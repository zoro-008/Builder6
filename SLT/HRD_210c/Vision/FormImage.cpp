...
void __fastcall TFrmImage::FormCreate(TObject *Sender)
{
    ...
    RectTracker = new TRectTracker  (); RectTracker -> LineColor = clLime ; RectTracker -> PointColor = clLime ;RectTracker -> Caption = "Rect" ; 
    LineTracker = new TLineTracker  (); LineTracker -> LineColor = clLime ; LineTracker -> PointColor = clLime ;LineTracker -> Caption = "Line" ;
...
void __fastcall TFrmImage::tmUpdateTimer(TObject *Sender)
{
    ...
    bool FrmHistoShowing = (FrmHisto != NULL && FrmHisto -> Showing) || (Frm3d    != NULL && Frm3d    -> Showing);
...
//    if(m_bPaintLock) {
//        Trace("m_bPaintLock",("m_bPaintLock return" + AnsiString(m_iVisnId)).c_str());
//        return ;
//    }
//    Trace("g_csPaint","PaintStart");
//
//    m_bPaintLock = true ;
...
//
    ...
    Label1 -> Caption = (int)hDc ;
    Label2 -> Caption = (int)pnDraw->Handle ;
    ...
    Label3 -> Caption = VISN[m_iVisnId]->m_iYameRet ? "Success":"Failed" ;
...
{
    CRunThread<Func> * func = new CRunThread<Func>(&OnPaintT) ;
}
...
void __fastcall TFrmImage::btInspClick(TObject *Sender)
{
    ...
    lbTime -> Caption = GetTickCount() -  lInspTime ;
...
void __fastcall TFrmImage::Timer2Timer(TObject *Sender)
{

    Timer2 -> Enabled = false ;

    VISN[m_iVisnId]->Inspect();
...
void __fastcall TFrmImage::Button2Click(TObject *Sender)
{
    ...
    //Search.
    SerchFile = ExpandFileName(Path + "*.*");
    if (FindFirst(SerchFile, faAnyFile, sr) == 0) {
        do {
            if ((sr.Attr & faAnyFile) == sr.Attr) {
                if ((sr.Name == ".") || (sr.Name == "..")) continue;
                    Item [Count] = sr.Name;
                    IDate[Count] = File.SearchFileDate(NormalDir(Path) + sr.Name  + "\\*.*" , sr.Time);
     ...
     for (int i = 0 ; i < Count ; i++) {
         VISN[m_iVisnId]->Img->LoadBmp((Path + Item[i]).c_str());
         //btInspClick(Sender);
...
void __fastcall TFrmImage::Button3Click(TObject *Sender)
{
    ...
    for(int i = 0 ; i < 100 ; i++)
    CRunThread<Func> * xxx = new CRunThread<Func>(&sun);//VISN[m_iVisnId]->Img->Rotate(TRect(50,50,690,530),1.0);
...
void __fastcall TFrmImage::Button4Click(TObject *Sender)
{
    ...
    unsigned long lTime =   GetTickCount();
    for(int y = 0 ; y < VISN[m_iVisnId]->Img->GetHeight(); y++) {
        for(int x = 0 ; x < VISN[m_iVisnId]->Img->GetWidth(); x++) {
             VISN[m_iVisnId]->Img->SetPixel(x,y,VISN[m_iVisnId]->Img->GetPixel(x,y)+30);
...
void __fastcall TFrmImage::bt3dClick(TObject *Sender)
{
    Trace("Operator", "Image Form3d Button Click");
    if(Frm3d) {
        delete Frm3d ;
    }
    Frm3d = new TFrm3d (this , VISN[m_iVisnId]->Img  , RectTracker);
    Frm3d -> Show() ;
}
//---------------------------------------------------------------------------

void __fastcall TFrmImage::lbTimeClick(TObject *Sender)
{
//    VISN[m_iVisnId]->Img-> Rotate(RectTracker -> GetTRect() , 5);
}
//---------------------------------------------------------------------------
