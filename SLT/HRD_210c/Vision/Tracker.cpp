...
void TTracker::OnLoadSave(bool _bLoad,AnsiString _sFilePath,AnsiString _sCaption)
{
    ...
    if(_bLoad) {
        ...
        UserINI.Load(_sFilePath, _sCaption , "m_iStdRefX    ", m_iStdRefX    );
        UserINI.Load(_sFilePath, _sCaption , "m_iStdRefY    ", m_iStdRefY    );
        ...
        UserINI.Save(_sFilePath, _sCaption , "m_iStdRefX    ", m_iStdRefX    );
        UserINI.Save(_sFilePath, _sCaption , "m_iStdRefY    ", m_iStdRefY    );
...
void __fastcall TRectTracker::Paint(HDC _Dc , float _fScaleX , float _fScaleY )
{
    ...
    if(FBandVisible && FFocused) {
        ...
        Gdi -> Rect(true, iBtmRectCntX-PointSize * 3, iBtmRectTopY-PointSize, iBtmRectCntX-PointSize, iBtmRectTopY+PointSize);
    }
...
bool __fastcall TRectTracker::TrackerDw(TMouseButton Button,TShiftState Shift ,int x, int y , float _fScaleX , float _fScaleY )
{
    ...
    else if (x > iBCntX -PointSize && x < iBCntX +PointSize && y > iBCntY -PointSize && y < iBCntY +PointSize ) {ret = Stat.IsBND  = true; FFocused = true ;}
...
void TRectTracker::SetRefOfs(int _iRefX , int _iRefY)
{
    FLeft += _iRefX - m_iStdRefX ;
    FTop  += _iRefY - m_iStdRefY ;
}
...
void TLineTracker::SetRefOfs(int _iRefX , int _iRefY)
{
    FStartX += _iRefX - m_iStdRefX ;
    FStartY += _iRefY - m_iStdRefY ;
    FEndX   += _iRefX - m_iStdRefX ;
    FEndY   += _iRefY - m_iStdRefY ;
}
...
void TCircleTracker::SetRefOfs(int _iRefX , int _iRefY)
{
    FX += _iRefX - m_iStdRefX ;
    FY += _iRefY - m_iStdRefY ;
}
...
void TPolyTracker::SetRefOfs(int _iRefX , int _iRefY)
{
    TRectTracker::SetRefOfs(_iRefX , _iRefY);

    for( int i = 0 ; i < FPointCount ; i++) {
        FPoints[i].x += _iRefX - m_iStdRefX ;
        FPoints[i].y += _iRefY - m_iStdRefY ;
    }
}
...
void TPolyLineTracker::SetRefOfs(int _iRefX , int _iRefY)
{
    TRectTracker::SetRefOfs(_iRefX , _iRefY);
