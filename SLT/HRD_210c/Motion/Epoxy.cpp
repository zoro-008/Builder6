...
/*
    int r1,c1;
    if(_riWhre == riREV) {
        c = DM.ARAY[riREV].FindLastCol(csUnkwn);
        if(c % 2)DM.ARAY[riREV].FindLastColRow    (csUnkwn , r1 , c1) ;
        else     DM.ARAY[riREV].FindLastColFrstRow(csUnkwn , r1 , c1) ;
        r = r1 ;
        c = c1 ;
        return (r > -1 && c > -1);
    }
...
*/
    ...
    if(_riWhre == riREF) {
        ...
        if(c % 2)DM.ARAY[riREF].FindLastColRow    (csUnkwn , r , c) ;
        else     DM.ARAY[riREF].FindLastColFrstRow(csUnkwn , r , c) ;
...
//            case piWRE_XEpxWork    : FindChip(riREV,r,c);
//                                     r = OM.DevInfo.iRowCnt - r - 1 ;
...
bool CEpoxy::ToStart(void) //스타트를 하기 위한 함수.
{
    ...
    switch (Step.iToStart) {
        ...
        case 10: //if(DM.ARAY[riREF].CheckAllStat(csUnkwn)){
                 //    DM.ARAY[riREF].SetStep(0);
                 //}
//                 FindChip(riREF,r,c);
//                   if(DM.ARAY[riREV].GetStat(r,c) == csRslt0 && DM.ARAY[riREF].GetStat(r,c) && bCheckIdxPos)
...
bool CEpoxy::Autorun(void) //오토런닝시에 계속 타는 함수.
{
    ...
    if (Step.iSeq == 0) {
        ...
        bool isCycleClean = !OM.MstOptn.bIgnrEpoxy && OM.DevOptn.iEpxCleanShotCnt != 0 &&
                           ((OM.DevOptn.iEpxClenCnt != 0 && OM.DevOptn.iEpxClenCnt <= m_iEpxCnt) || m_bToStarted );
        bool isCycleOri   =  false ; //Manual.
        bool isCycleWork  = !OM.MstOptn.bIgnrEpoxy && !RAE.bRaeSkip && !OM.CmnOptn.bEpoxyStop &&
                             DM.ARAY[riREF].GetCntStat(csUnkwn) /*&& DM.ARAY[riREV].GetCntStat(csUnkwn)*/ && bCheckIdxPos &&
                             RAE.GetSeqStep() == CRailEpx::scIdle && DM.ARAY[riREF].GetStep() > 0 ;

        bool isCycleWorkOut =!OM.MstOptn.bIgnrEpoxy &&
                             !DM.ARAY[riREF].CheckAllStat(csNone) /*&& DM.ARAY[riREV].GetCntStat(csUnkwn)*/ && bCheckIdxPos &&
                              RAE.GetSeqStep() != CRailEpx::scWork && DM.ARAY[riREF].GetStep() > 0 &&
                              DM.ARAY[riRDF].CheckAllStat(csNone) &&
                              DM.ARAY[riREV].GetCntStat(csWork) ;


        bool isConEnd     =  DM.ARAY[riREV].CheckAllStat(csNone) ;
...
//Check Cycle Time Out.
    ...
    switch (Step.iCycle) {
        ...
        case  10: if( FindChip(riREF,r,c)) {
                      DM.ARAY[riREV].SetStat(r,c,csRslt0);
                      DM.ARAY[riREF].SetStat(r,c,csWork);
                  }
                  Step.iCycle = 0;
                  return true ;
    }
...
/*
                  FindChip(riREV,r,c);
                  if(DM.ARAY[riREV].GetStat(r,c) == csRslt0) {
//                      DM.ARAY[riREV].SetStat(r,c,csWork);
                      DM.ARAY[riREF].SetStat(r,c,csWork);
                  }
                  else {
                      DM.ARAY[riREV].SetStat(r,c,csWork);
