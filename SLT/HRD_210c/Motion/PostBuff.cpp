...
bool CPostBuff::CycleHome()     //sun DLL direct access.
{
    ...
    switch (Step.iHome) {
        ...
        case 10: MT_SetHomeEnd(miPSB_XIdx,false);
                 AT_MoveCyl(aiPSB_Clamp  , ccBwd);
                 bPSBNone = DM.ARAY[riPSF].CheckAllStat(csNone) ;
                 Step.iHome++;
                 return false ;
...
bool CPostBuff::ToStart(void) //스타트를 하기 위한 함수.
{
    ...
    switch (Step.iToStart) {
        ...
        case 10: bPSBNone = DM.ARAY[riPSF].CheckAllStat(csNone);
...
bool CPostBuff::Autorun(void) //오토런닝시에 계속 타는 함수.
{
    ...
    if (Step.iSeq == 0) {
        ...
        //Check         miWRD_XIdx
        int  iReGribStep  = DM.ARAY[riPSF].GetStep() ;
        bool bAllNone     = DM.ARAY[riPSF].CheckAllStat(csNone) ;
...
//Check Cycle Time Out.
    ...
    switch (Step.iCycle) {
        ...
        case  13: if(!MoveActr(aiPSB_Clamp  , ccFwd)) return false ;
                  if(DM.ARAY[riPSF].GetStep() >= OM.DevOptn.iPSB_XRgCnt) MT_GoAbsSlow(miPSB_XIdx,GetMotrPos(miPSB_XIdx,piPSB_XIdxOut2) );
                  else                                                   MoveMotr(miPSB_XIdx,piPSB_XIdxOut1);

                  Step.iCycle++;
                  return false;

        case  14: if(DM.ARAY[riPSF].GetStep() >= OM.DevOptn.iPSB_XRgCnt) {if(!MT_GoAbsSlow(miPSB_XIdx,GetMotrPos(miPSB_XIdx,piPSB_XIdxOut2) )) return false ;}
                  else                                                   {if(!MoveMotr(miPSB_XIdx,piPSB_XIdxOut1)) return false ;}
                  if(DM.ARAY[riPSF].GetStep() >= OM.DevOptn.iPSB_XRgCnt) {MoveMotr(miPSB_XIdx,piPSB_XIdxWait); }
                  else                                                   {MoveActr(aiPSB_Clamp, ccBwd);        }
                  Step.iCycle++;
                  return false ;

        case  15: if(DM.ARAY[riPSF].GetStep() >= OM.DevOptn.iPSB_XRgCnt) {if(!MoveMotr(miPSB_XIdx,piPSB_XIdxWait)) return false; }
                  //else                                                   {MoveActr(aiPSB_Clamp, ccBwd);        }


                  if(!MoveActr(aiPSB_Clamp, ccBwd)) return false ;

                  m_sWorkedId    = DM.ARAY[riPSF].GetID   ();
                  m_sWorkedLotNo = DM.ARAY[riPSF].GetLotNo();

                  if(DM.ARAY[riPSF].GetStep() >=  OM.DevOptn.iPSB_XRgCnt && IO_GetX(xPSB_OutDetect)) {
                      EM_SetErr(eiPSB_OutFeedingFail);
                      Step.iCycle = 0 ;
                      return true ;
                  }
                  else if (DM.ARAY[riPSF].GetStep() >=  OM.DevOptn.iPSB_XRgCnt && !IO_GetX(xPSB_OutDetect)) {
                      LotInfoAdd(&DM.ARAY[riPSV]);
                      if(DM.ARAY[riULD].CheckAllStat(csEmpty)) {
                          DM.ARAY[riULD].SetLotNo(DM.ARAY[riPSF].GetLotNo());
                          DM.ARAY[riULD].SetID   (DM.ARAY[riPSF].GetID   ());
                      }
                  ...
                  DM.ARAY[riPSF].SetStep(DM.ARAY[riPSF].GetStep() + 1 );
...
bool CPostBuff::IsMgzLastStrip()
{
    AnsiString sLastLotNo ;

    if(DM.ARAY[riPSF].GetCntExist()) sLastLotNo = DM.ARAY[riPSF].GetLotNo() ;
...
//    bool bPSBExist = !DM.ARAY[riPSV].CheckAllStat(csNone) ;
...
//    bool bPSBDiff  = sLastLotNo != DM.ARAY[riPSV].GetLotNo() ;
