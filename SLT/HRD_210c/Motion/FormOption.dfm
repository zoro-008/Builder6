object FrmOption: TFrmOption
  Left = 511
  Top = 92
  BorderStyle = bsNone
  Caption = 'FrmOption'
  ClientHeight = 870
  ClientWidth = 1280
  Color = clBtnFace
  Constraints.MinHeight = 897
  Constraints.MinWidth = 1288
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 1280
    Height = 870
    Align = alClient
    Color = clMoneyGreen
    TabOrder = 0
    object Panel2: TPanel
      Left = 409
      Top = 0
      Width = 424
      Height = 401
      TabOrder = 0
      object Label67: TLabel
        Left = 4
        Top = 312
        Width = 300
        Height = 23
        AutoSize = False
        Caption = 'WAFER TOOL Theta Limit '#48372#51221#44050
        Color = clInactiveBorder
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
      end
      object Label68: TLabel
        Left = 367
        Top = 312
        Width = 36
        Height = 23
        AutoSize = False
        Caption = 'Ang'
        Color = clInactiveBorder
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
      end
      object Label6: TLabel
        Left = 4
        Top = 280
        Width = 300
        Height = 23
        AutoSize = False
        Caption = 'RAIL TOOL Theta Limit '#48372#51221#44050
        Color = clInactiveBorder
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
      end
      object Label7: TLabel
        Left = 365
        Top = 280
        Width = 36
        Height = 23
        AutoSize = False
        Caption = 'Ang'
        Color = clInactiveBorder
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
      end
      object Panel3: TPanel
        Left = 1
        Top = 1
        Width = 422
        Height = 35
        Align = alTop
        BevelOuter = bvNone
        Caption = 'VISION OPTION'
        Color = clSkyBlue
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -20
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        TabOrder = 0
      end
      object cbbIgnrRalAtVs: TCheckBox
        Tag = 6
        Left = 10
        Top = 96
        Width = 400
        Height = 22
        Caption = 'CHIP PLACE '#44160#49324' VISION '#49324#50857' '#50504#54632
        Color = clScrollBar
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
        TabOrder = 1
        OnClick = cbLoadingStopClick
      end
      object cbbUseStgTheta: TCheckBox
        Tag = 6
        Left = 10
        Top = 368
        Width = 400
        Height = 22
        Caption = 'USE STAGE THETA'
        Color = clScrollBar
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
        TabOrder = 2
        Visible = False
        OnClick = cbLoadingStopClick
      end
      object cbbIgnrRalBfVsEpoxyFail: TCheckBox
        Tag = 6
        Left = 10
        Top = 72
        Width = 400
        Height = 22
        Caption = 'EPOXY'#44160#49324' FAIL '#47924#49884
        Color = clScrollBar
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
        TabOrder = 3
        OnClick = cbLoadingStopClick
      end
      object cbIgnrOriVs: TCheckBox
        Tag = 6
        Left = 10
        Top = 48
        Width = 400
        Height = 22
        Caption = #48169#54693#44160#49324' VISION '#49324#50857' '#50504#54632
        Color = clScrollBar
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
        TabOrder = 4
        OnClick = cbLoadingStopClick
      end
      object cbUseHeadTWfr: TCheckBox
        Tag = 6
        Left = 10
        Top = 144
        Width = 400
        Height = 22
        Caption = 'WAFER '#44208#44284#50640' '#46384#47480' TOOL Theta '#48372#51221' '#49324#50857
        Color = clScrollBar
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
        TabOrder = 5
        OnClick = cbLoadingStopClick
      end
      object cbWfrVisnRetest: TCheckBox
        Tag = 6
        Left = 10
        Top = 208
        Width = 400
        Height = 22
        Caption = 'WAFER VISION 2'#48264' '#44160#49324' '#49324#50857
        Color = clScrollBar
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
        TabOrder = 6
        OnClick = cbLoadingStopClick
      end
      object cbUseHeadTRal: TCheckBox
        Tag = 6
        Left = 10
        Top = 168
        Width = 400
        Height = 22
        Caption = 'RAIL '#44208#44284#50640' '#46384#47480' TOOL Theta '#48372#51221' '#49324#50857
        Color = clScrollBar
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
        TabOrder = 7
        OnClick = cbLoadingStopClick
      end
      object edStageTLmtErr: TEdit
        Left = 309
        Top = 312
        Width = 56
        Height = 23
        AutoSize = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ImeName = 'Microsoft Office IME 2007'
        ParentFont = False
        TabOrder = 8
        Text = '1'
      end
      object edHedTLmtErr: TEdit
        Left = 309
        Top = 280
        Width = 56
        Height = 23
        AutoSize = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ImeName = 'Microsoft Office IME 2007'
        ParentFont = False
        TabOrder = 9
        Text = '1'
      end
    end
    object Panel4: TPanel
      Left = 0
      Top = 0
      Width = 409
      Height = 401
      TabOrder = 1
      object Label2: TLabel
        Left = 4
        Top = 344
        Width = 300
        Height = 23
        AutoSize = False
        Caption = 'EJECTOR AFTER DOWN  DELAY'
        Color = clInactiveBorder
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
        Visible = False
      end
      object Label3: TLabel
        Left = 365
        Top = 344
        Width = 36
        Height = 23
        AutoSize = False
        Caption = 'ms'
        Color = clInactiveBorder
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
        Visible = False
      end
      object Label4: TLabel
        Left = 4
        Top = 375
        Width = 300
        Height = 23
        AutoSize = False
        Caption = 'EJECTOR AFTER UP  DELAY'
        Color = clInactiveBorder
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
        Visible = False
      end
      object Label5: TLabel
        Left = 365
        Top = 375
        Width = 36
        Height = 23
        AutoSize = False
        Caption = 'ms'
        Color = clInactiveBorder
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
        Visible = False
      end
      object Panel5: TPanel
        Left = 1
        Top = 1
        Width = 407
        Height = 35
        Align = alTop
        BevelOuter = bvNone
        Caption = 'RUNNING OPTION'
        Color = clSkyBlue
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -20
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        TabOrder = 0
      end
      object cbLoadingStop: TCheckBox
        Tag = 6
        Left = 10
        Top = 72
        Width = 350
        Height = 22
        Caption = 'LOADING STOP'
        Color = clScrollBar
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
        TabOrder = 1
        OnClick = cbLoadingStopClick
      end
      object cbbDoorSkip: TCheckBox
        Tag = 6
        Left = 10
        Top = 48
        Width = 350
        Height = 22
        Caption = 'IGNORE DOOR OPEN'
        Color = clScrollBar
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
        TabOrder = 2
        OnClick = cbLoadingStopClick
      end
      object cbbIgnrVcSsr: TCheckBox
        Tag = 6
        Left = 10
        Top = 96
        Width = 350
        Height = 22
        Caption = 'IGNORE VACCUM SENSOR'
        Color = clScrollBar
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
        TabOrder = 3
        OnClick = cbLoadingStopClick
      end
      object cbUseMapFile: TCheckBox
        Tag = 6
        Left = 10
        Top = 120
        Width = 350
        Height = 22
        Caption = 'USE MAP FILE'
        Color = clScrollBar
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
        TabOrder = 4
        OnClick = cbLoadingStopClick
      end
      object cbUseBtAlign: TCheckBox
        Tag = 6
        Left = 10
        Top = 192
        Width = 350
        Height = 22
        Caption = 'USE BOAT ALIGN CYLINDER'
        Color = clScrollBar
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
        TabOrder = 5
        OnClick = cbLoadingStopClick
      end
      object cbIgnrHedOverload: TCheckBox
        Tag = 6
        Left = 10
        Top = 216
        Width = 350
        Height = 22
        Caption = 'IGNORE HED OVERLOAD'
        Color = clScrollBar
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
        TabOrder = 6
        OnClick = cbLoadingStopClick
      end
      object edEjctrDnAfterDelay: TEdit
        Left = 309
        Top = 344
        Width = 56
        Height = 23
        AutoSize = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ImeName = 'Microsoft Office IME 2007'
        ParentFont = False
        TabOrder = 7
        Text = '1'
        Visible = False
      end
      object edEjctrUpAfterDelay: TEdit
        Left = 309
        Top = 375
        Width = 56
        Height = 23
        AutoSize = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ImeName = 'Microsoft Office IME 2007'
        ParentFont = False
        TabOrder = 8
        Text = '1'
        Visible = False
      end
      object cbUseVcCoil: TCheckBox
        Tag = 6
        Left = 10
        Top = 168
        Width = 350
        Height = 22
        Caption = 'USE VOICE COIL'
        Color = clScrollBar
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
        TabOrder = 9
        OnClick = cbLoadingStopClick
      end
      object cbEpoxyStop: TCheckBox
        Tag = 6
        Left = 10
        Top = 240
        Width = 350
        Height = 22
        Caption = 'EPOXY STOP'
        Color = clScrollBar
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
        TabOrder = 10
        OnClick = cbLoadingStopClick
      end
      object cbInkDieWork: TCheckBox
        Tag = 6
        Left = 10
        Top = 144
        Width = 350
        Height = 22
        Caption = 'INK DIE WORK MODE (Only NoMap)'
        Color = clScrollBar
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
        TabOrder = 11
        OnClick = cbLoadingStopClick
      end
    end
    object btSave: TBitBtn
      Left = 1016
      Top = 760
      Width = 233
      Height = 81
      Caption = ' SAVE'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -33
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 2
      OnClick = btSaveClick
      Glyph.Data = {
        
        7878787878FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    end
    object Panel6: TPanel
      Left = 0
      Top = 401
      Width = 409
      Height = 305
      TabOrder = 3
      object Label9: TLabel
        Left = 5
        Top = 50
        Width = 267
        Height = 23
        AutoSize = False
        Caption = #51089#50629' CHIP '#49688#47049
        Color = clInactiveBorder
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
      end
      object Label10: TLabel
        Left = 355
        Top = 50
        Width = 50
        Height = 23
        AutoSize = False
        Caption = 'Count'
        Color = clInactiveBorder
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
      end
      object Panel7: TPanel
        Left = 1
        Top = 1
        Width = 407
        Height = 35
        Align = alTop
        BevelOuter = bvNone
        Caption = 'LOT END OPTION'
        Color = clSkyBlue
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -20
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        TabOrder = 0
      end
      object edLotEndChipCnt: TEdit
        Left = 277
        Top = 49
        Width = 71
        Height = 23
        AutoSize = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ImeName = 'Microsoft Office IME 2007'
        ParentFont = False
        TabOrder = 1
        Text = '1'
      end
    end
  end
end
