...
bool CCalibration::Inspect(CImage *_pImg , TRect _tRect , CAL_Para _tPara , CAL_Rslt * _pRslt)
{
    ...
    int iFirstRowY  = Blob1.fCentroidY ;
    int iLastRowY ;
    ...
    for (register int i = 0 ; i < _pRslt->BlbRslt.iBlobCnt ; i++ ) {
        ...
        if(iRowBandTop < Blob1.fCentroidY && iRowBandBtm > Blob1.fCentroidY) iColCnt++ ;
        if(iColBandLft < Blob1.fCentroidX && iColBandRgt > Blob1.fCentroidX) iRowCnt++ ;
        iLastRowY = Blob1.fCentroidY ;
    }
    ...
    for (register int y = 0 ; y < iRowCnt - 1; y++ ) {
        for (register int x = 0 ; x < iColCnt - 1; x++ ) {
            dSumX += _pRslt->BlbRslt.pBlobs[ iColCnt *  y + x  + 1 ].fCentroidX - _pRslt->BlbRslt.pBlobs[ iColCnt * y + x ].fCentroidX ; iCntX++ ;
            dSumY += _pRslt->BlbRslt.pBlobs[ iColCnt * (y + 1) + x ].fCentroidY - _pRslt->BlbRslt.pBlobs[ iColCnt * y + x ].fCentroidY ; iCntY++ ;
        }
    }
