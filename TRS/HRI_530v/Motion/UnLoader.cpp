...
bool CUnLoader::ToStart(void) //스타트를 하기 위한 함수.
{
    ...
    switch (Step.iToStart) {
        ...
        case 10: if(!DM.ARAY[riULD].CheckAllStat(csNone)){ //특별히 뭐 없을때.'
                     Step.iToStart=20 ;
                 }
                 else {
                     Step.iToStart=30 ;
...
bool CUnLoader::CheckSafe(EN_ACTR_ID _iActr , bool _bFwd)
{
    ...
    return bRet ;
                                        ...
                                        //sunsun SORT_Y 조그방향 반대. 언로더인덱스 업다운 실린더 방향 반대
                                        //언로더 세퍼레이터 실린더 버튼 없음.
...
{
    if(m_dLastIdxPos != MT_GetCmdPos(miULD_YIndx)) return true  ; //이파트는 필요 없다.
    else                                           return false ;
