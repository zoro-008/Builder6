...
//Check Cycle Time Out.
    ...
    bNeedInsp[0] = !OM.CmnOptn.bSkip1stTol ;//(DM.ARAY[riSTL].GetStat(0,0)!=csEmpty ||GetSeqStep()==scIdle) && !OM.CmnOptn.bSkip1stTol ;
    bNeedInsp[1] = !OM.CmnOptn.bSkip2ndTol ;//(DM.ARAY[riSTL].GetStat(1,0)!=csEmpty ||GetSeqStep()==scIdle) && !OM.CmnOptn.bSkip2ndTol ;
    bNeedInsp[2] = !OM.CmnOptn.bSkip3thTol ;//(DM.ARAY[riSTL].GetStat(2,0)!=csEmpty ||GetSeqStep()==scIdle) && !OM.CmnOptn.bSkip3thTol ;
    bNeedInsp[3] = !OM.CmnOptn.bSkip4thTol ;//(DM.ARAY[riSTL].GetStat(3,0)!=csEmpty ||GetSeqStep()==scIdle) && !OM.CmnOptn.bSkip4thTol ;
        ...
        case  16: if(!MoveActr(aiOST_PshrDnUp , ccBwd))return false;
                  ...
                  if(!bSkipInsp){
                       for(int r = 0 ; r < MAX_TOOL_SOCK_COL ; r++){
                           ...
                           if(GetOSTFail(r , m_pRs232_OST -> sData , bRet)){ //bRet이 true면 NG
                               //우선순위 OST
                               if(bRet&&!OM.CmnOptn.bOSTNotUseOst){ //1개라도 뻬일이 있으면 페일처리하고 OST 검사 안하기 때문에 //원래는 오프셑 먹어서 1ma 이런식이였는데 오프셑 게인 적용 없이 그냥 0.0으로 모두 쎄팅 하도록.. 수정. 20140105테라셈 요청.
...
bool CStageOST::WriteIDD_IDStoCSV(AnsiString _sFileName)
{
    ...
    UserFile.ClearDirDate(OST_FOLDER , Now() - OM.CmnOptn.iOSTLogDays); //7days.
