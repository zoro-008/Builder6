...
bool CLoader::FindChip(int &r , int &c )
{
    ...
    if(DM.ARAY[riLDR].FindFrstRowCol(csUnkwn , r,c)){
        return true ;
    }
    else {
        r=0 ;
...
double CLoader::GetMotrPos(EN_MOTR_ID _iMotr , EN_PSTN_ID _iPstnId )
{
    ...
    dWorkPos = dWorkPos + (OM.DevInfo.dLDRRowPitch * iR) ;
...
bool CLoader::ToStart(void) //스타트를 하기 위한 함수.
{
    ...
    switch (Step.iToStart) {
        ...
        case 10: if(!DM.ARAY[riLDR].CheckAllStat(csNone)){ //특별히 뭐 없을때.'
                     Step.iToStart=20 ;
                 }
                 else {
                     Step.iToStart=30 ;
        ...
        //Work존에 자제 있을때.
        case 20: MoveActr(aiLDR_IndxUpDn , ccBwd);
                 Step.iToStart++;
                 return false ;

        case 21: if(!MoveActr(aiLDR_IndxUpDn , ccBwd)) return false ;
                 //MoveMotr(miLDR_YIndx , piLDR_YIndxWork );
                 MT_GoAbsMan(miLDR_YIndx , m_dLastIdxPos);
                 Step.iToStart++;
                 return false ;

        case 22: if(!MT_GetStopInpos(miLDR_YIndx)) return false ;
                 MT_GoIncMan(miLDR_YIndx , -1.0);
                 Step.iToStart++;
                 return false ;

        case 23: if(!MT_GetStopInpos(miLDR_YIndx)) return false ;
                 MoveActr(aiLDR_IndxUpDn , ccFwd);
                 Step.iToStart++;
                 return false ;

        case 24: if(!MoveActr(aiLDR_IndxUpDn , ccFwd)) return false ;
                 MT_GoIncMan(miLDR_YIndx , 1.0);

                 Step.iToStart++;
                 return false ;

        case 25: if(!MT_GetStopInpos(miLDR_YIndx)) return false ;
                 Step.iToStart=0;
                 return true ;

        //워크 존에 자제 없얼때.
        case 30: MoveActr(aiLDR_IndxUpDn , ccBwd);
                 Step.iToStart ++ ;
                 return false ;

        case 31: if(!MoveActr(aiLDR_IndxUpDn , ccBwd)) return false;
        ...
        case 32: if(!MoveMotr(miLDR_YIndx , piLDR_YIndxWait)) return false ;
                 ...
                 return true ;
        /*
        case 10: MoveActr(aiLDR_IndxUpDn , ccFwd);
                 Step.iToStart++;
                 return false ;

        case 11: if(!MoveActr(aiLDR_IndxUpDn , ccFwd)) return false ;
                 MoveMotr(miLDR_YIndx , piLDR_YIndxWork );
                 Step.iToStart++;
                 return false ;
...
bool CLoader::Autorun(void) //오토런닝시에 계속 타는 함수.
{
    ...
    if (Step.iSeq == 0) {
        ...
        bool isCycleWork = !DM.ARAY[riLDR].CheckAllStat(csNone ) && DM.ARAY[riLDR].GetCntStat  (csUnkwn) && !GetInWorkPos();
        ...
        //모르는 스트립에러.
        if(  DM.ARAY[riLDR].CheckAllStat(csNone) && IO_GetX(xLDR_Tray3Work))
            EM_SetErrMsg(eiPKG_Unknwn , DM.ARAY[riLDR].GetName().c_str());
        //카세트 사라짐.
        if( !DM.ARAY[riLDR].CheckAllStat(csNone) &&!IO_GetX(xLDR_Tray3Work))
            EM_SetErrMsg(eiPKG_Dispr  , DM.ARAY[riLDR].GetName().c_str());
        //카세트 OUT 매거진 풀
...
bool CLoader::CycleHome()     //sun DLL direct access.
{
    ...
    switch (Step.iHome) {
        ...
        case 10: MoveActr(aiLDR_IndxUpDn, ccBwd);
                 Step.iHome++;
                 return false ;

        case 11: if(!MoveActr(aiLDR_IndxUpDn, ccBwd)) return false ;
...
//Check Cycle Time Out.
    ...
    switch (Step.iCycle) {
        ...
        case  12: if(!MoveMotr(miLDR_YIndx , piLDR_YIndxGet)) return false ;
                  IO_SetY(yLDR_AirBlow , true);
                  MoveActr(aiLDR_IndxUpDn , ccFwd);
                  Step.iCycle++;
                  return false ;

        case  13: if(!MoveActr(aiLDR_IndxUpDn , ccFwd)) return false ;
...
//Check Cycle Time Out.
    ...
    switch (Step.iCycle) {
        ...
        case  11: if(!MoveActr(aiLST_StckUpDn , ccBwd)) return false ;
                  if(!MoveActr(aiLDR_IndxUpDn , ccFwd)) return false ;
                  MoveMotr(miLDR_YIndx , piLDR_YIndxOut);
                  Step.iCycle++;
                  return false;

        case  12: if(!MoveMotr(miLDR_YIndx , piLDR_YIndxOut)) return false;

                  DM.ShiftArrayData(riLDR , riPSL);
                  MoveMotr(miLDR_YIndx , piLDR_YIndxGet);
                  MoveActr(aiLDR_IndxUpDn , ccBwd);
                  Step.iCycle++;
                  return false ;

        case  13: if(!MoveMotr(miLDR_YIndx , piLDR_YIndxGet)) return false ;
                  if(!MoveActr(aiLDR_IndxUpDn , ccBwd)) return false ;
                  Step.iCycle = 0 ;
                  return true ;
    }
}
...
bool CLoader::CheckSafe(EN_MOTR_ID _iMotr , EN_PSTN_ID _iPstnId)
{
    ...
    bool bIndxFwd    = !AT_Complete(aiLDR_IndxUpDn , ccBwd);
...
bool CLoader::CheckStop()
{
    ...
    if(!AT_Done(aiLDR_IndxUpDn)) return false ;
...
void CLoader::Load(bool _bLoad)
{
    ...
    if(m_dLastIdxPos > MT_GetMaxPos(miLDR_YIndx)) m_dLastIdxPos = MT_GetMaxPos(miLDR_YIndx) ;
    if ( _bLoad ) {
...
bool CLoader::GetInWorkPos()
{

    //졸라 이상함 dMotrPos == dCmdPos 으로 비교하면 같은 값인데 가끔 리턴 false가 있다.

    double dMotrPos   = GetMotrPos(miLDR_YIndx , piLDR_YIndxWork) ;
    double dCmdPos    = MT_GetCmdPos(miLDR_YIndx);
    bool   bStopInpos = MT_GetStopInpos(miLDR_YIndx);
