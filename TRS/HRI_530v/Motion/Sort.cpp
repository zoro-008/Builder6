...
bool CSort::CyclePickSTG(void)
{
    ...
    switch (Step.iCycle) {
        ...
        case  10: m_bRetryStgPick = false ;

                  //배큠에러에서 리트라이 누르고 그냥 제거 했을 경우.
                  if(DM.ARAY[riSTF].CheckAllStat(csEmpty)) {
                      Step.iCycle = 0 ;
                      return true ;
                  }
        ...
        case  16: bVacErr[0] = SortInfo.bDn[0]&&!IO_GetX(xSRT_VccSnsr1) && !OM.MstOptn.bVacErrIgnr ;
                  ...
                  sTemp2  = DM.ARAY[riSTF].GetStat(0,0) ; sTemp2 += "," ;
                  ...
                  sTemp2  = DM.ARAY[riSRT].GetStat(0,0) ; sTemp2 += "," ;
                  sTemp2 += DM.ARAY[riSRT].GetStat(0,1) ; sTemp2 += "," ;
                  sTemp2 += DM.ARAY[riSRT].GetStat(0,2) ; sTemp2 += "," ;
                  sTemp2 += DM.ARAY[riSRT].GetStat(0,3) ; sTemp2 += "," ;
                  Trace("Bf ShifSockSortInfo SRT",sTemp2.c_str());
                  ...
                  sTemp2 += DM.ARAY[riSTF].GetStat(0,2) ; sTemp2 += "," ;
                  sTemp2 += DM.ARAY[riSTF].GetStat(0,3) ; sTemp2 += "," ;
                  Trace("At ShifSockSortInfo STF",sTemp2.c_str());

                  sTemp2  = DM.ARAY[riSRT].GetStat(0,0) ; sTemp2 += "," ;
                  sTemp2 += DM.ARAY[riSRT].GetStat(0,1) ; sTemp2 += "," ;
                  sTemp2 += DM.ARAY[riSRT].GetStat(0,2) ; sTemp2 += "," ;
                  sTemp2 += DM.ARAY[riSRT].GetStat(0,3) ; sTemp2 += "," ;
                  Trace("At ShifSockSortInfo SRT",sTemp2.c_str());
...
bool CSort::CyclePickZIG(void)
{
    ...
    switch (Step.iCycle) {
        ...
        case  10:
                  m_bRetryZigPick = false ;
                  //배큠에러후에 그냥 제거 했을 경우 오토런에서 이리로 보내는데 클리어가 안되어 계속 리트라이 한다.
                  //삽질 하는것 클리어.
                  if(DM.ARAY[riZIG].CheckAllStat(csEmpty)) {
                      Step.iCycle = 0;
                      return true;
                  }
...
bool CSort::CheckSafe(EN_MOTR_ID _iMotr , EN_PSTN_ID _iPstnId)
{
    ...
    bool bPckrOnStgPos = MT_GetCmdPos(miSRT_XPckr) <= PM.GetValue(miSRT_XPckr , pvSRT_XPckrPocket) + 0.1 &&
                         ...
                         MT_GetCmdPos(miSRT_YPckr) >= PM.GetValue(miSRT_YPckr , pvSRT_YPckrPocket) - 0.1 ;
    bool bStgMoving   = !MT_GetStop(miSTG_TTble);
    ...
    if(_iMotr == miSRT_YPckr){
        ...
        if(bPckrOnStgPos && bStgMoving) { sMsg = AnsiString("Stage is Moving") ; bRet = false ;}
...
void CSort::Test()
{
    if(DM.ARAY[riSRT].GetCntStat(csEmpty)==4&& DM.ARAY[riSTF].GetCntStat(csEmpty)==0){
        String sTemp2 ;
        CSort::TSortInfo SortInfo ;
        SRT.GetSockSortInfo(true , riSRT , riSTF , SortInfo);

        sTemp2  = DM.ARAY[riSTF].GetStat(0,0) ;
        sTemp2 += DM.ARAY[riSTF].GetStat(0,1) ;
        sTemp2 += DM.ARAY[riSTF].GetStat(0,2) ;
        sTemp2 += DM.ARAY[riSTF].GetStat(0,3) ;
        Trace("Bf ShifSockSortInfo STF",sTemp2.c_str());

        sTemp2  = DM.ARAY[riSRT].GetStat(0,0) ;
        sTemp2 += DM.ARAY[riSRT].GetStat(0,1) ;
        sTemp2 += DM.ARAY[riSRT].GetStat(0,2) ;
        sTemp2 += DM.ARAY[riSRT].GetStat(0,3) ;
        Trace("Bf ShifSockSortInfo SRT",sTemp2.c_str());


        SRT.ShiftSockSortInfo(SortInfo , false,
                                         false,
                                         false,
                                         false);
    ...
    else if(DM.ARAY[riSRT].GetCntStat(csEmpty)==0&& DM.ARAY[riSTF].GetCntStat(csEmpty)==4){
        DM.ARAY[riSTF].SetStat(0,0,DM.ARAY[riSRT].GetStat(0,0));
        DM.ARAY[riSTF].SetStat(0,1,DM.ARAY[riSRT].GetStat(0,1));
        DM.ARAY[riSTF].SetStat(0,2,DM.ARAY[riSRT].GetStat(0,2));
        DM.ARAY[riSTF].SetStat(0,3,DM.ARAY[riSRT].GetStat(0,3));
