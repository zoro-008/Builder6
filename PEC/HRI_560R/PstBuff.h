...
class CPstBuff
{
	...
	double GetMotrPos      (EN_MOTR_ID _iMotr , EN_PSTN_ID _iPstnId ); //모터가 가야할 포지션을 
        double GetMotrVsPos    (EN_MOTR_ID _iMotr , EN_PSTN_ID _iPstnId ); //모터가 가야할 포지션을 리턴함. Double형.
        double GetMotrMrkPos   (EN_MOTR_ID _iMotr , EN_PSTN_ID _iPstnId ); //모터가 가야할 포지션을 리턴함. Double형.
        void   SetTrgPos   ();
        void   ResetTrgPos ();
	...
	bool MoveActr (EN_ACTR_ID _iActr , bool _bFwd); //실린더를 움직일때 쓰는 함수.
        ...
        bool CycleWork  (void); //마킹 Cycle
        bool CycleRemove(void); //자재 제거 Cycle
