...
void CHead::SetTrgPos()
{
    ...
    //TODO :: 560장비에서 대기위치에서 트리거가 터지는 현상떄문에
    //_bOnLevel를 false로 입력함. JS
    //_bOnLevel true  = High -> Low 로 떨어질 시 Trigger 터짐.
    //_bOnLevel false = Low -> High 로 올라갈 시 Trigger 터짐.
    //테스트기기로 check 함. JS
...
bool CHead::CycleInsp(void) //
{
    ...
    switch (Step.iCycle) {
        ...
        //17 Step Used When TimeOut
        case  18: //if(!MoveMotr(miWRK_XVsn , piWRK_XVsnWait)) return false;  //나중에 수정해야 함... 텍타임때문에 현재는 수정 안함..JS
                  ...
                  if(OM.CmnOptn.bVs1Skip) {
                      ...
                      if(DM.ARAY[riWR1].CheckAllExist()){
                          ...
                          if(!VC.ReadResult(1,riWR1)){ //비젼 데이터 읽어 오기.
                              ...
                              if(DM.ARAY[riVS2].GetCntExist()) DM.ARAY[riVS2].SetStat(csUnkwn) ;
                              if(DM.ARAY[riVS3].GetCntExist()) DM.ARAY[riVS3].SetStat(csUnkwn) ;
                              //VC.SendReset(vsVisn1); //쓰레드를 켜서 비젼 리셑.
                              //if(!OM.CmnOptn.bVs2Skip) VC.SendReset(vsVisn2) ;
                              //if(!OM.CmnOptn.bVs3Skip) VC.SendReset(vsVisn3) ;
                              Step.iCycle=0 ;
                              return true ;
                  ...
                  if(OM.CmnOptn.bVs2Skip) {
                      ...
                      if(DM.ARAY[riWR2].CheckAllExist() ){ //가라 같네요.밑에도.
                          ...
                          if(!VC.ReadResult(2,riWR2)){
                              ...
                              if(DM.ARAY[riVS3].GetCntExist()) DM.ARAY[riVS3].SetStat(csUnkwn) ;
                              //VC.SendReset(vsVisn2); //쓰레드를 켜서 비젼 리셑.
                              //if(!OM.CmnOptn.bVs1Skip) VC.SendReset(vsVisn1) ;
                              //if(!OM.CmnOptn.bVs3Skip) VC.SendReset(vsVisn3) ;
                              Step.iCycle=0 ;
                              return true ;
                  ...
                  if(OM.CmnOptn.bVs3Skip) {
                      ...
                      if(DM.ARAY[riWR3].CheckAllExist() ){
                          ...
                          if(!VC.ReadResult(3,riWR3)){
                              ...
                              //VC.SendReset(vsVisn3); //쓰레드를 켜서 비젼 리셑.
                              //if(!OM.CmnOptn.bVs1Skip) VC.SendReset(vsVisn1) ;
                              //if(!OM.CmnOptn.bVs2Skip) VC.SendReset(vsVisn2) ;
                              Step.iCycle=0 ;
                              return true ;
