...
bool CMotor::GoAbsAcDc(double _dPos , double _dVel , double _dAcc , double _dDcc)
{
    ...
    if (_dPos > Para.dMaxPos) {
        if(m_dLastCmdPos != GetCmdPos()) {MsgOk("Warrning","Motor Max Position is larger than Move Position."); m_dLastCmdPos = GetCmdPos();}
        Stop(); Trace(Para.sMotorName.c_str() , "GoAbs Para.dMaxPos return "); return false;
    }

    if (_dPos < Para.dMinPos) {
        if(m_dLastCmdPos != GetCmdPos()) {MsgOk("Warrning","Motor Min Position is small than Move Position." ); m_dLastCmdPos = GetCmdPos();}
    ...
    //if (GetStopInpos()) { //인포지션이 안들어와서 바로 else 로 가서 오버라이딩을 타는 것때문에 주석처리 (정지시에도 이런다...;;; dd
        ...
        if(GetTrgPos() != _dPos)Trace(Para.sMotorName.c_str() , ("Move to " + AnsiString(_dPos)).c_str());
    }
    else {
