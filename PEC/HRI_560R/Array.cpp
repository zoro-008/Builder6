...
__fastcall CArray::CArray(void)
{
    ...
    if(bFirst) {
        //Default Set.
        clStat[csRslt0  ] = clInactiveCaption ;//clInactiveCaption = 0  ,
        clStat[csRslt1  ] = clBlack           ;//clMaroon          = 1  ,
        clStat[csRslt2  ] = clBlue            ;//clPurple          = 2  ,
        clStat[csRslt3  ] = clRed             ;//clTeal            = 3  ,
        clStat[csRslt4  ] = 0x00C08080        ;//clOlive           = 4  ,
        clStat[csRslt5  ] = 0x000080FF        ;//clNavy            = 5  ,
        clStat[csRslt6  ] = 0x00FF80FF        ;//clMoneyGreen      = 6  ,
        clStat[csRslt7  ] = clMaroon          ;//clGreen           = 7  ,
        clStat[csRslt8  ] = 0x00004000        ;//clHighlight       = 8  ,
        clStat[csRslt9  ] = 0x00808040        ;//clBlack           = 9  ,
        clStat[csRslt10 ] = clYellow          ;//clBlack           = 10 ,
        clStat[csRslt11 ] = clNavy            ;//clBlack           = 11 ,
        clStat[csRslt12 ] = clOlive           ;//clBlack           = 12 ,
        clStat[csRslt13 ] = clAqua            ;//clBlack           = 13 ,
        clStat[csRslt14 ] = clSkyBlue         ;//clBlack           = 14 ,
        clStat[csRslt15 ] = clBlack           ;//clBlack           = 15 ,
        clStat[csRslt16 ] = clBlack           ;//clBlack           = 16 ,
        clStat[csNone   ] = clWhite           ;//clWhite           = 17 ,//스트립이나 카세트가 없는 상태.                                            >
        clStat[csMask   ] = clSkyBlue         ;//clSkyBlue         = 18 ,//PKG가 없지만 채크를 해놓은 상태.                                          >  요거 세개가 PKG 없는거.
        clStat[csEmpty  ] = clMoneyGreen      ;//clSilver          = 19 ,//카세트에 스트립이 없는 상태 혹은 더미패드에 토출자국이 없는경우. 프로브끝 >
        clStat[csUnkwn  ] = clAqua            ;//clAqua            = 20 ,//작업전 있는지 없는지 모르는 경우.
        clStat[csWork   ] = clSilver          ;//clBlue            = 21 ,// 작업 끝.
        clStat[csWait   ] = clYellow          ;//clYellow          = 22 ,
        clStat[csFail   ] = clFuchsia         ;//clFuchsia         = 23 ,
        clStat[csEror   ] = clRed             ;//clRed             = 24 ,
        clStat[csGood   ] = clLime            ;//clLime            = 25 ,
        clStat[csEtc    ] = clNone            ;//clNone            = 26 ,
        clStat[csToBuf  ] = clBlack           ;//clBlack           = 27,
        clStat[csFromBuf] = clBlack           ;//clBlack           = 28,
        clStat[csMarking] = clScrollBar       ;
        clStat[csMask   ] = clDkGray          ;

        sMenuName[csRslt0  ] = "RSLT0"     ;
        sMenuName[csRslt1  ] = "RSLT1"     ;
        sMenuName[csRslt2  ] = "RSLT2"     ;
        sMenuName[csRslt3  ] = "RSLT3"     ;
        sMenuName[csRslt4  ] = "RSLT4"     ;
        sMenuName[csRslt5  ] = "RSLT5"     ;
        sMenuName[csRslt6  ] = "RSLT6"     ;
        sMenuName[csRslt7  ] = "RSLT7"     ;
        sMenuName[csRslt8  ] = "RSLT8"     ;
        sMenuName[csRslt9  ] = "RSLT9"     ;
        sMenuName[csRslt10 ] = "RSLT10"    ;
        sMenuName[csRslt11 ] = "RSLT11"    ;
        sMenuName[csRslt12 ] = "RSLT12"    ;
        sMenuName[csRslt13 ] = "RSLT13"    ;
        sMenuName[csRslt14 ] = "RSLT14"    ;
        sMenuName[csRslt15 ] = "RSLT15"    ;
        sMenuName[csRslt16 ] = "RSLT16"    ;
...
//---------------------------------------------------------------------------
void __fastcall CArray::Load(bool IsLoad , FILE *fp , int _iNo)
{
    //Local Var.
    TUserINI   UserINI;
    AnsiString sPath  ;
