...
//    LotInfo.iLastTrayCnt = (DM.ARAY[riTRY].GetCntStat(csWork) * OM.DevInfo.iULDPkgCnt) + DM.ARAY[riUPK].GetCntStat(csWork); //TODO :: ??
...
//    LDR._iMgzCnt = 0 ; //안대서 추가 하긴 하는데 이게 여기가 맞는지 모르겟넹.
    ...
    //TODO :: SetStep == 1이면 UnLoaderTrayMove를 하는데 SetStep을 0으로 초기화 해주는게 trayMove를 해야지만 되기 떄문에
    //Lot End시에 Tray Step을 초기화 해줌. JS
    DM.ARAY[riTRY].SetStep(0);

    //WriteLotLog();
    TrayInfo.iTrayNo         = 0;
    TrayInfo.iTrayGoodCavity = 0;
    TrayInfo.iTrayBadCavity  = 0;
...
void __fastcall CLot::LoadLotInfo()
{
    ...
    UserINI.Load(Path.c_str() , "LotInfo" , "iLastTrayCnt" , LotInfo.iLastTrayCnt);
...
//    UserINI.Save(Path.c_str() , "LotInfo" , "iCntHeatFail" , LotInfo.iCntHeatFail);
    UserINI.Save(Path.c_str() , "LotInfo" , "iLastTrayCnt" , LotInfo.iLastTrayCnt);
}

void __fastcall CLot::LoadTrayInfo()
{
    //Local Var.
    TUserINI   UserINI;
    AnsiString Path;
    ...
    //Make Dir.
    Path = ExtractFilePath(Application->ExeName) + "SeqData\\" + "TrayInfo.ini";

    //Current Lot Informations.
    UserINI.Load(Path.c_str() , "Member " , "m_bLotOpen      " , m_bLotOpen               );
    UserINI.Load(Path.c_str() , "LotInfo" , "iTrayNo         " , TrayInfo.iTrayNo         );
    UserINI.Load(Path.c_str() , "LotInfo" , "iTrayGoodCavity " , TrayInfo.iTrayGoodCavity );
    UserINI.Load(Path.c_str() , "LotInfo" , "iTrayBadCavity  " , TrayInfo.iTrayBadCavity  );
}
void __fastcall CLot::SaveTrayInfo()
{
    //Local Var.
    TUserINI   UserINI;
    AnsiString Path;
    ...
    //Current Lot Informations.
    UserINI.Save(Path.c_str() , "Member " , "m_bLotOpen      " , m_bLotOpen               );
    UserINI.Save(Path.c_str() , "LotInfo" , "iTrayNo         " , TrayInfo.iTrayNo         );
    UserINI.Save(Path.c_str() , "LotInfo" , "iTrayGoodCavity " , TrayInfo.iTrayGoodCavity );
    UserINI.Save(Path.c_str() , "LotInfo" , "iTrayBadCavity  " , TrayInfo.iTrayBadCavity  );
}

void __fastcall CLot::LoadPSBInfo()
{
    //Local Var.
    TUserINI   UserINI;
    AnsiString Path;
    ...
    //Make Dir.
    Path = ExtractFilePath(Application->ExeName) + "SeqData\\" + "PSBInfo.ini";

    //Current Lot Informations.
    UserINI.Load(Path.c_str() , "Member " , "m_bLotOpen     " , m_bLotOpen             );
    UserINI.Load(Path.c_str() , "PSBInfo" , "iPSBStripCnt   " , PSBInfo.iPSBStripCnt   );
    UserINI.Load(Path.c_str() , "PSBInfo" , "iPSBGoodCavity " , PSBInfo.iPSBGoodCavity );
    UserINI.Load(Path.c_str() , "PSBInfo" , "iPSBBadCavity  " , PSBInfo.iPSBBadCavity  );
}
void __fastcall CLot::SavePSBInfo()
{
    //Local Var.
    TUserINI   UserINI;
    AnsiString Path;
    ...
    //Current Lot Informations.
    UserINI.Save(Path.c_str() , "Member " , "m_bLotOpen     " , m_bLotOpen             );
    UserINI.Save(Path.c_str() , "PSBInfo" , "iPSBStripCnt   " , PSBInfo.iPSBStripCnt   );
    UserINI.Save(Path.c_str() , "PSBInfo" , "iPSBGoodCavity " , PSBInfo.iPSBGoodCavity );
    UserINI.Save(Path.c_str() , "PSBInfo" , "iPSBBadCavity  " , PSBInfo.iPSBBadCavity  );
}
...
//    UserINI.Load(Path.c_str() , "DayInfo" , "iCntHeatFail" , DayInfo.iCntHeatFail);
    UserINI.Load(Path.c_str() , "DayInfo" , "iLastTrayCnt" , DayInfo.iLastTrayCnt);
}
...
//    UserINI.Save(Path.c_str() , "DayInfo" , "iCntHeatFail" , DayInfo.iCntHeatFail);
    UserINI.Save(Path.c_str() , "DayInfo" , "iLastTrayCnt" , DayInfo.iLastTrayCnt);
}
...
//    Temp += (AnsiString)DayInfo.iCntHeatFail + ",";
    Temp += (AnsiString)DayInfo.iLastTrayCnt + ",";
    Temp += (AnsiString)"\r\n"                    ;
...
void __fastcall CLot::WriteLotLog()
{
    ...
    if(!UserFile.FileExist(Path)) {
        ...
        DayInfo.iLastTrayCnt= LotInfo.iLastTrayCnt ;
...
//    Temp += (AnsiString)LotInfo.iCntHeatFail + ",";
    Temp += (AnsiString)LotInfo.iLastTrayCnt + ",";
...
void __fastcall CLot::WriteTrayLog()
{
    //Local Var.
    int        hFile   ;
    AnsiString Path    ;
    ...
    CurrDateTime = Now();
    Temp         = CurrDateTime.CurrentDate().FormatString("yyyymmdd");
    Path         = TRAY_INFO_FOLDER + Temp ;//".csv";
    tTemp.Val    = LotInfo.dStartTime ;
    sDate        = tTemp.FormatString("hh_nn_ss") ;
    ...
    Path         = TRAY_INFO_FOLDER + Temp + "\\" + "("+ sDate +")" + LotInfo.sLotNo + ".csv";
    ...
    //Save.
    Temp  = (AnsiString)TrayInfo.iTrayNo         + "," ;
    Temp += (AnsiString)TrayInfo.iTrayGoodCavity + "," ;
    Temp += (AnsiString)TrayInfo.iTrayBadCavity  + "," ;
    Temp += (AnsiString)"\r\n"                    ;
...
void __fastcall CLot::WritePSBLog()
{
    //Local Var.
    int        hFile   ;
    AnsiString Path    ;
    ...
    tTemp.Val    = LotInfo.dStartTime ;
    sDate        = tTemp.FormatString("hh_nn_ss") ;
    Path         = PSB_FAIL_DATA_FOLDER + Temp + "\\" + "("+ sDate +")" + LotInfo.sLotNo ;//".csv";

    if(!UserFile.FileExist(Path)) {
        CreateDir(Path);
    }

    Path         = PSB_FAIL_DATA_FOLDER + Temp + "\\" + "("+ sDate +")" + LotInfo.sLotNo + "\\" + "("+ sDate +")" + LotInfo.sLotNo + ".csv";
    ...
    //Save.
    Temp  = (AnsiString)PSBInfo.iPSBStripCnt   + "," ;
    Temp += (AnsiString)PSBInfo.iPSBGoodCavity + "," ;
    Temp += (AnsiString)PSBInfo.iPSBBadCavity  + "," ;
    Temp += (AnsiString)"\r\n"                    ;
...
void __fastcall CLot::UpdateTrayDate(TStringGrid * _sgDate)
{
    UserFile.GridSearchDir(TRAY_INFO_FOLDER , _sgDate , 1 , false);     // 디렉토리 읽어와서 날짜와 알파벳 순으로 정렬
}
...
void __fastcall CLot::UpdateTrayLotName(TStringGrid * _sgDate , TStringGrid * _sgLot)
{
    AnsiString sDay ,sPath ;
    sDay = _sgDate->Cells[1][_sgDate -> Row] ;
    sPath = TRAY_INFO_FOLDER + sDay + "\\" ;
    UserFile.GridSearchFile(sPath , _sgLot , 1 , true);     // 디렉토리 읽어와서 날짜와 알파벳 순으로 정렬
}
...
void __fastcall CLot::DispTrayDate(TStringGrid * _sgDate , TStringGrid * _sgLot , TStringGrid * _sgLotInfo)
{
    AnsiString sDay  = _sgDate -> Cells[1][ _sgDate -> Row] ;
    AnsiString sPath = TRAY_INFO_FOLDER + sDay + "\\" + _sgLot -> Cells[1][ _sgLot -> Row] ;
    ...
    _sgLotInfo -> ColCount         = 4   ;
    _sgLotInfo -> DefaultColWidth  = 80  ;
    _sgLotInfo -> DefaultRowHeight = 25  ;
    _sgLotInfo -> ColWidths[0]     = 30  ;
    _sgLotInfo -> ColWidths[1]     = 200 ;
    _sgLotInfo -> ColWidths[2]     = 200 ;
    _sgLotInfo -> ColWidths[3]     = 200 ;
...
bool __fastcall CLot::WriteArayData(EN_ARAY_ID riId ) //패스를 빼야 되나 ...매번 고쳐야 되자나...
{
...
//    if(
    ...
    iSlotNo = DM.ARAY[riId].GetID().ToIntDef(999999)%1000 ;
    sSlotNo.sprintf("%02d",iSlotNo);
    iMgzNo  = DM.ARAY[riId].GetID().ToIntDef(999999)/1000 ;
    ...
    //아오 ㅄ같네 진짜.
    sPath = LOG_FOLDER + CurrDateTime.CurrentDate().FormatString("yyyymmdd") + "\\" + "(" +sDate + ")" + sLotNo + "\\" + (String)iMgzNo ;
...
bool __fastcall CLot::WritePSBData(EN_ARAY_ID riId ) //대략 시간 없어서 그냥 추가.
{
    Trace("WriteArayData","1111111");
    //Local Var.
    TUserINI   UserINI;
    ...
    //아오 ㅄ같네 진짜. 그르게.
    sPath = PSB_FAIL_DATA_FOLDER + CurrDateTime.CurrentDate().FormatString("yyyymmdd") + "\\" ;

    if(!DirectoryExists(sPath)) UserFile.CreateDir(sPath.c_str());

    sPath = sPath + sLotNo + "_" + (String)iMgzNo + "_" + sSlotNo +".ini" ;
