...
//    iLen = Rs232_H->ReadData(_cbInQue, RcvBuff);
    ...
    if(sRcvMsg != "") memcpy(Rs232_E->bData , RcvBuff , sizeof(Rs232_E->bData));
    Trace("procComE",(char*)Rs232_E->bData);
...
void CRs232Man::Init()
{
    ...
    if(!Rs232_E->Open(0)) ShowMessage("Dispenser Rs232 Port Open Fail") ;
    else                  Rs232_E->CallBackReg(Dispenser);

    ChangeCh(0);
    SetTime(OM.DevOptn.iDisprdelay);
...
// ShotEnd 추가 2014.12.08 juhyeon
void CRs232Man::ShotEnd()
{
    BYTE RcvBuff[2];
    memset(&RcvBuff, 0 , sizeof(RcvBuff));
    ...
    RcvBuff[0] = 0x54 ;   // STEAD MODE에서 사용 가능.JS 12/12.
    RcvBuff[1] = 0xAA ;
