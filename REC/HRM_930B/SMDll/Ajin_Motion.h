...
struct SAjinParaUi {
    ...
    TCheckBox * cbUseGntr       ; TLabel * lbGntrSubAdd  ; TEdit * edGntrSubAdd  ;
    TComboBox * cbGntrMathod    ; TLabel * lbGntrOfs     ; TEdit * edGntrOfs     ;
                                  TLabel * lbGntrOfsRange; TEdit * edGntrOfsRange;
...
class CAjinMotion
{
    public:
        ...
        struct SAjinPara{ //Board Level Direct Access AjinPara. (INI)
            ...
            bool   bEncReverse   ; //CFS20set_enc_reverse      // 엔코더 입력 수치 반전. RM에서는 안쓴다.
            bool   bUse360Count  ; //0~360도 까지만 포지션을 쓰게 함. RM에서만 해놨음.
            //Gentry Add.
            bool   bUseGntr      ; //Gantry 사용 유무.
            int    iGntrMathod   ; //Gantry Home Type.
            int    iGntrSubAdd   ; //Sub Gantry Address.
            double dGntrOfs      ; //Gantry Off Set.
            double dGntrOfsRange ; //Gantry Off Set Range.
...
//        SAjinPara AjinPara   ;
    ...
    public:
        ...
        void SetSignalInpos(DWORD _dwSel);  //0 : 비접  1:에이접 3:안씀.
	...
	//Save AjinPara Functions.
        ...
        //겐트리..구동.
        void SetGantryEnable    (int _iSlaveAxis , int _iHomeUse , double _dSlaveOffset , double _dSlOffsetRange);
        void SetGantryDisable   (int _iSlaveAxis);

        //겐트리 구동 때문에 써야되는  홈함수들.
        void SetHomeMethod      (int _iHomeDir , DWORD _dwHomeSignal , DWORD _dwUseZ , double _dHomeClrTime , double _dHomeOffset);
        void SetHomeVel         (double _dVel1 , double _dVel2 , double _dVel3 , double _dLast ,double _dAcc1 , double _dAcc2);
        void SetHomeStart       ();
        void GetHomeResult      (DWORD * _dwResult);
};
