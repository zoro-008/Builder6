...
enum EN_DATA_ID {
    diXData     = 0 ,  //Vision X Data
    diYData     = 1 ,  //Vision Y Data
    MAX_DATA_ID
};
...
//        __fastcall ~CChip(void);
        ...
        void __fastcall SetData(EN_DATA_ID _iData, double _dData){
            dData[_iData] = _dData;
        }
        double __fastcall GetData(EN_DATA_ID _iData){
            return dData[_iData];
        }

        void __fastcall SetID(String _sID){
            sID = _sID;
        }
        String __fastcall GetID(){
            return sID;
        }
        ...
        //Operator
        void operator = (CChip& rhs) {
//            m_iStat    = csNone;
            memcpy(m_iSubStat , rhs.m_iSubStat , sizeof(EN_CHIP_STAT) * MAX_SUBSTAT);
            memcpy(dData      , rhs.dData      , sizeof(double) * MAX_DATA_ID);
            sID = rhs.sID ;
            m_iStat = rhs.GetStat() ;
            //memcpy(this , &rhs ,sizeof(CChip)); /*m_iStat = rhs.GetStat() ;*/
        }
};
...
class CArrayData
{
    ...
    public:
        ...
        EN_CHIP_STAT __fastcall GetStat   (int r , int c                    ) { CHECK_RETURN csNone    ; return CHPS[r][c].GetStat()         ; }
        ...
        bool         __fastcall CheckAllStat (EN_CHIP_STAT Stat1,EN_CHIP_STAT Stat2) {
            for(int r = 0 ; r < m_iMaxRow ; r++)
                for (int c = 0 ; c < m_iMaxCol ; c++)
                    if (!CheckStat (r,c,Stat1)&&!CheckStat (r,c,Stat2)) return false;
            return true;
        }
        bool         __fastcall CheckAllStat (EN_CHIP_STAT Stat1,EN_CHIP_STAT Stat2,EN_CHIP_STAT Stat3) {
            for(int r = 0 ; r < m_iMaxRow ; r++)
                for (int c = 0 ; c < m_iMaxCol ; c++)
                    if (!CheckStat (r,c,Stat1)&&!CheckStat (r,c,Stat2)&&!CheckStat (r,c,Stat3)) return false;
            return true;
        }
