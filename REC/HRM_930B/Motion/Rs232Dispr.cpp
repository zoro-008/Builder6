...
CRs232Dispr   Rs232_DisprFt;
CRs232Dispr   Rs232_DisprRr;
...
void CRs232Dispr::Init(EN_RS232 _iId)
{
    m_iId = _iId ;
    m_iSeqCycle = 0;
    m_iPreSeqCycle = 0;
    ...
    if(!m_pRS232C->Open(_iId)) ShowMessage("RS-232C Dispensor Open Failed!");
    else                       m_pRS232C->CallBackReg(RcvDisprData);


    //GetDispData(OM.DevOptn.iDspChFt+1);
    //CDelayTimer TimeOut;
    //TimeOut.Clear();
