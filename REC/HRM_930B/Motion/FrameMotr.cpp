...
bool TFraMotr::SetIdType(EN_MOTR_ID _iId , EN_MTOR_DIRECTION _iType )
{
    ...
    */
    ...
    const bool bUseEpipView = true ;
    const int iLeftPos  = 2   ;
    const int iRightPos = 142 ;
    ...
    else if(_iType == mdRL)
    {
        ...
        if(bUseEpipView){
            btNeg -> Left = iRightPos ; btNeg -> Layout = blGlyphRight ;
            btPos -> Left = iLeftPos  ; btPos -> Layout = blGlyphLeft  ;
        }
    }
    else if(_iType == mdBF)
    {
...
bool TFraMotr::CheckSafe(int _iId)
{
    ...
    if(_iId == (int)miLTL_XGenFt){}
    if(_iId == (int)miLTL_XGenRr){}
    if(_iId == (int)miLTL_YDispr){}
    if(_iId == (int)miLTL_ZDispr){}
    if(_iId == (int)miRTL_XGenFt){}
    if(_iId == (int)miRTL_XGenRr){}
    if(_iId == (int)miRTL_YVisn ){}
    if(_iId == (int)miRTL_ZVisn ){}
    if(_iId == (int)miLTL_YVisn ){}
    if(_iId == (int)miRTL_YTrspr){}
    if(_iId == (int)miSTG_XStage){}
    if(_iId == (int)miSTG_YStage){}
    if(_iId == (int)miLTL_ZVisn ){}
    if(_iId == (int)miIDX_XIndxR){}
    if(_iId == (int)miRTL_ZTrsfr){}
    if(_iId == (int)miRTL_YConvs){}
    if(_iId == (int)miSTG_YConvs){}
    if(_iId == (int)miIDX_TRotat){}
    if(_iId == (int)miLDR_YConvR){}
    if(_iId == (int)miLDR_ZElevR){}
    if(_iId == (int)miLDR_ZElevF){}
    if(_iId == (int)miSpare1    ){}
    if(_iId == (int)miULD_ZElevt){}
    if(_iId == (int)miSpare2    ){}

    if(!bRet){
        Trace(MT_GetName(_iId), sMsg.c_str());
  ...
  //  if(((int)miRAL_XVStp == _iId) && (IO_GetX(xRAL_LMask) || IO_GetX(xRAL_VMask))){
  //      bRet = false;
  //      Trace("miRAL_YCvsn", "CheckSafe");
  //      FM_MsgOk("WARNING" , "Exist Rail on MaskBlock");
  //  }
      ...
      TMouseButton Button, TShiftState Shift, int X, int Y)
{
    ...
    if(!cbSlow -> Checked){
             ...
             if(iUnitType == utJog   ) MT_JogN    (m_iId) ;
        else if(iUnitType == utMove  ) MT_GoInc   (m_iId , -dUnit ,MT_GetManVel(m_iId)/10.0) ;
        else if(iUnitType == utPitch ) MT_GoInc   (m_iId , -dPitch,MT_GetManVel(m_iId)/10.0) ;
        else return ;
    }
}
//---------------------------------------------------------------------------
...
void __fastcall TFraMotr::cbSlowClick(TObject *Sender)
{

    if(cbSlow -> Checked) {
        FM_MsgOk(MT_GetName(m_iId) , "상대이동값이 10% 로 감속됩니다.");
    }
    else {
        FM_MsgOk(MT_GetName(m_iId) , "상대이동값이 ManualSpeed로 변경 됩니다.");
    }
}
//---------------------------------------------------------------------------

void __fastcall TFraMotr::btStopClick(TObject *Sender)
{
    MT_Stop(m_iId);
}
//---------------------------------------------------------------------------
