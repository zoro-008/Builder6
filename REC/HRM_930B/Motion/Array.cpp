...
void __fastcall CArrayData::Load(bool IsLoad)
{
    ...
    if(IsLoad) {
        ...
        for (int r = 0 ; r < m_iMaxRow ; r++) {
            for (int c = 0 ; c < m_iMaxCol ; c++) {
                sIndex.sprintf("R%03d_C%03d_ID", r , c ) ;
                UserINI.Load(sPath, m_sName , sIndex, sTemp );
                CHPS[r][c].SetID(sTemp);
            }
        }
        OnChange();
    }
        ...
        UserINI.Save(sPath, m_sName , "m_sID"     , m_sID               );
        ...
        for (int r = 0 ; r < m_iMaxRow ; r++) {
            for (int c = 0 ; c < m_iMaxCol ; c++) {
                sIndex.sprintf("R%03d_C%03d_ID", r , c ) ;
                sTemp = CHPS[r][c].GetID();
                UserINI.Save(sPath, m_sName , sIndex, sTemp );
...
void __fastcall CArray::ImageMouseUp(TObject *Sender,TMouseButton Button, TShiftState Shift, int X, int Y)
{
    ...
    else if(Button == mbRight && Shift.Contains(ssCtrl)){
        FrmArrayInfo -> m_sArayName    = m_sName    ;
        FrmArrayInfo -> m_sArayLotNo   = m_sLotNo   ;
        FrmArrayInfo -> m_sArayID      = m_sID      ;
        FrmArrayInfo -> m_iArayStep    = m_iStep    ;
        FrmArrayInfo -> m_iAraySubStep = m_iSubStep ;

        FrmArrayInfo -> m_sChipID      = CHPS[ClickStat.iSelR2][ClickStat.iSelC2].GetID() ;
        FrmArrayInfo -> m_iChipStat    = CHPS[ClickStat.iSelR2][ClickStat.iSelC2].GetStat() ;

        FrmArrayInfo -> m_iCol         = ClickStat.iSelC2 ;
        FrmArrayInfo -> m_iRow         = ClickStat.iSelR2 ;

        FrmArrayInfo -> UpdateVal() ;
        FrmArrayInfo -> Show() ;
...
void __fastcall CArray::UpdateArayNrm()
{
    ...
    if(CheckAllStat(csNone)) {
        ...
        //View Chip Info.
        imArray -> Canvas -> Font -> Name = "Arial";
        ...
        for (int r = 0 ; r < uRow ; r++) {
            for (int c = 0 ; c < uCol ; c++) {
                ...
                imArray -> Canvas -> TextOut(uX1 , uY1 , CHPS[sRow][sCol].GetID().c_str());
            }
        }
    }
    ...
    //sprintf(str , GetName().c_str() );
    //imArray -> Canvas -> TextOutA(1 , 1 , GetName().c_str());
