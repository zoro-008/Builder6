...
bool CErrData::SetErr(int _iErrNo , AnsiString _sErrName , AnsiString _sErrMsg , AnsiString _sLotId)
{
    ...
    //기존에 있던것들 지우기.
    UserFile.ClearDirDate(ERR_FOLDER , Now() - 30 * 12); //90일.
...
bool CLotData::SaveArrayData(TData & _tData , int _iMgzNo , int _iSlotNo , CArray & _rData)
{
    ...
    //기존에 있던것들 지우기.
    UserFile.ClearDirDate(LOT_FOLDER , Now() - 30 * 12); //90일.
    ...
    UserINI.Save(sPath.c_str() , "Data" , "dUPEH     " , _tData.dUPEH     );
    UserINI.Save(sPath.c_str() , "Data" , "dUPH      " , _tData.dUPH      );
...
void CLotData::LoadSaveLotIni(bool _bLoad)
{
    //Local Var.
    TUserINI   UserINI;
    AnsiString sPath  ;
    ...
    if(_bLoad) {
        ...
        UserINI.Load(sPath.c_str() , "Data" , "dUPEH     " , m_tData.dUPEH     );
        UserINI.Load(sPath.c_str() , "Data" , "dUPH      " , m_tData.dUPH      );
        ...
        UserINI.Save(sPath.c_str() , "Data" , "dUPEH     " , m_tData.dUPEH     );
        UserINI.Save(sPath.c_str() , "Data" , "dUPH      " , m_tData.dUPH      );
...
void CLotData::DispWorkDateList(TStringGrid * _sgDateList)
{
    ...
    UserFile.GridSearchFile(WRK_FOLDER , _sgDateList , 1 , true);     // 디렉토리 읽어와서 날짜와 알파벳 순으로 정렬
}
...
void CLotData::DispLotList(AnsiString _sDate , TStringGrid * _sgLotList)
{
    ...
    _sgLotList -> Cells[4][0] = "UPH"      ;             _sgLotList -> ColWidths[4] = _sgLotList -> Cells[4][0].Length() * 15 ;
    ...
    if (FindFirst(sSerchFile , faAnyFile , sr) == 0) {
        do {
            if ((sr.Attr & faDirectory) != sr.Attr) {
                ...
                sTemp = sTemp.sprintf("%0.3f",tData.dUPEH);
                UserINI.Load(sIniPath.c_str() , "Data" , "dUPEH     " , tData.dUPEH     ); _sgLotList -> Cells[3][iLotCnt+1] = sTemp     ;
                sTemp = sTemp.sprintf("%0.3f",tData.dUPH);
                UserINI.Load(sIniPath.c_str() , "Data" , "dUPH      " , tData.dUPH      ); _sgLotList -> Cells[4][iLotCnt+1] = sTemp     ;
...
void CLotData::DispLotInfo(TStringGrid * _sgLotInfo) //오퍼레이션 창용.
{
    ...
    sTemp.sprintf("%.4f",m_tData.dUPEH );
    //_sgLotInfo -> Cells[0][1] = "UPEH     "  ;                                       _sgLotInfo -> Cells[1][1] = sTemp     ;
    sTemp.sprintf("%.4f",m_tData.dUPH );
    _sgLotInfo -> Cells[0][1] = "UPH      "  ;                                       _sgLotInfo -> Cells[1][1] = sTemp     ;
    ...
    _sgLotInfo -> Cells[0][6] = "CycleTime"  ;                                       _sgLotInfo -> Cells[1][6] = String((int)m_tData.dTickTime) + "sec";

    sTemp.sprintf("%.4f",m_tData.dUPH ? 1/m_tData.dUPH : 0 );
    _sgLotInfo -> Cells[0][7] = "HPU      "  ;                                       _sgLotInfo -> Cells[1][7] = sTemp + " hour";
...
void CLotData::Update(EN_SEQ_STAT Stat)
{
    ...
    static bool bPreWorkEndAll = DM.ARAY[riSTG].CheckAllStat(csWorkEnd);
    ...
    static bool bPreSTGExist = !DM.ARAY[riSTG].CheckAllStat(csNone);
...
void CDayData::LoadSaveDayIni(bool _bLoad , TData & _tData)
{
    //Local Var.
    TUserINI   UserINI;
    AnsiString sPath;
    ...
    if(_bLoad) {
        ...
        UserINI.Load(sPath.c_str() , "Data" , "dUPEH     " , _tData.dUPEH      );
        UserINI.Load(sPath.c_str() , "Data" , "dUPH      " , _tData.dUPH       );
...
void CDayData::DispDayInfo (TStringGrid * _sgDayInfo) //오퍼레이션 창용.
{
    ...
    //sTemp.sprintf("%.3f",m_tData.dUPEH);
    //_sgDayInfo -> Cells[0][4] = "UPEH     ";                                      _sgDayInfo -> Cells[1][4] = sTemp    ;
    sTemp.sprintf("%.4f",m_tData.dUPH);
    _sgDayInfo -> Cells[0][4] = "UPH      ";                                      _sgDayInfo -> Cells[1][4] = sTemp ;
