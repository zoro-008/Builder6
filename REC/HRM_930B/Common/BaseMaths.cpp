...
double __stdcall CMath::GetLineA(double y, double  x, double b)
{
    if(x==0){
             if((y-b) ==0 ) return      0.0 ;
        else if((y-b) > 0 ) return  DBL_MAX ;   //오차 발생.
        else if((y-b) < 0 ) return -DBL_MAX ;   //오차 발생.
    }
    return (y-b)/x;
}
...
{
    //if(x2==x1)return DBL_MAX ; //오차발생. //일직선상에 있으면 오류남.X점이 수직으로 있을경우.
    if(x2==x1){
             if((y2-y1) ==0 ) return      0.0 ;
        else if((y2-y1) > 0 ) return  DBL_MAX ;   //오차 발생.
        else if((y2-y1) < 0 ) return -DBL_MAX ;   //오차 발생.
    }
    return (y2-y1)/(x2-x1);
}
...
double __stdcall CMath::GetLengthPntToLine(double _tPntX , double _tPntY , double _dLine1X, double _dLine1Y , double _dLint2X , double _dLine2Y)
{
    if(_dLine1X == _dLint2X){//직선 기울기가 무한대...
        return fabs(_tPntX - _dLine1X) ;
    }

    if(_dLine1Y == _dLine2Y){
        return fabs(_tPntY - _dLine1Y) ;
    }

    double dA = GetLineA(_dLine1X, _dLine1Y , _dLint2X , _dLine2Y);
    double dB = GetLineA(_dLine1X, _dLine1Y , _dLint2X , _dLine2Y);

    TDPoint tPnt ;
    tPnt.x = _tPntX ;
    tPnt.y = _tPntY ;

    return CMath::GetLengthPntToLine(tPnt , dA , dB) ;

}


//썅썅 오버플로우 문제 생긴다. _dA가 무한대 일때....
double __stdcall CMath::GetLengthPntToLine(TDPoint _tPnt , double _dA , double _dB)
{
    double A,B,C ;
    double fRet ;
...
bool __stdcall CMath::GetLineABFromPntByLeastSqure(int _iPntCnt , double * _pPntX , double * _pPntY , double & _dA , double & _dB)
{
    if(_iPntCnt < 2) return false ; //2개 이상이여야지만 데이터가 나옴.
    ...
    for (int i = 0; i < _iPntCnt; ++i){
        ...
        sxx += (_pPntX[i] * _pPntX[i]) ;
        sxy += (_pPntX[i] * _pPntY[i]) ;
    }
    ex = sx/_iPntCnt ;
    ey = sy/_iPntCnt ;
...
bool  CMath::IsPntInPolygonOutRect(TDPoint *_pPoints, int _iPntCnt, TDPoint _tPnt , TDRect &_tOutRect)
{
    ...
    _tOutRect.left   = tOutRect.left   = iXMin ;
    _tOutRect.top    = tOutRect.top    = iYMin ;
    _tOutRect.right  = tOutRect.right  = iXMax ;
    _tOutRect.bottom = tOutRect.bottom = iYMax ;
...
bool CMath::IsPntInPolygon(TDPoint *_pPoints, int _iPntCnt, TDPoint _tPnt)
{
    ...
    if (!CMath::IsPntInPolygonOutRect(_pPoints,_iPntCnt,_tPnt,r)) return false ;

    pt1 = pt2 = _tPnt ;
    pt2.x = r.right + 1 ; //최외각 사각형에서 오른쪽으로 1만 가면 관통된다.
                          ...
                          double   *_pCntrX , double *_pCntrY  , double *_pRadius , double *_pScore)
{
    ...
    *_pCntrX = *_pCntrY = *_pRadius = *_pScore = 0.0 ;
    ...
    fDevMinSum    = DBL_MAX ; //~(1<<(sizeof(double)*8-1)) 20150519 수정.
    iGapInCntMax = 0   ;
    int a = 0 , b = 0 , c = 0 ;
