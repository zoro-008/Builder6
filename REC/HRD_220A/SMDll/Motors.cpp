...
void CMotors::ContiSetAxisMap(int _iCoord, int _iAxisCnt, int *_ipAxisNo) // _ipAxisNo 값이 쓸모가 없어지는... 이 문제 해결해야함...
{
    ...
    m_iMasterAxisPhyAdd = 9999 ; // 마스터 축 피지컬 주소 값 9999로 초기화
    ...
    for(int i = 0; i < _iAxisCnt ; i++){
        ...
        if(iPhyscAdd < m_iMasterAxisPhyAdd) m_iMasterAxisPhyAdd = iPhyscAdd ; //가장 작은 축이 Master가 되게
...
//    for (int j = 0 ; '\0' != _dpEndPos[j] ; j++) { _dpEndPos[j] = Motor[m_iMasterAxisPhyAdd] -> GetPulseFromUnit(_dpEndPos[j]) ; }
    ...
    CAjinMotion::LineMove(_iCoord, _dpEndPos, Motor[m_iMasterAxisPhyAdd] -> GetPulseFromUnit(_dVel), // 인자 * ( Pulse / Unit ), Unit를 Pulse로 바꾸어줌
                                              Motor[m_iMasterAxisPhyAdd] -> GetPulseFromUnit(_dAcc), // pulses / Sec^2 ??
                                              Motor[m_iMasterAxisPhyAdd] -> GetPulseFromUnit(_dDec)) ;

    delete []ipRealAxisNo ;

    Trace("LineMove _dVel" , String(_dVel).c_str());
    Trace("LineMove _dVel Pulse" , String(Motor[m_iMasterAxisPhyAdd] -> GetPulseFromUnit(_dVel)).c_str());
}
...
void CMotors::CircleCenterMove(int _iCoord, int *_ipAxisNo, int _iAxisCnt, double *_dpCenterPos, double *_dpEndPos, double _dVel, double _dAcc, double _dDec, DWORD _dwCWDir)
{
    ...
    for (int j = 0 ; '\0' != _dpCenterPos[j] ; j++) { _dpCenterPos[j] = Motor[m_iMasterAxisPhyAdd] -> GetPulseFromUnit(_dpCenterPos[j]) ;
                                                      _dpEndPos[j]    = Motor[m_iMasterAxisPhyAdd] -> GetPulseFromUnit(_dpEndPos[j]   ) ; }

    CAjinMotion::CircleCenterMove(_iCoord, ipRealAxisNo, _dpCenterPos, _dpEndPos, Motor[m_iMasterAxisPhyAdd] -> GetPulseFromUnit(_dVel),
                                                                                     Motor[m_iMasterAxisPhyAdd] -> GetPulseFromUnit(_dAcc),
                                                                                     Motor[m_iMasterAxisPhyAdd] -> GetPulseFromUnit(_dDec), _dwCWDir) ;
    delete []ipRealAxisNo ;
}
...
void CMotors::CirclePointMove(int _iCoord, int *_ipAxisNo, int _iAxisCnt, double *_dpMidPos, double *_dpEndPos, double _dVel, double _dAcc, double _dDec, int _iArcCircle)
{
    ...
    for (int j = 0 ; '\0' != _dpMidPos[j] ; j++) { _dpMidPos[j] = Motor[m_iMasterAxisPhyAdd] -> GetPulseFromUnit(_dpMidPos[j]) ;
                                                   _dpEndPos[j] = Motor[m_iMasterAxisPhyAdd] -> GetPulseFromUnit(_dpEndPos[j]) ; }

    CAjinMotion::CirclePointMove(_iCoord, ipRealAxisNo, _dpMidPos, _dpEndPos, Motor[m_iMasterAxisPhyAdd] -> GetPulseFromUnit(_dVel),
                                                                                 Motor[m_iMasterAxisPhyAdd] -> GetPulseFromUnit(_dAcc),
                                                                                 Motor[m_iMasterAxisPhyAdd] -> GetPulseFromUnit(_dDec), _iArcCircle);
    delete []ipRealAxisNo ;
}
...
void CMotors::CircleRadiusMove(int _iCoord, int *_ipAxisNo, int _iAxisCnt, double _dRadius, double *_dpEndPos, double _dVel, double _dAcc, double _dDec, DWORD _dwCWDir, DWORD _dwShortDistance)
{
    ...
    CAjinMotion::CircleRadiusMove(_iCoord, ipRealAxisNo, _dRadius, _dpEndPos, Motor[m_iMasterAxisPhyAdd] -> GetPulseFromUnit(_dVel),
                                                                                 Motor[m_iMasterAxisPhyAdd] -> GetPulseFromUnit(_dAcc),
                                                                                 Motor[m_iMasterAxisPhyAdd] -> GetPulseFromUnit(_dDec), _dwCWDir, _dwShortDistance);
    delete []ipRealAxisNo ;
}
...
void CMotors::CircleAngleMove(int _iCoord, int *_ipAxisNo, int _iAxisCnt, double *_dCenterPos, double _dAngle, double _dVel, double _dAcc, double _dDec, DWORD _dwCWDir)
{
    ...
    for (int j = 0 ; '\0' != _dCenterPos[j] ; j++) { _dCenterPos[j] = Motor[m_iMasterAxisPhyAdd] -> GetPulseFromUnit(_dCenterPos[j]) ; }

    CAjinMotion::CircleAngleMove(_iCoord, ipRealAxisNo, _dCenterPos, _dAngle, Motor[m_iMasterAxisPhyAdd] -> GetPulseFromUnit(_dVel),
                                                                              Motor[m_iMasterAxisPhyAdd] -> GetPulseFromUnit(_dAcc),
                                                                              Motor[m_iMasterAxisPhyAdd] -> GetPulseFromUnit(_dDec), _dwCWDir);
    delete []ipRealAxisNo ;
