...
__fastcall TFrmHisto::TFrmHisto(TComponent* Owner , CImage * _pImage , CTracker * _pTracker)
    : TForm(Owner)
{
    Left = 100 ;
    Top  = 100 ;
...
void __fastcall TFrmHisto::OnSetImage(CImage * _pImage)
{
    m_pImage   = _pImage   ;
}

void __fastcall TFrmHisto::OnSetTracker(CTracker * _pTracker)
{
    m_pTracker = _pTracker ;
}
//---------------------------------------------------------------------------
...
void __fastcall TFrmHisto::FormShow(TObject *Sender)
{
    ...
    CGdi * Gdi = new CGdi(PView->Handle);
...
void __fastcall TFrmHisto::UpdateView()
{
    ...
    TDRect tDRect = m_pTracker -> GetDRect() ;
    //fRect.left   += *m_pRefOstX ;
    //fRect.Top    += *m_pRefOstY ;
    //fRect.right  += *m_pRefOstX ;
    //fRect.bottom += *m_pRefOstY ;

    TRect rect ;
    rect.left   = tDRect.left   ;
    rect.Top    = tDRect.Top    ;
    rect.right  = tDRect.right  ;
    rect.bottom = tDRect.bottom ;

    m_pImage->SetRect(&rect);
    ...
    //m_pImage -> SetRect(m_CImage , &rect) ; //Image -> SetRect(&rect) ;
    ...
    //if(!iRectHeight==0 || !iRectWidth==0) return ;
