...
Trace("<@SETBUFF>", (String(myRs232c->Para.PortNo+10) + "," + String(myRs232c->Para.PortNo) +"Port CriticalSection Enter Stt").c_str());
EnterCriticalSection(&myRs232c->myCriticalSection);
Trace("<@SETBUFF>", (String(myRs232c->Para.PortNo+10) + "," + String(myRs232c->Para.PortNo) +"Port CriticalSection Enter End").c_str());
...
//        if(CallArr[myRs232c->myID] != NULL)
        {
            if(dwEventMask & EV_RXCHAR)
            {
                ...
                myRs232c->CallArr(CommStatus.cbInQue);
            }
...
Trace("<@SETBUFF>", (String(myRs232c->Para.PortNo+10) + "," + String(myRs232c->Para.PortNo) +"Port CriticalSection Leave Stt").c_str());
LeaveCriticalSection(&myRs232c->myCriticalSection);
Trace("<@SETBUFF>", (String(myRs232c->Para.PortNo+10) + "," + String(myRs232c->Para.PortNo) +"Port CriticalSection Leave End").c_str());
    }
}
...
bool __fastcall TRS232C::CallBackReg(CallbackKey _CallbackKey)
{
    ...
    CallArr = _CallbackKey ;
    return true ;
}

bool __fastcall TRS232C::Open(TPara _tPara)
{
    char DeviceName[10];
    ...
    //-- Device Name 생성
    sprintf(DeviceName, "COM%d", _tPara.PortNo);
    ...
    //-- 통신 속도등의 DCD값 지정
    myDCB.BaudRate = (DWORD)_tPara.BaudRate;
    myDCB.ByteSize = (BYTE)_tPara.ByteSize;

    switch(_tPara.StopBits)
    {
        case 0:
