...
void __fastcall CSequence::UpdateButton(void) //외부 버튼들 및 윈도우즈 버튼들이 클릭이 되었을때 취합해서 동작진행 시키는 함수.
{
    ...
    if (bStartSw) {
        ...
        if(!OM.CmnOptn.bTempIgnrErrorChk) {
            ...
            if(iTemp[0] > OM.DevOptn.iRailBtmTemp + OM.CmnOptn.dTempErrorTol) FM_MsgOk("ERROR" , (String("Rail Bottom온도가 설정 온도 보다 ")+ OM.CmnOptn.dTempErrorTol + "도 이상 높습니다.").c_str());
            if(iTemp[1] > OM.DevOptn.iRailTopTemp + OM.CmnOptn.dTempErrorTol) FM_MsgOk("ERROR" , (String("Rail Top온도가 설정 온도 보다 "   )+ OM.CmnOptn.dTempErrorTol + "도 이상 높습니다.").c_str());
            if(iTemp[2] > OM.DevOptn.iStageTemp   + OM.CmnOptn.dTempErrorTol) FM_MsgOk("ERROR" , (String("Stage 온도가 설정 온도 보다 "     )+ OM.CmnOptn.dTempErrorTol + "도 이상 높습니다.").c_str());

            if(iTemp[0] < OM.DevOptn.iRailBtmTemp - OM.CmnOptn.dTempErrorTol) FM_MsgOk("ERROR" , (String("Rail Bottom온도가 설정 온도 보다 ")+ OM.CmnOptn.dTempErrorTol + "도 이상 낮습니다.").c_str());
            if(iTemp[1] < OM.DevOptn.iRailTopTemp - OM.CmnOptn.dTempErrorTol) FM_MsgOk("ERROR" , (String("Rail Top온도가 설정 온도 보다 "   )+ OM.CmnOptn.dTempErrorTol + "도 이상 낮습니다.").c_str());
            if(iTemp[2] < OM.DevOptn.iStageTemp   - OM.CmnOptn.dTempErrorTol) FM_MsgOk("ERROR" , (String("Stage 온도가 설정 온도 보다 "     )+ OM.CmnOptn.dTempErrorTol + "도 이상 낮습니다.").c_str());
        }
...
bool  __fastcall CSequence::InspectCrash(void)
{
    if(IO_GetX(xDSP_HeadDamege)) {
        if(MM.GetManNo() != mcAllHome){ //하드웨어 충돌 알람시에 올홈으로 풀수 있다.
            ...
            EM_SetErrMsg(eiPRT_Crash , "툴 하드웨어 충돌 센서가 감지 되었습니다.");
            return false ;
        }
    }
    return true ;
