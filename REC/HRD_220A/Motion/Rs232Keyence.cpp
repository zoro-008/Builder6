    ...
    if(m_iCmdNo == hcHeight){
        ...
        if(sChanel == "0") { //두채널 모두.
            ...
            if(StrToFloatDef(sHeight1 , -1) == StrToFloatDef(sHeight1 , 0)){
                m_dHeightData1 = StrToFloatDef(sHeight1 , KEYENCE_ERR) ;
            }
            else {
                m_dHeightData1 = KEYENCE_ERR ;
                Trace("Keynce1 Err", String(sHeight1).c_str());
            }

            if(StrToFloatDef(sHeight2 , -1) == StrToFloatDef(sHeight2 , 0)){
                m_dHeightData2 = StrToFloatDef(sHeight2 , KEYENCE_ERR) ;
            }
            else {
                m_dHeightData2 = KEYENCE_ERR ;
