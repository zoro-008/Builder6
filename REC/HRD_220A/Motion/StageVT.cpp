...
bool CStageVT::CycleWork(void)
{
    ...
    //bNeedInsp1 = (DM.ARAY[riSTB].GetStat(0,3)!=csEmpty || GetSeqStep()==scIdle ) && !OM.CmnOptn.bSkip1stTol && (DM.ARAY[riSTB].GetSubStat(0,3,iOSTRslt)==csGood||GetSeqStep()==scIdle) ;
    //bNeedInsp2 = (DM.ARAY[riSTB].GetStat(0,2)!=csEmpty || GetSeqStep()==scIdle ) && !OM.CmnOptn.bSkip2ndTol && (DM.ARAY[riSTB].GetSubStat(0,2,iOSTRslt)==csGood||GetSeqStep()==scIdle) ;
    //bNeedInsp3 = (DM.ARAY[riSTB].GetStat(0,1)!=csEmpty || GetSeqStep()==scIdle ) && !OM.CmnOptn.bSkip3thTol && (DM.ARAY[riSTB].GetSubStat(0,1,iOSTRslt)==csGood||GetSeqStep()==scIdle) ;
    //bNeedInsp4 = (DM.ARAY[riSTB].GetStat(0,0)!=csEmpty || GetSeqStep()==scIdle ) && !OM.CmnOptn.bSkip4thTol && (DM.ARAY[riSTB].GetSubStat(0,0,iOSTRslt)==csGood||GetSeqStep()==scIdle) ;
    ...
    */
    ...
    //static int iBLKCntPock1 = 0 ;
    //static int iBLKCntPock2 = 0 ;
    //static int iBLKCntPock3 = 0 ;
    //static int iBLKCntPock4 = 0 ;
                 ...
                 */
                 ...
                 if(GetSeqStep() != scIdle) {  //메뉴얼로 세팅할때 자꾸 스테이지 돌려 놓고 검사 하면 삑사리 나서...
                     if(DM.ARAY[riSTB].GetStat(0,3) != csEmpty && //자제가 있고.
                        DM.ARAY[riSTB].GetSubStat(0,3,iOSTRslt) == csGood) { //자제 있는 놈만.
                     ...
                     if(OM.CmnOptn.iVTContBlkCnt != 0){ //0으로 설정 되어 있으면 사용 안함.
                          sTemp = "";
                          if(OM.CmnOptn.iVTContBlkCnt<=m_iBLKCntPock1){
                              if(sTemp=="") sTemp =  "1";
                              else          sTemp+= ",1";
                          }
                          if(OM.CmnOptn.iVTContBlkCnt<=m_iBLKCntPock2){
                              if(sTemp=="") sTemp =  "2";
                              else          sTemp+= ",2";
                          }
                          if(OM.CmnOptn.iVTContBlkCnt<=m_iBLKCntPock3){
                              if(sTemp=="") sTemp =  "3";
                              else          sTemp+= ",3";
                          }
                          if(OM.CmnOptn.iVTContBlkCnt<=m_iBLKCntPock4){
                              if(sTemp=="") sTemp =  "4";
                              else          sTemp+= ",4";
                          }
                     ...
                     /*
                     if(DM.ARAY[riMVT].GetStat(0,3) == (EN_CHIP_STAT)OM.CmnOptn.iVTBLKBin+1) m_iBLKCntPock1++ ;
                     else                                                                    m_iBLKCntPock1=0 ;
                     if(DM.ARAY[riMVT].GetStat(0,2) == (EN_CHIP_STAT)OM.CmnOptn.iVTBLKBin+1) m_iBLKCntPock2++ ;
                     else                                                                    m_iBLKCntPock2=0 ;
                     if(DM.ARAY[riMVT].GetStat(0,1) == (EN_CHIP_STAT)OM.CmnOptn.iVTBLKBin+1) m_iBLKCntPock3++ ;
                     else                                                                    m_iBLKCntPock3=0 ;
                     if(DM.ARAY[riMVT].GetStat(0,0) == (EN_CHIP_STAT)OM.CmnOptn.iVTBLKBin+1) m_iBLKCntPock4++ ;
                     else                                                                    m_iBLKCntPock4=0 ;


                     if(OM.CmnOptn.iVTContBlkCnt != 0){ //0으로 설정 되어 있으면 사용 안함.
                         if(OM.CmnOptn.iVTContBlkCnt<=m_iBLKCntPock1){EM_SetErrMsg(eiVTI_SerialNG , "1번포켓 BLK 불량 연속"); m_iBLKCntPock1 = 0 ;}
                         if(OM.CmnOptn.iVTContBlkCnt<=m_iBLKCntPock2){EM_SetErrMsg(eiVTI_SerialNG , "2번포켓 BLK 불량 연속"); m_iBLKCntPock2 = 0 ;}
                         if(OM.CmnOptn.iVTContBlkCnt<=m_iBLKCntPock3){EM_SetErrMsg(eiVTI_SerialNG , "3번포켓 BLK 불량 연속"); m_iBLKCntPock3 = 0 ;}
                         if(OM.CmnOptn.iVTContBlkCnt<=m_iBLKCntPock4){EM_SetErrMsg(eiVTI_SerialNG , "4번포켓 BLK 불량 연속"); m_iBLKCntPock4 = 0 ;}
                     }
                     */
