...
#define MAX_MOTR 2
//#define PULSE_PER_REV 32768   //최대 분해능이 안나와 속도가 2000rpm이상 안된다.
#define PULSE_PER_REV 16384
...
public:
    bool m_bMLimit[4],m_bPLimit[4],m_bZ[4],m_bNear[4],
        m_bInpo[4],m_bAlarm[4],m_bError[4],m_bBusy[4],m_bOrg[4];
    bool m_bHomeDone[4];
    long m_lPPS[4];
    long m_lEncPos[4],m_lCmdPos[4];
    bool m_bEmer;
    NMCDATA nmcData;
/*
    bool m_bAxis1Start;
    ...
    bool m_bAxis2Stop;
*/
    double m_fEncPos[4],m_fCmdPos[4];
public:
...
//    bool SetDriveSpeed(int nAxis , int RPM , int Acc_mSec , int Rot_mSec);
    ...
    int iORCnt ;
    bool OR_Start(int dAllTime , int iCnt , int *Speed , int *Time);
