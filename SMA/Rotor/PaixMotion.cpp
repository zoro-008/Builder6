...
//    m_bAxis2Stop = false;
    ...
    {
        ShowMessage("Motion Connection Failed [192.168.2.20]");
...
{
    return nmc_get_ipaddress(nField0,nField1,nField2,nField3);
}
//---------------------------------------------------------------------------
...
//     SetSpeedDist    (nAxis,40, AccRate , APS ,0 );  //잠시 수정.
     SetSpeedDist    (nAxis,40, MaxAccRate , MaxAPS ,0 );  //잠시 수정.
//     SetSpeedPPS     (nAxis,40, AccRateP, PPS ,0 );
     SetSpeedPPS     (nAxis,40, MaxAccRateP, MaxPPS ,0 );
...
bool __fastcall CPaixMotion::HomeProc(int nAxis)
{
    ...
    if(nAxis == Axis1) {
        switch(Stat[nAxis].iHomeStep) {
            ...
            case 10 : SetSpeedDist    (nAxis,40, 360,36,0 );    //StartSpeed , Acc , Speed , Jerk
            ...
            case 11 : if(!nmcData.nmcBit.nNear0)   return false ;
                      StopMotor(nAxis);
                      Stat[nAxis].iHomeStep++ ;
                      return false ;

            case 12 : if(nmcData.nmcBit.nBusy0)    return false ;
                      SetSpeedDist    (nAxis,40, 360,3,0 );    //StartSpeed , Acc , Speed , Jerk
                      JogMove (nAxis,CW  );
                      Stat[nAxis].iHomeStep++ ;
                      return false ;

            case 13 : if(nmcData.nmcBit.nNear0) return false ;
                      StopMotor(nAxis);
                      Stat[nAxis].iHomeStep++ ;
                      return false ;

            case 14 : if(nmcData.nmcBit.nBusy0)    return false ;
                      SetSpeedDist    (nAxis,40, 1800,360,0 );    //StartSpeed , Acc , Speed , Jerk
                      SetEnc(nAxis , 0.0) ;
                      SetCmd(nAxis , 0.0) ;
...
bool CPaixMotion::OR_Start(int iAllTime , int iCnt , int *Speed , int *Time)
{
    int nAxis = Axis1 ;
...
//                  m_pMsgFunc("Start OR_START");
                  iORCnt = 0;
                  MoveRPM(nAxis,Stat[nAxis].iRPM,100,Stat[nAxis].dTotal);
                  Stat[nAxis].iORStep++ ;
                  return false ;
