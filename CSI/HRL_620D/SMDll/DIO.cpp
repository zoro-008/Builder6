...
void CDIO::SetOut(int _iNo , bool _bVal)
{
...
void CDIO::SetY(int _iNo , bool _bVal , bool _bDirect) //다이렉트가 트루이면 테스트 모드에서도 동작.
{
...
bool CDIO::GetY(int _iNo , bool _bDirect )
{
...
bool CDIO::GetX(int _iNo , bool _bDirect)
{
...
bool CDIO::GetInModuleNo (int _iNo , int &_iModuleNo , int &_iOffset)
{
    if( _iNo < 0 || _iNo >= m_iMaxIn) {/*ShowMessageT("Range is wrong");*/ return false; }
...
bool CDIO::GetOutModuleNo(int _iNo , int &_iModuleNo , int &_iOffset)
{
    if( _iNo < 0 || _iNo >= m_iMaxOut) {/*ShowMessageT("Range is wrong");*/ return false; }
...
void CDIO::LoadDnmVar()
{
    ...
    //Load Device.
    UserINI.Load(sPath , "CDIO"    , "m_iMaxIn "  , m_iMaxIn );
    UserINI.Load(sPath , "CDIO"    , "m_iMaxOut"  , m_iMaxOut);
    UserINI.Load(sPath , "Member"  , "m_iLangSel" , iLangSel ); m_iLangSel = (EN_LAN_SEL)iLangSel ;
}

void CDIO::SaveDnmVar()
{
    //Local Var.
    TUserINI   UserINI;
    AnsiString sPath  ;
    ...
    //Load Device.
    UserINI.Save(sPath , "CDIO"   , "m_iMaxIn "  ,      m_iMaxIn  );
    UserINI.Save(sPath , "CDIO"   , "m_iMaxOut"  ,      m_iMaxOut );
    UserINI.Save(sPath , "Member" , "m_iLangSel" , (int)m_iLangSel);
}

void CDIO::DisplayList(bool _bOutput , TStringGrid *_sgTable , bool _bFixedWidth )
{
    //Local Var.
    ...
    if(_bFixedWidth)_sgTable -> ColWidths[5] = 150;  _sgTable -> Cells[5][0] = "Comment" ;
    if(_bFixedWidth)_sgTable -> ColWidths[6] = 55 ;  _sgTable -> Cells[6][0] = "Delay"   ;
    if(_bFixedWidth)_sgTable -> ColWidths[7] = 50 ;  _sgTable -> Cells[7][0] = "V Val"   ;
    if(_bFixedWidth)_sgTable -> ColWidths[8] = 50 ;  _sgTable -> Cells[8][0] = "A Val"   ;
    if(_bFixedWidth)_sgTable -> ColWidths[9] = 100;  _sgTable -> Cells[9][0] = "Address" ;
    //Display.
    if (_bOutput) {
        //Clear View Index.
        for (int i = 0 ; i < m_iMaxOut ; i++) {
...
//            else             sAdd = "Y" + sAdd.sprintf ("%04d",m_pOut[i].iAdd) ;
            ...
            _sgTable -> Cells[9][i + 1] = sTemp.sprintf("DO%d_%02d" ,iIOModuleNo+1 , iIOOffset );
        }
    }
    else {
        //Clear View Index.
...
//            else             sAdd = "X" + sAdd.sprintf ("%04d",m_pIn[i].iAdd) ;
            ...
            _sgTable -> Cells[9][i + 1] = sTemp.sprintf("DI%d_%02d" ,iIOModuleNo+1 , iIOOffset ) ;        }
    }
...
void CDIO::UpdateStatus(bool _bOutput , TStringGrid *_sgTable )
{
    //Local Var.
...
void CDIO::DisplayProperty(bool          _bTable      ,
                           int           _bOutput     ,
                           TEdit       * _edIndex     ,
                           TEdit       * _edEnum      ,
                           TEdit       * _edAdd       ,
