...
bool CMarkVisn::SaveVisnDevice(AnsiString _sDevice)
{
    ...
    if (hFile == -1) { Trace("Err",(sData + "is Can't made").c_str());  return false ; }
...
bool CMarkVisn::VisnJobChange(AnsiString _sDevName)
{
    if(!SaveVisnDevice(_sDevName)) {
...
//Check Cycle Time Out.
    ...
    switch (Step.iCycle) {
        ...
        case  11:
                  MoveMotr(miVSN_YInsp , piVSN_YInspTrg   );
...
//Check Cycle Time Out.
    ...
    switch (Step.iCycle) {
        ...
        case  12: if(!MoveMotr(miVSN_YInsp , piVSN_YInspTrg   )) return false ;
                  ...
                  //if(!IO_GetX(xVSC_MrkBusy)) return false;
