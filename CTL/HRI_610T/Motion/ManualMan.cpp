...
bool __fastcall CManualMan::SetManCycle(EN_MANUAL_CYCLE _iNo)
{
    ...
    //                       !IO_GetX(xRAL_VPkg4 ) && !IO_GetX(xPSB_PkgIn5) && !IO_GetX(xPSB_Pkg5 ) && !IO_GetX(xPSB_PkgOut6 );
         ...
         if(m_iManNo == mcAllHome         ) {if(IO_GetX(xPRB_PkgIn1 )              ){FM_MsgOk("ERROR","Check Rail In Sensor!" ); bRet = false ;}
                                             ...
                                             if( IO_GetX(xRAL_VPkg4)){ FM_MsgOk("ERROR","Vision Zone Sensor has been detected." ); bRet = false ; }}
    else if(m_iManNo == mcRAL_CycleCnvr   ) {}
    ...
    else if(m_iManNo == mcLSR_CycleCntrMrk) {}
    ...
    else if(m_iManNo == mcVSN_CycleReject ) {}
    ...
    //                    MT_GetCmdPos(miLSR_YAlVs) <= PM.GetValue(miLSR_YAlVs , pvLSR_YAlVsWork) + 0.1  ;
         ...
         if(m_iManNo == mcAllHome         ) {for(int i = 0; i < MAX_MOTR; i++) MT_ClearPos(i);
                                             PRB.InitHomeStep ();
                                             RAL.InitHomeStep ();
                                             LSR.InitHomeStep ();
                                             VSN.InitHomeStep ();
    ...
    else if(m_iManNo == mcRAL_UnBind      ) {RAL.MoveActr(aiRAL_LAlnFwBw , ccBwd); RAL.MoveActr(aiRAL_VAlnFwBw , ccBwd);
                                             RAL.MoveActr(aiRAL_LStnUpDn , ccBwd); RAL.MoveActr(aiRAL_VStnUpDn , ccBwd);}
    ...
    else if(m_iManNo == mcRAL_CycleCnvr   ) {RAL.InitHomeStep();                                       }
    ...
    else if(m_iManNo == mcLSR_CycleCntrMrk) {m_iManStep = 10 ; m_iPreManStep = 0;                      }
    ...
    else if(m_iManNo == mcVSN_CycleReject ) { m_iManStep = 10; m_iPreManStep = 0;                      }
    ...
    else if (m_iManNo == mcETC_LDRULDPlace    ) { LDR.InitCycleStep();
                                                  ULD.InitCycleStep();                                 }
...
void __fastcall CManualMan::Update(void)
{
    ...
    else if(m_iManNo == mcRAL_CycleCnvr   ) {if(RAL.CycleHome         ()) m_iManNo = mcNoneCycle; }
    ...
    else if(m_iManNo == mcLSR_CycleCntrMrk) {if(CycleCntrMrk          ()) m_iManNo = mcNoneCycle; }
    ...
    else if(m_iManNo == mcVSN_CycleReject ) {if(CycleVisnReject       ()) m_iManNo = mcNoneCycle; } //sun sun 이거 동작 제대로 하게 되어 있는지 확인.
...
bool CManualMan::CycleRailMoveNextPart()
{
    ...
    switch (m_iManStep) {
        ...
        case  11: RAL.MoveActr(aiRAL_VStnUpDn , ccBwd);
                  ...
                  RAL.MoveActr(aiRAL_VStpUpDn , ccFwd);
                  RAL.MoveActr(aiRAL_LStnUpDn , ccBwd);
                  RAL.MoveActr(aiRAL_LAlnFwBw , ccBwd);
        ...
        case  12: if(!RAL.MoveActr(aiRAL_VStnUpDn , ccBwd)) return false;
                  ...
                  if(!RAL.MoveActr(aiRAL_VStpUpDn , ccFwd)) return false;
...
bool CManualMan::CycleCntrMrk()
{
        //Check Cycle Time Out.
    AnsiString sTemp ;
    if (m_tmCycle.OnDelay(m_iManStep && m_iManStep == m_iPreManStep &&!OM.MstOptn.bDebugMode, 15000 )) {
        EM_SetErr(eiETC_ManCycleTO);
        sTemp = sTemp.sprintf("%s TIMEOUT STATUS : Step.iHome=%02d" , __FUNC__ , m_iManStep );
        Trace("Manual Man",sTemp.c_str());
        m_iManStep = 0 ;
    ...
    //진섭아 이렇게 하면 계속 초기화 된다.
    //AnsiString sLsrCalDev = "Laser Center";
    //AnsiString sBeforeDev;
    sLsrCalDev = "Laser Center";
    ...
    switch (m_iManStep) {
        ...
        case  10: if(OM.GetCrntDev() != "Laser Center") sBeforeDev = OM.GetCrntDev();
                  //IO_SetY(yLSC_DataRead , false);
                  RAL.MoveActr(aiRAL_LStnUpDn, ccBwd          );
                  LSR.MoveMotr(miLSR_YAlVs   , piLSR_YAlVsWait);
        ...
        case  11: if(!m_tmDelay.OnDelay(1000))return false;
                  if(!RAL.MoveActr(aiRAL_LStnUpDn, ccBwd          )) return false;
                  if(!LSR.MoveMotr(miLSR_YAlVs   , piLSR_YAlVsWait)) return false;
                  if(sLsrCalDev != ""){
                      OM.LoadJobFile(sLsrCalDev);
                  }

                  LSR.LaserJobChange(sLsrCalDev);
                  //VSN.VisnJobChange (sLsrCalDev);
                  m_tmDelay.Clear();

                  m_iManStep++;
                  return false ;

        case  12: if(!m_tmDelay.OnDelay(1000))return false;
                  RAL.MoveActr(aiRAL_LStnUpDn, ccFwd);
        ...
        case  13: //if(!m_tmDelay.OnDelay(true, 1000))return false;
                  if(!RAL.MoveActr(aiRAL_LStnUpDn, ccFwd))return false;
                  Rs232Laser.ClearReadMsg();
                  //JS.
                  //Rs232Laser.SendMsg(0, 0, 0, 0, 0);
        ...
        case  15: //if(!m_tmDelay.OnDelay(true, 1000))return false;
                  //if(!IO_GetX(xLSC_DataReadComp)) return false ;
                  //IO_SetY(yLSC_DataRead , false);
        ...
        case  16: //if(!m_tmDelay.OnDelay(true, 1000))return false;
                  //if(IO_GetX(xLSC_DataReadComp)) return false ;
        ...
        case  18: OM.LoadJobFile(sBeforeDev);

                  LSR.LaserJobChange(sBeforeDev);
                  //VSN.VisnJobChange (sBeforeDev);

                  m_iManStep ++;
                  return false;

        case  19: //if(!m_tmDelay.OnDelay(1000)) return false;
                  MT_GoAbsRun(miLSR_YAlVs, OM.MstOptn.dVsnLsrYCntrPos);
...
bool CManualMan::CycleVisnReject()
{
        //Check Cycle Time Out.
    AnsiString sTemp ;
    if (m_tmCycle.OnDelay(m_iManStep && m_iManStep == m_iPreManStep &&!OM.MstOptn.bDebugMode, 15000 )) {
    ...
    switch (m_iManStep) {
        ...
        case  10: RAL.MoveActr(aiRAL_VStnUpDn, ccBwd          );
                  RAL.MoveActr(aiRAL_VAlnFwBw, ccBwd          );
                  m_iManStep++;
                  return false;

        case  11: if(!RAL.MoveActr(aiRAL_VStnUpDn , ccBwd)) return false;
                  if(!RAL.MoveActr(aiRAL_VAlnFwBw , ccBwd)) return false;
                  RAL.MoveActr(aiRAL_VStpUpDn , ccBwd);
                  m_tmDelay.Clear();
                  m_iManStep++;
                  return false;

        case  12: if(!RAL.MoveActr(aiRAL_VStpUpDn , ccBwd)) return false;
                  if(!m_tmDelay.OnDelay(true, 100))return false;
                  IO_SetY(yRAL_FeedAc , true);
                  m_tmDelay.Clear();
                  m_iManStep++;
                  return false;
        ...
        case  14: if(!PSB.CycleReject()) return false ;  //배출 사이클.
                  m_iManStep = 0;
                  return true;
    }
}
