...
void __fastcall TFrmDeviceSet::tmUpdateTimerTimer(TObject *Sender)
{
    ...
    if(FM_GetLevel() == (EN_LEVEL)lvMaster){
        btOfsHelp    -> Visible = true ;
        btFormatHelp -> Visible = true ;
    }
    if(btFormatHelp -> Visible && FM_GetLevel() != (EN_LEVEL)lvMaster) btFormatHelp -> Visible = false;
    if(btOfsHelp    -> Visible && FM_GetLevel() != (EN_LEVEL)lvMaster) btOfsHelp    -> Visible = false;
...
void __fastcall TFrmDeviceSet::UpdateDevOptn(bool bTo)
{
    ...
    if(bTo){
        ...
        //edHoleXCntrOfs    -> Text      = OM.DevOptn.dHoleXCntrOfs    ;
        //edHoleYCntrOfs    -> Text      = OM.DevOptn.dHoleYCntrOfs    ;
        //edHoleTCntrOfs    -> Text      = OM.DevOptn.dHoleTCntrOfs    ;

        edLotNo           -> Text      = OM.DevOptn.sSampleLotNo     ;
        edLaser           -> Text      = OM.DevOptn.sSampleFormat    ;

        edVSNXLSRCntrOfs  -> Text      = OM.DevOptn.dVSNXLSRCntrOfs  ;
        edVSNYLSRCntrOfs  -> Text      = OM.DevOptn.dVSNYLSRCntrOfs  ;
        edVSNTLSRCntrOfs  -> Text      = OM.DevOptn.dVSNTLSRCntrOfs  ;

        cbLSRTest         -> Checked   = OM.DevOptn.bLSRTest         ;

        edOriVsnDelay     -> Text      = OM.DevOptn.dOriVsnDelay     ;
        ...
        //OM.DevOptn.dHoleXCntrOfs     = StrToFloatDef(edHoleXCntrOfs    -> Text , 0.0  ) ;
        //OM.DevOptn.dHoleYCntrOfs     = StrToFloatDef(edHoleYCntrOfs    -> Text , 0.0  ) ;
        //OM.DevOptn.dHoleTCntrOfs     = StrToFloatDef(edHoleTCntrOfs    -> Text , 0.0  ) ;

        OM.DevOptn.dVSNXLSRCntrOfs     = StrToFloatDef(edVSNXLSRCntrOfs    -> Text , 0.0  ) ;
        OM.DevOptn.dVSNYLSRCntrOfs     = StrToFloatDef(edVSNYLSRCntrOfs    -> Text , 0.0  ) ;
        OM.DevOptn.dVSNTLSRCntrOfs     = StrToFloatDef(edVSNTLSRCntrOfs    -> Text , 0.0  ) ;


        OM.DevOptn.bLSRTest          = cbLSRTest -> Checked;

        OM.DevOptn.sSampleLotNo      =               edLotNo           -> Text          ;
        OM.DevOptn.sSampleFormat     =               edLaser           -> Text          ;

        OM.DevOptn.dOriVsnDelay      = StrToIntDef(edOriVsnDelay       -> Text , 100    ) ;
    }
}
//---------------------------------------------------------------------------
...
void __fastcall TFrmDeviceSet::btTestLaserTextClick(TObject *Sender)
{
    ...
    if(mrYes == FM_MsgYesNo("ERROR","Would you like to Save the Data")) {
        if(!LSR.SaveLaserLotNo(edLotNo -> Text)){
...
void __fastcall TFrmDeviceSet::btOfsHelpClick(TObject *Sender)
{
    FrmHelp -> Show();
    FrmHelp -> tsHelp1 -> Show();
}
//---------------------------------------------------------------------------

void __fastcall TFrmDeviceSet::btFormatHelpClick(TObject *Sender)
{
    FrmHelp -> Show();
    FrmHelp -> tsHelp2 -> Show();
}
//---------------------------------------------------------------------------
