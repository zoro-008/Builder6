...
void __fastcall TFrmMain::tmUpdateTimer(TObject *Sender)
{
    ...
    if(IO_GetX(xLSC_Ready))pnReady -> Color = clRed;
    else                   pnReady -> Color = clLime;
    if(IO_GetX(xLSC_Progress))pnProgress -> Color = clRed;
    else                      pnProgress -> Color = clLime;
    ...
    pnOri -> Color = IO_GetX(xVSC_OriReady) ? clGreen : clRed ;
    pnAlg -> Color = IO_GetX(xVSC_AlnReady) ? clGreen : clRed ;
    pnMrk -> Color = IO_GetX(xVSC_MrkReady) ? clGreen : clRed ;
    pnLsr -> Color = IO_GetX(xLSC_Ready   ) ? clGreen : clRed ;
