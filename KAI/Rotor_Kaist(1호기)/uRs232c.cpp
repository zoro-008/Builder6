	...
	while(!Terminated)
    {
        ...
        if(myRs232c->hmyWnd)
        {
            if(dwEventMask & EV_RXCHAR)
            {
                ...
                if(myRs232c ->myID == 0) SendMessage(myRs232c->hmyWnd, UM_EVN_RXCHAR_PB1 , CommStatus.cbInQue, 0);
//                if(myRs232c ->myID == 1) SendMessage(myRs232c->hmyWnd, UM_EVN_RXCHAR_PB2 , CommStatus.cbInQue, 0);
//                if(myRs232c ->myID == 2) SendMessage(myRs232c->hmyWnd, UM_EVN_RXCHAR_PB3 , CommStatus.cbInQue, 0);
//                if(myRs232c ->myID == 1) SendMessage(myRs232c->hmyWnd, UM_EVN_RXCHAR_BAR1, CommStatus.cbInQue, 0);
//                if(myRs232c ->myID == 2) SendMessage(myRs232c->hmyWnd, UM_EVN_RXCHAR_BAR2, CommStatus.cbInQue, 0);
...
bool __fastcall TRS232C::Open(int _iId,HWND hWnd)
{
    hmyWnd = hWnd;
    ...
    //myDCB.StopBits = Para.StopBits;
    //myDCB.Parity   = Para.Parity;
