...
bool CSEQ::GetFuncName(AnsiString &_sLine , EN_FUNC &_fcName )
{
    ...
    else if (_sLine.Pos   ("MOVEANG"       ) == 1) { _fcName = fcMOVEANG       ; _sLine.Delete(1,AnsiString("MOVEANG"       ).Length()) ; return true  ; }
    else if (_sLine.Pos   ("MOVEACTR"      ) == 1) { _fcName = fcMOVEACTR      ; _sLine.Delete(1,AnsiString("MOVEACTR"      ).Length()) ; return true  ; }
    else                                           {                                                                                      return false ; }
}
    ...
    for(int i = 0 ; i < _slSourse -> Count ; i++) {
        ...
        switch(iFunc) {
            ...
            case fcMOVE0RPM      : //if(!GetPara(sTemp , dPara)) {m_sErrName = "Line " + AnsiString(i) + "파라미터1의 형태가 이상합니다." ; return false ;} m_FuncInfo[i].dPara1 = dPara ;
                                   if(!GetPara(sTemp , dPara)) {m_sErrName = "Line " + AnsiString(i) + "파라미터1의 형태가 이상합니다." ; return false ;} m_FuncInfo[i].dPara1 = dPara / MOTOR_CONVERSION;
...
*/
            ...
            case fcOR_SETSPEED0  : if(!GetPara(sTemp , dPara)) {m_sErrName = "Line " + AnsiString(i) + "파라미터1의 형태가 이상합니다." ; return false ;} m_iMaxVal = dPara ;
                                   ...
                                   for(int iCnt = 0 ; iCnt< m_iMaxVal ; iCnt++ ) {
                                       ...
                                       m_dORSpeed[iCnt] = dPara / MOTOR_CONVERSION;
                                   }
                                   break ;
            ...
            case fcMOVEANG       : if(!GetPara(sTemp , dPara)) {m_sErrName = "Line " + AnsiString(i) + "파라미터1의 형태가 이상합니다." ; return false ;} m_FuncInfo[i].dPara1 = dPara ;
                                   if(!GetPara(sTemp , dPara)) {m_sErrName = "Line " + AnsiString(i) + "파라미터2의 형태가 이상합니다." ; return false ;} m_FuncInfo[i].dPara2 = dPara ;
                                   if(!GetPara(sTemp , dPara)) {m_sErrName = "Line " + AnsiString(i) + "파라미터3의 형태가 이상합니다." ; return false ;} m_FuncInfo[i].dPara3 = dPara / MOTOR_CONVERSION;
                                   if(!GetPara(sTemp , dPara)) {m_sErrName = "Line " + AnsiString(i) + "파라미터4의 형태가 이상합니다." ; return false ;} m_FuncInfo[i].dPara4 = dPara / MOTOR_CONVERSION;
                                   break ;


            case fcMOVEACTR      : if(!GetPara(sTemp , dPara)) {m_sErrName = "Line " + AnsiString(i) + "파라미터1의 형태가 이상합니다." ; return false ;} m_FuncInfo[i].dPara1 = dPara ;
                                   m_FuncInfo[i].dPara1 = (int)dPara ;
                                   if(!GetPara(sTemp , dPara)) {m_sErrName = "Line " + AnsiString(i) + "파라미터1의 형태가 이상합니다." ; return false ;} m_FuncInfo[i].dPara2 = dPara ;break ;
                                   m_FuncInfo[i].dPara2 = (int)dPara ;
                                   break ;
    ...
    while( !Terminated )
    {
        ...
        switch(m_FuncInfo[iStep].iFunc) {
            case fcCHECKFINISH   : if(!pPaix -> GetbHomeDone (m_FuncInfo[iStep].dPara1)) continue;
                                   ...
                                   if( pPaix -> GetbMoveAng  (m_FuncInfo[iStep].dPara1)) continue;
                                   //if( pPaix -> GetbMoveActr (m_FuncInfo[iStep].dPara1)) continue;
                                   break ;

            case fcMOVE0RPM      :
                                   pPaix -> MoveRPM    (Axis1 ,m_FuncInfo[iStep].dPara1 ,m_FuncInfo[iStep].dPara2,m_FuncInfo[iStep].dPara3 );
            ...
            case fcCOOLING       : //JH
                                   ...
                                   pPaix -> MoveActr(aiHeaterDnUp, false);
                                   break ;
            ...
            case fcMOVEANG       : //pPaix -> SetSpeedDist (m_FuncInfo[iStep].dPara1,1,m_FuncInfo[iStep].dPara2, m_FuncInfo[iStep].dPara3,0 );    //StartSpeed , Acc , Speed , Jerk
                                   //pPaix -> RelMove      (m_FuncInfo[iStep].dPara1,m_FuncInfo[iStep].dPara4 ) ;
                                   pPaix -> MoveAng(m_FuncInfo[iStep].dPara1, m_FuncInfo[iStep].dPara2, m_FuncInfo[iStep].dPara3, m_FuncInfo[iStep].dPara4);
                                   break ;

            case fcMOVEACTR      : pPaix -> MoveActr(m_FuncInfo[iStep].dPara1 , m_FuncInfo[iStep].dPara2);
                                   break;
...
bool __fastcall CSEQ::SeqUpdate()
{
    static int  iStep    = 0     ;
    static int  iPreStep = 0     ;
    static int  iForStep = 0     ;
    ...
    if(m_FuncInfo[iStep].iFunc == fcMOVE0RPM || m_FuncInfo[iStep].iFunc == fcOR_SETSPEED0){
        m_bSaveImg = true;
    }


    switch(m_FuncInfo[iStep].iFunc) {
        case fcCHECKFINISH   : if(!pPaix -> GetbHomeDone (m_FuncInfo[iStep].dPara1)) return false;
                               if( pPaix -> GetBusy      (m_FuncInfo[iStep].dPara1)) return false;
                               if(!pPaix -> GetInpo      (m_FuncInfo[iStep].dPara1)) return false;
                               if( pPaix -> GetbShaking  (m_FuncInfo[iStep].dPara1)) return false;
                               if( pPaix -> GetbOverRide (m_FuncInfo[iStep].dPara1)) return false;
                               if( pPaix -> GetbMoveAng  (m_FuncInfo[iStep].dPara1)) return false;
                               m_bSaveImg = false;
                               //if( pPaix -> GetbMoveActr (m_FuncInfo[iStep].dPara1)) continue;
                               break;
        ...
        case fcMOVEABS       : if(m_FuncInfo[iStep].dPara1 == 0) {pPaix -> RotateReset(Axis1); }
                               else m_bSaveImg = false;
                                   pPaix -> SetSpeedDist (m_FuncInfo[iStep].dPara1,1,m_FuncInfo[iStep].dPara2, m_FuncInfo[iStep].dPara3,0 );    //StartSpeed , Acc , Speed , Jerk
                                   pPaix -> AbsMove      (m_FuncInfo[iStep].dPara1,m_FuncInfo[iStep].dPara4 ) ;
                               break;

        case fcMOVEINC       : pPaix -> SetSpeedDist (m_FuncInfo[iStep].dPara1,1,m_FuncInfo[iStep].dPara2, m_FuncInfo[iStep].dPara3,0 );    //StartSpeed , Acc , Speed , Jerk
                               pPaix -> RelMove      (m_FuncInfo[iStep].dPara1,m_FuncInfo[iStep].dPara4 ) ;
                               break;
        ...
        case fcDELAY         : if(!TempTimer.OnDelay(true,m_FuncInfo[iStep].dPara1)) return false;
                               break;
        ...
        case fcSHAKE0        : pPaix -> SetSpeedDist (0, 1, m_FuncInfo[iStep].dPara2, m_FuncInfo[iStep].dPara3, 0 );    //Axis , StartSpeed , Acc , Speed , Jerk
                               pPaix -> MoveShake(0, m_FuncInfo[iStep].dPara1, m_FuncInfo[iStep].dPara2, m_FuncInfo[iStep].dPara3, m_FuncInfo[iStep].dPara4); // juhyeon.
                               break;
        case fcOR_SETSPEED0  : //할일 없음.   
                               break;
        ...
        case fcCOOLING       : pPaix -> MoveActr(aiCoverUpDn , true );
                               pPaix -> MoveActr(aiHeaterDnUp, false);
                               break;
    ...
    iStep++ ;
     ...
     if(FrmMain -> m_bGoCheck){ // juhyeon
//         FrmMain -> mmSeq -> Lines -> Clear();
         FrmMain -> mmSeq -> Lines = FrmMain -> mmTemp -> Lines ;

         FrmMain -> m_bGoCheck = false ;
     }
...
AnsiString CSEQ::GetFuncNameString(EN_FUNC iFunc)
{
     ...
     switch(iFunc) {
         ...
         case fcMOVEANG     : sRet = "MOVEANG      "  ; break ;
         case fcMOVEACTR    : sRet = "MOVEACTR     "  ; break ;
         default            : sRet = ""               ; break ;
     }
