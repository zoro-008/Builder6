...
//주의!!!!
//20160118.JS
//모터 스피드 10000RPM안올라 가서
//PULSE_PER_REV 속도 1440에서 2880으로 2배 뻥튀기
//강제로 속도 10000RPM까지 올림...
//전송 값이랑 모터 받는 속도가 다름....
//---------------------------------------------------------------------------
...
bool CPaixMotion::JogMove(int nAxis, int nDir)
{
    ...
    if(nAxis == 0) SetServoOnOff(nAxis , false) ;
    if(nAxis == 1) SetServoOnOff(nAxis , true ) ;
...
//---------------------------------------------------------------------------
//bool CPaixMotion::MoveActr(int nActr, bool _bFwd)
//{
//    if(nActr > 2) return false;
//
//    if(nActr == 0){
//        pPaix->SetOutputBit(1, _bFwd);
//    }
//        
//
//
...
void __fastcall CPaixMotion::Execute()
{
    ...
    while( !Terminated )
    {
        ...
        for(int i = 0 ; i < MAX_MOTR ; i++) {
            ...
            if(Stat[i].bMoveAngle ) AngleProc(i);
        ...
        for(int i = 0; i < MAX_ACTR ; i++){
            if(i == 0) ActrStat[i].bFwdSnsor = !NmcData.nmcBit.nInput6;
            else       ActrStat[i].bFwdSnsor =  NmcData.nmcBit.nInput7;
            if(ActrStat[i].bActrStart) ActrProc(i) ;
            //센서 번호 확인후 넣기.JS.
        }
...
void __fastcall CPaixMotion::SetRotorPara()
{
    ...
    SetAlarmLogic   (Axis1,0    ) ; //0:NC 1:NO
    ...
    //Pulse 변환 Clock 0:2Pulse방식 1:1Pulse방식
    SetClock        (Axis1,1    ) ; //0:2Clock 1:1Clock
    ...
    SetMotorRes     (Axis2,10000) ; //드라이버에서 설정. 5000
    SetLeed         (Axis2,4    ) ; //4mm
    SetEncCountMode (Axis2,0    ) ; //0:4체배 1:2체배 2:1체배
    SetZLogic       (Axis2,1    ) ; //0:NC 1:NO
    SetEncInputMode (Axis2,1    ) ; //0:EA|EB 1:EB|EA 2:CW|CCW 3:CCW|CW
    SetNearLogic    (Axis2,0    ) ; //0:NC 1:NO
    SetLimitLogic   (Axis2,0    ) ; //0:NC 1:NO
    SetAlarmLogic   (Axis2,1    ) ; //0:NC 1:NO
    PulseOutInverse (Axis2,1    ) ; //0:CCW|CW 1:CW|CCW
    ...
    SetSpeedDist    (Axis2,1, 50 , 10 , 0 );    //StartSpeed , Acc , Speed , Jerk //10이하면 삽질함.
...
//bool CPaixMotion::SetSpeedDist(int nAxis,double lStart, double lAcc,double lMax,long lJerk)
    ...
    //나중에 IO번호 입력.JS.
    ActrStat[0].iActrIONum = yETC_CoverUpDn  ;
    ActrStat[1].iActrIONum = yETC_HeaterDnUp ;

    ActrStat[0].bMoveDone = false;
    ActrStat[1].bMoveDone = false;

    ActrStat[0].bActrStart = false;
    ActrStat[1].bActrStart = false;
...
bool __fastcall CPaixMotion::HomeProc(int nAxis)
{
    ...
    if(nAxis == Axis1) {
        switch(Stat[nAxis].iHomeStep) {
            ...
            case 10 : // 앞쪽 실린더 내리기
                      MoveActr(aiHeaterDnUp, true);
                      Stat[nAxis].iHomeStep++ ;
                      return false ;

            case 11 : if(ActrStat[aiHeaterDnUp].bMoveDone)return false;
                      SetSpeedDist    (nAxis,1, 360,36,0 );    //StartSpeed , Acc , Speed , Jerk
            ...
            case 12: //if(!nmcData.nmcBit.nOrg0)   return false ;
            ...
            case 14 : if(nmcData.nmcBit.nOrg0) return false ;
                      StopMotor(nAxis);
                      Stat[nAxis].iHomeStep++ ;
                      return false ;

            case 15 : if(nmcData.nmcBit.nBusy0)    return false ;
...
*/
        ...
        switch(Stat[nAxis].iHomeStep) {
            ...
            case 10 : // 뒤쪽 실린더 들어주기
                      MoveActr(aiCoverUpDn, true);
                      Stat[nAxis].iHomeStep++ ;
                      return false ;

            case 11 : if(ActrStat[aiCoverUpDn].bMoveDone)return false;
                      SetSpeedDist    (nAxis,1, 200 , 20 , 1 );    //StartSpeed , Acc , Speed , Jerk
                      SetMotionMode   (nAxis,2              );    //0:Pulse 1:RPM 2:Position
                      SetMotorRes     (nAxis,10000) ; //드라이버에서 설정. 5000
                      SetLeed         (nAxis,4    ) ; //2mm
                      SetMotorOnOff   (nAxis,true );
                      JogMove         (nAxis,CCW  );
            ...
            case 12 : if(!nmcData.nmcBit.nOrg1)return false ;
                      StopMotor(nAxis);
                      Stat[nAxis].iHomeStep++ ;
                      return false ;

            case 13 : if(nmcData.nmcBit.nBusy1) return false ;
                      //SetSpeedDist (nAxis,1, 20 , 1 ,0 );    //StartSpeed , Acc , Speed , Jerk
                      //JogMove (nAxis,CW  );
                      Stat[nAxis].iHomeStep++ ;
                      return false ;

            case 14 : //if(nmcData.nmcBit.nNLimit1) return false ;
                      StopMotor(nAxis);
                      Stat[nAxis].iHomeStep++ ;
                      return false ;

            case 15 : if(nmcData.nmcBit.nBusy1)    return false ;
                      SetSpeedDist (nAxis,1, 30, 10,0 );    //StartSpeed , Acc , Speed , Jerk
                      SetEnc(nAxis , 0.0) ;
                      SetCmd(nAxis , 0.0) ;
                      Stat[nAxis].iHomeStep = 0 ;
                      Stat[nAxis].bHomeStart = false ;
...
void __fastcall CPaixMotion::MoveActr(int nActr, bool _bFwd)
{
    if( nActr < 0 && nActr >= MAX_ACTR || !Remote) return ;

    ActrStat[nActr].bFwd       = _bFwd ;
    ActrStat[nActr].bMoveDone  = true  ;
    ActrStat[nActr].iActrStep  = 10    ;
    ActrStat[nActr].bActrStart = true  ;
}

bool __fastcall CPaixMotion::ActrProc(int nActr)
{
    //ReadSensorAll(&nmcData);
    int iTemp = nActr;
    BYTE OutputIO;
    ...
    switch(ActrStat[nActr].iActrStep) {

        default : ActrStat[nActr].iActrStep = 0 ; break ;

        case 10 : pPaix->GetOutput(&OutputIO);
                  SetOutputBit(ActrStat[nActr].iActrIONum, ActrStat[nActr].bFwd); //nmc_out SetSpeedDist    (nAxis,1, 360,36,0 );    //StartSpeed , Acc , Speed , Jerk
                  pPaix->GetOutput(&OutputIO);

                  ActrStat[nActr].iActrStep++ ;
                  return false ;

        case 11 : //if(!ActrStat[nActr].bFwd){
                  //    if(!ActrStat[nActr].bFwdSnsor) return false;
                  //}
                  //else {
                  //    if( ActrStat[nActr].bFwdSnsor) return false;
                  //
                  if( ActrStat[nActr].bFwd && !ActrStat[nActr].bFwdSnsor) return false;
                  if(!ActrStat[nActr].bFwd &&  ActrStat[nActr].bFwdSnsor) return false;
                  m_tmTemp.Clear();
                  ActrStat[nActr].iActrStep++ ;
                  return false ;
        ...
        case 12 : if(!m_tmTemp.OnDelay(true , 500)) return false;
                  sTemp.printf("Axis%d HomeDone" , nActr);
                  m_pMsgFunc(sTemp.c_str());
                  ActrStat[nActr].bMoveDone  = false;
                  ActrStat[nActr].bActrStart = false;
                  ActrStat[nActr].iActrStep = 0;
                  return true ;
        }

    }

void __fastcall CPaixMotion::MoveAng(int nAxis , double _dTime , double _dFirstAngle, double _dAngle)
{
    Stat[nAxis].iAngleStep    = 10   ;
    Stat[nAxis].dAngDelayTime = _dTime;
    Stat[nAxis].dFirstAngle   = _dFirstAngle;
    Stat[nAxis].dMoveAngle    = _dAngle;
    Stat[nAxis].bMoveAngle    = true ;
}
bool __fastcall CPaixMotion::AngleProc(int nAxis)
{
    int iMovePos = 0;
    static int iMoveCnt = 0;
    static int iPreMoveCnt = 0;
    static double dAngleTime = 0;

    AnsiString sTemp = "";

    if(nAxis != Axis1) return false ;

    switch(Stat[nAxis].iAngleStep){
        default : Stat[nAxis].iAngleStep = 0; break ;

        case 10 : //모터 위치 초기화 하기.
                  m_pMsgFunc("Start MoveAngle");
                  SetSpeedDist    (nAxis,1, 360,36,0 );    //StartSpeed , Acc , Speed , Jerk
                  RotateReset(nAxis);
                  //iMovePos = CIRCLEANGLE - GetCmdPos(nAxis);
                  //pPaix -> RelMove(nAxis , iMovePos);
                  iPreMoveCnt = 1;
                  dAngleTime = GetTickTime();
                  Stat[nAxis].iAngleStep++;
                  return false;

        case  11: if(nmcData.nmcBit.nBusy0)return false ;
                  RotateReset(nAxis);
                  pPaix -> AbsMove(nAxis , Stat[nAxis].dFirstAngle);
                  Stat[nAxis].iAngleStep++;
                  return false;

        case  12: if(nmcData.nmcBit.nBusy0)return false ;
                  iMoveCnt = CIRCLEANGLE % ((int)Stat[nAxis].dMoveAngle * 2);
                  if(!iMoveCnt) iMoveCnt = CIRCLEANGLE / (Stat[nAxis].dMoveAngle * 2);
                  else          iMoveCnt = CIRCLEANGLE / (Stat[nAxis].dMoveAngle * 2) + 1;  //한바퀴를 못돌아서... JS
                  pPaix -> AbsMove(nAxis , Stat[nAxis].dMoveAngle * iPreMoveCnt + Stat[nAxis].dFirstAngle);
                  Stat[nAxis].iAngleStep++;
                  return false;
        ...
        case  14: if(!m_AngleTimer.OnDelay(true , Stat[nAxis].dAngDelayTime)) return false;
                  if(m_bUseOVSave) {
                      OceanViewShow(); }
                  m_OceanTimer.Clear();
                  Stat[nAxis].iAngleStep++;
        ...
        case  15: if(!m_OceanTimer.OnDelay(true , 200)) return false;
                  if(m_bUseOVSave) {
                      OceanViewClick(); }
                  m_OceanTimer.Clear();
                  Stat[nAxis].iAngleStep++;
        ...
        case  16: if(!m_OceanTimer.OnDelay(true , 200)) return false;
                  iPreMoveCnt++;
                  if(iMoveCnt >= iPreMoveCnt) {
                      Stat[nAxis].iAngleStep = 12;
                      return false;
                  }

                  // juhyeon. 소수점 둘째자리까지 출력하게 만듬. 수정이 필요할수도...
                  m_dMoveAng = (GetTickTime() - dAngleTime) / 1000;
                  sTemp = m_dMoveAng ;
                  sTemp.Delete(6, 99); // 일단 소수 둘째자리까지 출력할 수 있게 해놓음.
                  m_dMoveAng = sTemp.ToDouble();

                  Stat[nAxis].iAngleStep = 0;
                  Stat[nAxis].bMoveAngle = false;
                  return true;

    }

}
void __fastcall CPaixMotion::MoveShake(int nAxis , double _dTime , double _dRange, int _iAcc, int _iSpeed)
{
    Stat[nAxis].iShakeStep  = 10      ;
    Stat[nAxis].bShaking    = true    ;
    Stat[nAxis].dShakeTime  = _dTime  ;
    Stat[nAxis].dShakeRange = _dRange ;
    Stat[nAxis].iShakeAcc   = _iAcc   ;
    Stat[nAxis].iShakeSpeed = _iSpeed ;
...
bool __fastcall CPaixMotion::ShakeProc(int nAxis)
{
    ...
    switch(Stat[nAxis].iShakeStep){
        ...
        case 10 : m_ShakeTimer.Clear();
                  ...
                  pPaix -> SetSpeedDist(0, 1, Stat[nAxis].iShakeAcc, Stat[nAxis].iShakeSpeed, 0);
...
//    int iCnt   = 0;
    ...
    switch(Stat[nAxis].iORStep){
        ...
        default : Stat[nAxis].iORStep = 0 ; break ;
...
void CPaixMotion::OceanViewShow()
{
    HWND  hwnd;

    hwnd = NULL;

    hwnd = FindWindow(NULL, "OceanView 1.5.0 (Licensed)");

    if(hwnd == NULL) {
        FrmMain -> Msg("OceanView Not Opened.");
        return ;
    }

    ShowWindow(hwnd, SW_RESTORE);
    SetWindowPos(hwnd, HWND_TOPMOST  , -1280, 1, 1280, 1024, SWP_NOREDRAW );
...
void CPaixMotion::OceanViewClick()
{
    POINT pPos ;
    GetCursorPos(&pPos);

//    if(FrmMain -> edPx -> Text == "") return ;
    SetCursorPos(FrmMain -> edPx -> Text.ToInt() , FrmMain -> edPy -> Text.ToInt());
