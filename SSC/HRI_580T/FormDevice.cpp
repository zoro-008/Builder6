...
//     //Check Running Status.
    if(LT.m_bLotOpen) {
        FM_MsgOk("Error" , "JOB FILE을 DOWNLOAD 할 수 없습니다.\nLOT OPEN중 입니다.");
        return;
    }

    if (FM_MsgYesNo("Confirm","새로운 JOB FILE을 DOWNLOAD 하시겠습니까?") != mrYes) return ;


    if(!IO_GetX(xHED_1Ready) && !OM.CmnOptn.bVisn1Skip && OM.CmnOptn.bAutoVsChange ) { FM_MsgOk("ERROR","Vision1 is not Ready!") ; return ; }
    if(!IO_GetX(xHED_2Ready) && !OM.CmnOptn.bVisn2Skip && OM.CmnOptn.bAutoVsChange ) { FM_MsgOk("ERROR","Vision2 is not Ready!") ; return ; }
    ...
    DM.ARAY[riWK1].SetMaxColRow(OM.DevInfo.iColCnt,OM.DevInfo.iRowCnt );  pbStatus -> Position = 30  ;
    DM.ARAY[riVS1].SetMaxColRow(OM.DevInfo.iColCnt,OM.DevInfo.iRowCnt );  pbStatus -> Position = 40  ;
    DM.ARAY[riWK2].SetMaxColRow(OM.DevInfo.iColCnt,OM.DevInfo.iRowCnt );  pbStatus -> Position = 50  ;
    ...
    DM.ARAY[riULD].SetMaxColRow(1                 ,OM.DevInfo.iCsSlCnt);  pbStatus -> Position = 80  ;
...
//      case lvControl  : btSetting -> Enabled = true ; break ;
        default         :                               break ;
    }
