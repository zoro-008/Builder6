...
{
    if(_bValue){
        HomeStat.bHomeEnded  = true ;
    }
    else {
        HomeStat.bHomeEnded  = false;
    }
    Update();
}
    ...
    if (GetStopInpos()) {
        ...
        if (Stat.dTrgPos != _dPos) {//Position Override.
            if (fabs(_dPos - Stat.dTrgPos) > UsrPara.dInposition) {
                ...
                iReceive = AxtFS20 -> GoOvr(_dPos * PULSE_PER_UNIT); //감속 프로파일은 전에 무브 동작것을 자동적으로 따라감.
            }
        }
    }
                        ...
                        TLabel *_lbBreakOff    )
{
    ...
    if(_lbHomeEnded  ){_lbHomeEnded   -> Color   = HomeStat.bHomeEnded   ? clLime : clSilver ;}
...
//                        TCheckBox  * _cbHomeWithNSnsr ,
                        ...
                        TEdit      * _edBreakAdd      )  //SUB UI 디스플레이 OR 저장.)
{
     if(_bToTable) {
...
void CMotor::Update()
{
    ...
    //Stat.bOk          =!Stat.bPEndLimSnsr && !Stat.bNEndLimSnsr && !Stat.bAlarm && HomeStat.bHomeEnded && Stat.bServoOn ;  //OK for Equip running
    ...
    Stat.bReady       = Stat.bOk && !Stat.bPEndLimSnsr && !Stat.bNEndLimSnsr && HomeStat.bHomeEnded ;                                                        //OK for Move Again.

    //If Servo is off, Set target position to current position.
    if (!Stat.bOk) Stat.dTrgPos = Stat.dEncPos;

    //Check Error.
    if ( Stat.bAlarm  ) { if(!GetStop())Stop(); HomeStat.bHomeEnded = false; }
    if (!Stat.bServoOn) { if(!Para.bExistEnc)   HomeStat.bHomeEnded = false; }

    //Check Limit Software Pos.
    if(Para.bExistEnc) {
        if (Para.dMaxPos && (Stat.dEncPos > Para.dMaxPos + UsrPara.dInposition) && Stat.bServoOn && HomeStat.bHomeEnded && !Stat.bJogN) {
            if(!GetStop()) {
                Stop();
                Trace("Motr Stop","Para.dMaxPos && (Stat.dEncPos > Para.dMaxPos + UsrPara.dInposition) && Stat.bServoOn && HomeStat.bHomeEnded && !Stat.bJogN");
            }
        }
        if (                (Stat.dEncPos < Para.dMinPos - UsrPara.dInposition) && Stat.bServoOn && HomeStat.bHomeEnded && !Stat.bJogP) {
            if(!GetStop()) {
                Stop();
                Trace("Motr Stop","                (Stat.dEncPos < Para.dMinPos - UsrPara.dInposition) && Stat.bServoOn && HomeStat.bHomeEnded && !Stat.bJogP");
            }
        }
    }
    else {
        if (Para.dMaxPos && (Stat.dCmdPos > Para.dMaxPos + UsrPara.dInposition) && Stat.bServoOn && HomeStat.bHomeEnded && !Stat.bJogN) {Stop(); Trace("Motr Stop","Para.dMaxPos && (Stat.dCmdPos > Para.dMaxPos + UsrPara.dInposition) && Stat.bServoOn && HomeStat.bHomeEnded && !Stat.bJogN");}
        if (                (Stat.dCmdPos < Para.dMinPos - UsrPara.dInposition) && Stat.bServoOn && HomeStat.bHomeEnded && !Stat.bJogP) {Stop(); Trace("Motr Stop","                (Stat.dCmdPos < Para.dMinPos - UsrPara.dInposition) && Stat.bServoOn && HomeStat.bHomeEnded && !Stat.bJogP");}
    }
...
bool CMotor::CycleHome()
{
    ...
    switch(Para.iHomeMethod) {
        case hmOrigin    :  //홈센서 레벨은 아진보드에서 레벨 설정이 안되서 AxtFS20 클레스에서 가공(CycleHome) 해야 한다.
