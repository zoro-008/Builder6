...
//    pTracker ->Height  = 100;
    ...
    IO.DisplayProperty(true,cbIO->ItemIndex , edIOIndex, edIOAdd , edIOHexAdd , edIOName , edIOComt , cbIOInverse , edIOOnDelay , edIOOffDelay ,sgIO , cbHex -> Checked ) ;
...
void __fastcall TFrmDllMain::sgIODblClick(TObject *Sender)
{
    ...
    IO.SetY(sgIO -> Row -1 , !IO.GetY(sgIO -> Row -1) , true);
}
//---------------------------------------------------------------------------
...
{
    Trace(((TBitBtn *)Sender)->Name.c_str() , "Clicked");
    IO.DisplayProperty(false,cbIO->ItemIndex , edIOIndex , edIOAdd , edIOHexAdd , edIOName , edIOComt , cbIOInverse , edIOOnDelay , edIOOffDelay , sgIO , cbHex -> Checked ) ;
    IO.DisplayList(cbIO -> ItemIndex , sgIO , true );
}
//---------------------------------------------------------------------------
