...
class CArray
{
    ...
    public:
        ...
        void __fastcall SetStat(EN_CHIP_STAT _iStat) {for(int r = 0 ; r < m_iMaxRow ; r++) {for(int c = 0 ; c < m_iMaxCol ; c++) {CHPS[r][c].SetStat(_iStat);}} if(_iStat == csNone){SetStep(0);}}//{SetID(""); SetLotNo(""); SetStep(0);}} 별로 쓸일 없을거 같음...로더2매거진 이상일때 문제되서 삭제.
