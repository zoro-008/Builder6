...
double CUnLoader::GetMotrPos(EN_MOTR_ID _iMotr , EN_PSTN_ID _iPstnId )
{
    ...
    if(_iMotr == miULD_ZCmp){
        ...
        switch(_iPstnId) {
            default                  : dPos = MT_GetCmdPos  (_iMotr                       ); break ;
            case piULD_YCmpWait      : dPos = PM.GetValue   (_iMotr , pvULD_YCmpWaitPs    ); break ;
            case piULD_YCmpPick      : dPos = PM.GetValue   (_iMotr , pvULD_YCmpPickPs    ); break ;
            case piULD_YCmpWork      : dPos = PM.GetValue   (_iMotr , pvULD_YCmpWorkPs    ); break ;
            case piULD_YCmpPlce      : dPos = PM.GetValue   (_iMotr , pvULD_YCmpPlcePs    ); break ;
        }
    }
    ...
    if(_iMotr == miULD_ZCmp){
        if(isRailGateCnsr) { sMsg = "Strip is feeding now" ; bRet = false ;}
            ...
            switch(_iPstnId) {
                ...
                case piULD_YCmpPick      : if(isClampZCenter) {sMsg = MT_GetName(miULD_ZCmp)+" is Middle Position" ; bRet = false ;} break ;
                case piULD_YCmpPlce      : if(isClampZCenter) {sMsg = MT_GetName(miULD_ZCmp)+" is Middle Position" ; bRet = false ;} break ;
            }
    }
    else {
        bRet = false ;
...
bool CUnLoader::CycleHome()
{
    ...
    switch (Step.iHome) {
        ...
        case 11: MT_SetHomeEnd(miULD_ZCmp , true) ;
...
bool CUnLoader::Autorun(void) //오토런닝시에 계속 타는 함수.
{
    ...
    if (Step.iSeq == 0) {
        ...
        bool isLotFinished = DM.ARAY[riULD].GetCntStat  (csWork) &&
                            (
                                 (
                                     !DM.ARAY[riPSB].CheckAllStat(csNone) && (DM.ARAY[riPSB].GetLotNo() != DM.ARAY[riULD].GetLotNo())
                                 )
                             ||
                                 (
                                     !DM.ARAY[riWK2].CheckAllStat(csNone) && (DM.ARAY[riWK2].GetLotNo() != DM.ARAY[riULD].GetLotNo())&& DM.ARAY[riPSB].CheckAllStat(csNone)
                                 )
                            );

        bool isExstStrip   =!DM.ARAY[riPRB].CheckAllStat(csNone) ||
                            !DM.ARAY[riWK1].CheckAllStat(csNone) || !DM.ARAY[riVS1].CheckAllStat(csNone) ||
                            !DM.ARAY[riWK2].CheckAllStat(csNone) || !DM.ARAY[riVS2].CheckAllStat(csNone) ||
                            !DM.ARAY[riPSB].CheckAllStat(csNone) ;
        bool isPreEqpEnd   = EC.GetPreEqpLotEnd() ;
        ...
        bool isCyclePlace  = DM.ARAY[riULD].CheckAllStat(csWork ) ||(!isExstStrip && isPreEqpEnd && !DM.ARAY[riULD].CheckAllStat(csNone )) || isLotFinished;
        bool isCycleEnd    = DM.ARAY[riULD].CheckAllStat(csNone ) && !isExstStrip && isPreEqpEnd  ;
...
//Check Cycle Time Out.
    ...
    switch (Step.iCycle) {
        ...
        case  16: if(!MoveMotr(miULD_ZCmp , piULD_ZCmpPickBwd)) return false ;
                  if(OM.MstOptn.bUld_YHome) { m_bIsHome = true; MT_DoHome(miULD_YCmp); }
                  Step.iCycle++;
                  return false ;

        case  17: if(OM.MstOptn.bUld_YHome && !MT_GetHomeEnd(miULD_YCmp) ) return false ;
                  m_bIsHome = false ;
                  MoveMotr(miULD_YCmp , piULD_YCmpWork) ;
                  Step.iCycle++;
                  return false ;

        case  18: if(!MoveMotr(miULD_YCmp , piULD_YCmpWork)) return false ;
                  IO_SetY(yULD_MgzInAC,false);
                  Step.iCycle = 0 ;
                  return true ;
    }
...
//Check Cycle Time Out.
    ...
    switch (Step.iCycle) {
        ...
        case  18: if(MoveMotr(miULD_ZCmp , piULD_ZCmpPickFwd)) return false ;
