...
{
    if(DM.ARAY[riPostBuff].GetCntStat(csFail )) DM.ARAY[riPostBuff].FindFrstRowCol(csFail  , r , c);
    if(DM.ARAY[riPostBuff].GetCntStat(csRslt0)) DM.ARAY[riPostBuff].FindFrstRowCol(csRslt0 , r , c);
    if(DM.ARAY[riPostBuff].GetCntStat(csRslt1)) DM.ARAY[riPostBuff].FindFrstRowCol(csRslt1 , r , c);
    if(DM.ARAY[riPostBuff].GetCntStat(csRslt2)) DM.ARAY[riPostBuff].FindFrstRowCol(csRslt2 , r , c);
...
double CTrim::GetMotrPos(EN_MOTR_ID _iMotr , EN_PSTN_ID _iPstnId )
{
    ...
    //X , Y Pos Setting.
    double dXPos = PM.GetValue(miTRM_X , pvTRM_XWorkSttPs) +
                   c * OM.DevInfo.dColPitch +
                   (OM.DevInfo.dColGrGap - OM.DevInfo.dColPitch) * c / OM.DevInfo.iColGrCnt ;
    double dYPos = PM.GetValue(miTRM_Y , pvTRM_YWorkSttPs) +
    ...
    if(_iMotr == miTRM_X){
        switch(_iPstnId) {
            ...
            case piTRM_XWait    : dPos = PM.GetValue (_iMotr , pvVS2_XWaitPs    ); break ;
            case piTRM_XWorkStt : dPos = PM.GetValue (_iMotr , pvVS2_XInspSttPs ); break ;
        ...
        switch(_iPstnId) {
            default             : dPos = MT_GetCmdPos(_iMotr                    ); break ;
            case piTRM_YWait    : dPos = PM.GetValue (_iMotr , pvVS2_YWaitPs    ); break ;
            case piTRM_YWorkStt : dPos = PM.GetValue (_iMotr , pvVS2_YInspSttPs ); break ;
...
bool CTrim::CycleTrim(void) //Rail끝단에서 장비 밖으로 배출 되는 함수.
{
    ...
    switch (Step.iCycle) {
        ...
        case  13: if(!MoveActr(aiTR_Trimmer , ccFwd)) return false ;
                  FindChip(r,c);
                  DM.ARAY[riPostBuff].SetStat(r,c,csEmpty) ;
                  Step.iCycle++ ;
                  return false ;
...
bool CTrim::Autorun(void) //오토런닝시에 계속 타는 함수.
{
    //Check Cycle Time Out.
    AnsiString sTemp ;
    sTemp = sTemp.sprintf("%s Step.iCycle=%02d" , __FUNC__ , Step.iCycle );
    ...
    if (Step.iSeq == 0) {
        ...
        if(!IO_GetX(xTR_RjBox1 )) EM_SetErr(eiTR_NoRjBox1 );
        if(!IO_GetX(xTR_RjBox2 )) EM_SetErr(eiTR_NoRjBox2 );
        if(!IO_GetX(xTR_RjBox3 )) EM_SetErr(eiTR_NoRjBox3 );
        if( IO_GetX(xTR_SortPkg)) EM_SetErr(eiTR_FullRjBox);

        if(EM_IsErr()) return false ;

        bool isFLP_TrimPos    =  MT_CmprPos(miFRP_T , GetMotrPos(miFRP_T , piFRP_TTrim)) ;
        bool isCycleTrim      =  isFLP_TrimPos &&
                                (DM.ARAY[riPostBuff].GetCntStat(csFail ) ||
                                 DM.ARAY[riPostBuff].GetCntStat(csRslt0) ||
                                 DM.ARAY[riPostBuff].GetCntStat(csRslt1) ||
                                 DM.ARAY[riPostBuff].GetCntStat(csRslt2) );
...
{
    //Check Time Out.
    if (m_tmToStart.OnDelay(Step.iToStart && !PreStep.iToStart == Step.iToStart && CheckStop() && !OM.MstOptn.bDebugMode , 5000)) EM_SetErr(eiRL_ToStartTO);
...
{
    //Check Time Out.
    if (m_tmToStop.OnDelay(Step.iToStop && !PreStep.iToStop == Step.iToStop && CheckStop() && !OM.MstOptn.bDebugMode , 5000)) EM_SetErr(eiRL_ToStopTO);
...
{
    if(!MT_GetStopInpos(miTRM_X)) return false ;
    if(!MT_GetStopInpos(miTRM_Y)) return false ;
