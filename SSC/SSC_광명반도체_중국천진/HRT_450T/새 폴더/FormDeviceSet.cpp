    ...
    : TForm(Owner)                                                                                                                                                             
{
    ...
    PM.SetWindow(pnLd1Y , miPRB_XCmp);
    PM.SetWindow(pnLd1Z , miWRK_XCmp);
    PM.SetWindow(pnRl3X , miPSB_XCmp);
    PM.SetWindow(pnPbT  , miWRK_ZTop);
    PM.SetWindow(pnPbB  , miWRK_ZBtm);
    PM.SetWindow(pnLd1Y , miLDR_YCmp);
    PM.SetWindow(pnLd1Z , miLDR_ZCmp);
...
void __fastcall TFrmDeviceSet::pcDeviceSetChange(TObject *Sender)
{
    switch ( pcDeviceSet -> ActivePageIndex )
    {
        ...
        case 1 : FraMotor[miLDR_YCmp]->Parent = pnLDR_Y  ;
                 FraMotor[miLDR_ZCmp]->Parent = pnLDR_Z  ;

                 FraCylinder[aiLDR_Cmp]->Parent = pnLDR_Cmp1 ;
                 FraCylinder[aiLDR_Psh]->Parent = pnLDR_Psh1 ;
                 FraCylinder[aiLDR_Cmp]->Parent = pnLDR_Cmp2 ;
                 FraCylinder[aiLDR_Psh]->Parent = pnLDR_Psh2 ;

                 PM.SetParent(pnLd1Y , miLDR_YCmp);
                 PM.SetParent(pnLd1Z , miLDR_ZCmp);
        break;

        case 2 : FraMotor[miPR1_XCmp]->Parent = pnRAl_X1 ;
                 FraMotor[miPR2_XCmp]->Parent = pnRAl_X2 ;                                
                 FraMotor[miWRK_XCmp]->Parent = pnRAl_X3 ;                                
                 FraMotor[miPSB_XCmp]->Parent = pnRAl_X4 ;                                
                                                                                       //
                 FraCylinder[aiPRB_Cmp  ]->Parent = pnPRB_Cmp1 ;                       // 
                 FraCylinder[aiPRB_IdChk]->Parent = pnPRB_IdC1 ;                       // 
                 FraCylinder[aiWRK_Cmp  ]->Parent = pnWRK_Cmp1 ;
                 FraCylinder[aiPSB_Cmp  ]->Parent = pnPSB_Cmp1 ;                       
                 FraCylinder[aiPSB_Psh  ]->Parent = pnPSB_Psh1 ;                       
                 FraCylinder[aiPRB_Cmp  ]->Parent = pnPRB_Cmp2 ;                       
                 FraCylinder[aiPRB_IdChk]->Parent = pnPRB_IdC2 ;                       
                 FraCylinder[aiWRK_Cmp  ]->Parent = pnWRK_Cmp2 ;                       
                 FraCylinder[aiPSB_Cmp  ]->Parent = pnPSB_Cmp2 ;                       
                 FraCylinder[aiPSB_Psh  ]->Parent = pnPSB_Psh2 ;

                 PM.SetParent(pnRl1X , miPR1_XCmp);
                 PM.SetParent(pnRl2X , miPR2_XCmp);
                 PM.SetParent(pnRl3X , miWRK_XCmp);
                 PM.SetParent(pnRl4X , miPSB_XCmp);
        break;

        case 3 : FraMotor[miWRK_ZTop]->Parent = pnPRB_ZTop ;
                 FraMotor[miWRK_ZBtm]->Parent = pnPRB_ZBot ;
                 FraMotor[miWRK_YWrk]->Parent = pnPRB_YTop ;

                 PM.SetParent(pnSTG_TStgPs , miWRK_ZTop);
                 PM.SetParent(pnSTG_XTrsPs , miWRK_ZBtm);
                 PM.SetParent(pnSTG_ZCstPs , miWRK_YWrk);
        break;

        case 4 : FraMotor[miSTL_YTop]->Parent = pnSTR_YTop ;
                 FraMotor[miREJ_XBtm]->Parent = pnRJT_XBot ;
                 FraMotor[miSTL_YTop]->Parent = pnSTR_YTop ;

                 FraCylinder[aiSTL_Top]->Parent = pnSTR_Top1 ;
                 FraCylinder[aiSTL_Btm]->Parent = pnSTR_Bot1 ;
                 FraCylinder[aiSTL_Top]->Parent = pnSTR_Top2 ;
                 FraCylinder[aiSTL_Btm]->Parent = pnSTR_Bot2 ;

                 PM.SetParent(pnPbBY , miSTL_YBot);
                 PM.SetParent(pnPbT  , miSTL_YTop);
                 PM.SetParent(pnPbBZ , miREJ_XBtm);
        break;

        case 5 : FraMotor[miULD_YCmp]->Parent = pnHED_XHed2 ;
                 FraMotor[miULD_ZCmp]->Parent = pnHED_YHed2 ;

                 FraCylinder[aiULD_Cmp]->Parent = pnSTG_Chk2 ;
                 FraCylinder[aiULD_Cmp]->Parent = pnSTG_Alg2 ;
