...
//    RcvData = "PUT:";
    ...
    if(OM.DevOptn.bSideLEDControl){
        if(g_bPb2Rslt && _iCh == PROBE_CH2){
            for(int j=0; j>-OM.DevOptn.iContColCnt / 2; j-- ){
            sArray[OM.DevOptn.iContColCnt / 2 - j] = g_sPb2Bin[-j];
            //sArray[OM.DevOptn.iContColCnt - 1 + j] = g_sPb1Bin[-j];
                for(int i = OM.DevInfo.iRowCnt - 1 ; i >= 0 ; i --){
                    iArray[OM.DevOptn.iContColCnt / 2 - j][i] = sArray[OM.DevOptn.iContColCnt / 2 - j].SubString(i+1 , 1).ToInt();
                }
            }
        }
        else if(g_bPb1Rslt && _iCh == PROBE_CH1){
            for(int j=0; j>-OM.DevOptn.iContColCnt / 2; j-- ){
            sArray[- j] = g_sPb1Bin[-j];
                for(int i = OM.DevInfo.iRowCnt - 1 ; i >= 0 ; i --){
                iArray[-j][i] = sArray[-j].SubString(i+1 , 1).ToInt();
                }
            }
        }
    }
