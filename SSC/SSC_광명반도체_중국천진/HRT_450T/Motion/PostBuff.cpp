...
//Check Cycle Time Out.
    ...
    switch (Step.iCycle) {
        ...
        case  13: //if(!MoveMotr(miPSB_XCmp , piPSB_XGripOfs))  return false ;
        ...
        case  15: if(!MoveMotr(miPSB_XCmp , piPSB_XWRKWork)) return false;
                  MoveMotr(miWRK_ZTop , piWRK_ZTWork);
                  Step.iCycle++;
                  return false;

        case  16: if(!MoveMotr(miWRK_ZTop , piWRK_ZTWork)) return false ;
                  if(!OM.MstOptn.bUseReContact) MoveActr(aiPSB_Cmp  , ccBwd );
                  MoveMotr(miWRK_ZBtm , piWRK_ZBWork);
                  Step.iCycle++;
                  return false;

        case  17: if(!OM.MstOptn.bUseReContact && !MoveActr(aiPSB_Cmp  , ccBwd)) return false;
                  if(!MoveMotr(miWRK_ZBtm , piWRK_ZBWork)) return false ;
                  if(!OM.MstOptn.bUseReContact) MoveMotr(miPSB_XCmp , piPSB_XWRKGrip);
                  //if(!OM.MstOptn.bUseReContact) MoveMotr(miPSB_XCmp , piPSB_XGripOfs);
                  MoveMotr(miWRK_YBtm, piWRK_YBWork) ;
                  Step.iCycle++;
                  return false ;

        case  18: if(!MoveMotr(miWRK_YBtm, piWRK_YBWork)) return false ;
        ...
        //주의 밑에서 사용.
        case  19: OM.DevOptn.iContactPinCnt++;
        ...
        case  20: if(!m_tmTemp.OnDelay(true ,OM.DevOptn.iPrbSttDelay)) return false ;
        ...
        case  21: if(m_tmTemp.OnDelay(true , OM.DevOptn.i1ChLedTime * 100 + 1000)) {
                      if(!g_bPb1Rslt) {
        ...
        case  22: if(m_tmTemp.OnDelay(true,OM.DevOptn.i2ChLedTime * 100 + 1000)) {
                      if(!g_bPb2Rslt) {
                          MoveMotr(miWRK_YBtm, piWRK_YBWait) ;
                          MoveActr(aiPSB_Cmp , ccFwd);
                          Step.iCycle = 50 ;
        ...
        case  25: if(!MoveActr(aiPSB_Cmp , ccBwd)) return false;
                  MoveMotr(miPSB_XCmp , piPSB_XWRKGrip);
                  //MoveMotr(miPSB_XCmp , piPSB_XGripOfs);
                  Step.iCycle++;
                  return false;

        case  26: if(!MoveMotr(miPSB_XCmp , piPSB_XWRKGrip)) return false;
        ...
        case  31: if(!MoveActr(aiPSB_Cmp , ccFwd)) return false;
        ...
        case  51: if(!MoveActr(aiPSB_Cmp , ccBwd)) return false;
                  Step.iCycle = 0 ;
                  return false ;
    }
}
