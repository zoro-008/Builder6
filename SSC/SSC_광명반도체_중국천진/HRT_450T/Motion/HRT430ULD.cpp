    ...
    if (Step.iSeq == 0) {
        ...
        bool isLotFinished = DM.ARAY[riULD].GetCntStat  (csWork) &&
                          (
                           ...
                           (!DM.ARAY[riWRK].CheckAllStat(csNone) && (DM.ARAY[riWRK].GetLotNo() != DM.ARAY[riULD].GetLotNo())&& DM.ARAY[riPS2].CheckAllStat(csNone)));

        bool isExstStrip   = !DM.ARAY[riPR2].CheckAllStat(csNone) || !DM.ARAY[riWRK].CheckAllStat(csNone) || !DM.ARAY[riWRK].CheckAllStat(csNone) || !DM.ARAY[riPS2].CheckAllStat(csNone);
                                                                //|| !DM.ARAY[riVS1].CheckAllStat(csNone) ||!DM.ARAY[riVS2].CheckAllStat(csNone) ;
        ...
        bool isCyclePlace  = DM.ARAY[riULD].CheckAllStat(csWork ) ||(!isExstStrip && !DM.ARAY[riULD].CheckAllStat(csNone))|| isLotFinished; // && isPreEqpEnd )
        bool isCycleEnd    = DM.ARAY[riULD].CheckAllStat(csNone ) && !isExstStrip ;//&& isPreEqpEnd  ;

        Stat.bReadyMgz = isZWorkPos ;

        //모르는 스트립에러.
        if(  DM.ARAY[riULD].CheckAllStat(csNone) && (  IO_GetX(xULD_MgzClamp1 ) ||  IO_GetX(xULD_MgzClamp2 ))                       ) EM_SetErr(eiULD_MgzUnknown) ;
        //카세트 사라짐.
        if( !DM.ARAY[riULD].CheckAllStat(csNone) && ( !IO_GetX(xULD_MgzClamp1 ) && !IO_GetX(xULD_MgzClamp2 ))&& !OM.CmnOptn.bDryRun ) EM_SetErr(eiULD_MgzDispr  ) ; //sun bDryRun 옵션 추가.
        //카세트 OUT 매거진 풀
