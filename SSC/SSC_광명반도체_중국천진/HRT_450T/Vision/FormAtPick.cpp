...
#pragma link "CSPIN"
#pragma resource "*.dfm"
TFrmAtPick *FrmAtPick;
//---------------------------------------------------------------------------
__fastcall TFrmAtPick::TFrmAtPick(TComponent* Owner)
        : TForm(Owner)
{

}

void TFrmAtPick::Init()
{
    Left = 0 ;
    Top  = 0 ;
    ...
    FrmTrain -> SetTrainArea ( ((CAtPick *)VISN[m_iVisnId])-> Orig.pTrArea );
    FrmTrain -> SetTrainImg  ( ((CAtPick *)VISN[m_iVisnId])-> Orig.pTrImage);
...
void TFrmAtPick::ApplyPara(bool _bToTable)
{
    ...
    if(_bToTable) {
        ...
        //Common Option
        cbImageSave     -> ItemIndex = VOM.VCmnOptn.iAtPickImageSave      ;

        //Dev Optn
        edCountX        -> Value     = ((CAtPick *)VISN[m_iVisnId])->GetInspColCnt() ;
        edCountY        -> Value     = ((CAtPick *)VISN[m_iVisnId])->GetInspRowCnt() ;

        edOriSinc       -> Value     = VOM.VDevOptn.dAtPickOriSinc        ;

        cbOverUnder     -> ItemIndex = VOM.VDevOptn.bAtPickOverUnder ? 1 : 0 ;

        //DevPara.
        sbThresholdLow  -> Position  = VOM.VDevPara.AtPickTsd.iThresholdLow  ;
        sbThresholdHigh -> Position  = VOM.VDevPara.AtPickTsd.iThresholdHigh ;
        sbPxMinPer      -> Position  = VOM.VDevOptn.dAtPickPxMinPer          ;
    }
    else {
        //Cam Light Para
        ...
        //Common Option
        VOM.VCmnOptn.iAtPickImageSave         = cbImageSave     -> ItemIndex ;

        //Dev Optn
        ((CAtPick *)VISN[m_iVisnId])->SetInspColCnt(edCountX        -> Value);
        ((CAtPick *)VISN[m_iVisnId])->SetInspRowCnt(edCountY        -> Value);

        VOM.VDevOptn.dAtPickOriSinc           = edOriSinc       -> Value     ;

        VOM.VDevOptn.bAtPickOverUnder         = cbOverUnder     -> ItemIndex ;

        //DevPara.
        VOM.VDevPara.AtPickTsd.iThresholdLow  = sbThresholdLow  -> Position  ;
        VOM.VDevPara.AtPickTsd.iThresholdHigh = sbThresholdHigh -> Position  ;
        VOM.VDevOptn.dAtPickPxMinPer          = sbPxMinPer      -> Position  ;
...
void __fastcall TFrmAtPick::btSaveClick(TObject *Sender)
{
    ApplyPara(false);
...
void __fastcall TFrmAtPick::FormShow(TObject *Sender)
{
...
void TFrmAtPick::FormShow(EN_PAGE_ID _iPage)
{
...
void __fastcall TFrmAtPick::sbThresholdLowChange(TObject *Sender)
{
    edThresholdLow -> Text = sbThresholdLow -> Position ;
}
//---------------------------------------------------------------------------

void __fastcall TFrmAtPick::sbThresholdHighChange(TObject *Sender)
{
    edThresholdHigh -> Text = sbThresholdHigh -> Position ;
}
//---------------------------------------------------------------------------

void __fastcall TFrmAtPick::sbPxMinPerChange(TObject *Sender)
{
...
void __fastcall TFrmAtPick::pcSetChange(TObject *Sender)
{
    VISN[m_iVisnId]->SetActivPage((EN_PAGE_ID)pcSet -> ActivePageIndex) ;
    btTrain -> Visible = pcSet -> ActivePageIndex == piAtPickOcv ;

    FrmImage[m_iVisnId] -> OnPaint();
}
//---------------------------------------------------------------------------

void __fastcall TFrmAtPick::btTrainClick(TObject *Sender)
{
    if(VISN[m_iVisnId]->GetActivPage() == piAtPickLCS) {
    }
    else if(VISN[m_iVisnId]->GetActivPage() == piAtPickOcv) {
        VISN[m_iVisnId]->Train();
        FrmTrain -> OnPaint() ;
    }
    else if(VISN[m_iVisnId]->GetActivPage() == piAtPickTsd) {
    }
}
...
void __fastcall TFrmAtPick::tmUpdateTimer(TObject *Sender)
{
    ...
    if(iPreX != ((CAtPick *)VISN[m_iVisnId]) -> Insp.pTrckInsp1 -> Left  ) bChangedTracker = true ;
    if(iPreY != ((CAtPick *)VISN[m_iVisnId]) -> Insp.pTrckInsp1 -> Top   ) bChangedTracker = true ;
    if(iPreW != ((CAtPick *)VISN[m_iVisnId]) -> Insp.pTrckInsp1 -> Width ) bChangedTracker = true ;
    if(iPreH != ((CAtPick *)VISN[m_iVisnId]) -> Insp.pTrckInsp1 -> Height) bChangedTracker = true ;
...
void __fastcall TFrmAtPick::btLtClick(TObject *Sender)
{
    ...
    float fXPitch   = (((CAtPick *)VISN[m_iVisnId])->Insp.pTrckInsp2 -> Left - ((CAtPick *)VISN[m_iVisnId])->Insp.pTrckInsp1 -> Left) / (float)VOM.VDevOptn.iCountX ;
    float fYPitch   = (((CAtPick *)VISN[m_iVisnId])->Insp.pTrckInsp2 -> Top  - ((CAtPick *)VISN[m_iVisnId])->Insp.pTrckInsp1 -> Top ) / (float)VOM.VDevOptn.iCountY ;
    float fTrackerW =  ((CAtPick *)VISN[m_iVisnId])->Insp.pTrckInsp1 -> Width  ;
    float fTrackerH =  ((CAtPick *)VISN[m_iVisnId])->Insp.pTrckInsp1 -> Height ;

    if(iSelButton == 0 ) {//left top
        fWidth  = fXPitch * 2;
        fHeight = fYPitch * 2;
        fLeft   = ((CAtPick *)VISN[m_iVisnId])->Insp.pTrckInsp1 -> Left - fWidth /2 + fTrackerW/2;
        fTop    = ((CAtPick *)VISN[m_iVisnId])->Insp.pTrckInsp1 -> Top  - fHeight/2 + fTrackerH/2;
    }
    else if(iSelButton == 1) { //right top
        fWidth  = fXPitch * 2;
        fHeight = fYPitch * 2;
        fLeft   = ((CAtPick *)VISN[m_iVisnId])->Insp.pTrckInsp2 -> Left - fWidth /2 + fTrackerW/2;
        fTop    = ((CAtPick *)VISN[m_iVisnId])->Insp.pTrckInsp1 -> Top  - fHeight/2 + fTrackerH/2;
    }
        ...
        fHeight = fYPitch * 2;
        fLeft   = ((CAtPick *)VISN[m_iVisnId])->Insp.pTrckInsp1 -> Left - fWidth /2 + fTrackerW/2;
        fTop    = ((CAtPick *)VISN[m_iVisnId])->Insp.pTrckInsp2 -> Top  - fHeight/2 + fTrackerH/2;
    }
