...
COcv::COcv()
{
    ...
    TrackerT = new TRectTracker() ;
    TrackerI = new TRectTracker() ;
    ...
    TrackerT -> Caption = "OCV_Train";
    TrackerI -> Caption = "OCV_Insp" ;
...
{
    TrackerT -> SetFunc(ChangeFunc) ;
    TrackerI -> SetFunc(ChangeFunc) ;
}

void COcv::SetTrackerVisible(bool _bVisible)
{
    TrackerT -> Visible = _bVisible ;
    TrackerI -> Visible = _bVisible ;
}

void COcv::SetTrackerEnabled(bool _bEnabled)
{
    TrackerT -> Enabled = _bEnabled ;
    TrackerI -> Enabled = _bEnabled ;
}

bool COcv::MouseTrackerDn(TMouseButton Button, TShiftState Shift, int X, int Y , float _fScaleX , float _fScaleY)
{
    TrackerT -> Focused = false ;
    TrackerI -> Focused = false ;

    if(TrackerT -> TrackerDw(Button, Shift , X,Y, _fScaleX , _fScaleY)) { m_fInspOfsX = 0.0 ; m_fInspOfsY = 0.0 ; return true ;}
    if(TrackerI -> TrackerDw(Button, Shift , X,Y, _fScaleX , _fScaleY)) { m_fInspOfsX = 0.0 ; m_fInspOfsY = 0.0 ; return true ;}
...
{
    if(TrackerT -> TrackerMv(Shift, X, Y , _fScaleX , _fScaleY)) return true ;
    if(TrackerI -> TrackerMv(Shift, X, Y , _fScaleX , _fScaleY)) return true ;
...
{
    TrackerT -> TrackerUp(Button, Shift );
    TrackerI -> TrackerUp(Button, Shift );
}

void COcv::PaintTracker(HDC _hHdc , float _fScaleX , float _fScaleY)
{
    if(TrackerI -> Visible)PaintOfsRect(_hHdc , _fScaleX , _fScaleY , GetOfsRect(TrackerI -> GetTRect()));
    TrackerT -> Paint(_hHdc , _fScaleX , _fScaleY);
    TrackerI -> Paint(_hHdc , _fScaleX , _fScaleY);
...
void COcv::LoadPara(bool _bLoad , AnsiString _sFilePath , AnsiString _sId)
{
    ...
    AnsiString sTrArea = _sFilePath + "_" + _sId+"_OcvTrainArea.bmp" ;
    ...
    TrackerT -> OnLoadSave(_bLoad , sTrakPath , sTemp + "_TrackerT");
    TrackerI -> OnLoadSave(_bLoad , sTrakPath , sTemp + "_TrackerI");
    ...
    if(_bLoad) {
        ...
        UserINI.Load(sFilePath, sTemp , "iInspFreq ", Para.iInspFreq );
        ...
        UserINI.Save(sFilePath, sTemp , "iInspFreq ", Para.iInspFreq );
...
void COcv::ApplyPara(bool _bTable)
{
    if(_bTable) {
        ...
        ParaUi.edInspFreq   -> Text = Para.iInspFreq  ;
        ...
        iTemp = StrToIntDef(ParaUi.edInspFreq   -> Text , 10 ); if (Para.iInspFreq  != iTemp) { Trace("Operator" , "Both Ocv_InspFreq values are different."); }
        ...
        Para.iInspFreq  = StrToIntDef(ParaUi.edInspFreq   -> Text , 10 ) ; ParaUi.edInspFreq   -> Text = Para.iInspFreq ;
...
{
    bool bRet ;
    bRet = OCV_Train(_pImg , TrackerT -> GetTRect() , TrPr , m_pTrArea , m_pTrImage);
...
bool COcv::Process(IImage * _pImg , OCV_Rslt * _pRslt)
{
    ...
    bRet = OCV_Inspect(_pImg , m_pTrArea , m_pTrImage , GetOfsRect(TrackerI -> GetTRect()) , Para , m_pRsltArea , _pRslt);
