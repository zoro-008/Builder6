...
//    if(MAX_MOTR != MT_GetMotorCnt()) {ShowMessageT("Motor Count is wrong");}
    ...
    memset( Value    , 0 , sizeof(SValue)*MAX_TAB*MAX_TAB_MOTR );

    Value[tiLDR  ][miLD1_ZCmp].sName = "Loader Z"          ;
    Value[tiLDR  ][miLD1_YCmp].sName = "Loader Y"          ;
    Value[tiRAIL ][miPRB_XCmp].sName = "1st Insert X Axis" ;
    Value[tiRAIL ][miWRK_XCmp].sName = "2st Insert X Axis" ;
    Value[tiRAIL ][miPSB_XCmp].sName = "3st Insert X Axis" ;
    Value[tiPROBE][miWRK_ZTop].sName = "Probe Top"         ;
    Value[tiPROBE][miWRK_ZBtm].sName = "Probe Bottom"      ;

    Value[tiLDR  ][miLD1_ZCmp].iType = vtUpDown  ;
    Value[tiLDR  ][miLD1_YCmp].iType = vtBackFor ;
    Value[tiRAIL ][miPRB_XCmp].iType = vtBackFor ;
    Value[tiRAIL ][miWRK_XCmp].iType = vtBackFor ;
    Value[tiRAIL ][miPSB_XCmp].iType = vtBackFor ;
    Value[tiPROBE][miWRK_ZTop].iType = vtUpDown ;
    Value[tiPROBE][miWRK_ZBtm].iType = vtUpDown ;
...
double CDispMan::GetValue(int _iAxisNo , EN_PSTN_VALUE _iPstnNo)
{
...
/*
bool CDispMan::CmprPos(int _iAxisNo , EN_PSTN_VALUE _iPstnNo)
{
    if( _iAxisNo < 0 || _iAxisNo >= MAX_MOTR          ) {ShowMessageT("Motor Axis Range is wrong"    ); return 0.0 ;}
    if( _iPstnNo < 0 || _iPstnNo >= PSTN_CNT[_iAxisNo]) {ShowMessageT("Position Index Range is wrong"); return 0.0 ;}
...
*/
void CDispMan::Save(AnsiString sJobName)
{
    //Local Var.
    TUserINI   UserINI ;
    AnsiString sPath   ;
...
void CDispMan::Load(AnsiString sJobName)
{
    //Local Var.
    TUserINI   UserINI ;
    AnsiString sPath   ;
...
void CDispMan::UpdatePstn(bool _bToTable)
{
    if(_bToTable) {
        for(int m = 0 ; m < MAX_MOTR ; m++) {
            for(int i = 0 ; i < PSTN_CNT[m] ; i++) {
...
//---------------------------------------------------------------------------
void __fastcall CDispMan::pPstnListDrawCell(TObject *Sender, int ACol,
...
//---------------------------------------------------------------------------
void __fastcall CDispMan::pPstnListMouseDown(TObject *Sender,
...
//---------------------------------------------------------------------------
void __fastcall CDispMan::pPstnListMouseUp(TObject *Sender,
      TMouseButton Button, TShiftState Shift, int X, int Y)
{
...
void CDispMan::SetWindow(TPanel * _pnBase )
{
...
//---------------------------------------------------------------------------
void __fastcall CDispMan::pPstnEditKeyDown(TObject *Sender, WORD &Key,
      TShiftState Shift)
{
   if(Key == VK_RETURN){
...
void __fastcall CDispMan::pPstnEditExit(TObject *Sender)
{
    Trace(MT_GetName(GridInfo.iGrid).c_str() , (m_pPstnList[GridInfo.iGrid] -> Cells[GridInfo.iCol][GridInfo.iRow]+"->"+m_pPstnEdit[GridInfo.iGrid] -> Text.ToDouble()).c_str());
    m_pPstnList[GridInfo.iGrid] -> Cells[GridInfo.iCol][GridInfo.iRow] = m_pPstnEdit[GridInfo.iGrid] -> Text.ToDouble() ;
    m_pPstnEdit[GridInfo.iGrid] -> Visible = false ;
