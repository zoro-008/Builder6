...
bool CLoader::CheckSafe(EN_MOTR_ID _iMotr , EN_PSTN_ID _iPstnId)
{
    ...
    bool isFwPusher      =  IO_GetX(xLD1_PusherFw) || !IO_GetX(xLD1_PusherBw);
//    bool isBwPusher      = !IO_GetX(xLD1_PusherFw) ||  IO_GetX(xLD1_PusherBw);
...
bool CLoader::CycleHome()     //sun DLL direct access.
{
    ...
    switch (Step.iHome) {
        ...
        case 10: IO_SetY(yLD1_PusherFw,false);
                 IO_SetY(yLD1_PusherBw,true );
...
bool CLoader::Autorun(void) //오토런닝시에 계속 타는 함수.
{
    ...
    if (Step.iSeq == 0) {
        ...
        bool isCycleSupply =  DM.ARAY[riLD1].CheckAllStat(csNone ) && !Stat.bChckSplyEmpty && !IO_GetX(xLD1_MgzIn )&& ((m_iMgzCnt < OM.DevOptn.iLotEndMgzCnt)  || GetLotNo()!="") ;
        bool isCyclePick   =  DM.ARAY[riLD1].CheckAllStat(csNone ) && (IO_GetX(xLD1_MgzIn) ||  OM.CmnOptn.bDryRun )&& ((m_iMgzCnt < OM.DevOptn.iLotEndMgzCnt)  || GetLotNo()!="") ;
        bool isCycleWork   =  DM.ARAY[riLD1].GetCntStat  (csUnkwn) &&  DM.ARAY[riPR1].CheckAllStat(csNone ) && DM.ARAY[riPR2].CheckAllStat(csNone ) &&
                              PRB.GetCycleStep() != CPreBuff::scIn && !OM.CmnOptn.bLoadingStop;
...
//Check Cycle Time Out.
    ...
    switch (Step.iCycle) {
        ...
        case  11: if(!MoveActr(aiLD1_Cmp    , ccFwd      )) return false ;
                  ...
                  if(!IO_GetX (xLD1_PusherBw             )) return false ;
                  IO_SetY(yLD1_PusherBw,false);
        ...
        case  14: if(!MoveMotr(miLD1_ZCmp , piLD1_ZClampOn)) return false ;
                  ...
                  if(m_iMgzCnt > OM.DevOptn.iLotEndMgzCnt) {
                      ...
                      DM.ARAY[riLD1].SetLotNo(PopLotNo()) ;
...
//Check Cycle Time Out.
    ...
    switch (Step.iCycle) {
        ...
        case  12: if(!MoveMotr(miLD1_ZCmp , piLD1_ZWork)) return false ;
                  ...
                  iSltCnt = OM.DevInfo.iCsSlCnt - DM.ARAY[riLD1].FindLastRow(csUnkwn) ; //case 14에서 일로 이사옴.
                  DM.ARAY[riPR1].SetID(m_iMgzCnt * 100 + iSltCnt);
                  DM.ARAY[riPR1].SetLotNo(DM.ARAY[riLD1].GetLotNo());
                  Step.iCycle++;
                  return false;

        case  13: if(IO_GetX(xLD1_PusherOverLoad)) {
                      IO_SetY(yLD1_PusherFw,false);
                      IO_SetY(yLD1_PusherBw,true );
...
//                  DM.ShiftArrayData(riLD1,riPRB);
        ...
        case  14: if(!IO_GetX(xLD1_PusherBw))return false ;
