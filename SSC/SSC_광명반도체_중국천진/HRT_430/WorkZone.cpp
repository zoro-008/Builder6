...
double CWorkZone::GetMotrPos(EN_MOTR_ID _iMotr , EN_PSTN_ID _iPstnId )
{
    ...
    double dX1Pos = PM.GetValue(miWRK_XCmp , pvWRK_XWorkStartPs) + (OM.DevInfo.dColPitch * fc) ;
    double dX2Pos = PM.GetValue(miWRK_XCmp , pvWRK_XWorkStartPs) + (OM.DevInfo.dColPitch * fc) - OM.DevOptn.dReGrabPs1                         ;
    double dX3Pos = PM.GetValue(miWRK_XCmp , pvWRK_XWorkStartPs) + (OM.DevInfo.dColPitch * fc) - OM.DevOptn.dReGrabPs1 - OM.DevOptn.dReGrabPs2 ;

    //Regrib pos.
    double dX2PosRg = PM.GetValue(miWRK_XCmp , pvWRK_XWorkStartPs) + (OM.DevInfo.dColPitch * rc) - OM.DevOptn.dReGrabPs1                         ;
    double dX3PosRg = PM.GetValue(miWRK_XCmp , pvWRK_XWorkStartPs) + (OM.DevInfo.dColPitch * rc) - OM.DevOptn.dReGrabPs1 - OM.DevOptn.dReGrabPs2 ;
    ...
    if( OM.DevInfo.iColGrCnt > 1 ) {
        ...
        dX3Pos += (OM.DevInfo.dColGrGap - OM.DevInfo.dColPitch) * iColGrCnt ;

        iColGrCnt = rc / OM.DevInfo.iColGrCnt ;
        dX2PosRg += (OM.DevInfo.dColGrGap - OM.DevInfo.dColPitch) * iColGrCnt ;
        dX3PosRg += (OM.DevInfo.dColGrGap - OM.DevInfo.dColPitch) * iColGrCnt ;
    }
...
bool CWorkZone::Autorun(void) //오토런닝시에 계속 타는 함수.
{
    ...
    if (Step.iSeq == 0) {
        ...
        bool isReGrip1       =  fc >= OM.DevOptn.iReGrabCnt1 && m_iReGripCnt == 0 ;
        bool isReGrip2       =  fc >= OM.DevOptn.iReGrabCnt2 && m_iReGripCnt == 1 ;
        ...
        bool isCycleWork     =  DM.ARAY[riWRK].GetCntStat  (csUnkwn) &&( isPstEmpty || !isHalfWorked ) ;
        bool isCyclePostWork = !DM.ARAY[riWRK].GetCntStat  (csUnkwn) && !DM.ARAY[riWRK].CheckAllStat(csNone)&& DM.ARAY[riPS1].CheckAllStat(csNone) && DM.ARAY[riPS2].CheckAllStat(csNone);
...
//Check Cycle Time Out.
    AnsiString sTemp ;
    if (m_tmCycle.OnDelay(Step.iCycle && Step.iCycle == PreStep.iCycle && CheckStop() && Step.iCycle != 18 && Step.iCycle != 19 && !OM.MstOptn.bDebugMode, 5000 )) {
        EM_SetErr(eiWRK_CycleTO);
        sTemp = sTemp.sprintf( "%s TIMEOUT STATUS : Step.iCycle=%02d" , __FUNC__ , Step.iCycle );
        Trace(m_sPartName.c_str(),sTemp.c_str());
        Step.iCycle = 0 ;
    ...
    switch (Step.iCycle) {
        ...
        case  12: if(!MoveMotr(miWRK_XCmp,piWRK_XWork))  return false ;
                  ...
                  if(!OM.CmnOptn.bIgnrInsp) {
                      ...
                      OM.DevOptn.iPinCtCnt++;
                      m_sTemp = OM.DevOptn.iPinCtCnt ;
                      Trace(OM.m_sCrntDev.c_str(),m_sTemp.c_str());
                  }
                  if(OM.DevOptn.iLimPinCtCnt < OM.DevOptn.iPinCtCnt) {
                      EM_SetErr(eiWRK_ContactCntOver);
                      Step.iCycle = 0;
                      return true;
                  }
                  Step.iCycle++;
        ...
        case  13: //if(!MoveMotr(miWRK_ZBtm,piWRK_ZBWork)) return false ;
                  if(!OM.CmnOptn.bIgnrInsp ) if(!MoveMotr(miWRK_ZTop,piWRK_ZTWork)) return false ;
                  if( OM.MstOptn.bGripTest )  MoveActr(aiWRK_Cmp,ccBwd); //차민철 과장님 요청사항.
                  if(!OM.CmnOptn.bIgnrInsp )  MoveMotr(miWRK_ZBtm,piWRK_ZBWork);
                  Step.iCycle++;
                  return false;


        case  14: if(!OM.CmnOptn.bIgnrInsp) if(!MoveMotr(miWRK_ZBtm,piWRK_ZBWork)) return false ;
        ...
        case  18: m_i1ChDelay  = OM.DevInfo.iRowCnt*OM.DevOptn.iContColCnt*OM.DevOptn.i1ChLedTime + OM.DevInfo.iRowCnt*OM.DevOptn.iContColCnt*OM.DevOptn.i1ChZenTime + 1000 ;
                  if(m_tmTemp.OnDelay(true, m_i1ChDelay)) {
                      if(!g_bPb1Rslt)EM_SetErr(eiWRK_FailProbeComm1) ;
//                      if(!g_bPb2Rslt)EM_SetErr(eiWRK_FailProbeComm2) ;
//                      IO_SetY(yWRK_Probe1,false);
//                      IO_SetY(yWRK_Probe2,false);
                      Step.iCycle = 0 ;
                      return false ;
                  }
        ...
        case  19: m_i2ChDelay  = OM.DevInfo.iRowCnt*OM.DevOptn.iContColCnt*OM.DevOptn.i2ChLedTime + OM.DevInfo.iRowCnt*OM.DevOptn.iContColCnt*OM.DevOptn.i2ChZenTime + 1000 ;
                  if(m_tmTemp.OnDelay(true,m_i2ChDelay)) {
//                      if(!g_bPb1Rslt)EM_SetErr(eiWRK_FailProbeComm1) ;
                      if(!g_bPb2Rslt)EM_SetErr(eiWRK_FailProbeComm2) ;
//                      IO_SetY(yWRK_Probe1,false);
//                      IO_SetY(yWRK_Probe2,false);
...
//                  IO_SetY(yWRK_Probe2,false);
        ...
        case  20: if(OM.MstOptn.bGripTest) { if(!MoveActr(aiWRK_Cmp,ccFwd)) return false ; } //차민철 과장님 요청사항.
                  ...
                  /*
                  //if(OM.CmnOptn.bIgnrInsp) iFail = 0;
                  for(int j = 0 ; j < OM.DevOptn.iContactCnt ;j++) {
                      iFail += DM.ARAY[riWRK].GetCntColStat((iMaxCol - c) - j,csFail) ;
                  }
                  */
                  if(OM.MstOptn.bGripTest) MoveActr(aiWRK_Cmp,ccFwd); //차민철 과장님 요청사항.
                  MoveMotr(miWRK_ZBtm,piWRK_ZBWait);
...
//                          }
                  ...
                  //memset(iPinFailCnt , 0 , sizeof(int)*MAX_PROB_COL *  MAX_ARRAY_ROW);
        ...
        case  21: if(OM.MstOptn.bGripTest) { if(!MoveActr(aiWRK_Cmp,ccFwd)) return false ; } //차민철 과장님 요청사항.
                  if(!MoveMotr(miWRK_ZBtm,piWRK_ZBWait)) return false ;
                  if(!MoveMotr(miWRK_ZTop,piWRK_ZTWait)) return false ;
                  //if (!DM.ARAY[riWRK].GetCntStat(csUnkwn) && DM.ARAY[riWRK].GetCntStat(csFail) >= OM.DevOptn.iLimAllFailCnt && !OM.CmnOptn.bIgnrInsp) EM_SetErr(eiWRK_FailCntOver);
...
//Check Cycle Time Out.
    ...
    switch (Step.iCycle) {
        ...
        case  10: //PCM.ClearData();
                  bRepeat = Stat.bRepeat;
        ...
        case  11: if(!MoveMotr(miWRK_ZBtm,piWRK_ZBWait)) return false ;
                  if(!MoveMotr(miWRK_ZTop,piWRK_ZTWait)) return false ;
                  if(!MoveActr(aiWRK_Cmp,ccFwd))         return false ;
                  DM.ARAY[riTmp].SetStat(csUnkwn);
                  MoveMotr(miWRK_ZTop,piWRK_ZTWork);
                  Step.iCycle++;
                  return false;

        case  12: //if(!MoveMotr(miWRK_ZBtm,piWRK_ZBWork)) return false ;
        ...
        case  13: if(OM.MstOptn.bGripTest) { if(!MoveActr(aiWRK_Cmp,ccBwd)) return false ; } //차민철 과장님 요청사항.
        ...
        case  14: if(!m_tmTemp.OnDelay(true ,OM.DevOptn.iPrbSttDelay)) return false ;
        ...
        case  15: //if(m_tmInspect1.OnDelay(IO_GetY(yWRK_Probe1),1000) && !OM.CmnOptn.bIgnrInsp) { Step.iCycle = 0; EM_SetErr(eiWRK_FailProbeComm1); return true; }
                  //if(m_tmInspect2.OnDelay(IO_GetY(yWRK_Probe2),1000) && !OM.CmnOptn.bIgnrInsp) { Step.iCycle = 0; EM_SetErr(eiWRK_FailProbeComm2); return true; }
                  ...
                  if(!g_bPb1Rslt || !g_bPb2Rslt) return false ;
        ...
        case  16: if(OM.MstOptn.bGripTest) { if(!MoveActr(aiWRK_Cmp,ccFwd)) return false ; } //차민철 과장님 요청사항.
                  //마스킹 패턴 및 재 검사 패턴.
                  FindChip(r,c);
                  //if(!DM.ARAY[riWRK].GetCntStat(csUnknwn)&&!DM.ARAY[riWRK].GetCntStat(csNone)){   //메뉴얼 동작시에는 제자리만 찍으려고 미리 지움.
                  //    for( int j =0 ; j < OM.DevOptn.iContColCnt ; j++ ) {
                  //        for(int i = 0 ; i < iMaxRow ; i++ ) DM.ARAY[riWRK].SetStat(i , c + j + 1, csGood); //데이타 마스킹.
                  //    }
                  //}
                  FindChip(r,c);
                  ...
                  if(OM.CmnOptn.bIgnrInsp) {
                      ...
                      r1 = PCM.CheckDataCnt(riTmp , g_sPb1Msg);
                      r2 = PCM.CheckDataCnt(riTmp , g_sPb2Msg);

                      if(!r1)EM_SetErr(eiWRK_Probe1DataWrong) ; //데이터 마스킹.  나중에 PCM에 ARAY 걷어 내고 여기서 데이터 받아서 마스킹 한다.
                      if(!r2)EM_SetErr(eiWRK_Probe2DataWrong) ;

                      if(r1 && r2) {
                          PCM.GetDataCh(riWRK , g_sPb1Msg , PROBE_CH1 ,true); //true = repeat ;
                          PCM.GetDataCh(riWRK , g_sPb2Msg , PROBE_CH2 ,true);
                      }
                  }
                  iFail = 0;
                  for( int j =0 ; j < OM.DevOptn.iContColCnt ; j++ ) {
                  ...
                  //검사하고 Data맵 지움.
                  FindChip(r,c);
                  if(DM.ARAY[riWRK].GetCntStat(csWork)||DM.ARAY[riWRK].GetCntStat(csFail)){   //메뉴얼 동작시에는 제자리만 찍으려고 미리 지움.
                      for( int j =0 ; j < OM.DevOptn.iContColCnt ; j++ ) {
                          for(int i = 0 ; i < iMaxRow ; i++ ) DM.ARAY[riWRK].SetStat(i , c + j + 1, csUnkwn); //데이타 마스킹.
                      }
                  }
                  if(OM.MstOptn.bGripTest) MoveActr(aiWRK_Cmp,ccFwd); //차민철 과장님 요청사항.
                  MoveMotr(miWRK_ZBtm,piWRK_ZBWait);
        ...
        case  17: if(OM.MstOptn.bGripTest) { if(!MoveActr(aiWRK_Cmp,ccFwd)) return false ; } //차민철 과장님 요청사항.
                  if(!MoveMotr(miWRK_ZBtm,piWRK_ZBWait)) return false ;
                  if(!MoveMotr(miWRK_ZTop,piWRK_ZTWait)) return false ;
...
//Check Cycle Time Out.
    AnsiString sTemp ;
    if (m_tmCycle.OnDelay(Step.iCycle && Step.iCycle == PreStep.iCycle && CheckStop() && Step.iCycle != 18 && Step.iCycle != 19 && !OM.MstOptn.bDebugMode, 11000 )) {
        EM_SetErr(eiWRK_CycleTO);
        sTemp = sTemp.sprintf( "%s TIMEOUT STATUS : Step.iCycle=%02d" , __FUNC__ , Step.iCycle );
        Trace(m_sPartName.c_str(),sTemp.c_str());
        Step.iCycle = 0 ;
    ...
    switch (Step.iCycle) {
        ...
        case  14: if(OM.MstOptn.bGripTest) { if(!MoveActr(aiWRK_Cmp,ccBwd)) return false ; } //차민철 과장님 요청사항.
                  if(!OM.CmnOptn.bIgnrInsp) if(!MoveMotr(miWRK_ZBtm,piWRK_ZBWork)) return false ;
        /*          MT_GoIncRun(miWRK_ZBtm , -2.0);
                  Step.iCycle++;
                  return false ;
