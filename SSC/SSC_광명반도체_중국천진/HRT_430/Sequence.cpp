...
void __fastcall CSequence::Reset(void)
{
    ...
    if(OM.CmnOptn.bAutoOperation)EC.SendPstEqp(psReset);
    //g_sPb2Msg = "" ;
    //g_sPb1Msg = "" ;
...
//    bool bResetSw = IO_GetXUp(xETC_LResetSw) || IO_GetXUp(xETC_RResetSw) || IO_GetXUp(xETC_ResetSw) || m_bBtnReset ;
    ...
    if (bStartSw) {
        ...
        if(OM.CmnOptn.bAutoOperation)EC.SendPstEqp(psStart);
//        if(!IO_GetX(xETC_PostEqpReady))EM_SetErr(eiETC_PostEqpNotReady);
    }

    if (bStopSw) {

        if(OM.CmnOptn.bAutoOperation)EC.SendPstEqp(psStop);
        //Reset() ;
...
{                                                                                         
    ...
    if(!IO_GetX(xLD1_MainAirCheck) && !IO_GetY(yETC_MainSol)){EM_SetErr(eiETC_MainAir); isOk = false ;}
...
bool __fastcall CSequence::InspectEmergency(void)
{
    ...
    //Check Emergency
    if (IO_GetX(xETC_LEmgSw) || IO_GetX(xETC_EmgSw1) || IO_GetX(xETC_EmgSw2) ) {
...
bool __fastcall CSequence::InspectActuator(void)
{
    ...
    if(AT_Err(aiLD1_Cmp  )) {EM_SetErr(eiLD1_CmpTO   ) ; isOk = false ; }
...
//    //Inspect Motor Alarm.
    ...
    if(OM.EqpOptn.bExistLoader){ if (MT_GetAlarm(miLD1_ZCmp)){EM_SetErr(eiLD1_ZCmpAlarm ); isOk = false; } }
    if(OM.EqpOptn.bExistLoader){ if (MT_GetAlarm(miLD1_YCmp)){EM_SetErr(eiLD1_YCmpAlarm ); isOk = false; } }
...
//    if (MT_GetNLimSnsr(miWRK_ZBtm)&&MT_GetHomeEnd(miWRK_ZBtm)){EM_SetErr(eiWRK_ZBtmNegLim); isOk = false;}
    if(OM.EqpOptn.bExistLoader){ if (MT_GetNLimSnsr(miLD1_ZCmp)&&MT_GetHomeEnd(miLD1_ZCmp)){EM_SetErr(eiLD1_ZCmpNegLim); isOk = false;} }
    if(OM.EqpOptn.bExistLoader){ if (MT_GetNLimSnsr(miLD1_YCmp)&&MT_GetHomeEnd(miLD1_YCmp)){EM_SetErr(eiLD1_YCmpNegLim); isOk = false;} }
...
//    if (MT_GetPLimSnsr(miWRK_ZBtm)&&MT_GetHomeEnd(miWRK_ZBtm)){EM_SetErr(eiWRK_ZBtmPosLim); isOk = false;} //Home
    if(OM.EqpOptn.bExistLoader){ if (MT_GetPLimSnsr(miLD1_ZCmp)&&MT_GetHomeEnd(miLD1_ZCmp)){EM_SetErr(eiLD1_ZCmpPosLim); isOk = false;} }
    if(OM.EqpOptn.bExistLoader){ if (MT_GetPLimSnsr(miLD1_YCmp)&&MT_GetHomeEnd(miLD1_YCmp)){EM_SetErr(eiLD1_YCmpPosLim); isOk = false;} }
...
bool __fastcall CSequence::InspectHomeEnd()
{
    ...
    if(OM.EqpOptn.bExistLoader && !LD1.m_bIsHome ){ if (!MT_GetHomeEnd(miLD1_ZCmp)){EM_SetErr(eiLD1_ZCmpHomeEnd); isOk = false; } }
    if(OM.EqpOptn.bExistLoader && !LD1.m_bIsHome ){ if (!MT_GetHomeEnd(miLD1_YCmp)){EM_SetErr(eiLD1_YCmpHomeEnd); isOk = false; } }
...
bool __fastcall CSequence::InspectStripDispr(void)
{
    ...
    else if(( DM.ARAY[riWRP].GetCntExist() || DM.ARAY[riWRK].GetCntExist()) &&  (!IO_GetX(xWRK_1Pkg) && !IO_GetX(xWRK_2Pkg) )  && !OM.CmnOptn.bDryRun ) return false ;
    else if ( DM.ARAY[riPS2].GetCntExist() && !IO_GetX(xPSB_1Pkg) && !OM.CmnOptn.bDryRun) return false ;
    else if (!DM.ARAY[riLD1].CheckAllStat(csNone) && ( !IO_GetX(xLD1_MgzDetect1) && !IO_GetX(xLD1_MgzDetect2) ) ) return false ;
    return true ;
}
...
bool __fastcall CSequence::ToStartCon(void)
{
    ...
    //Call ToStop.
    if(OM.EqpOptn.bExistLoader) Ret0 = LD1.ToStartCon();
    else Ret0 = true ;
    Ret1 = PRB.ToStartCon();
    Ret2 = WRK.ToStartCon();
    Ret3 = PSB.ToStartCon();
...
bool __fastcall CSequence::ToStopCon(void)
{
    bool Ret0 , Ret1 , Ret2 , Ret3 ;
    //Call ToStop.
    if(OM.EqpOptn.bExistLoader) Ret0 = LD1.ToStopCon();
    else Ret0 = true ;
    Ret1 = PRB.ToStopCon();
    Ret2 = WRK.ToStopCon();
    Ret3 = PSB.ToStopCon();
...
bool __fastcall CSequence::ToStart(void)
{
    bool Ret0 , Ret1 , Ret2 , Ret3 ;
    //Call ToStop.
    if(OM.EqpOptn.bExistLoader) Ret0 = LD1.ToStart();
    else Ret0 = true ;
    Ret1 = PRB.ToStart();
    Ret2 = WRK.ToStart();
    Ret3 = PSB.ToStart();
...
bool __fastcall CSequence::ToStop(void)
{
    bool Ret0 , Ret1 , Ret2 , Ret3 ;
    //Call ToStop.
    if(OM.EqpOptn.bExistLoader) Ret0 = LD1.ToStop();
    else Ret0 = true ;
    Ret1 = PRB.ToStop();
    Ret2 = WRK.ToStop();
    Ret3 = PSB.ToStop();
...
bool __fastcall CSequence::AutoRun(void)
{
    bool Ret0 , Ret1 , Ret2 , Ret3 ;
    //Call ToStop.
    if(OM.EqpOptn.bExistLoader) Ret0 = LD1.Autorun();
    else Ret0 = true;
    Ret1 = PRB.Autorun();
    Ret2 = WRK.Autorun();
    Ret3 = PSB.Autorun();
