...
double CMotrCtrMan::GetValue(int _iAxisNo , EN_PSTN_VALUE _iPstnNo)
{
...
/*
bool CMotrCtrMan::CmprPos(int _iAxisNo , EN_PSTN_VALUE _iPstnNo)
{
    if( _iAxisNo < 0 || _iAxisNo >= MAX_MOTR          ) {ShowMessageT("Motor Axis Range is wrong"    ); return 0.0 ;}
    if( _iPstnNo < 0 || _iPstnNo >= PSTN_CNT[_iAxisNo]) {ShowMessageT("Position Index Range is wrong"); return 0.0 ;}
...
*/
void CMotrCtrMan::Save(AnsiString sJobName)
{
    //Local Var.
    TUserINI   UserINI ;
    AnsiString sPath   ;
...
void CMotrCtrMan::Load(AnsiString sJobName)
{
    //Local Var.
    TUserINI   UserINI ;
    AnsiString sPath   ;
...
void CMotrCtrMan::UpdatePstn(bool _bToTable)
{
    if(_bToTable) {
        for(int m = 0 ; m < MAX_MOTR ; m++) {
            for(int i = 0 ; i < PSTN_CNT[m] ; i++) {
...
//---------------------------------------------------------------------------
void __fastcall CMotrCtrMan::pPstnListDrawCell(TObject *Sender, int ACol,
...
//---------------------------------------------------------------------------
void __fastcall CMotrCtrMan::pPstnListMouseDown(TObject *Sender,
...
//---------------------------------------------------------------------------
void __fastcall CMotrCtrMan::pPstnListMouseUp(TObject *Sender,
      TMouseButton Button, TShiftState Shift, int X, int Y)
{
...
void CMotrCtrMan::SetWindow(TPanel * _pnBase , int _iAxisNo)
{
...
//---------------------------------------------------------------------------
void __fastcall CMotrCtrMan::pPstnEditKeyDown(TObject *Sender, WORD &Key,
      TShiftState Shift)
{
   if(Key == VK_RETURN){
...
void __fastcall CMotrCtrMan::pPstnEditExit(TObject *Sender)
{
    Trace(MT_GetName(GridInfo.iGrid).c_str() , (m_pPstnList[GridInfo.iGrid] -> Cells[GridInfo.iCol][GridInfo.iRow]+"->"+m_pPstnEdit[GridInfo.iGrid] -> Text.ToDouble()).c_str());
    m_pPstnList[GridInfo.iGrid] -> Cells[GridInfo.iCol][GridInfo.iRow] = m_pPstnEdit[GridInfo.iGrid] -> Text.ToDouble() ;
    m_pPstnEdit[GridInfo.iGrid] -> Visible = false ;
