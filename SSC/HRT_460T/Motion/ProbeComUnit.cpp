...
//    RcvData = "PUT:";
    ...
    int iTemp[16];
    if(OM.DevOptn.bSideLEDControl && OM.MstOptn.bUsed4RowCntct){
        if(bPb2Rslt && _iCh == PROBE_CH2){
            for(int j=0; j>-OM.DevOptn.iContColCnt / 2; j-- ){
            sArray[OM.DevOptn.iContColCnt / 2 - j] = sPb2Bin[-j];
        ...
        else if(bPb1Rslt && _iCh == PROBE_CH1){
            for(int j=0; j>-OM.DevOptn.iContColCnt / 2; j-- ){
            sArray[- j] = sPb1Bin[-j];
                for(int i = OM.DevInfo.iRowCnt - 1 ; i >= 0 ; i --){
    ...
    if(g_bPbRelse2Rslt){
        if(_iCh == PROBE_CH2){
            for(int ArrColCnt=0; ArrColCnt < iContColCnt / 2; ArrColCnt++){
        ...
        if(_iCh == PROBE_CH1){
            for(int ArrColCnt = 2 ; ArrColCnt < iContColCnt ; ArrColCnt++){
...
void __fastcall CProbeComUnit::WriteProbeDataCsv(EN_ARAY_ID _iId)
{
    ...
    if(LOT.m_bLotOpen) Path += "\\" + (AnsiString)iMgzCnt + ".csv" ;
    else              Path += "\\" + sTime   + ".csv" ;
...
//    DelPastLog();
    ...
    if(LOT.m_bLotOpen)  sPath += "\\" + DM.ARAY[_iId].GetLotNo() + "\\Log";//   LOT.GetCrntLot() ;
    else               sPath += "\\NoLot"                      ;
    ...
    if(LOT.m_bLotOpen) sPath += "\\" + (AnsiString)iMgzCnt + ".ini" ;
...
//    if( OM.DevOptn.i1Ch3P && OM.DevOptn.i1Ch3N ){ iLedChip = 3 ; } 혼용이 안됨.
    ...
    int iContColCnt = 0;
    if(OM.MstOptn.bUsed4RowCntct) iContColCnt = OM.DevOptn.iContColCnt / 2;
    else                          iContColCnt = OM.DevOptn.iContColCnt    ;
    ...
    if(OM.DevOptn.bUseZener) {
        ...
        sMsg +="NUM" + sTemp.sprintf("%01d%03d",iLedChip, OM.DevInfo.iRowCnt * (iContColCnt)) + "#" ;    //
...
//밑에사망
        ...
        sMsg +="NUM" + sTemp.sprintf("%01d%03d",iLedChip, OM.DevInfo.iRowCnt * (OM.DevOptn.iContColCnt)) + "#" ;    //
...
void CProbeComUnit::HeatProbe1Ch(DWORD _cbInQue)
{
    //Sleep(1);
    int iLen;
    ...
    if(OM.MstOptn.bUsed4RowCntct){
        ...
        PCM.sPb1Bin[0] = sRcvQue[2];
        PCM.sPb1Bin[1] = sRcvQue[3];
    }
    ...
    if(FrmProbe && OM.MstOptn.bUsed4RowCntct)FrmProbe -> edPb1ChData2 -> Text = sRcvMsg ;
...
void CProbeComUnit::HeatProbe2Ch(DWORD _cbInQue)
{
//Sleep(1);
    int iLen;
    const int iBuffSize = 1024 ;
