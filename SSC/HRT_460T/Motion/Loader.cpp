...
bool CLoader::CheckSafe(EN_MOTR_ID _iMotr , EN_PSTN_ID _iPstnId)
{
    ...
    bool isFwPusher      = !AT_Complete(aiLDR_Psh,ccBwd); //IO_GetX(xLD1_PusherFw) || !IO_GetX(xLD1_PusherBw);
    ...
    if(_iMotr == miLDR_ZCmp){
        ...
        switch(_iPstnId) {
            default                   :                                                                                                                   break ;
                //UpStair
                case piLDR_ZPickBwd   : if(isClampYFwd &&                   isClampZDnstair)  { sMsg = (AnsiString)MT_GetName(miLDR_YCmp)+" is Fwd" ; bRet = false ;} break ;
                case piLDR_ZClampOn   : if(isClampYFwd &&                   isClampZDnstair)  { sMsg = (AnsiString)MT_GetName(miLDR_YCmp)+" is Fwd" ; bRet = false ;} break ;
                case piLDR_ZPickFwd   : if(isClampYFwd &&                   isClampZDnstair)  { sMsg = (AnsiString)MT_GetName(miLDR_YCmp)+" is Fwd" ; bRet = false ;} break ;
                //Middle
                case piLDR_ZWait      : if(isClampYFwd && (isClampZUpstair||isClampZDnstair)) { sMsg = (AnsiString)MT_GetName(miLDR_YCmp)+" is Fwd" ; bRet = false ;} break ;
                case piLDR_ZWorkStart : if(isClampYFwd && (isClampZUpstair||isClampZDnstair)) { sMsg = (AnsiString)MT_GetName(miLDR_YCmp)+" is Fwd" ; bRet = false ;} break ;
                case piLDR_ZWork      : if(isClampYFwd && (isClampZUpstair||isClampZDnstair)) { sMsg = (AnsiString)MT_GetName(miLDR_YCmp)+" is Fwd" ; bRet = false ;} break ;
                //DnStatir
                case piLDR_ZPlaceFwd  : if(isClampYFwd &&  isClampZUpstair)                   { sMsg = (AnsiString)MT_GetName(miLDR_YCmp)+" is Fwd" ; bRet = false ;} break ;
                case piLDR_ZClampOff  : if(isClampYFwd &&  isClampZUpstair)                   { sMsg = (AnsiString)MT_GetName(miLDR_YCmp)+" is Fwd" ; bRet = false ;} break ;
                case piLDR_ZPlaceBwd  : if(isClampYFwd &&  isClampZUpstair)                   { sMsg = (AnsiString)MT_GetName(miLDR_YCmp)+" is Fwd" ; bRet = false ;} break ;
        }
    }
        ...
        if(isFwPusher    ) { sMsg = "Pusher is not Bwd         " ; bRet = false ;}
            ...
            switch(_iPstnId) {
                default                   :                                                                                           break ;
                    ...
                    case piLDR_YPick      : if(isClampZCenter) {sMsg = (AnsiString)MT_GetName(miLDR_YCmp)+" is Middle Position" ; bRet = false ;} break ;
                    case piLDR_YPlace     : if(isClampZCenter) {sMsg = (AnsiString)MT_GetName(miLDR_YCmp)+" is Middle Position" ; bRet = false ;} break ;
            }
        }
    }
    else {
...
bool CLoader::Autorun(void) //오토런닝시에 계속 타는 함수.
{
    ...
    if (Step.iSeq == 0) {
        ...
        bool isCyclePick   =  DM.ARAY[riLDR].CheckAllStat(csNone )    && (IO_GetX(xLDR_MgzIn) ||  OM.CmnOptn.bDryRun )&& ((m_iMgzCnt < OM.DevOptn.iLotEndMgzCnt)  || (GetLotNo()!="" || OM.MstOptn.bNotUstLot)) ;
        bool isCycleWork   =  DM.ARAY[riLDR].GetCntStat  (csUnkwn)    &&  DM.ARAY[riPRB].CheckAllStat(csNone ) &&
                              PRB.GetCycleStep() == CPreBuffer::scIdle && !OM.CmnOptn.bLoadingStop;
        bool isCyclePlace  =  DM.ARAY[riLDR].CheckAllStat(csEmpty) ;
        bool isCycleEnd    =  DM.ARAY[riLDR].CheckAllStat(csNone ) && GetLotNo()=="" && m_iMgzCnt >= OM.DevOptn.iLotEndMgzCnt && !isCycleSupply && !isCyclePick ;
                              /*m_iMgzCnt > OM.DevOptn.iLotEndMgzCnt*/


        //모르는 스트립에러.
        if(  DM.ARAY[riLDR].CheckAllStat(csNone) && (  IO_GetX(xLDR_MgzDetect1) ||  IO_GetX(xLDR_MgzDetect2) )     ) EM_SetErrMsg(eiPKG_Unknwn, m_sPartName.c_str()) ;
        //카세트 사라짐.
        if( !DM.ARAY[riLDR].CheckAllStat(csNone) && ( !IO_GetX(xLDR_MgzDetect1) && !IO_GetX(xLDR_MgzDetect2) )     ) EM_SetErrMsg(eiPKG_Dispr , m_sPartName.c_str()) ;
        //카세트 OUT 매거진 풀

        if (((m_iMgzCnt < OM.DevOptn.iLotEndMgzCnt) || GetLotNo()!="") && Stat.bChckSplyEmpty && !IO_GetX(xLDR_MgzIn) && !OM.CmnOptn.bLoadingStop) {
            EM_SetErrMsg(eiPRT_NeedMgz, m_sPartName.c_str()) ;
...
{
    //Check Time Out.
    if (m_tmToStop.OnDelay(Step.iToStop && !PreStep.iToStop == Step.iToStop && CheckStop(), 10000)) EM_SetErrMsg(eiPRT_ToStopTO, m_sPartName.c_str());
...
//                  DM.ARAY[riPR1].SetLotNo(DM.ARAY[riLDR].GetLotNo());
        ...
        case  20: if(!MoveActr(aiLDR_Psh , ccBwd))return false;
                  Step.iCycle = 0;
                  return true ;
    }
...
bool CLoader::CheckStop()
{
    ...
    if(!AT_Done(aiLDR_Psh   )) return false ;
