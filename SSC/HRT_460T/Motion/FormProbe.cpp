...
//    Stat.VrMin = dCChipVr[1][1];
    ...
    */
    ...
    if(Stat.VfMin == 999) { lbVfMin  -> Caption = 0 ;
                            ...
                            lbVfMin2 -> Caption = Stat.VfMin ; }

    if(Stat.VrMin == 999) { lbVrMin  -> Caption = 0 ;
                            lbVrMin2 -> Caption = 0 ; }
    else                  { lbVrMin  -> Caption = Stat.VrMin ;
                            lbVrMin2 -> Caption = Stat.VrMin ; }

    lbVfMax  -> Caption = Stat.VfMax ;
    lbVrMax2 -> Caption = Stat.VrMax ;
    lbVrMax  -> Caption = Stat.VrMax ;
    lbVrMax2 -> Caption = Stat.VrMax ;

    if( Stat.VfCnt ) {
        lbVfAvr  -> Caption = sAvr.sprintf("%0.2f", Stat.VfSum / Stat.VfCnt ) ;
        lbVfAvr2 -> Caption = sAvr.sprintf("%0.2f", Stat.VfSum / Stat.VfCnt ) ;
    }
    else {
        lbVfAvr  -> Caption = 0.000;
        lbVfAvr2 -> Caption = 0.000;
    }

    if( Stat.VrCnt ) {
        lbVrAvr  -> Caption = sAvr.sprintf("%0.2f", Stat.VrSum / Stat.VrCnt ) ;
        lbVrAvr2 -> Caption = sAvr.sprintf("%0.2f", Stat.VrSum / Stat.VrCnt ) ;
    }
    else {
        lbVrAvr  -> Caption = 0.000;
        lbVrAvr2 -> Caption = 0.000;
    }
...
//                     sgProbeCur->DefaultColWidth   = 25 ;
    ...
    for(int i=0 ; i<OM.DevOptn.iDgChipCnt ; i++ )
    {
        ...
        else if( iCChipRslt[ARow-1][ACol-1][i] == prDRNg) {
            sgProbeCur->Canvas->Brush->Color = clGray ;
            sgProbeCur->Canvas->FillRect(dRect[iCount])          ;
            sgProbeCur->Canvas->TextOut(dRect[iCount].Left + 1, dRect[iCount].Top , sItemVf[i]);
            //sgProbeCur->Canvas->Brush->Color = clRed  ;
            //sgProbeCur->Canvas->FillRect(DnRect)      ;
...
//                     sgProbePast->DefaultColWidth   = 25 ;
    ...
    for(int i=0 ; i<OM.DevOptn.iDgChipCnt ; i++ )
    {
        ...
        else if( iPChipRslt[ARow-1][ACol-1][i] == prDRNg) {
            sgProbePast->Canvas->Brush->Color = clGray ;
            sgProbePast->Canvas->FillRect(dRect[iCount])          ;
            sgProbePast->Canvas->TextOut(dRect[iCount].Left + 1, dRect[iCount].Top , sItemVf[i]);
            //sgProbePast->Canvas->Brush->Color = clRed  ;
            //sgProbePast->Canvas->FillRect(DnRect)      ;
...
void __fastcall TFrmProbe::FormClose(TObject *Sender, TCloseAction &Action)
{
    tmUpdate -> Enabled = false;
}
//---------------------------------------------------------------------------
