...
double CUnLoader::GetMotrPos(EN_MOTR_ID _iMotr , EN_PSTN_ID _iPstnId )
{
    ...
    //X , Y Pos Setting.
    double dZ1Pos = PM.GetValue(miULD_ZCmp , pvULD_ZWorkStart) - r * OM.DevInfo.dCsSlPitch ;
    double dPos  = 0.0 ;

    if(_iMotr == miULD_ZCmp){
        switch(_iPstnId) {
            default                  : dPos = MT_GetCmdPos(_iMotr                       ); break ;
            case piULD_ZWait         : dPos = PM.GetValue (_iMotr , pvULD_ZWait         ); break ;
            case piULD_ZPickFwd      : dPos = PM.GetValue (_iMotr , pvULD_ZPickFwd      ); break ;
            case piULD_ZClampOn      : dPos = PM.GetValue (_iMotr , pvULD_ZClampOn      ); break ;
            case piULD_ZPickBwd      : dPos = PM.GetValue (_iMotr , pvULD_ZPickBwd      ); break ;
            case piULD_ZWorkStart    : dPos = PM.GetValue (_iMotr , pvULD_ZWorkStart    ); break ;
            case piULD_ZPlaceFwd     : dPos = PM.GetValue (_iMotr , pvULD_ZPlaceFwd     ); break ;
            case piULD_ZClampOff     : dPos = PM.GetValue (_iMotr , pvULD_ZClampOff     ); break ;
            case piULD_ZPlaceBwd     : dPos = PM.GetValue (_iMotr , pvULD_ZPlaceBwd     ); break ;
        ...
        switch(_iPstnId) {
            default                  : dPos = MT_GetCmdPos  (_iMotr                     ); break ;
            case piULD_YWait         : dPos = PM.GetValue   (_iMotr , pvULD_YWait       ); break ;
            case piULD_YPick         : dPos = PM.GetValue   (_iMotr , pvULD_YPick       ); break ;
            case piULD_YWork         : dPos = PM.GetValue   (_iMotr , pvULD_YWork       ); break ;
            case piULD_YPlace        : dPos = PM.GetValue   (_iMotr , pvULD_YPlace      ); break ;
        }
    }
    ...
    if(_iMotr == miULD_ZCmp){
        if(isRailGateCnsr) { sMsg = "Strip is feeding now      " ; bRet = false ;}
            switch(_iPstnId) {
                default               :                                                                                                                   break ;
                //UpStair
                case piULD_ZPickBwd   : if(isClampYFwd &&                   isClampZDnstair)  { sMsg = (AnsiString)MT_GetName(miULD_YCmp)+" is Fwd" ; bRet = false ;} break ;
                case piULD_ZClampOn   : if(isClampYFwd &&                   isClampZDnstair)  { sMsg = (AnsiString)MT_GetName(miULD_YCmp)+" is Fwd" ; bRet = false ;} break ;
                case piULD_ZPickFwd   : if(isClampYFwd &&                   isClampZDnstair)  { sMsg = (AnsiString)MT_GetName(miULD_YCmp)+" is Fwd" ; bRet = false ;} break ;
                //Middle
                case piULD_ZWait      : if(isClampYFwd && (isClampZUpstair||isClampZDnstair)) { sMsg = (AnsiString)MT_GetName(miULD_YCmp)+" is Fwd" ; bRet = false ;} break ;
                case piULD_ZWorkStart : if(isClampYFwd && (isClampZUpstair||isClampZDnstair)) { sMsg = (AnsiString)MT_GetName(miULD_YCmp)+" is Fwd" ; bRet = false ;} break ;
                case piULD_ZWork      : if(isClampYFwd && (isClampZUpstair||isClampZDnstair)) { sMsg = (AnsiString)MT_GetName(miULD_YCmp)+" is Fwd" ; bRet = false ;} break ;
                //DnStatir
                case piULD_ZPlaceFwd  : if(isClampYFwd &&  isClampZUpstair)                   { sMsg = (AnsiString)MT_GetName(miULD_YCmp)+" is Fwd" ; bRet = false ;} break ;
                case piULD_ZClampOff  : if(isClampYFwd &&  isClampZUpstair)                   { sMsg = (AnsiString)MT_GetName(miULD_YCmp)+" is Fwd" ; bRet = false ;} break ;
                case piULD_ZPlaceBwd  : if(isClampYFwd &&  isClampZUpstair)                   { sMsg = (AnsiString)MT_GetName(miULD_YCmp)+" is Fwd" ; bRet = false ;} break ;
        ...
        switch(_iPstnId) {
            ...
            case piULD_YPick      : if(isClampZCenter) {sMsg = (AnsiString)MT_GetName(miULD_ZCmp)+" is Middle Position" ; bRet = false ;} break ;
            case piULD_YPlace     : if(isClampZCenter) {sMsg = (AnsiString)MT_GetName(miULD_ZCmp)+" is Middle Position" ; bRet = false ;} break ;
        }
    }
    else {
        bRet = false ;
...
bool CUnLoader::Autorun(void) //오토런닝시에 계속 타는 함수.
{
    ...
    if (Step.iSeq == 0) {
        ...
        bool isExstStrip   = !DM.ARAY[riPRB].CheckAllStat(csNone) || !DM.ARAY[riWRK].CheckAllStat(csNone) ||
                             !DM.ARAY[riPSB].CheckAllStat(csNone) || !DM.ARAY[riLDR].CheckAllStat(csNone) ;

        bool isLotChanged = (DM.ARAY[riULD].GetLotNo()!="" && DM.ARAY[riULD].GetLotNo() != DM.ARAY[riPSB].GetLotNo() && !DM.ARAY[riPSB].CheckAllStat(csNone));
        ...
        bool isCycleSupply = DM.ARAY[riULD].CheckAllStat(csNone ) && !IO_GetX(xULD_MgzIn) && isExstStrip; //LDR.GetStat().bWorkEnd;//&& !Stat.bChckSplyEmpty ;
        bool isCyclePick   = DM.ARAY[riULD].CheckAllStat(csNone ) && (IO_GetX(xULD_MgzIn) ||  OM.CmnOptn.bDryRun ) &&
                             (isExstStrip);// ||!isPreEqpEnd) ;
        bool isCycleWork   = !isCycleSupply && !isCyclePick && DM.ARAY[riULD].GetCntStat  (csEmpty) && !isZWorkPos ;//&& PSB.GetSeqStep() != scOut ;
        ...
        bool isCyclePlace  = !DM.ARAY[riULD].CheckAllStat(csNone ) &&
                             (DM.ARAY[riULD].CheckAllStat(csWork ) || isLotChanged || (!isExstStrip && LDR.GetWorkEnd()));



        bool isCycleEnd    = DM.ARAY[riULD].CheckAllStat(csNone ) && !isExstStrip && LDR.GetWorkEnd();//  isPreEqpEnd  ;

        Stat.bReadyMgz = isZWorkPos ;

        //모르는 스트립에러.
        if(!isCyclePick){
            if(  DM.ARAY[riULD].CheckAllStat(csNone) && (  IO_GetX(xULD_MgzDetect1 ) ||  IO_GetX(xULD_MgzDetect2 ))                        ) EM_SetErrMsg(eiPKG_Unknwn, m_sPartName.c_str()) ;
        }
        //카세트 사라짐.
        if(!isCyclePlace){
            if( !DM.ARAY[riULD].CheckAllStat(csNone) && ( !IO_GetX(xULD_MgzDetect1 ) && !IO_GetX(xULD_MgzDetect2 )) && !OM.CmnOptn.bDryRun ) EM_SetErrMsg(eiPKG_Dispr, m_sPartName.c_str()) ; //sun bDryRun 옵션 추가.
        }
        //카세트 OUT 매거진 풀

        if(  DM.ARAY[riULD].CheckAllStat(csNone) && !IO_GetX(xULD_MgzIn) && Stat.bChckSplyEmpty && (DM.ARAY[riPSB].GetCntExist()) && !OM.CmnOptn.bDryRun ) EM_SetErrMsg(eiPRT_NeedMgz, m_sPartName.c_str()) ;
...
{
    //Check Time Out.
    if (m_tmToStop.OnDelay(Step.iToStop && !PreStep.iToStop == Step.iToStop && CheckStop(), 5000)) EM_SetErrMsg(eiPRT_ToStopTO, m_sPartName.c_str());
...
//Check Cycle Time Out.
    ...
    switch (Step.iCycle) {
        ...
        case  11: r1 = m_tmDelay.OnDelay(true , 3000) ;
        ...
        case  13: IO_SetY(yULD_MgzInAC, false);
                  if(IO_GetY(yULD_MgzInAC)) return false;
                  Step.iCycle = 0 ;
                  return true ;
    }
}
...
//Check Cycle Time Out.
    ...
    switch (Step.iCycle) {
        ...
        case  10: MoveActr(aiULD_Cmp    , ccFwd );
                  MoveMotr(miULD_YCmp   , piULD_YWait) ;
                  IO_SetY(yULD_MgzInAC,true);
                  Step.iCycle++;
                  return false;

        case  11: if(!MoveActr(aiULD_Cmp    , ccFwd  )) return false ;
                  if(!MoveMotr(miULD_YCmp   , piULD_YWait)) return false ;
        ...
        case  19: IO_SetY(yULD_MgzInAC,false);
                  if(IO_GetY(yULD_MgzInAC)) return false;
                  Step.iCycle = 0 ;
                  return true ;
    }
...
//Check Cycle Time Out.
    ...
    switch (Step.iCycle) {
        ...
        case  10: MoveMotr(miULD_YCmp , piULD_YWait) ;
                  IO_SetY(yULD_MgzOutAC,true);
                  Step.iCycle++;
                  return false;

        case  11: if(!MoveMotr(miULD_YCmp , piULD_YWait)) return false ;
        ...
        case  16: if(!m_tmDelay.OnDelay(true, 500)) return false;
        ...
        case  17: if(!MoveMotr(miULD_ZCmp , piULD_ZPlaceBwd)) return false ;
                  IO_SetY(yULD_MgzOutAC,true);
                  MoveMotr(miULD_YCmp , piULD_YWait) ;
                  Step.iCycle++;
                  return false ;

        case  18: if(!MoveMotr(miULD_YCmp , piULD_YWait)) return false ;
                  MoveMotr(miULD_ZCmp , piULD_ZPickFwd);
                  Step.iCycle++;
                  return false ;

        case  19: if(MoveMotr(miULD_ZCmp , piULD_ZPickFwd)) return false ;
                  m_tmDelay.Clear();
                  Step.iCycle++;
                  return false ;

                  /////////////////
        case  20: if(m_tmDelay.OnDelay(true , 2000)){
                      if(IO_GetX(xULD_MgzOutFull)) {
                          IO_SetY(yULD_MgzOutAC,false);
                          EM_SetErrMsg(eiPRT_MgzFull , m_sPartName.c_str());
                          Step.iCycle = 0 ;
                          return true ;
                      }
                  }
        ...
        case  21: if(!m_tmDelay.OnDelay(true , 3000)) return false;
                  IO_SetY(yULD_MgzOutAC,false);
