...
double __stdcall CImage::GetScaleX() { return m_dScaleX ;}
double __stdcall CImage::GetScaleY() { return m_dScaleY ;}

double __stdcall CImage::GetOffsetX() { return m_dOffsetX;}
double __stdcall CImage::GetOffsetY() { return m_dOffsetY;}
...
void __stdcall CImage::Del()
{
    ...
    if(m_pImage  ){GlobalFree(m_pImage );m_pImage =NULL;}//{delete [] m_pImage  ;m_pBitInfo8  = NULL ;}if(m_pImage  ){delete [] m_pImage  ;}
    if(m_pImageC ){GlobalFree(m_pImageC);m_pImageC=NULL;}//{delete [] m_pImageC ;m_pBitInfo24 = NULL ;}if(m_pImageC ){delete [] m_pImageC ;}
}
...
bool __stdcall CImage::SetPalette(int _iWidth , int _iHeight)
{
    ...
    m_pBitInfo8  = (BITMAPINFO *) new BYTE[BMP_INFO_SIZE_8 ]; ZeroMemory(m_pBitInfo8 ,sizeof(BMP_INFO_SIZE_8 ));
    m_pBitInfo24 = (BITMAPINFO *) new BYTE[BMP_INFO_SIZE_24]; ZeroMemory(m_pBitInfo24,sizeof(BMP_INFO_SIZE_24));
...
//  원래 14바이트 인BITMAPFILEHEADER를 sizeof(BITMAPF9ILEHEADER) 해 보십시요 16 이 나올겁니다.
    ...
    fread(&bih.biPlanes        , 2, 1, file); //1        1          1           1            1            //WORD   biPlanes;         1      컬러 Plane의 수.(???) 예전그래픽 카드 때문에 있는데 지금은 전혀 사용하지 않는다.
...
int __stdcall CImage::ShowImgGrey(HDC _hHdc , int _iDstX, int _iDstY, int _iDstW, int _iDstH , int _iSrcX, int _iSrcY, int _iSrcW, int _iSrcH)
{
    ...
    m_dScaleX = _iDstW / (double)_iSrcW ;
    m_dScaleY = _iDstH / (double)_iSrcH ;
...
void __stdcall CImage::Rotate(TRect _tRect , double _dAngle)
{

    //double TempTime = GetTickTimeDouble() ;
    SetRect(&_tRect);
    ...
    int   iPnt;
    int   y,x;//타겟 이미지 좌표
    double dOriY,dOriX;//원 이미지 상의 해당 좌표 (실수값)
    int   iOriY,iOriX;//원 이미지 상의 해당 좌표 (정수값)
    double dRestY,dRestX;// 예 1.24-1=0.24
    int   iLTPx,iRTPx,iRBPx,iLBPx;
    double dCosRad,dSinRad;
    double dRotRad=(double)(-_dAngle*MATH_PI/180.0);//angle->radian으로 변환
    dCosRad=(double)cos(dRotRad);
    dSinRad=(double)sin(dRotRad);
    ...
    for(y = _tRect.top ; y < _tRect.top + iHeight ; y++) {
        for(x = _tRect.left ; x < _tRect.left + iWidth ; x++) {
            ...
            //회전전의 원 이미지상의 좌표 구함
            dOriY =-dSinRad * (x-iRotCntX) + dCosRad * (y-iRotCntY) + iRotCntY;
            dOriX = dCosRad * (x-iRotCntX) + dSinRad * (y-iRotCntY) + iRotCntX;
            iOriY = floor(dOriY);//예: floor(2.8)=2.0
            iOriX = floor(dOriX);
            dRestY= dOriY-iOriY;
            dRestX= dOriX-iOriX;
            //범위 조사
            ...
            newValue=(BYTE)(iLTPx*(1-dRestX)*(1-dRestY)+
                            iRTPx*   dRestX *(1-dRestY)+
                            iRBPx*   dRestX *   dRestY +
                            iLBPx*(1-dRestX)*   dRestY);
...
void __stdcall CImage::TrimScale(TRect _tRect , CImage * _tDstImg , double _dScale)
{
    ...
    int iWidth  =_tRect.Width () * _dScale;
    int iHeight =_tRect.Height() * _dScale;
    ...
    double        PxSubX , PxSubY ; //픽셀 소수점. 픽셀비중을 이것으로 구한다.
    double        SubPxVal = 0 ;
    double        SumPxVal = 0 ;
    ...
    for(int y = 0 ; y < iHeight ; y++){
        for(int x = 0 ; x < iWidth; x++){
            for(int y1 = 0 ; y1 < 1 / _dScale ; y1 ++){
                for(int x1 = 0 ; x1 < 1 / _dScale ; x1++){
                    PxSubX = (_tRect.left+(x*(1/_dScale))+x1) - (_tRect.left+(x*(1/_dScale))+x1) ;
                    PxSubY = (_tRect.top +(y*(1/_dScale))+y1) - (_tRect.top +(y*(1/_dScale))+y1) ;

                    PxV00 = GetPixel(_tRect.left+(x*(1/_dScale))+x1,_tRect.top+(y*(1/_dScale))+y1);

                    SubPxVal = (1-PxSubX)*(1-PxSubY)*PxV00;
                    SumPxVal += SubPxVal;
                }
            }
            SumPxVal = SumPxVal / ((1/_dScale) * (1/_dScale));
...
bool __stdcall CImage::SetPoint (double * _dX , double * _dY , int _iMargin)
{
    bool bRet = false ;
    if(*_dX < _iMargin) {*_dX = _iMargin ; bRet = true ;}
    if(*_dX >= m_iWidth  - _iMargin) {*_dX = m_iWidth  - _iMargin - 1 ; bRet = true ;}

    if(*_dY < _iMargin) {*_dY = _iMargin ; bRet = true ;}
    if(*_dY >= m_iHeight - _iMargin) {*_dY = m_iHeight - _iMargin - 1 ; bRet = true ;}
