...
int CAtPick::GetInspRowCnt(){return VOM.VDevOptn.iCountX ;}
int CAtPick::GetInspColCnt(){return VOM.VDevOptn.iCountY ;}
void CAtPick::SetInspRowCnt(int _iVal){VOM.VDevOptn.iCountX = _iVal;}
void CAtPick::SetInspColCnt(int _iVal){VOM.VDevOptn.iCountY = _iVal;}
...
void CAtPick::Inspection()
{
    ...
    EN_VISN_ERR_ID iInspRslt    = veAtPickOk ;
    int            iInspFailCnt = 0       ;
    bool           bRet         = true    ;
    ...
    while(1) { //고투문 대용.
        ...
        bRet = OCV_Inspect(m_pImg , Orig.pTrArea , Orig.pTrImage , Orig.pTrckInsp->GetOutRect() , VOM.VDevPara.AtPickOcv , Orig.pRsltArea , &Orig.tRslt) ;
        //}
        if(!bRet) {
            iInspRslt = veAtPickOriFail ;
            m_sRsltMsg = AnsiString("FAILED ORIGIN : ") + SVL_GetErrMsg();
            break ;
        }

        if(Orig.tRslt.fSinc < VOM.VDevOptn.dAtPickOriSinc) {
            iInspRslt = veAtPickOriNg ;
        ...
        for(int r = 0 ; r < VOM.VDevOptn.iCountY ; r++) {
            for(int c = 0 ; c < VOM.VDevOptn.iCountX ; c++) {
                ...
                bRet = TSD_Inspect(m_pImg , tInspRect , VOM.VDevPara.AtPickTsd , &Insp.pRslt[VOM.VDevOptn.iCountX * r + c]);

                iInspPxCnt = Insp.pRslt[VOM.VDevOptn.iCountX * r + c].tRect.Height() * Insp.pRslt[VOM.VDevOptn.iCountX * r + c].tRect.Width() ;

                if(!bRet) {
                    iInspRslt = veAtPickInspFail ;
                ...
                */
                ...
                if(fPxPer < VOM.VDevOptn.dAtPickPxMinPer) {
                    iInspRslt = veAtPickInspNg ;
                    Rslt.bFailMap[r][c] = true ;
                    iInspFailCnt++;
                }
            }
            if(!iInspRslt == veAtPickInspFail) break ;
        }
        if(iInspRslt != veAtPickOk) {
...
void CAtPick::Train()
{
    if(m_iActivPage == piAtPickOcv) {
        //(IImage * _pImg , TRect _Rect , OCV_Para _tPara ,  IArea * _pTrainArea , IImage * _pTrainImg) ;
        if(!OCV_Train(GetImg() , Orig.pTrckInsp->GetTRect() , VOM.VDevPara.AtPickOcv , Orig.pTrArea , Orig.pTrImage)) {
                              ...
                              float _fScaleX , float _fScaleY ,int _iThreshold , bool _bCntrLine)
{
    ...
    if(g_bSettingMode){
        if(m_iActivPage == piAtPickLCS) {
            PaintCntLine(_hDc , _iDstW , _iDstH);
        }
        else if(m_iActivPage == piAtPickOcv) {
        ...
        else if(m_iActivPage == piAtPickTsd) {
            for(int r = 0 ; r < VOM.VDevOptn.iCountY ; r++) {
                for(int c = 0 ; c < VOM.VDevOptn.iCountX ; c++) {
    ...
    if(Stat.bDispRslt){
        ...
        dPxPer = Insp.pRslt[VOM.VDevOptn.iCountX * r + c].iPxCnt * 100 / iInspPxCnt ;
                ...
                if(dPxPer < VOM.VDevOptn.dAtPickPxMinPer) {
                    iInspRslt = veAtPickInspNg ;
                    iInspFailCnt++;
                }
        ...
        */
        ...
        for(int r = 0 ; r < VOM.VDevOptn.iCountY ; r++) {
            for(int c = 0 ; c < VOM.VDevOptn.iCountX ; c++) {
                iInspPxCnt = Insp.pRslt[VOM.VDevOptn.iCountX * r + c].tRect.Width() * Insp.pRslt[VOM.VDevOptn.iCountX * r + c].tRect.Height() ;
                fPercent = Insp.pRslt[VOM.VDevOptn.iCountX * r + c].iPxCnt * 100 /(float)iInspPxCnt ;
                if(fPercent  < VOM.VDevOptn.dAtPickPxMinPer) {Gdi -> m_tPen.Color = clRed  ; Gdi -> m_tText.Color = clRed  ;}
...
{
    if(m_iActivPage == piAtPickOcv) {
        UpdateParaOcv(_bTable , _vePara , VOM.VDevPara.AtPickOcv);
    }
    else if(m_iActivPage == piAtPickTsd) {
        UpdateParaTsd(_bTable , _vePara , VOM.VDevPara.AtPickTsd);
    }
}
...
void CAtPick::Load(bool _bLoad)
{
    ...
    //Origin
    LoadParaOcv(_bLoad , VOM.VDevPara.AtPickOcv , sJobPath , "Orig");
    ...
    //Insp
    LoadParaTsd(_bLoad , VOM.VDevPara.AtPickTsd , sJobPath , "Insp");
