...
__fastcall TFrmPixel::TFrmPixel(TComponent* Owner , CImage * _pImage , CTracker * _pTracker)
    : TForm(Owner)
{
    Left = 100 ;
    Top  = 100 ;
...
void __fastcall TFrmPixel::OnSetTracker(CTracker * _pTracker)
{
    m_pTracker = _pTracker ;
}
...
void __fastcall TFrmPixel::UpdateView()
{
    ...
    sp.x = m_pTracker -> GetPointX(CTracker::piLineStt) ; sp.y = m_pTracker -> GetPointY(CTracker::piLineStt) ;
    ep.x = m_pTracker -> GetPointX(CTracker::piLineEnd) ; ep.y = m_pTracker -> GetPointY(CTracker::piLineEnd) ;
