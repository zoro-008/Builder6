...
{
    m_pValueList -> Add(edName -> Text , edComment -> Text , cbType -> Text , StrToFloatDef(edMinVal -> Text,0.0) , StrToFloatDef(edMaxVal -> Text,0.0)  );
    DisplayList();
}
//---------------------------------------------------------------------------
...
{
    m_pValueList -> Insert(sgValueList -> Row-1 , edName -> Text , edComment -> Text , cbType -> Text , StrToFloatDef(edMinVal -> Text,0.0) , StrToFloatDef(edMaxVal -> Text,0.0)  );
    DisplayList();
}
//---------------------------------------------------------------------------
...
void __fastcall TFrmValueList::btSaveClick(TObject *Sender)
{
    if(m_pSelVal) {
        m_pSelVal -> SetName   (edName    -> Text     ) ;
        m_pSelVal -> SetComment(edComment -> Text     ) ;
        m_pSelVal -> SetValType(cbType    -> Text     ) ;
...
void TFrmValueList::UpdateProperty(int _iNo)
{
    ...
    cbType    -> Text      = m_pSelVal -> GetValType() ;
...
void __fastcall TFrmValueList::sgValueListSelectCell(TObject *Sender,
      int ACol, int ARow, bool &CanSelect)
{
//    if(ACol==6) {
//        sgValueList -> Options= sgValueList -> Options <<goEditing ;
//    }
//    else {
//        sgValueList -> Options= sgValueList -> Options >>goEditing ;
//
