...
bool CImi::InitImi()
{
    ...
    //m_bDllLoaded = LoadNeptuneApi("D:\\Work\\001_BC6Works\\Luxpia\\HRT_430S\\bin\\NeptuneC_MD_VC80.dll");//LoadImCamApi("ImCam.dll") ;
...
bool CImi::Init()
{
    ...
    ENeptuneError iRet ;
    if(m_iMaxCam){
        iRet = ntcOpen(m_pCamInfo[Para.iPhysicalNo].strCamID, &m_hCamHandle);
        if (iRet  != NEPTUNE_ERR_Success ){
        	ShowMessage("Can not Open IMI camera_"+String(Para.iPhysicalNo) + "(ErrCode:" + iRet +")");
        	return false;
        }
    }
