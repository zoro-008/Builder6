...
bool TFraMotr::CheckSafe(int _iId)
{
    ...
    if(_iId == (int)miLDR_ZCmp){
        if(!AT_Complete(aiLDR_Psh , ccBwd)){
            FM_MsgOk("WARRING","Loader Pusher Cylinder is Not Bwd");
            bRet = false;
        }
    }
    else if(_iId == (int)miLDR_YCmp){
        if(!AT_Complete(aiLDR_Psh , ccBwd)){
            FM_MsgOk("WARRING","Loader Pusher Cylinder is Not Bwd");
            bRet = false;
        }
    }
    else if(_iId == (int)miWRK_XCmp){
        if(MT_GetCmdPos(miWRK_ZBtm) >= PM.GetValue(miWRK_ZBtm , pvWRK_ZBWork)){
            FM_MsgOk("WARRING" , "Work Zone Z Button Motor Work Position");
            bRet = false;
        }
        if(MT_GetCmdPos(miWRK_YBtm) >= PM.GetValue(miWRK_YBtm , pvWRK_YBWork)){
            FM_MsgOk("WARRING" , "Work Zone Y Button Motor Work Position");
            bRet = false;
        }
    }
    else if(_iId == (int)miSTL_YTop) {
        if(!AT_Complete(aiSTL_Top , ccBwd)) {
            FM_MsgOk("WARRING","Sorting Tool Top Cylinder is Not Bwd");
            bRet = false;
        }
    }
    else {
