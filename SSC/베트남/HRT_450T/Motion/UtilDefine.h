...
//1.1.2.4
//20180511 베트남개조로 인해 1021장비에서만 검증한 소스....
#define EQP_SERIAL_LIST \
"\
SSC-14-1021,\
"

//프로그램 수정은 밑에꺼 모두 수렴할수 있게 했으나
//실제 확인을 할 수 없어서 일단 나중에 확인 되는 장비만 EQP_SERIAL_LIST에 넣어서 쓰자.
#define EQP_SERIAL_LIST_BF_1124 \
"\
SSC-13-1043,\
SSC-14-1019,\
SSC-14-1020,\
...
enum EN_MOTR_ID {
    ...
    miULD_YCmp  =12 ,    //                       원
    miULD_ZCmp  =13 ,    //
    miSTL_YBtm  =14 ,    // 서반 배트남행 개조시에 추가 되어서피지컬어드레스 7번쓰고 7번모터는 99번으로 세팅 20180425 선계
...
enum EN_ERR_ID {
    ...
    eiULD_YCmpAlarm              ,  //092 ;
    eiSTL_YBtmHomeEnd            ,  //093 ;
    eiSTL_YBtmNegLim             ,  //094 ;
    eiSTL_YBtmPosLim             ,  //095 ;
    eiSTL_YBtmAlarm              ,  //096 ;
    eiPR1_CmpTO                  ,  //097 ;
    eiPR2_CmpTO                  ,  //098 ;
    eiPRB_IdChkTO                ,  //099 ;
...
enum EN_PSTN_ID {
    ...
    piPSB_XWait       = 0  ,  //
    piPSB_XGrip            ,  // CycleIn 트림 워크 스타트  piPSB_XWorkStart로 가기 위해 집는 위치.
    piPSB_XPull            ,  //
    piPSB_XBack            ,  //
    piPSB_XWorkStart       ,  //
    piPSB_XOut             ,  //
    ...
    piWRK_ZTWait      =  0 ,  //
    piWRK_ZTWork           ,  //
    piWRK_ZTChange         ,  // 20180514 이상하게 이축만 체인지가 없어서 추가.
    MAX_PSTN_ID_MOTR4      ,  //
    ...
    piSTL_YTWorkStart      ,  //
    piSTL_YTDevCheck       ,  //
    piSTL_YTPlace          ,  //Place 포지션. 픽해서 깥데기에 버리는  타입.
    piSTL_YVoid            ,  //작은 자재 포스트버퍼 그리퍼에 소팅툴이 부딪힐까봐 피해가는 포지션.
    piSTL_YTWork           ,  //
    MAX_PSTN_ID_MOTR8      ,  //
...
//    pvSTL_YVoid            , // 2018 0515 서반 배트남 탑뷰개조 건으로 트림툴을 장비 전면으로 땡겨 달면서 0포지션이 너무 오래걸려 추가.
    MAX_PSTN_MOTR8         , //
