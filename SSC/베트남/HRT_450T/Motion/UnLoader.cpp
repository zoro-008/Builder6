...
bool CUnLoader::Autorun(void) //오토런닝시에 계속 타는 함수.
{
    ...
    if (Step.iSeq == 0) {
        ...
        bool isExstStrip   = !DM.ARAY[riPR1].CheckAllStat(csNone) || !DM.ARAY[riPR2].CheckAllStat(csNone) ||
                             !DM.ARAY[riWRK].CheckAllStat(csNone) || !DM.ARAY[riPSB].CheckAllStat(csNone) ||
                             !DM.ARAY[riLDR].CheckAllStat(csNone) ;
        ...
        if(  DM.ARAY[riULD].CheckAllStat(csNone) && !IO_GetX(xULD_MgzIn) && Stat.bChckSplyEmpty && (DM.ARAY[riPSB].GetCntExist()) && !OM.CmnOptn.bDryRun ) EM_SetErr(eiULD_NeedMgz  ) ;
