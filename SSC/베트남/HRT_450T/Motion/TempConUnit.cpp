...
__fastcall CTempConUnit::CTempConUnit(void)
{
    ...
    if(OM.MstOptn.bUsedTempRS232) {
        //RS232C_TempCon = new TRS232C ;
        if(!RS232C_TempCon->Open(2)) ShowMessagePos("RS-232C TempCon 3 Ch Open Failed!",0,0);
...
void CTempConUnit::SetRcvMsg(AnsiString _sMsg)
{
    ...
    if(bDispErr && sSum != sCheckSum) { //체크썸 확인 해보자.
        ShowMessagePos("TempCon CheckSum is Wrong!!!!",0,0);
        bDispErr = false ;
    }

    if(!m_sRcvMsg.Pos("OK")) {
        if(bDispErr) {
            ShowMessagePos("TempCon NG Received",0,0);
            bDispErr = false ;
        }
        return ;
    }
