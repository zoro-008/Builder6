...
void __fastcall CLot::Close()
{
    SaveLotInfo();
    SaveDayInfo();
}
void __fastcall CLot::Reset(void)
{
    m_bLotEnd = false;
}
                                  ...
                                  TPanel * _pnGoodChip )
{
    ...
    if(_pnDropChip) _pnDropChip -> Caption = LotInfo.iDropChip  ;
    if(_pnGoodChip) _pnGoodChip -> Caption = LotInfo.iGoodChip  ;
                                  ...
                                  TPanel * _pnWorkLot   )
{
    ...
    if(_pnDropChip) _pnDropChip -> Caption = DayInfo.iDropChip  ;
...
void __fastcall CLot::LoadDayInfo()
{
    ...
    UserINI.Load(Path.c_str() , "DayInfo" , "iDropChip " , DayInfo.iDropChip  );
    UserINI.Load(Path.c_str() , "DayInfo" , "iGoodChip " , DayInfo.iGoodChip  );
...
void __fastcall CLot::SaveDayInfo()
{
    ...
    UserINI.Save(Path.c_str() , "DayInfo" , "iDropChip " , DayInfo.iDropChip  );
    UserINI.Save(Path.c_str() , "DayInfo" , "iGoodChip " , DayInfo.iGoodChip  );
...
void __fastcall CLot::WriteLotDayLog()
{
    ...
    if (hFile == -1) {
        ...
        if(bNewDay) { //새로운 날이면 전날 마지막 돌리던 물량 데이터 인계 받음.
            ...
            tTemp.Val = DayInfo.dTotalTime ; Trace("DayInfo.dTotalTime",tTemp.FormatString("hh:nn:ss").c_str());
                                             ...
                                             Trace("DayInfo.iDropChip" ,AnsiString(DayInfo.iDropChip ).c_str());
                                             Trace("DayInfo.iGoodChip" ,AnsiString(DayInfo.iGoodChip ).c_str());
            ...
            DayInfo.iDropChip   = LotInfo.iDropChip    ;
            DayInfo.iGoodChip   = LotInfo.iGoodChip    ;
            DayInfo.iLotCnt     = 1                    ;
    ...
    Temp += (AnsiString)DayInfo.iDropChip    + ",";
    Temp += (AnsiString)DayInfo.iGoodChip    + ",";
...
//        Temp += "iFail"     ;
    ...
    Temp += (AnsiString)LotInfo.iDropChip + "," ;
    Temp += (AnsiString)LotInfo.iGoodChip + "," ;
    Temp += (AnsiString)"\r\n"                  ;
...
{
    UserFile.ClearDirDate(LOG_FOLDER , Now() - 30 * 3);
}
...
bool __fastcall CLot::WriteTrimInfo(AnsiString _sLotNo, int _Trimcnt , int _iBinNo)
{
    //Local Var.
    TUserINI   UserINI;
    AnsiString sPath  , sArayName ;
    ...
    sPath   = sPath + "TrimLog.ini" ;

    UserINI.Save(sPath.c_str()  , "TrimLog"  , "iTrimCnt" , _Trimcnt );
    //UserINI.Save(sPath.c_str()  , "TrimLog"  , "iBinNo  " , _iBinNo  );

    return true ;
}
bool __fastcall CLot::ReadTrimInfo(TStringGrid * _sgDate, TStringGrid * _sgLot , int *_iTrimCnt, int * _iBinNo)
{
    TUserINI   UserINI;
    ...
    UserINI.Load(sPath.c_str()  , "TrimLog"  , "iTrimCnt " , _iTrimCnt );
    //UserINI.Load(sPath.c_str()  , "TrimLog"  , "iBinNo   " , _iBinNo   );
}
