...
CWorkZone::CWorkZone(void)
{
    ...
    if(!RS232C_Pb1ch->Open(0)) ShowMessagePos("RS-232C Probe 1 Ch Open Failed!",0,0);
    else                       RS232C_Pb1ch->CallBackReg(HeatProbe1Ch);
    if(!RS232C_Pb2ch->Open(1)) ShowMessagePos("RS-232C Probe 2 Ch Open Failed!",0,0);
...
//    double dCmpGrGap   = OM.DevInfo.dColGrGap - OM.DevInfo.dColPitch ;
    ...
    //1.1.2.3
    //추가 COL 그룹 갭 적용하기 수정
    double dCmpMoveOfs = (OM.DevInfo.dColPitch+ OM.DevOptn.dWRKPtchOfs) * OM.DevOptn.iContColCnt ;
    //     16             4mm                    4ea                       0.0mm
    ...
    double dCmpMovePos = PM.GetValue(miWRK_XCmp , pvWRK_XGrip) + dCmpMoveOfs ;
        /*
    if( OM.DevInfo.iColGrCnt < 0) OM.DevInfo.iColGrCnt = 1;
    int iGrCnt =  OM.DevInfo.iColCnt / OM.DevInfo.iColGrCnt ;

    if( OM.DevInfo.iColGrCnt > 1 && iGrCnt > 0) { //그룹겝 계산하여 더하기.
        dCmpMovePos += (OM.DevInfo.dColGrGap - OM.DevInfo.dColPitch) * (OM.DevOptn.iContColCnt / iGrCnt) ;
    }
    */
    ...
    */
    ...
    if(_iMotr == miWRK_XCmp){
        switch(_iPstnId) {
            ...
            case piWRK_XWorkStart  : dPos = PM.GetValue (_iMotr , pvWRK_XWorkStart ); break ;
            case piWRK_XWork       : dPos = dCmpMovePos                             ; break ;
//            case piWRK_XWork       : if (bGrGrip) dPos = dCmpMovePos + dCmpGrGap    ;
//                                     else         dPos = dCmpMovePos                ; break ;
    ...
    //}
    /* bUsedXAxis 굳이 이옵션으로 쪼갤필요 없을듯 해서 일단 주석 처리 하고 옵션 깜.
...
bool CWorkZone::ToStop(void) //스탑을 하기 위한 함수.
{
    ...
    switch (Step.iToStop) {
        ...
        case 10: MoveMotr(miWRK_ZTop , piWRK_ZTChange);

                 Step.iToStop ++ ;
                 return false ;

        case 11: if(!MoveMotr(miWRK_ZTop , piWRK_ZTChange)) return false ;
                 if(!MT_GetAlarm   (miWRK_XCmp) &&
...
//Check Cycle Time Out.
    ...
    switch (Step.iCycle) {
        ...
        case  10: if(DM.ARAY[riWRK].CheckAllStat(csUnkwn) && OM.MstOptn.bUsedVision && !OM.CmnOptn.bIgnrIdCehck){ //1호기 비전 없어서 Option처리 JH
                      SV_Grab(viOri);
                      m_tmTemp.Clear();
                      Step.iCycle++;
                      return false ;
...
//Check Cycle Time Out.
    ...
    switch (Step.iCycle) {
        ...
        case  12: if(!MoveMotr(miWRK_YBtm , piWRK_YBWait)) return false ;
                  MoveMotr(miWRK_ZTop , piWRK_ZTChange  );
                  MoveMotr(miWRK_ZBtm , piWRK_ZBChange);
                  Step.iCycle++;
                  return false;

        case  13: if(!MoveMotr(miWRK_ZTop , piWRK_ZTChange  )) return false ;
...
/*
    //1.1.2.3
    //추가 COL 그룹 갭 적용하기 수정
    double dCmpMovePos = PM.GetValue(miWRK_XCmp , pvWRK_XGrip) + (OM.DevInfo.dColPitch * OM.DevOptn.iContColCnt) + OM.DevOptn.dWRKPtchOfs;
    int iGrCnt =  OM.DevInfo.iColCnt / OM.DevInfo.iColGrCnt ;
    if( OM.DevInfo.iColGrCnt > 1 && iGrCnt > 0) { //그룹겝 계산하여 더하기.
        dCmpMovePos += (OM.DevInfo.dColGrGap - OM.DevInfo.dColPitch) * (OM.DevOptn.iContColCnt % iGrCnt) ;
    }
...
//    int iGrCnt =  OM.DevInfo.iColCnt / OM.DevInfo.iColGrCnt ;
//    if( OM.DevInfo.iColGrCnt > 1 && iGrCnt > 0) { //그룹겝 계산하여 더하기.
//        dCmpMovePos += (OM.DevInfo.dColGrGap - OM.DevInfo.dColPitch) * (fc % iGrCnt) ;
//    }
