...
bool InitAjinBoard()
{
    ...
    //1.3.3.11
    //2018.0508 어버이날 업데이트 옵션처리로 피지컬 어드레스를 실제 박혀 있는 보드 보다 많이 쓸때.
    //리밋센서와 알람시그널이 반전 플레그도 먹지 않아 이렇게 추가하여 실제 박혀있는 보드 보다 많을때
    //적절히 운용 한다.
    //CAjinMotion::m_iTotalNoOfAxis = GetAjinAxisCnt() ;
    m_iTotalNoOfAxis = GetAjinAxisCnt() ;
...
bool CAjinMotion::GetNLimSnsr()
{
    ...
    //리밋센서와 알람시그널이 반전 플레그도 먹지 않아 이렇게 추가하여 실제 박혀있는 보드 보다 많을때
    //적절히 운용 한다.
    if(m_iTotalNoOfAxis <= AjinPara.iPhysicalNo) return false ;
    if(0                >  AjinPara.iPhysicalNo) return false ;
...
#endif //AJIN_AXT
#ifdef AJIN_AXL
    //Go Motor.           //시발 ..... 상대값 인지 ... 뭔지 오동작 작열.
    AxmOverridePos(AjinPara.iPhysicalNo , _dNewTrgPos - _dSttPos ); //감속 프로파일은 전에 무브 동작것을 자동적으로 따라감.
...
bool CAjinMotion::FindEdgeStop (double _dVel ,double _dAcc , EN_FINDEDGE_TYPE _ftFindEdgeType)
{
    ...
    */
    ...
    int iRet = AxmMoveSignalSearch(AjinPara.iPhysicalNo, _dVel, _dAcc, iSignal, iEdge , SLOWDOWN_STOP) ;
...
void CAjinMotion::SetTrgPos(int _iPosCnt , double * _dPos , double _dTrgTime , bool _bActual ,bool _bOnLevel)
{
    ...
    double dTrigTime = _dTrgTime; // 4ms D
