...
AnsiString TFrmDllMain::OnGetAge()
{
   ...
   AnsiString Date = FileDateToDateTime(Age).FormatString("''yyyy'. 'm'. 'd'. 'AM/PM' 'hh': 'nn'");
...
void __fastcall TFrmDllMain::tmUpdateTimer(TObject *Sender)
{
    ...
    switch(pcMain -> ActivePageIndex) {
        ...
        case 4 : //Motor
            ...
            //Master Level.  10/06 JS.
            pnMtStat    -> Visible = FrmPassword -> GetLevel() >= lvMaster ;
            pnAxtPara   -> Visible = FrmPassword -> GetLevel() >= lvMaster ;

            btTest      -> Visible = FrmPassword -> GetLevel() == lvContol ;
            btJogN      -> Visible = FrmPassword -> GetLevel() == lvContol ;
            btJogP      -> Visible = FrmPassword -> GetLevel() == lvContol ;
            btMtClrPos  -> Visible = FrmPassword -> GetLevel() == lvContol ;
            btMtHome    -> Visible = FrmPassword -> GetLevel() == lvContol ;
            btAllSrvOn  -> Visible = FrmPassword -> GetLevel() == lvContol ;
            btAllSrvOff -> Visible = FrmPassword -> GetLevel() == lvContol ;
            btAllHome   -> Visible = FrmPassword -> GetLevel() == lvContol ;
