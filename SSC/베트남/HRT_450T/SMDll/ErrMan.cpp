...
AnsiString CErrMan::GetErrImgPath (int _iNo)
{
    return Err[_iNo].sImgPath ;
}

double CErrMan::GetRectLeft   (int _iNo){return Err[_iNo].dRectLeft  ;}
double CErrMan::GetRectTop    (int _iNo){return Err[_iNo].dRectTop   ;}
double CErrMan::GetRectWidth  (int _iNo){return Err[_iNo].dRectWidth ;}
double CErrMan::GetRectHeight (int _iNo){return Err[_iNo].dRectHeight;}
...
void __fastcall CErrMan::DisplayList(TStringGrid * _sgGrid) //스트링 그리드 업데이트.
{
    ...
    _sgGrid -> ColWidths[1 ] = 250;
    _sgGrid -> ColWidths[2 ] = 700;
                                         ...
                                         TShape *_spErrRect  )
{
    ...
    if (Err[iErrNo].sImgPath == "" || !FileExists(Err[iErrNo].sImgPath)) {
        ...
        _imErr -> Picture -> LoadFromFile("D:\\Bc6Works\\SSC\\HRT_450T\\bin\\Util\\Camera\\1.bmp");
    }
