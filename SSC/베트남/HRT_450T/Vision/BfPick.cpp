...
int CBfPick::GetInspRowCnt(){return VOM.VDevOptn.iCountX ;}
int CBfPick::GetInspColCnt(){return VOM.VDevOptn.iCountY ;}
void CBfPick::SetInspRowCnt(int _iVal){VOM.VDevOptn.iCountX = _iVal;}
void CBfPick::SetInspColCnt(int _iVal){VOM.VDevOptn.iCountY = _iVal;}
...
void CBfPick::Inspection()
{
    ...
    EN_VISN_ERR_ID iInspRslt    = veBfPickOk ;
    int            iInspFailCnt = 0       ;
    bool           bRet         = true    ;
    ...
    while(1) { //고투문 대용.
        ...
        bRet = OCV_Inspect(m_pImg , Orig.pTrArea , Orig.pTrImage , Orig.pTrckInsp->GetOutRect() , VOM.VDevPara.BfPickOcv , Orig.pRsltArea , &Orig.tRslt) ;
        //}
        if(!bRet) {
            iInspRslt = veBfPickOriFail ;
            m_sRsltMsg = AnsiString("FAILED ORIGIN : ") + SVL_GetErrMsg();
            break ;
        }

        if(Orig.tRslt.fSinc < VOM.VDevOptn.dBfPickOriSinc) {
            iInspRslt = veBfPickOriNg ;
        ...
        for(int r = 0 ; r < GetInspRowCnt() ; r++) {
            for(int c = 0 ; c < GetInspColCnt() ; c++) {
                ...
                bRet = TSD_Inspect(m_pImg , tInspRect , VOM.VDevPara.BfPickTsd , &Insp.pRslt[VOM.VDevOptn.iCountX * r + c]);

                iInspPxCnt = Insp.pRslt[VOM.VDevOptn.iCountX * r + c].tRect.Height() * Insp.pRslt[VOM.VDevOptn.iCountX * r + c].tRect.Width() ;

                if(!bRet) {
                    iInspRslt = veBfPickInspFail ;
                ...
                */
                ...
                if(fPxPer < VOM.VDevOptn.dBfPickPxMinPer) {
                    iInspRslt = veBfPickInspNg ;
                    Rslt.bFailMap[r][c] = true ;
                    iInspFailCnt++;
                }
            }
            if(!iInspRslt == veBfPickInspFail) break ;
        }
        if(iInspRslt != veBfPickOk) {
...
void CBfPick::Train()
{
    if(m_iActivPage == piBfPickOcv) {
        //(IImage * _pImg , TRect _Rect , OCV_Para _tPara ,  IArea * _pTrainArea , IImage * _pTrainImg) ;
        if(!OCV_Train(GetImg() , Orig.pTrckInsp->GetTRect() , VOM.VDevPara.BfPickOcv , Orig.pTrArea , Orig.pTrImage)) {
                              ...
                              float _fScaleX , float _fScaleY ,int _iThreshold , bool _bCntrLine)
{
    ...
    if(g_bSettingMode){
        if(m_iActivPage == piBfPickLCS) {
            PaintCntLine(_hDc , _iDstW , _iDstH);
        }
        else if(m_iActivPage == piBfPickOcv) {
        ...
        else if(m_iActivPage == piBfPickTsd) {
    ...
    if(Stat.bDispRslt){
        ...
        dPxPer = Insp.pRslt[VOM.VDevOptn.iCountX * r + c].iPxCnt * 100 / iInspPxCnt ;
                ...
                if(dPxPer < VOM.VDevOptn.dBfPickPxMinPer) {
                    iInspRslt = veBfPickInspNg ;
                    iInspFailCnt++;
                }
        ...
        */
        ...
        for(int r = 0 ; r < VOM.VDevOptn.iCountY ; r++) {
            for(int c = 0 ; c < VOM.VDevOptn.iCountX ; c++) {
                iInspPxCnt = Insp.pRslt[VOM.VDevOptn.iCountX * r + c].tRect.Width() * Insp.pRslt[VOM.VDevOptn.iCountX * r + c].tRect.Height() ;
                fPercent = Insp.pRslt[VOM.VDevOptn.iCountX * r + c].iPxCnt * 100 /(float)iInspPxCnt ;
                if(fPercent  < VOM.VDevOptn.dBfPickPxMinPer) {Gdi -> m_tPen.Color = clRed  ; Gdi -> m_tText.Color = clRed  ;}
...
{
    if(m_iActivPage == piBfPickOcv) {
        UpdateParaOcv(_bTable , _vePara , VOM.VDevPara.BfPickOcv);
    }
    else if(m_iActivPage == piBfPickTsd) {
        UpdateParaTsd(_bTable , _vePara , VOM.VDevPara.BfPickTsd);
    }
}
...
void CBfPick::Load(bool _bLoad)
{
    ...
    //Origin
    LoadParaOcv(_bLoad , VOM.VDevPara.BfPickOcv , sJobPath , "Orig");
    ...
    //Insp
    LoadParaTsd(_bLoad , VOM.VDevPara.BfPickTsd , sJobPath , "Insp");
