...
class CArray
{
    ...
    private:
        ...
        bool       m_bLastStrip;
        bool       m_bCheck    ; //그냥 아무때나 가따 쓰기용. Ver 1.0.2.3
    ...
    public:
        ...
        void __fastcall SetCheck      (bool       _bCheck);
        bool __fastcall GetCheck      (void);
        ...
        //Zenner Value를 입력하게 하여 값을 가지고 있는다.
        ...
        void operator  = (CArray& rhs) {
            ...
            m_bCheck   = rhs.m_bCheck  ;

            //memcpy(CHPS , rhs.CHPS , sizeof(CChip) * m_iMaxCol * m_iMaxRow );
            for(int r = 0 ; r < m_iMaxRow ; r++) {
                for(int c = 0 ; c < m_iMaxCol ; c++) {
                    CHPS[r][c].SetStat(rhs.GetStat(r,c));

                    //VF 및 Zenner 값을 입력 받아서 분류 하기 위해서JS. 20150318
                    CHPS[r][c].SetVf  (rhs.GetVf  (r,c));
                    CHPS[r][c].SetVr  (rhs.GetVr  (r,c));
                }
            }
        }
};
