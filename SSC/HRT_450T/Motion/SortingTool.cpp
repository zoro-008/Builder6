    ...
    switch (Step.iCycle) {
        ...
        case  15: if(!OM.CmnOptn.bIgnrDecChk && Step.iSeq == scDevChk) {
                      if(DM.ARAY[riPSB].FindLastCol(csFail) != m_iLastDecCol && !DM.ARAY[riPSB].GetCntStat(csWork)){
                          if(!OM.DevOptn.bIgnrVoidMotion) MoveMotr(miSTL_YTop , piSTL_YTWait);
                          Step.iCycle++;
                          return false ;
                      }
                      else {
        ...
        case  16: MoveActr(aiSTL_Btm , ccBwd);
                  ...
                  //Ver 1.1.0.0
                  bExistFail     = DM.ARAY[riPSB].GetCntStat(csFail ); //FindChip(r1 , c1 , csFail);
                  bExistWork     = DM.ARAY[riPSB].GetCntStat(csWork ); //FindChip(r2 , c2 , csWork);
                  bExistMask     = DM.ARAY[riPSB].GetCntStat(csMask ); //FindChip(r3 , c3 , csMask);
                  bFinished      =(!bExistFail && !bExistWork && !bExistMask) || OM.CmnOptn.bIgnrTrim ; //현재 스트립의 작업이 끝났는지 확인함.
                  if(bFinished) MoveMotr(miSTL_YTop , piSTL_YTWait);

                  Step.iCycle++;
                  return false;

        case  17: if(!OM.DevOptn.bIgnrVoidMotion && !MoveMotr(miSTL_YTop , piSTL_YTWait)) return false;
                  if(bFinished && !MoveMotr(miSTL_YTop , piSTL_YTWait)) return false;
                  if(!MoveActr(aiSTL_Btm , ccBwd        )) return false;
                  Step.iCycle = 0 ;
                  return true ;
    }
...
//    else        DM.ARAY[riPS1].FindLastColRow    (csFail , c , r);
    ...
    switch (Step.iCycle) {
        ...
        case  18: if(!m_tmTemp.OnDelay(true, OM.DevOptn.iRejectMoveDeley)) return false;
                  Step.iCycle = 100;
                  return false;
        ...
        case 102: if(!MoveActr(aiSTL_Btm , ccBwd)) return false;
                  if(!OM.DevOptn.bIgnrVoidMotion && !MoveMotr(miSTL_YTop , piSTL_YTWait)) return false;
                  if(bFinished && !MoveMotr(miSTL_YTop , piSTL_YTWait)) return false;
                  Step.iCycle = 0 ;
                  return true ;
