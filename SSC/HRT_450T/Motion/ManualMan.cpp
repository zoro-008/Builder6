...
//    else if (m_iManNo == mcETC_PstRemoveStrip ){Trace("Manual Man","mcETC_PstRemoveStrip Stt");}
    ...
    //else if (m_iManNo == mcETC_6              ){}
                                                ...
                                                if(AT_Complete(aiULD_Cmp   , ccBwd)         ) {
                                                 ...
                                                 else { if(FM_MsgYesNo("Check", "Real Strip Remove?")==mrYes) {m_iManStep=10; m_iPreManStep = 0;}}
                                               }
    ...
    //                                              if(AT_Complete(aiPRB_IdChk , ccBwd)) { AT_MoveCyl(aiPRB_IdChk , ccFwd); } }
        ...
        if(iDevManRow >= OM.DevInfo.iRowCnt - 1) iDevManRow = OM.DevInfo.iRowCnt - 1;
...
double CManualMan::GetMotrPosTable(int _iRow , int _iType)
{
    ...
    dPos = dStartPos ;//+ (OM.DevInfo.dRowPitch * r);
    ...
    int iRowGrCnt    = OM.DevInfo.iRowGrCnt   >1 ? r / OM.DevInfo.iRowGrCnt : 0 ;

    //현재 Row까지의 서브그룹카운트.
    //왜 1이상이냐면 0이면 자동으로 1로 세팅되게 되어 있는데 1이면 의미가 없어서 그냥 0임.
    int iRowSubGrCnt = OM.DevInfo.iRowSubGrCnt >1 ? (OM.DevInfo.iRowGrCnt / OM.DevInfo.iRowSubGrCnt -iRowGrCnt) : 0 ;

    dPosOffset += r            *  OM.DevInfo.dRowPitch ;                           //첨에 일단 피치만 더하고.
