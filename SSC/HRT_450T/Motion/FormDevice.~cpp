...
//     //Check Running Status.
    ...
    AnsiString sTemp = "DOWNLOAD J OB FILE. (" + lbSelDevice -> Caption + ")";
    ...
    DM.ARAY[riPR2   ].SetMaxColRow(OM.DevInfo.iColCnt , OM.DevInfo.iRowCnt );  pbStatus -> Position = 30 ;
    ...
    DM.ARAY[riOPT   ].SetMaxColRow(OM.DevInfo.iColCnt , OM.DevInfo.iRowCnt );  pbStatus -> Position = 60 ;
//    DM.ARAY[riSSI   ].SetMaxColRow(1                  , OM.DevInfo.iCsSlCnt);  pbStatus -> Position = 65 ;
    WRK.ArraySSI.SetMaxColRow(1 , OM.DevInfo.iCsSlCnt);  pbStatus -> Position = 65 ;
    DM.ClearMap    (      );                                                   pbStatus -> Position = 70 ;
    OM.SaveLastInfo(      );                                                   pbStatus -> Position = 80 ;
    PM.Load        (sName );                                                   pbStatus -> Position = 90 ;
    //Ver 1.0.2.3
    //로더 SetMaxColRow 뒤쪽에 있어야 함
    WRK.ArrayLoad  (true  );                                                   pbStatus -> Position = 95 ;
...
void __fastcall TFrmDevice::btNewClick(TObject *Sender)
{
    ...
    if(FM_GetLan() == lsKorean) {
        switch (index) {
        	case 1: //New.
                 if (edToName -> Text == "") {   //아무것도 입력이 되지 않은 경우
                     FM_MsgOk("Error" , "입력된 JOB FILE 이름이 없습니다." ); return;
                 }
                 ...
                 if (!UserFile.CreateDir(Path)) {
                     if      (FM_GetLan() == lsKorean) { FM_MsgOk("Warning" , "같은 이름의 File이 존재합니다."); return ; }
                 }
            ...
            case 2: //Copy.
                ...
                if (edFromName -> Text == "") {   //아무것도 선택되지 않은 경우.
                    FM_MsgOk("Error" , "선택된 JOB FILE 이름이 없습니다." ); return;
                }
                ...
                if (UserFile.DirExists(Path + edToName -> Text)) {
                    FM_MsgOk("Warning" , "같은 이름의 File이 존재합니다."); return ;
                }
            ...
            case 4: //Delete
                ...
                if (edFromName -> Text == OM.m_sCrntDev) {
                    FM_MsgOk("Warning" , "현재 사용중 인JOB FILE은 삭제할 수 없습니다."); return;
                }
        ...
        switch (index) {
        	case 1: //New.
                 if (edToName -> Text == "") {   //아무것도 입력이 되지 않은 경우
                     FM_MsgOk("Error" , "JOB FILE Name is not Inputed."    ); return;
                 }

                 if(FM_MsgYesNo("Confirm" , edToName -> Text + " Create a JOB FILE?"         ) == mrNo) return;

                 Path = ExtractFilePath(Application -> ExeName) + "JobFile\\" + edToName -> Text ;

                 if (!UserFile.CreateDir(Path)) {
                     FM_MsgOk("Warning" , "Exist of same name file."      ); return ; 
                 }
            ...
            case 2: //Copy.
                //Check None Name.
                if (edFromName -> Text == "") {   //아무것도 선택되지 않은 경우.
                    FM_MsgOk("Error" , "JOB FILE Name is not selected."   ); return;
                }
                ...
                if(FM_MsgYesNo("Confirm" , edFromName -> Text +" of into "+ edToName -> Text + " to JOB FILE of copy created?"        ) == mrNo) return ;
            ...
            case 3: //Rename.
                ...
                if(FM_MsgYesNo("Confirm" , "Change is name of the JOB FILE?"   ) == mrNo) return;
            ...
            case 4: //Delete
                ...
                if(FM_MsgYesNo("Confirm" , "Would you like to delete the selected job file?" ) == mrNo)  return;

                if (edFromName -> Text == OM.m_sCrntDev) {
                    FM_MsgOk("Warning" , "Now using JOB FILE is not delete."           ); return;
                }
