...
//                 Trace(m_sPartName.c_str(), sTemp.c_str());
        ...
        case 14: if(!MT_GoAbsMan(miWRK_YBtm, GetMotrPos(miWRK_YBtm,piWRK_YBWait)))return false;
                 if(OM.MstOptn.bUsedXAxis && !MT_GoAbsMan(miWRK_XBtm, GetMotrPos(miWRK_XBtm,piWRK_XBWait))) return false;
...
void CWorkZone::ArrayLoad(bool IsLoad)
{
    //Local Var.
    TUserINI   UserINI;
    AnsiString sPath  ;
    ...
    sPath = EXE_FOLDER + "JobFile\\" + OM.m_sCrntDev + "\\ArraySSI.INI";


    //Load
    AnsiString sName    = "ArraySSI" ;
    AnsiString sRowData ;
    AnsiString sIndex   ;
    AnsiString sTemp    ;
    int iMaxCol = DM.ARAY[riLDR].GetMaxCol(); //WRK.ArraySSI.GetMaxCol();
    int iMaxRow = DM.ARAY[riLDR].GetMaxRow(); //WRK.ArraySSI.GetMaxRow();
    if(IsLoad) {
        //UserINI.Load(sPath, sName , "m_iMaxRow" , iMaxRow             );
        //UserINI.Load(sPath, sName , "m_iMaxCol" , iMaxCol             );
        //if(!iMaxRow) iMaxRow = 1 ;
        //if(!iMaxCol) iMaxCol = 1 ;

        //메모리 재할당.
        WRK.ArraySSI.SetMaxColRow(iMaxCol , iMaxRow);

        for (int r = 0 ; r < iMaxRow ; r++) {
            sIndex.sprintf("Row%03d", r ) ;
            UserINI.Load(sPath, sName , sIndex, sRowData );
            for (int c = 0 ; c < iMaxCol ; c++) {
                sTemp = sRowData.SubString(c*3+1,3);
                //ArraySSI.CHPS[r][c].SetStat((EN_CHIP_STAT)sTemp.ToIntDef(0));
                WRK.ArraySSI.SetStat(r,c,(EN_CHIP_STAT)sTemp.ToIntDef(0));
            }
        }
    }
    else {
        //UserINI.Save(sPath, sName , "iMaxRow" , iMaxRow           );
        //UserINI.Save(sPath, sName , "iMaxCol" , iMaxCol           );
        for (int r = 0 ; r < iMaxRow ; r++) {
            sRowData = "" ;
            sIndex.sprintf("Row%03d", r ) ;
            for (int c = 0 ; c < iMaxCol ; c++) {
                //sTemp.sprintf("%03d",(int)CHPS[r][c].GetStat()) ;
                sTemp.sprintf("%03d",(int)WRK.ArraySSI.GetStat(r,c)) ;
                sRowData += sTemp ;
            }
            UserINI.Save(sPath, sName , sIndex, sRowData );
        }
    }
