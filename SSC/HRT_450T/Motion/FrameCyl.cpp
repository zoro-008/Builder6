...
bool TFraCyl::CheckSafe(int _iId , bool _bFwd)
{
    ...
    AnsiString sMsg = "";
         ...
         if(_iId == (int)aiLDR_Cmp   ) { bRet = LDR.CheckSafe((EN_ACTR_ID)_iId,_bFwd) ; }
    else if(_iId == (int)aiLDR_Psh   ) { bRet = LDR.CheckSafe((EN_ACTR_ID)_iId,_bFwd) ; }
    else if(_iId == (int)aiPR1_Cmp   ) { bRet = PR1.CheckSafe((EN_ACTR_ID)_iId,_bFwd) ; }
    else if(_iId == (int)aiPR2_Cmp   ) { bRet = PR2.CheckSafe((EN_ACTR_ID)_iId,_bFwd) ; }
    //else if(_iId == (int)aiPRB_IdChk ) { bRet = PR2.CheckSafe((EN_ACTR_ID)_iId,_bFwd) ; }
    ...
    else if(_iId == (int)aiPSB_Rolr  ) { bRet = PSB.CheckSafe((EN_ACTR_ID)_iId,_bFwd) ; }
