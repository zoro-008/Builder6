...
bool CLoader::Autorun(void) //오토런닝시에 계속 타는 함수.
{
    ...
    if (Step.iSeq == 0) {
        ...
        bool isCycleWork   =  DM.ARAY[riLDR].GetCntStat  (csUnkwn)    &&  DM.ARAY[riPR1].CheckAllStat(csNone ) &&
                              PR1.GetCycleStep() == CPreBuff1::scIdle && !OM.CmnOptn.bLoadingStop;
        ...
        //모르는 스트립에러.
        if(  DM.ARAY[riLDR].CheckAllStat(csNone) && (  IO_GetX(xLDR_MgzDetect1) ||  IO_GetX(xLDR_MgzDetect2) )     ) EM_SetErr(eiLDR_ClampUnknown) ;
        //카세트 사라짐.
        if( !DM.ARAY[riLDR].CheckAllStat(csNone) && ( !IO_GetX(xLDR_MgzDetect1) && !IO_GetX(xLDR_MgzDetect2) )     ) EM_SetErr(eiLDR_ClampDispr)   ;
        //카세트 OUT 매거진 풀
...
//                  else {
                    ...
                    //Ver 1.0.2.3
                    if(OM.DevInfo.bSSI){
                        if(WRK.ArraySSI.GetStat(DM.ARAY[riLDR].FindLastRow(csUnkwn),0) != csWork){
                            DM.ARAY[riPR1].SetCheck(true);
                        }
                    }
