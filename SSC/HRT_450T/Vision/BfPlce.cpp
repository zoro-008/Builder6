...
int CBfPlce::GetInspRowCnt(){return VOM.VDevOptn.iCountY ;}
int CBfPlce::GetInspColCnt(){return VOM.VDevOptn.iCountX ;}
void CBfPlce::SetInspRowCnt(int _iVal){VOM.VDevOptn.iCountY = _iVal;}
void CBfPlce::SetInspColCnt(int _iVal){VOM.VDevOptn.iCountX = _iVal;}

bool CBfPlce::Init(TVisnStaticPara _tVisnStaticPara)
{
...
void CBfPlce::Inspection()
{
    DWORD dwStartTime = GetTickCount() ;

    memset(&Rslt , 0 , sizeof(TRetResult));

    EN_VISN_ERR_ID iInspRslt    = veBfPlceOk ;
    int            iInspFailCnt = 0          ;
    bool           bRet         = true       ;
    ...
    while(1) { //고투문 대용.
        ...
        bRet = OCV_Inspect(m_pImg , Orig.pTrArea , Orig.pTrImage , Orig.pTrckInsp->GetOutRect() , VOM.VDevPara.BfPlceOcv , Orig.pRsltArea , &Orig.tRslt) ;
        //}
        if(!bRet) {
            iInspRslt = veBfPlceOriFail ;
            m_sRsltMsg = AnsiString("FAILED ORIGIN : ") + SVL_GetErrMsg();
            break ;
        }

        if(Orig.tRslt.fSinc < VOM.VDevOptn.dBfPlceOriSinc) {
            iInspRslt = veBfPlceOriNg ;
        ...
        for(int r = 0 ; r < GetInspRowCnt() ; r++) {
            for(int c = 0 ; c < GetInspColCnt() ; c++) {
                ...
                bRet = TSD_Inspect(m_pImg , tInspRect , VOM.VDevPara.BfPlceTsd , &Insp.pRslt[GetInspColCnt() * r + c]);

                iInspPxCnt = Insp.pRslt[GetInspColCnt() * r + c].tRect.Height() * Insp.pRslt[GetInspColCnt() * r + c].tRect.Width() ;

                if(!bRet) {
                    iInspRslt = veBfPlceInspFail ;
                ...
                */
                ...
                if(fPxPer < VOM.VDevOptn.dBfPlcePxMinPer) {
                    iInspRslt = veBfPlceInspNg ;
                    Rslt.bFailMap[r][c] = true ;
                    iInspFailCnt++;
                }
            }
            if(!iInspRslt == veBfPlceInspFail) break ;
        }
        if(iInspRslt != veBfPlceOk) {
...
void CBfPlce::Train()
{
    if(m_iActivPage == piBfPlceOcv) {
        //(IImage * _pImg , TRect _Rect , OCV_Para _tPara ,  IArea * _pTrainArea , IImage * _pTrainImg) ;
        if(!OCV_Train(GetImg() , Orig.pTrckInsp->GetTRect() , VOM.VDevPara.BfPlceOcv , Orig.pTrArea , Orig.pTrImage)) {
            ShowMessage(SVL_GetErrMsg());
        }
    }
}

void CBfPlce::Paint(HDC _hDc , EN_DISP_OPTN _iDpOptn ,
                              ...
                              float _fScaleX , float _fScaleY ,int _iThreshold , bool _bCntrLine)
{
    ...
    if(g_bSettingMode){
        if(m_iActivPage == piBfPlceLCS) {
            PaintCntLine(_hDc , _iDstW , _iDstH);
        }
        else if(m_iActivPage == piBfPlceOcv) {
        ...
        else if(m_iActivPage == piBfPlceTsd) {
            for(int r = 0 ; r < GetInspRowCnt() ; r++) {
                for(int c = 0 ; c < GetInspColCnt() ; c++) {
        ...
        */
        ...
        for(int r = 0 ; r < VOM.VDevOptn.iCountY ; r++) {
            for(int c = 0 ; c < VOM.VDevOptn.iCountX ; c++) {
                ...
                fPercent = Insp.pRslt[VOM.VDevOptn.iCountX * r + c].iPxCnt * 100 /(float)iInspPxCnt ;
                if(fPercent  < VOM.VDevOptn.dBfPlcePxMinPer) {Gdi -> m_tPen.Color = clRed  ; Gdi -> m_tText.Color = clRed  ;}
...
void CBfPlce::SetPaintCallback(PaintCallbackFunc _pCalback)
{
...
bool CBfPlce::OnMouseDw(TMouseButton Button, TShiftState Shift, int X, int Y , float _fScaleX , float _fScaleY, float _fOffsetX, float _fOffsetY)
{
    if(g_bSettingMode){
        if(m_iActivPage == piBfPlceLCS) {
        }
...
bool CBfPlce::OnMouseMv(TShiftState Shift, int X, int Y , float _fScaleX , float _fScaleY, float _fOffsetX, float _fOffsetY)
{
...
void CBfPlce::OnMouseUp(TMouseButton Button, TShiftState Shift, int X, int Y , float _fScaleX , float _fScaleY, float _fOffsetX, float _fOffsetY)
{
...
void CBfPlce::ShowAlgoPara(TValueListEditor * _vePara)
{
...
void CBfPlce::UpdateAlgoPara(bool _bTable , TValueListEditor * _vePara)
{
    if(m_iActivPage == piBfPlceOcv) {
        UpdateParaOcv(_bTable , _vePara , VOM.VDevPara.BfPlceOcv);
    }
    else if(m_iActivPage == piBfPlceTsd) {
        UpdateParaTsd(_bTable , _vePara , VOM.VDevPara.BfPlceTsd);
    }
}

void CBfPlce::Load(bool _bLoad)
{
    ...
    //Origin
    LoadParaOcv(_bLoad , VOM.VDevPara.BfPlceOcv , sJobPath , "Orig");
    ...
    //Insp
    LoadParaTsd(_bLoad , VOM.VDevPara.BfPlceTsd , sJobPath , "Insp");
