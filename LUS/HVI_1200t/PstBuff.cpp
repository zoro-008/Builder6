...
bool CPstBuff::FindChipMrk( int &r , int &c ) //
{
    ...
    {
        if(OM.MstOptn.bEmptyNoTrm && i == csRslt1 ) continue ; //Empty == csRslt1
...
*/
    ...
    if( OM.DevInfo.iColSubGroupCount   ) {
        dYTrmPos = dYTrmPos + ((dSubColGroupGap ) * (iFlipR / OM.DevInfo.iColSubGroupCount)) ;
        dYVsnPos = dYVsnPos + ((dSubColGroupGap ) * (r      / OM.DevInfo.iColSubGroupCount)) ;
        dYMrkPos = dYMrkPos + ((dSubColGroupGap ) * (mr     / OM.DevInfo.iColSubGroupCount)) ;
    }



    if( OM.DevInfo.iRowSubGroupCount   ) {
        dYTrmPos = dYTrmPos + ((dSubRowGroupGap ) * (iFlipR / OM.DevInfo.iRowSubGroupCount)) ;
        dYVsnPos = dYVsnPos + ((dSubRowGroupGap ) * (r      / OM.DevInfo.iRowSubGroupCount)) ;
        dYMrkPos = dYMrkPos + ((dSubRowGroupGap ) * (mr     / OM.DevInfo.iRowSubGroupCount)) ;
    }
...
//        case 15: if(!MoveActr(aiPSB_FlprCmp,ccBwd)) return false ;
        ...
        case 27: if(!MoveActr(aiPSB_FlprOpCl,ccFwd)) return false ;
                 MoveMotr(miPSB_XIns     , piPSB_XInsWait);
                 MoveActr(aiPSB_IndxUpDn , ccBwd         );
...
//2012.09.24 dd
        ...
        case 19 : MoveActr(aiPSB_IndxUpDn , ccBwd);
...
//    int          iCol , iRow ;                                                                                               //1,2,3 bin
    ...
    if(!OM.MstOptn.bEmptyNoTrm      &&  //밑에 순서 주의 0,1번  1번이 엠티라 서 그럼.
...
bool CPstBuff::CycleOut(void) //자제 아웃.
{
    ...
    switch (Step.iCycle) {
        ...
        case 18 : if(!MoveActr(aiPSB_PnchDnUp , ccFwd )) return false ;
                  MoveActr(aiPSB_PshrFwBw , ccFwd  ) ;
                  Step.iCycle++;
                  return false ;

        case 19 : if(!MoveActr(aiPSB_PshrFwBw , ccFwd )) return false ;
...
//                      if(OM.CmnOptn.bUseTrimMachine) EC.SendPstEqp(psLotEnd);
                  ...
                  DM.ARAY[riULD].SetStat(DM.ARAY[riULD].FindLastRow(csEmpty),0 , csWork);
...
//
                  ...
                  return false ;
*/
        case 20 : if(!MoveActr(aiPSB_PshrFwBw,ccBwd)) return false ;
                  MoveActr(aiPSB_PshrDnUp, ccBwd );

                  Step.iCycle++;
                  return false ;

        case 21 : if(!MoveActr(aiPSB_PshrDnUp, ccBwd )) return false ;
                  if(!MoveMotr(miPSB_XIns    , piPSB_XInsWait)) return false ;
