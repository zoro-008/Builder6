...
void CEqpComUnit::ThreadPstEqpMsg()
{
    ...
    else if(iPstSendMsg == psSending   ) {IO_SetY(yETC_PstEqpSending   , true ); Sleep(1000); IO_SetY(yETC_PstEqpSending   , false ); }
...
void CEqpComUnit::SendPstEqp(EN_PST_EQP_SEND _iMsg)
{
    ...
    else if(_iMsg == psSending   ) {TRACE("Smd PstEqp","Sending  "); WriteArayData(riPSB); }
...
bool __fastcall CEqpComUnit::WriteArayData(EN_ARAY_ID riId) //다음장비 네트워크 드라이브에 스트립 데이터를 쓴다. X
{                                                           //현장비에 스트림 데이터를 쓴다.dd
    //Local Var.
    TUserINI   UserINI;
    AnsiString sPath  ;
