...
//    gbSkipInfo -> Enabled >= FM_GetLevel() == lvEngineer ;
    ...
    cbUseTrimMachine   -> Enabled = FM_GetLevel() >= lvEngineer ;
    cbVisionAllSkip    -> Enabled = FM_GetLevel() >= lvEngineer ;
    cbUsePenMrk        -> Enabled = FM_GetLevel() >= lvEngineer && !SEQ._bRun; //Run 때 바꾸면 동작 안되서 여기서 일단 막았음.dd
...
void __fastcall TFormOperation::UpdateCmmOptn(bool bTable)
{
    if(bTable) { //Checked 속성 건드리면 OnClick 이벤트가 발생되나 그래서 이렇게 함. dd
        ...
        CbChecked(cbUseTrimMachine , OM.CmnOptn.bUseTrimMachine );
        CbChecked(cbIdleRun        , OM.CmnOptn.bIdleRun        );
        CbChecked(cbUsePenMrk      , OM.CmnOptn.bUsePenMrk      );
    }
    else {
        ...
        OM.CmnOptn.bUseTrimMachine = cbUseTrimMachine -> Checked  ;
        OM.CmnOptn.bIdleRun        = cbIdleRun        -> Checked  ;
        OM.CmnOptn.bUsePenMrk      = cbUsePenMrk      -> Checked  ;
    }
}
//---------------------------------------------------------------------------
...
void __fastcall TFormOperation::cbTrm1SkipClick(TObject *Sender)
{
    ...
    //처리를 해야 할듯 하네요. dd
    bool bUsePenMrk = CbChecked(cbUsePenMrk);
    if(sTemp == "cbUsePenMrk" && bUsePenMrk) {
        CbChecked(cbTrmAllSkip,true);
    }

    bool bTrmAllSkip = CbChecked(cbTrmAllSkip);
    if(sTemp == "cbTrmAllSkip" && !bTrmAllSkip) {
        CbChecked(cbUsePenMrk,false);
    }
