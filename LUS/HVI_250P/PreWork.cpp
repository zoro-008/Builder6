...
bool CPreWork::FindChip(int &r , int &c )
{
...
double CPreWork::RejectMove(void)
{
...
double CPreWork::GetMotrPos(EN_MOTR_ID _iMotr , EN_PSTN_ID _iPstnId )
{
    double dPos       = 0.0 ;
...
//---------------------------------------------------------------------------
bool CPreWork::CheckSafe(EN_MOTR_ID _iMotr , EN_PSTN_ID _iPstnId)
{
...
//---------------------------------------------------------------------------
bool  CPreWork::CheckSafe(EN_ACTR_ID _iActr , bool _bFwd)
{
...
bool CPreWork::MoveMotr(EN_MOTR_ID _iMotr , EN_PSTN_ID _iPstnId) // 모터를 움직일때 쓰는 함수.
{
...
bool CPreWork::MoveActr(EN_ACTR_ID _iActr , bool _bFwd) //실린더를 움직일때 쓰는 함수.
{
...
//                 MT_GoAbsMan(miWRK_ZPch , PM.GetValue(miWRK_ZPch , pvWRK_ZPchWait));
        ...
        case 13: if(!MT_GoAbsMan(miWRK_1Idx ,  m_dLast1IdxPos)) return false ;
                 if(!MT_GoAbsMan(miWRK_2Idx ,  m_dLast2IdxPos)) return false ;
                 if(!MT_GoAbsMan(miWRK_XRjt ,  m_dLastXSrtPos)) return false ;
...
bool CPreWork::ToStopCon(void) //스탑을 하기 위한 조건을 보는 함수.
{
    //Clear Timer.
    Stat.bReqStop = true ;
//    m_ToStopTimer.Clear();
...
bool CPreWork::ToStartCon(void) //스타트를 하기 위한 조건을 보는 함수.
{
...
bool CPreWork::ToStart(void) //스타트를 하기 위한 함수.
{
    //Check Time Out.
...
bool CPreWork::ToStop(void) //스탑을 하기 위한 함수.
{
    //Check Time Out.
...
bool CPreWork::Autorun(void) //오토런닝시에 계속 타는 함수.
{
    //Check Cycle Time Out.
...
//        MT_SetServo(miWRK_ZPch,false);
    ...
    if(Step.iSeq == 0 && Step.iSubSeq == 0) {
        bool isConEnd          = (DM.ARAY[riWRK].CheckAllStat(csNone) );//&& DM.ARAY[riWRK].GetStep() == asWRK_None );
    ...
    if (Step.iSubSeq == 0 ) { //생각해 보니...안되네 나눔.dd
        //Sub
        bool isCycle2IdxWorkStt=  !DM.ARAY[riWRK].CheckAllStat(csNone) &&  DM.ARAY[riWRK].GetCntStat  (csEmpty) && DM.ARAY[riWRK].GetStep() == asWRK_2IdxWorkStt;
        bool isCycle2IdxWork   =  !DM.ARAY[riWRK].CheckAllStat(csNone) &&  DM.ARAY[riWRK].GetCntStat  (csEmpty) && DM.ARAY[riWRK].GetStep() == asWRK_2IdxWork   ;
        bool isCycle2IdxReWork =  !DM.ARAY[riWRK].CheckAllStat(csNone) &&  DM.ARAY[riWRK].GetCntStat  (csEmpty) && DM.ARAY[riWRK].GetStep() == asWRK_2IdxReWork ; //3번이면 다찍는다네...dd
        bool isCycle2IdxOut    =  !DM.ARAY[riWRK].CheckAllStat(csNone) &&  DM.ARAY[riWRK].CheckAllStat(csEmpty) && DM.ARAY[riWRK].GetStep() >= asWRK_2IdxPull   ;
        bool isConEnd1         = (DM.ARAY[riWRK].CheckAllStat(csNone) && DM.ARAY[riWRK].GetStep() == asWRK_None );

        if(DM.ARAY[riWRK ].CheckAllStat(csNone) &&  IO_GetX(xPSB_Strip3) && !OM.CmnOptn.bDryRun &&
...
bool CPreWork::CycleToWrkStt(void) //
{
    //Check Cycle Time Out.
    AnsiString sTemp ;
...
bool CPreWork::Cycle1IdxWork(void) //
{
    //Check Cycle Time Out.
    AnsiString sTemp ;
    if (m_tmCycle.OnDelay(Step.iCycle == PreStep.iCycle && CheckStop() && !OM.MstOptn.bDebugMode , 5000 )) {
...
bool CPreWork::CycleTo2Idx(void) //
{
    //Check Cycle Time Out.
    AnsiString sTemp ;
    if (m_tmCycle.OnDelay(Step.iCycle == PreStep.iCycle && CheckStop() && !OM.MstOptn.bDebugMode , 5000 )) {
...
bool CPreWork::Cycle2IdxWorkStt(void) //
{
    //Check Cycle Time Out.
    AnsiString sTemp ;
...
bool CPreWork::Cycle2IdxWork(void) //
{
    //Check Cycle Time Out.
    AnsiString sTemp ;
    if (m_tmSubCycle.OnDelay(Step.iSubCycle == PreStep.iSubCycle && CheckStop() && !OM.MstOptn.bDebugMode , 5000 )) {
    ...
    switch (Step.iSubCycle) {
        ...
        case  13: if(!MoveMotr(miWRK_ZPch,piWRK_ZPchWorkStt)) return false ;
                  ...
                  if(OM.DevInfo.iColCnt / (float)OM.DevOptn.iTrimColCnt > 2) { //2줄이상일때에 보내버림.dd
                      DM.ARAY[riWRK].SetStep(asWRK_2IdxReWork );
                      Step.iSubCycle = 0 ;
                      return true ;
                  }
...
//                  LotInfoAdd();
        ...
        case  14: if(!MoveMotr(miWRK_ZPch,piWRK_ZPchWait)) return false ;
                  DM.ARAY[riWRK].SetStep(asWRK_2IdxOut    );
//                  if(OM.DevInfo.iColCnt / OM.DevOptn.iTrimColCnt == 3) DM.ARAY[riWRK].SetStep(asWRK_2IdxReWork );
...
bool CPreWork::Cycle2IdxReWork(void) //
{
    //Check Cycle Time Out.
    AnsiString sTemp ;
    if (m_tmSubCycle.OnDelay(Step.iSubCycle == PreStep.iSubCycle && CheckStop() && !OM.MstOptn.bDebugMode , 5000 )) {
...
bool CPreWork::Cycle2IdxOut(void) //
{
    //Check Cycle Time Out.
    AnsiString sTemp ;
    if (m_tmSubCycle.OnDelay(Step.iSubCycle == PreStep.iSubCycle && CheckStop() && !OM.MstOptn.bDebugMode , 5000 )) {
    ...
    switch (Step.iSubCycle) {
        ...
        case  11: if(!MoveActr(aiWRK_2Idx,ccFwd)) return false ;
                  ...
                  DM.ARAY[riWRK].SetStep(asWRK_2IdxPull);
                  Step.iSubCycle++;
                  return false ;
        ...
        case  15: if(!MoveActr(aiWRK_2Idx,ccFwd)) return false ;
                  ...
                  DM.ARAY[riWRK].SetStat(csNone); //Data Masking. dd
//                  DM.ARAY[riWRK].SetStep(asWRK_2IdxOut);
...
//    Bin5.iBinCnt += OM.DevInfo.iCsSlCnt * OM.DevOptn.iLotEndMgzCnt * (OM.DevInfo.iRowCnt * OM.DevInfo.iColCnt) ;
    ...
    if(DM.ARAY[riWRK].GetCntStat(csUnkwn))
    {
        switch(m_iBinCnt)
        {
...
//TODO::    else TODO:: 아 집에 가야되서 못함.ㅠㅠ dd
    ...
    switch(m_iBinCnt)
    {
        case 1 : if(GetLastStrip() ) { Bin1.bBinFull = true ; } break ;
        case 2 : if(GetLastStrip() ) { Bin2.bBinFull = true ; } break ;
        case 3 : if(GetLastStrip() ) { Bin3.bBinFull = true ; } break ;
        case 4 : if(GetLastStrip() ) { Bin4.bBinFull = true ; } break ;
        case 5 : if(GetLastStrip() ) { Bin5.bBinFull = true ; } break ;
        default : break ;
    }
}

bool CPreWork::LotInfoAdd  ()
{
    int iToolCnt ;
...
//    bool bWRKDiff  = DM.ARAY[riWRK].GetLotNo() != DM.ARAY[riPRB].GetLotNo() ;
    ...
    bool bPreEqpLotEnd = EC.GetPreEqpLotEnd() && DM.ARAY[riPRB].CheckAllStat(csNone) && DM.ARAY[riWRK].CheckAllStat(csNone) ;
...
void CPreWork::SetLastCmd()
{
    return ; //이파트는 필요 없다.

}

bool CPreWork::CheckMoved()
{
    return true ; //이파트는 필요 없다.
}

bool CPreWork::CheckStop()
{
...
void CPreWork::Load(bool IsLoad)
{
    //Local Var.
    TUserINI   UserINI;
    AnsiString sPath  ;
