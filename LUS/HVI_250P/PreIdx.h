...
class CPreIdx
{
	...
	bool ToStop      (void); //스탑을 하기 위한 함수.
	    ...
	    //One Cycle.
        bool CycleToWork   (void); //처음 들어온 위치에서 첫 트림 위치로 가는것.
        bool CycleToReWork1(void); //처음 들어온 위치에서 2번째 트림 위치로 가는것.
        bool CycleToReWork2(void); //처음 들어온 위치에서 3번째 트림 위치로 가는것.
        bool CycleOut      (void); //Post Index에게 넘겨주는 위치로 가는것.
