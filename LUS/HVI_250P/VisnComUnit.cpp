...
{
    iVisnResetSel = _iSel ;
    if(_iSel==vrVisn1) CRunThread<Func> * func = new CRunThread<Func>(&ThreadReset1) ;
    if(_iSel==vrVisn2) CRunThread<Func> * func = new CRunThread<Func>(&ThreadReset2) ;
    if(_iSel==vrVisn3) CRunThread<Func> * func = new CRunThread<Func>(&ThreadReset3) ;
    if(_iSel==vrVisn4) CRunThread<Func> * func = new CRunThread<Func>(&ThreadReset4) ;
...
//        switch(_iSel)
//        {
//            case vrVisn1 : ThreadReset1(); break;
//            case vrVisn2 : ThreadReset2(); break;
//            case vrVisn3 : ThreadReset3(); break;
//            case vrVisn4 : ThreadReset4(); break;
//            default : break ;
//
//        }
...
void CVisnComUnit::ThreadModelChange()
{
    ...
    while(1) {
        ...
        switch (iStep) {
            ...
            case  11: if(!IO_GetX(xHED_1Ready,true)) {
                          EM_SetErr(eiHED_Visn1NotReady);
                          EM_SetDisp(true) ;
                          return ;
                      }
                      ...
                      if(!IO_GetX(xHED_3Ready,true)) {
                          EM_SetErr(eiHED_Visn3NotReady);
                          EM_SetDisp(true) ;
                          return ;
                      }
                      if(!IO_GetX(xHED_4Ready,true)) {
                          EM_SetErr(eiHED_Visn4NotReady);
                          EM_SetDisp(true) ;
                          return ;
                      }
                      ...
                      IO_SetY(yHED_3JobChange , true) ;
                      IO_SetY(yHED_4JobChange , true) ;
                      Timer.Clear();
                      iStep++;
                      continue ;

            case  12: if(IO_GetX(xHED_1Ready,true)) continue ;
                      if(IO_GetX(xHED_2Ready,true)) continue ;
                      if(IO_GetX(xHED_3Ready,true)) continue ;
                      if(IO_GetX(xHED_4Ready,true)) continue ;
                      //if(!Timer.OnDelay(true , 1000)) continue ;

                      iStep++;
                      continue ;

            case  13: IO_SetY(yHED_1JobChange ,false) ;
                      IO_SetY(yHED_2JobChange ,false) ;
                      IO_SetY(yHED_3JobChange ,false) ;
                      IO_SetY(yHED_4JobChange ,false) ;
                      iStep++;
            ...
            case  14: if(!IO_GetX(xHED_1Ready,true)) continue ;
                      if(!IO_GetX(xHED_2Ready,true)) continue ;
                      if(!IO_GetX(xHED_3Ready,true)) continue ;
                      if(!IO_GetX(xHED_4Ready,true)) continue ;
                      Timer.Clear();
            ...
            case  15: //if(Timer.OnDelay(true , 1000)) {
                      ...
                      if(IO_GetX(xHED_3Busy,true)) continue ;
                      if(IO_GetX(xHED_4Busy,true)) continue ;
                      iStep = 0 ;
