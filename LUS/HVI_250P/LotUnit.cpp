...
{
    AnsiString sTempJobFile  = EC.EndCheckLot.sJobFile  ;
    AnsiString sTempOperator = EC.EndCheckLot.sOperator ;
    ClearLotInfo();

    LotInfo.sLotNo           = EC.EndCheckLot.sLotNo ;
...
//    WRK.m_iBinNo = 1 ; //Lot Open 에서 하기가 좀 애매해서...dd
...
void __fastcall CLot::pnLotWorkChip(TPanel * _pnLotNo    ,
                                    TPanel * _pnJobFile  ,
                                    TPanel * _pnOperator ,
                                    TPanel * _pnStartTime,
                                    TPanel * _pnEndTime  ,
...
//        DayInfo.iCntHeatFail= LotInfo.iCntHeatFail ;
    ...
    TDateTime tTemp ;
    String    sTemp ;
    sTemp = DM.ARAY[riPSI].GetLotNo();
//    String    sDate ;
//    tTemp.Val = LotInfo.dStartTime ;
//    sDate = tTemp.FormatString("hh_nn_ss") ;

    Path = LOG_FOLDER + Temp + "\\" + sTemp + ".csv"; // "(" + sDate + ")" //MgzNo때문에 JS
