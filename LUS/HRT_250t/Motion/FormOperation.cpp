...
void TFrmOperation::DM_Init ()
{
    ...
    DM.ARAY[riRAB].PopupMenuEnable(true);
...
void __fastcall TFrmOperation::tmUpdateTimer(TObject *Sender)
{
    ...
    if(IO_GetY(yETC_HeatOn1)){
        btHeatOn1->Caption = "HEAT 1 ON";
        btHeatOn1->Font->Color = clRed;
    }
    else {
        btHeatOn1->Caption = "HEAT 1 OFF";
        btHeatOn1->Font->Color = clBlack ;
    }
    if(IO_GetY(yETC_HeatOn2)){
        btHeatOn2->Caption = "HEAT 2 ON";
        btHeatOn2->Font->Color = clRed;
    }
    else {
        btHeatOn2->Caption = "HEAT 2 OFF";
        btHeatOn2->Font->Color = clBlack ;
    }

    btMan2_D ->Caption = IO_GetY(yRAL_AirBlower) ? "AIR BLOWER ON" : "AIR BLOWER OFF" ;
...
void __fastcall TFrmOperation::btHeatOn1Click(TObject *Sender)
{
    int iIndex = ((TButton *)Sender) -> Tag ;

    switch(iIndex) {
        case 1 : IO_SetY(yETC_HeatOn1,!IO_GetY(yETC_HeatOn1)); break ;
        case 2 : IO_SetY(yETC_HeatOn2,!IO_GetY(yETC_HeatOn2)); break ;
    }
}
//---------------------------------------------------------------------------
...
void __fastcall TFrmOperation::btFeederClick(TObject *Sender)
{
    IO_SetY(yETC_FeederOn,!IO_GetY(yETC_FeederOn));

    if(IO_GetY(yETC_FeederOn)) btFeeder -> Caption = "FEEDER ON ";
    else                       btFeeder -> Caption = "FEEDER OFF";
}
//---------------------------------------------------------------------------
