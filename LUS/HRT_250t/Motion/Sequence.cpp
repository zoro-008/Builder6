...
void __fastcall CSequence::UpdateButton(void) //외부 버튼들 및 윈도우즈 버튼들이 클릭이 되었을때 취합해서 동작진행 시키는 함수.
{
    ...
    IO_SetY(yETC_AirLp    , IO_GetY(yETC_MainAir));
    ...
    if(IO_GetX(xRAL_PkgInChk)) m_tmSttSec.Clear();

    if (isStopCon  &&  m_iStep) {m_bReqStop = true ; m_tmStopSec.Clear();}
...
{                                                                                         
    ...
    if(!IO_GetY(yETC_MainAir)) { isOk = false ; EM_SetErr(eiETC_MainAir  ) ; }
...
{
    if(OM.CmnOptn.bIgnrHeatErr || !IO_GetY(yETC_HeatOn1))return true ;
...
//    if(IO_GetX(xRAL_Heat2DnLmt  )) {isOk = false ; EM_SetErr(eiRAL_Heat2DnLmt ); }
    ...
    if(m_bRun) {
        if(OM.DevOptn.iTempSet1 + OM.DevOptn.iTempGap < HTR.GetTemp1() ){ isOk = false ; EM_SetErr(eiRAL_Heat1SoftLmt) ; IO_SetY(yETC_HeatOn1,false);}
        if(OM.DevOptn.iTempSet1 - OM.DevOptn.iTempGap > HTR.GetTemp1() ){ isOk = false ; EM_SetErr(eiRAL_Heat1SoftLmt) ;}

        if(OM.DevOptn.iTempSet2 + OM.DevOptn.iTempGap < HTR.GetTemp2() ){ isOk = false ; EM_SetErr(eiRAL_Heat2SoftLmt) ; IO_SetY(yETC_HeatOn2,false);}
...
bool __fastcall CSequence::InspectActuator(void)
{
    ...
    //Inspect.
    if(AT_Err(aiRAL_Index)) {EM_SetErr(eiRAL_Index   ) ; isOk = false ; }
...
//    for(int i=0; i<MAX_MOTR; i++) if (MT_GetPLimSnsr(i)){EM_SetErr((int)eiFED_TFedPosLim + i ); isOk = false; }
    ...
    if (MT_GetAlarm   (miRAL_XIdx)){EM_SetErr(eiRAL_XIdxAlarm); isOk = false; }
    if (MT_GetAlarm   (miWRK_ZPrb)){EM_SetErr(eiWRK_ZPrbAlarm); isOk = false; }
    if (MT_GetAlarm   (miSRT_TSrt)){EM_SetErr(eiSRT_TSrtAlarm); isOk = false; }
...
bool __fastcall CSequence::InspectHomeEnd()
{
    ...
    if (!MT_GetHomeEnd   (miWRK_ZPrb)){EM_SetErr(eiWRK_ZPrbHomeEnd); isOk = false; }
