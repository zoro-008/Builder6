...
double CSort::GetMotrPos(EN_MOTR_ID _iMotr , EN_PSTN_ID _iPstnId )
{
    ...
    //double dPosBin1 = PM.GetValue (_iMotr , pvSRT_TSrtBin1 ) + 0 * PM.GetValue (_iMotr , pvSRT_TSrtBin1 ) ;
    //double dPosBin2 = PM.GetValue (_iMotr , pvSRT_TSrtBin1 ) + 1 * PM.GetValue (_iMotr , pvSRT_TSrtBin2 ) ;
    //double dPosBin3 = PM.GetValue (_iMotr , pvSRT_TSrtBin1 ) + 2 * PM.GetValue (_iMotr , pvSRT_TSrtBin3 ) ;
    //double dPosBin4 = PM.GetValue (_iMotr , pvSRT_TSrtBin1 ) + 3 * PM.GetValue (_iMotr , pvSRT_TSrtBin4 ) ;
    //double dPosBin5 = PM.GetValue (_iMotr , pvSRT_TSrtBin1 ) + 4 * PM.GetValue (_iMotr , pvSRT_TSrtBin5 ) ;

    if(_iMotr == miSRT_TSrt){
        switch(_iPstnId) {
            default             : dPos = MT_GetCmdPos(_iMotr                  ); break ;
            case piSRT_TSrtWait : dPos = PM.GetValue (_iMotr , pvSRT_TSrtWait ); break ;
            case piSRT_TSrtBin1 : dPos = PM.GetValue (_iMotr , pvSRT_TSrtBin1 ); break ;
            case piSRT_TSrtBin2 : dPos = PM.GetValue (_iMotr , pvSRT_TSrtBin2 ); break ;
            case piSRT_TSrtBin3 : dPos = PM.GetValue (_iMotr , pvSRT_TSrtBin3 ); break ;
            case piSRT_TSrtBin4 : dPos = PM.GetValue (_iMotr , pvSRT_TSrtBin4 ); break ;
            case piSRT_TSrtBin5 : dPos = PM.GetValue (_iMotr , pvSRT_TSrtBin5 ); break ;
        }                                                                   
    }
...
//                  }
        ...
        case  14: if(m_tmTemp.OnDelay(true , 4000)){//OM.DevOptn.iSortDeley )){
                  ...
                  if(!IO_GetXDn(xSRT_PkgDetect) && !IO_GetXUp(xSRT_PkgDetect) && !IO_GetX(xSRT_PkgDetect)) return false ;
                  //if(!IO_GetXDn(xSRT_PkgDetect) && !IO_GetXUp(xSRT_PkgDetect) && !IO_GetX(xSRT_PkgDetect))
                  ...
                  //if(m_iCrntBin == m_iTempBin){
                  //    m_iTempBin = m_iCrntBin;
                  //    Step.iCycle = 0;
                  //    return true;
                  //}
                  //else {
        ...
        case 15:  if(!m_tmTemp.OnDelay(true, OM.DevOptn.iSortDeley)) return false; // 분류 시  모터 바로 움직이면 수량 달라짐 딜레이 추가...JH
                  IO_SetY(ySRT_AirBlower,false);
                  bSortEnd = true ;
                  Step.iCycle = 0 ;
                  return true ;
