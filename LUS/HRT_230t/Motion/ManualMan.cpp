...
//***********************************************************************************************
    ...
    else if(m_iManNo == mcFED_Home                 ){FED.InitHomeStep ();}
    ...
    else if(m_iManNo == mcRalEndBlow               ){IO_SetY(yRAL_Blower,!IO_GetY(yRAL_Blower));}
...
void __fastcall CManualMan::Update(void)
{
    ...
    else if(m_iManNo == mcFED_Home                 ){if(FED.CycleHome        ()) m_iManNo = mcNoneCycle; }
    ...
    else if(m_iManNo == mcRAL_CycleWorkIdx         ){if(RAL.CycleWorkFist    ()) m_iManNo = mcNoneCycle; }
...
bool CManualMan::CycleMoveBin3()
{
    ...
    switch (m_iManStep) {
        ...
        case  13: if(!MT_GoAbsMan(miRAL_XIdx , PM.GetValue(miRAL_XIdx , pvRAL_XIdxWait ))) return false;
                  if(!AT_MoveCyl(aiRAL_IdxUpDn, ccFwd)) return false;
                  AT_MoveCyl(aiRAL_IdxUpDn, ccBwd);
                  m_iManStep++;
                  return false ;

        case  14: if(!AT_MoveCyl(aiRAL_IdxUpDn, ccBwd)) return false;
//                  if(!IO_GetX(xWRK_PkgExist)) SRT.SetBin(3);
                  MT_GoAbsMan(miRAL_XIdx , PM.GetValue(miRAL_XIdx , pvRAL_XIdxOut ));
                  m_iManStep++;
                  return false ;

        case  15: if(!MT_GoAbsMan(miRAL_XIdx , PM.GetValue(miRAL_XIdx , pvRAL_XIdxOut ))) return false;
                  MT_GoAbsMan(miRAL_XIdx , PM.GetValue(miRAL_XIdx , pvRAL_XIdxWait ));
                  AT_MoveCyl(aiRAL_IdxUpDn, ccFwd);
                  m_iManStep++;
                  return false ;
        ...
        case  16: if(!MT_GoAbsMan(miRAL_XIdx , PM.GetValue(miRAL_XIdx , pvRAL_XIdxWait ))) return false;
                  if(!AT_MoveCyl(aiRAL_IdxUpDn, ccFwd)) return false;
                  if( IO_GetX(xWRK_Pkg)) DM.ARAY[riWRK].SetStat(csUnkwn);
