...
bool CWork::Autorun(void) //오토런닝시에 계속 타는 함수.
{
    ...
    if (Step.iSeq == 0) {
        ...
        //if(IO_GetX(xWRK_PkgExist) && DM.ARAY[riWRK].CheckAllStat(csNone)){
        ...
        //주석 제거 후에 실행. 확인후 안될시 다시 주석...JS
        //모르는 스트립에러.
        //if( DM.ARAY[riWRK ].CheckAllStat(csNone) &&  IO_GetX(xWRK_PkgExist)) EM_SetErr(eiWRK_PkgUnknown ) ;

        //스트립 사라짐 에러.
        if(isCycleWork){
            if(!DM.ARAY[riWRK ].CheckAllStat(csNone) && !IO_GetX(xWRK_Pkg)) EM_SetErr(eiWRK_PkgDispr   ) ;
        }
...
//Check Cycle Time Out.
    ...
    switch (Step.iCycle) {
        ...
        case  11:
                  if(!IO_GetX(xWRK_Pkg)) {
//                      if(m_tmTemp.OnDelay(true, OM.DevOptn.iBin3Deley)) return false; // TODO ::??
...
//                      LT.AddLotInfoWorkChip(1);
                  ...
                  if(m_iPinCnt >= OM.DevOptn.iPinChangeCnt) {
