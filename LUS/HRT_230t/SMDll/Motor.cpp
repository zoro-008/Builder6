...
bool CMotor::GoAbsAcDc(double _dPos , double _dVel , double _dAcc , double _dDcc)
{
    ...
    //Check Motor Limit.
    if (_dPos > Para.dMaxPos) { Stop(); Trace(Para.sMotorName.c_str() , "GoAbs Para.dMaxPos return "); return false; }
    if (_dPos < Para.dMinPos) { Stop(); Trace(Para.sMotorName.c_str() , "GoAbs Para.dMinPos return "); return false; }
...
bool CMotor::FindEdgeStop (double _dVel ,double _dAcc ,CAjinMotion::EN_FINDEDGE_TYPE _ftFindEdgeType)
{
    ...
    else                 return AjinMotion -> FindEdgeStop(_dVel                 , _dAcc                 , _ftFindEdgeType);
}
...
bool CMotor::FindEdgeEmgStop(double _dVel ,double _dAcc ,CAjinMotion::EN_FINDEDGE_TYPE _ftFindEdgeType)
{
    ...
    else                 return AjinMotion -> FindEdgeEmgStop(_dVel                 ,_dAcc                 , _ftFindEdgeType);
}
...
//  UserINI.Load(sPath, "Para   "  , "bHomeWithNSnsr" , Para   .bHomeWithNSnsr );
  ...
  //UserINI.Load(sPath, "UsrPara"  , "dAccTime      " , UsrPara.dAccTime       );
    ...
    UserINI.Load(sPath, "UsrPara"  , "dAcceleration " , UsrPara.dAcceleration  );
    if(UsrPara.dAcceleration == 0.0)UserINI.Load(sPath, "UsrPara"  , "dAcceleration" , UsrPara.dAcceleration  ); //Update.... exptional
    if(UsrPara.dDeceleration == 0.0)UserINI.Load(sPath, "UsrPara"  , "dDeceleration" , UsrPara.dDeceleration  );
...
bool CMotor::CycleHome()
{
    ...
    switch (HomeStat.iHomeStep) {
        ...
        case 15 :
            if(!GetStop()) return false ;
            if(bUseZphase) FindEdgeEmgStop(-1 * dLastVel , dAcc  , ftZPhsUpEdge);
            else           FindEdgeEmgStop(     dLastVel , dAcc  , ftSnsrDnEdge);
