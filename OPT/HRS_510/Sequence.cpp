...
//    bool bResetSw = IO_GetXUp(xETC_LResetSw) || IO_GetXUp(xETC_RResetSw) || IO_GetXUp(xETC_ResetSw) || m_bBtnReset ;
    ...
    IO_SetY(yETC_ResetLp  , IO_GetX(xETC_ResetSw ) || (m_bFlick && isErr));
    //IO_SetY(yETC_AirLp    , IO_GetX(xETC_AirSw   ) || IO_GetY(yETC_MainSol) || IO_GetY(yETC_VacuSol));
    //IO_SetY(yETC_AirLp    , IO_GetX(xETC_AirSw   ) || IO_GetY(yETC_MainSol) || IO_GetY(yETC_VacuSol));
...
//        if(!IO_GetX(xETC_PostEqpReady))EM_SetErr(eiETC_PostEqpNotReady);
        if(!IO_GetX(xETC_PowerOn )) {FM_MsgOk("ERR","Power On Plz"); return ; }
    }
...
bool __fastcall CSequence::InspectEmergency(void)
{
    ...
    //Check Emergency
    if (IO_GetX(xETC_EmgSw1) || IO_GetX(xETC_EmgSw2)) {
...
bool __fastcall CSequence::InspectActuator(void)
{
    ...
    //Inspect.
    if(AT_Err(aiLDR_SptrFB )) {EM_SetErr(eiLDR_SptrFBTO ) ; isOk = false ; }
    if(AT_Err(aiLDR_SplyFB )) {EM_SetErr(eiLDR_SplyFBTO ) ; isOk = false ; }
    if(AT_Err(aiRAL_IndxUD )) {EM_SetErr(eiRAL_IndxUDTO ) ; isOk = false ; }
    if(AT_Err(aiPCK_PckrDU )) {EM_SetErr(eiPCK_PckrDUTO ) ; isOk = false ; }
    if(AT_Err(aiSTN_GuidFB )) {EM_SetErr(eiSTN_GuidFBTO ) ; isOk = false ; }
    if(AT_Err(aiSRT_Grip1FB)) {EM_SetErr(eiSRT_Grip1FBTO) ; isOk = false ; }
    if(AT_Err(aiSRT_Grip2FB)) {EM_SetErr(eiSRT_Grip2FBTO) ; isOk = false ; }
    if(AT_Err(aiSRT_Grip3FB)) {EM_SetErr(eiSRT_Grip3FBTO) ; isOk = false ; }
    if(AT_Err(aiSRT_FlipFB )) {EM_SetErr(eiSRT_FlipFBTO ) ; isOk = false ; }
    if(AT_Err(aiULD_StckUD )) {EM_SetErr(eiULD_StckUDTO ) ; isOk = false ; }
