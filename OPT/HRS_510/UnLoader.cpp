...
bool CUnLoader::CycleHome()
{
    ...
    switch (Step.iHome) {
        ...
        case 10: MoveActr(aiULD_StckUD  ,ccBwd);
                 IO_SetY(yRAL_FeedingMt,false);
                 Step.iHome++;
                 return false ;

        case 11: if(!MoveActr(aiULD_StckUD  ,ccBwd)) return false ;
                 Step.iHome = 0;
                 return true ;
...
bool CUnLoader::Autorun(void) //오토런닝시에 계속 타는 함수.
{
    ...
    if (Step.iSeq == 0) {
        ...
        bool isCycleIn    =!DM.ARAY[riWKE].CheckAllStat(csNone) && IO_GetX(xRAL_Detect4);
...
//Check Cycle Time Out.
    ...
    switch (Step.iCycle) {
        ...
        case  11: if(!m_tmTemp.OnDelay(true , 200)) return false ;
                  IO_SetY(yRAL_FeedingMt , false);
                  MoveActr(aiULD_StckUD , ccFwd);
                  Step.iCycle++;
                  return false;

        case  12: if(!MoveActr(aiULD_StckUD , ccFwd))return false ;
                  MoveActr(aiULD_StckUD , ccBwd);
                  Step.iCycle++;
                  return false ;

        case  13: if(!MoveActr(aiULD_StckUD , ccBwd)) return false ;
                  //Lot Day Info Add
                  LT.AddDayInfoWorkStrp(1);
                  DM.ARAY[riPSB].SetStat(csNone);

                  if(IO_GetX(xULD_TrayFull)) EM_SetErr(eiULD_TrayFull);
                  Step.iCycle = 0 ;
                  return true ;
    }
}
...
{
    if(!AT_Done(aiULD_StckUD )) return false ;
