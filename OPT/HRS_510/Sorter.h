...
class CSorter
{
    ...
    protected:
        ...
        double GetMotrPos (EN_MOTR_ID _iMotr , EN_PSTN_ID _iPstnId , int _iPickerId = -1); //모터가 가야할 포지션을 리턴함. Double형.

        bool   GetLastStrip(); //아직 파트에 어레이 데이터가 존재 할때만 유용. 즉 보내기전에 체크해야함.

    public:    /* Direct Accessable Var.  */
        void ClearBinCnt();
        int  m_iBinCnt[MAX_BIN_CNT] ;
	    ...
	    //Motor.
	    bool MoveMotr (EN_MOTR_ID _iMotr , EN_PSTN_ID _iPstnId , int _iPickerId = -1); // 모터를 움직일때 쓰는 함수.
	    ...
	    //One Cycle.
        bool CyclePick   (void); //스트립을 PreBuff로 이송.
        bool CycleSort   (void); //스트립을 Work1으로 이송.
