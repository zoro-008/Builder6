...
double dRotorH[MAX_ARRAY_ROW][MAX_ARRAY_COL] ;
double g_dCrntZeroPoint ;
...
void __fastcall CSpcSub::WriteDataCsv()
{
    ...
    sPath = sPath + Now().FormatString("yyyy_mm") + "\\";
...
//    sTemp = "Rotor Height \r\n";
    ...
    for(int r = 0 ; r < OM.DevInfo.iRowCnt ; r++) {
        for(int c = 0 ; c < OM.DevInfo.iColCnt ; c++) {
            sTemp += sTemp1.sprintf("%.3f",dRotorH[r][c]) + "," ;
        }
        sTemp += "\r\n" ;
    }
    FileSeek (hFile , 0             , SEEK_END     );
...
{
    UserFile.ClearDirDate(SPC_FOLDER , Now() - 90);
}

void CSpcSub::LoadSave(bool _bLoad)
{
        //Local Var.
    TUserINI   UserINI;
    AnsiString sPath  ;
    ...
    //Set Dir.
    sPath = EXE_FOLDER + "SeqData\\dRotorH.INI";
    ...
    if(_bLoad) {
        for( int r = 0 ; r < OM.DevInfo.iRowCnt ; r++) {
            for(int c = 0 ; c < OM.DevInfo.iColCnt ; c++) {
                sTemp = "dRotorH"+AnsiString(r)+"_"+AnsiString(c);
                UserINI.Load(sPath.c_str()  , "CSpcSub"  , sTemp , dRotorH[r][c]);
            }
        }
        UserINI.Load(sPath.c_str()  , "CSpcSub" , "g_dCrntZeroPoint" , g_dCrntZeroPoint);
    }
        ...
        for( int r = 0 ; r < OM.DevInfo.iRowCnt ; r++) {
            for(int c = 0 ; c < OM.DevInfo.iColCnt ; c++) {
                sTemp = "dRotorH"+AnsiString(r)+"_"+AnsiString(c);
                UserINI.Save(sPath.c_str()  , "CSpcSub" , sTemp , dRotorH[r][c]);
            }
        }
        UserINI.Save(sPath.c_str()  , "CSpcSub" , "g_dCrntZeroPoint" , g_dCrntZeroPoint);
    }
