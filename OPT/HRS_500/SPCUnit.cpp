...
void __fastcall CSpc::WriteHeightData (double _dData)
{
    //Local Var.
    int        hFile   ;
    ...
    Path         = SPC_HEIGHT_FOLDER ;

    UserFile.ClearDirDate(Path , Now() - 90);

    if(!UserFile.FileExist(Path)) CreateDir(Path);

    FilePath = Path + Temp + ".csv";

    //File Open.
    hFile = FileOpen(FilePath.c_str() , fmOpenWrite);
    if (hFile == -1) {
        hFile = FileCreate(FilePath.c_str());
        if (hFile == -1) return;
        Temp  = "Time,"  ;
        Temp += "Height" ;
        Temp += "\r\n"   ;
    ...
    //Save.
    Temp  = CurrDateTime.FormatString("hh:nn:ss") + ","   ;
    Temp += (AnsiString)_dData                    + "\r\n";
...
void __fastcall CSpc::WriteLeftStatorData (double _dData)
{
    //Local Var.
    int        hFile   ;
    AnsiString Path , FilePath   ;
...
void __fastcall CSpc::WriteRightStatorData (double _dData)
{
    //Local Var.
    int        hFile   ;
    AnsiString Path , FilePath   ;
