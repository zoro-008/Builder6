...
bool CMotor::JogP()
{
    ...
    return AjinMotion -> JogP(PULSE_PER_UNIT * dVel , dAcc);
}
...
bool CMotor::JogN()
{
    ...
    return AjinMotion -> JogN(PULSE_PER_UNIT * dVel , dAcc);
}
...
bool CMotor::GoHomeOffset (double _dPos , double _dVel)
{
    ...
    if (GetStop()) {
        ...
        iReceive = AjinMotion -> GoAbs(_dPos * PULSE_PER_UNIT , _dVel * PULSE_PER_UNIT , UsrPara.dAccTime );
    }
...
{
    return AjinMotion -> FindEdgeStop(_dVel * PULSE_PER_UNIT, _dAcc , _ftFindEdgeType);
}

bool CMotor::FindEdgeEmgStop(double _dVel ,double _dAcc ,CAjinMotion::EN_FINDEDGE_TYPE _ftFindEdgeType)
{
    return AjinMotion -> FindEdgeEmgStop(_dVel * PULSE_PER_UNIT,_dAcc, _ftFindEdgeType);
}
