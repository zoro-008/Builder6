...
//  CFS20set_inposition_enable (AjinPara.iPhysicalNo, AjinPara.bInposEnable  ); // 인포지션(Inposition) 신호 사용유무 설정
    ...
    //이상하게 무용지물.
    //AxmSignalSetZphaseLevel (AjinPara.iPhysicalNo , LOW                           ); //Z상 접점.
    ...
    if(AjinPara.dSCurveAcPer == 0 && AjinPara.dSCurveDcPer == 0 ) {
        AxmMotSetProfileMode (AjinPara.iPhysicalNo, SYM_TRAPEZOIDE_MODE );
    }
...
bool CAjinMotion::JogP(double _dVel , double _dAcc)
{
#ifdef AJIN_AXT
    return CFS20v_move_ex(AjinPara.iPhysicalNo , _dVel , _dAcc);
#endif //AJIN_AXT
#ifdef AJIN_AXL
    bool Rslt = false;
	DWORD	dwRet;
	dwRet	= AxmMoveVel(AjinPara.iPhysicalNo,  _dVel , _dAcc, _dAcc);
...
bool CAjinMotion::JogN(double _dVel , double _dAcc)
{
#ifdef AJIN_AXT
    return CFS20v_move_ex(AjinPara.iPhysicalNo , -1 * _dVel , _dAcc);
#endif //AJIN_AXT
#ifdef AJIN_AXL
    bool Rslt = false;
	DWORD	dwRet;
	dwRet	= AxmMoveVel(AjinPara.iPhysicalNo,  -1 * _dVel , _dAcc, _dAcc);
	Rslt	= (BOOL)dwRet == AXT_RT_SUCCESS ? TRUE : FALSE;
...
bool CAjinMotion::GoAbs(double _dPos , double _dVel , double _dAcc)
{
#ifdef AJIN_AXT
    //Go Motor.
    if (GetStop()) {
        if(AjinPara.bUseSCurve) CFS20start_s_move_ex(AjinPara.iPhysicalNo , _dPos , _dVel , _dAcc );
        else                    CFS20start_move_ex  (AjinPara.iPhysicalNo , _dPos , _dVel , _dAcc );
    }
    ...
    //Go Motor.
    if (GetStop()) {
	    AxmMoveStartPos(AjinPara.iPhysicalNo, _dPos , _dVel , _dAcc , _dAcc );
    }
...
bool CAjinMotion::GoOvr(double _dPos)
{
#ifdef AJIN_AXT
    //Go Motor.
    CFS20position_override(AjinPara.iPhysicalNo , _dPos ); //감속 프로파일은 전에 무브 동작것을 자동적으로 따라감.
...
#ifdef AJIN_AXL
    //Go Motor.
    AxmOverridePos(AjinPara.iPhysicalNo , _dPos ); //감속 프로파일은 전에 무브 동작것을 자동적으로 따라감.
...
bool CAjinMotion::FindEdgeStop (double _dVel ,double _dAcc ,EN_FINDEDGE_TYPE _ftFindEdgeType)
{
    ...
    return CFS20start_signal_search1_ex(AjinPara.iPhysicalNo, _dVel , _dAcc , ftFindEdgeType);
#endif //AJIN_AXT
#ifdef AJIN_AXL
    ...
    */
    ...
    return (AXT_RT_SUCCESS ==AxmMoveSignalSearch(AjinPara.iPhysicalNo, _dVel, _dAcc, iSignal, iEdge , SLOWDOWN_STOP));
...
bool CAjinMotion::FindEdgeEmgStop(double _dVel ,double _dAcc ,EN_FINDEDGE_TYPE _ftFindEdgeType)
{
    ...
    // 원점 센서 검출 시작 : 사다리꼴 구동, 가속 시간 모드 사용, 감속 정지 방식
    return (AXT_RT_SUCCESS == AxmMoveSignalSearch(AjinPara.iPhysicalNo, _dVel, _dAcc, iSignal, iEdge , EMERGENCY_STOP)) ;
#endif //AJIN_AXL
