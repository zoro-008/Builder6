...
void COptionMan::LoadDevOptn(AnsiString _sDevName)
{
    ...
    for(int i=0;i<MAX_BIN_CNT;i++) {
        ...
        sTemp = "dMinBnS" + (String)i ;
        UserINI.Load(sPath.c_str()  , "DevOptn"  , sTemp , DevOptn.dMinBnS[i] );
        sTemp = "dMaxBnS" + (String)i ;
        UserINI.Load(sPath.c_str()  , "DevOptn"  , sTemp , DevOptn.dMaxBnS[i] );


        sTemp = "iBnLSel" + (String)i ;
        UserINI.Load(sPath.c_str()  , "DevOptn"  , sTemp , DevOptn.iBnLSel[i] );
        sTemp = "iBnLIdx" + (String)i ;
        UserINI.Load(sPath.c_str()  , "DevOptn"  , sTemp , DevOptn.iBnLIdx[i] );

        sTemp = "iBnRSel" + (String)i ;
        UserINI.Load(sPath.c_str()  , "DevOptn"  , sTemp , DevOptn.iBnRSel[i] );
        sTemp = "iBnRIdx" + (String)i ;
        UserINI.Load(sPath.c_str()  , "DevOptn"  , sTemp , DevOptn.iBnRIdx[i] );

    }

    UserINI.Load(sPath.c_str()  , "DevOptn"  , "iBinMaxCnt" , DevOptn.iBinMaxCnt);
...
void COptionMan::SaveDevOptn(AnsiString _sDevName)
{
    ...
    for(int i=0;i<MAX_BIN_CNT;i++) {
        ...
        sTemp = "dMinBnS" + (String)i ;
        UserINI.Save(sPath.c_str()  , "DevOptn"  , sTemp , DevOptn.dMinBnS[i] );
        sTemp = "dMaxBnS" + (String)i ;
        UserINI.Save(sPath.c_str()  , "DevOptn"  , sTemp , DevOptn.dMaxBnS[i] );


        sTemp = "iBnLSel" + (String)i ;
        UserINI.Save(sPath.c_str()  , "DevOptn"  , sTemp , DevOptn.iBnLSel[i] );
        sTemp = "iBnLIdx" + (String)i ;
        UserINI.Save(sPath.c_str()  , "DevOptn"  , sTemp , DevOptn.iBnLIdx[i] );

        sTemp = "iBnRSel" + (String)i ;
        UserINI.Save(sPath.c_str()  , "DevOptn"  , sTemp , DevOptn.iBnRSel[i] );
        sTemp = "iBnRIdx" + (String)i ;
        UserINI.Save(sPath.c_str()  , "DevOptn"  , sTemp , DevOptn.iBnRIdx[i] );

    }

    UserINI.Save(sPath.c_str()  , "DevOptn"  , "iBinMaxCnt" , DevOptn.iBinMaxCnt);
}
...
void COptionMan::LoadCmnOptn()
{
    ...
    UserINI.Load(sPath.c_str()  , "CmnOptn"  , "iRezeroPickCnt" , CmnOptn.iRezeroPickCnt);
    UserINI.Load(sPath.c_str()  , "CmnOptn"  , "dMasterOffset " , CmnOptn.dMasterOffset );
...
void COptionMan::SaveCmnOptn()
{
    ...
    UserINI.Save(sPath.c_str(),"CmnOptn","iRezeroPickCnt" , CmnOptn.iRezeroPickCnt);
    UserINI.Save(sPath.c_str(),"CmnOptn","dMasterOffset " , CmnOptn.dMasterOffset );
