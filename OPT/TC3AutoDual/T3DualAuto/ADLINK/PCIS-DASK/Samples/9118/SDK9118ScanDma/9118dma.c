         ...
         switch(LOWORD(wParam)) {
           ...
           case IDC_CHAN:
             ...
             case IDC_START:
                  ...
                  err=AI_ContScanChannels (card, channel, range, ai_buf, data_size, (F32)sample_rate, SYNCH_OP);
                  QueryPerformanceCounter(&current_count);
                  take_msec =((double)(current_count.QuadPart-start_count.QuadPart))/freq.QuadPart*1000;
                  sprintf(buf, "The time taken for DMA operation is: %4.3f msec",take_msec);
                  MessageBox(hWnd,buf,"DMA time",MB_OK);
