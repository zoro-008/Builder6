...
int nCmdShow;
{
  ...
  hWnd = CreateWindow("GenericWClass",
                      (LPCSTR)"PCI-9118 Single Buffer DMA with About Trigger Mode",
                      WS_OVERLAPPEDWINDOW,
                      CW_USEDEFAULT,
                      CW_USEDEFAULT,
                      520,
    ...
    case WM_COMMAND:
        switch(LOWORD(wParam)) {
           ...
           case IDC_START:
                ...
                if (data_size <= postCount) {
                   wsprintf(s, "Buffer Size have to be larger than %d\n", postCount+2);
                ...
                fstop =0;
				if(midcheck) {
                 if ((err=AI_9118_Config(card,P9118_AI_BiPolar|P9118_AI_SingEnded,P9118_AI_DtrgPositive|P9118_AI_EtrgPositive|P9118_AI_AboutTrgEn,0,postCount))!= NoError) {
                 ...
                 if ((err=AI_9118_Config(card,P9118_AI_BiPolar|P9118_AI_SingEnded, 0, 0, postCount))!= NoError) {
                      wsprintf(s, "AI_Config Error = %d\n", err);
                      MessageBox(hWnd, s, "Error", MB_OK);
                      SendMessage(hWnd, WM_CLOSE, 0, 0L);
                      break;
