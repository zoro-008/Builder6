...
HANDLE  hInst;
HWND hAddrComboBox, hCardComboBox;
HWND hADComboBox1;
HWND hADComboBox2;
HWND hADComboBox3;
HWND hADComboBox4;
...
LPSTR AD_Range[2][8]={{"+/-5V","+/-2.5V","+/-1.25V","+/-0.625V","0-10V","0-5V","0-2.5V","0-1.25V"},
                      {"+/-5V","+/-0.5V","+/-0.05V","+/-0.005V","0-10V","0-1V","0-0.1V","0-0.01V"}
                     } ;
I16 range_value[2][8]={{AD_B_5_V, AD_B_2_5_V, AD_B_1_25_V, AD_B_0_625_V, AD_U_10_V, AD_U_5_V, AD_U_2_5_V, AD_U_1_25_V},
                       {AD_B_5_V, AD_B_0_5_V, AD_B_0_05_V, AD_B_0_005_V, AD_U_10_V, AD_U_1_V, AD_U_0_1_V, AD_U_0_01_V}
                      };

I16 Card_Type=PCI_9118DG;
I16 card_value[3] = {PCI_9118DG, PCI_9118HR, PCI_9118HG};
LPSTR Card_Name[3]={"PCI9118DG", "PCI9118HR", "PCI9118HG"};
...
int Input_Mode=P9118_AI_SingEnded;
int Input_Signal=P9118_AI_BiPolar;
...
BOOL PASCAL setup(HWND hWnd, unsigned message, WPARAM wParam, LPARAM lParam)
{
  ...
  char s[50];
      switch (message) {
         case WM_INITDIALOG:
              for (i=0; i<3; i++)
                SendMessage(GetDlgItem(hWnd,IDC_COMBO1), CB_ADDSTRING, NULL, (LONG)(LPSTR) Card_Name[i]);
              for (i=0;i<16;i++) {
                wsprintf(n, "%d", i);
                _strupr(n);
                SendMessage(GetDlgItem(hWnd,IDC_COMBO4), CB_ADDSTRING, NULL, (LONG)(LPSTR) n);
              }
              SendMessage(GetDlgItem(hWnd,IDC_COMBO2), CB_ADDSTRING, NULL, (LONG)(LPSTR)"Bipolar");
              SendMessage(GetDlgItem(hWnd,IDC_COMBO2), CB_ADDSTRING, NULL, (LONG)(LPSTR)"Unipolar");
              ...
              SendMessage(GetDlgItem(hWnd,IDC_COMBO3) , CB_SETCURSEL, 0, 0L);
              ...
              switch(LOWORD(wParam)) {
                ...
                case IDC_COMBO2:
                     if (HIWORD(wParam) == CBN_CLOSEUP) {
                        ...
                        if (i == 0) Input_Signal = P9118_AI_BiPolar;
                        else Input_Signal = P9118_AI_UniPolar;
                     }
                     return(TRUE);
                case IDC_COMBO3:
                     if (HIWORD(wParam) == CBN_CLOSEUP) {
                        i = (int)SendMessage(GetDlgItem(hWnd,IDC_COMBO3), CB_GETCURSEL, 0, 0L);
                        SendMessage(GetDlgItem(hWnd,IDC_COMBO3), CB_GETLBTEXT, i, (LPARAM)((LPCSTR)n));
                        if (i == 0) Input_Mode = P9118_AI_SingEnded;
                        else Input_Mode = P9118_AI_Differential;
                     }
                     return(TRUE);
                case IDC_COMBO4:
                     if (HIWORD(wParam) == CBN_CLOSEUP) {
                ...
                case IDOK:
                     if (card<0) {
                      if ((card=Register_Card(PCI_9118DG, card_number)) < 0) {
                     ...
                     SendMessage(hAddrComboBox, CB_RESETCONTENT, 0L, 0L);
                     for (i=0;i<8;i++) {
                         SendMessage(hAddrComboBox, CB_ADDSTRING, NULL, (LONG)AD_Range[(Card_Type==PCI_9118HG)?1:0][i]);
                     }
...
LPARAM lParam;
{
  ...
  switch(message) {
    case WM_CREATE:
           ...
           for (i=0;i<8;i++) {
              SendMessage(hAddrComboBox, CB_ADDSTRING, NULL, (LONG)AD_Range[0][i]);
           }
           SendMessage(hAddrComboBox ,CB_SETCURSEL, 0, 0L);
    ...
    case WM_PAINT:
           ...
           //TextOut(hDC,output_pos[13].x+90, output_pos[15].y+56," PCI-9118 ",9);

           SetBkMode(hDC,TRANSPARENT);
                   TextOut(hDC,output_pos[8].x+20, io_pos[15].y+182,"-10    -6     -2    2     6    10",33);
           //TextOut(hDC,output_pos[8].x+20, io_pos[15].y+182,"0     1     2     3     4     5",31);
           ...
           switch (range) {
             ...
             case AD_B_0_05_V:
                       ...
                       break;
             case AD_B_0_005_V:
                       TextOut(hDC,input_pos[15].x-5, io_pos[15].y-5,"0.005",5);
                       TextOut(hDC,input_pos[15].x-8, io_pos[15].y+67,"-0.005",6);
                       TextOut(hDC,input_pos[15].x-5, io_pos[15].y+87,"0.005",5);
                       TextOut(hDC,input_pos[15].x-8, io_pos[15].y+159,"-0.005",6);
                       break;
             case AD_U_10_V:
    ...
    case WM_COMMAND:
        switch(LOWORD(wParam)) {
           ...
           case IDC_GAIN:
                if (HIWORD(wParam) == CBN_CLOSEUP) {
                    if ((i=(int)SendMessage(GetDlgItem(hWnd,IDC_GAIN),CB_GETCURSEL, 0, 0L))>=0 ) {
                       ...
                       range = range_value[(Card_Type==PCI_9118HG)?1:0][i];
           ...
           case IDC_COMBO1:
                if (HIWORD(wParam) == CBN_CLOSEUP) {
                    if ((i=(int)SendMessage(GetDlgItem(hWnd,IDC_COMBO1),CB_GETCURSEL, 0, 0L))>=0 ) {
                       SendMessage(GetDlgItem(hWnd,IDC_COMBO1), CB_GETLBTEXT, i, (LPARAM)((LPCSTR)n));
                       Card_Type = card_value[i];
                       InvalidateRect(hWnd, NULL, FALSE);
                    }
        ...
        if (card >= 0) {
           ...
           for (i=0;i<8;i++ ) {
              if (bCheck1[i]) {
				  ...
				  if(Card_Type==PCI_9118HR) {
					 analog_input[i] = (I16)(analog_input[i] & 0xffff);
                     adinput[i]=io_pos[0].y+42-(43*(I16)analog_input[i])/32767;
                  }
				  else {
                     analog_input[i] = (analog_input[i] >> 4) & 0x0fff;
                     adinput[i]=io_pos[0].y+83-(85*(unsigned long)analog_input[i])/4095;
				  }
                  if (old_adinput1[i]!=-1 ) {
           ...
           for (i=0;i<8;i++ ) {
              if (bCheck2[i]) {
        		  ...
        		  if(Card_Type==PCI_9118HR) {
					 analog_input[i+8] = (I16)(analog_input[i+8] & 0xffff);
                     adinput[i+8]=io_pos[0].y+125-(43*(I16)analog_input[i+8])/32767;
                  }
				  else {
                     analog_input[i+8] = (analog_input[i+8]>>4) & 0x0fff;
                     adinput[i+8]=io_pos[0].y+172-(85*(unsigned long)analog_input[i+8])/4095;
				  }
                  if (old_adinput2[i]!=-1 ) {
