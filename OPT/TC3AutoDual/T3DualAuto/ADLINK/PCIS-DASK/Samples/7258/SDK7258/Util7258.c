/*;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;  Copyright (c) 1997-2001, ADLink Technology Inc.  All rights reserved.  ;;
;;                                                                         ;;
;;      File Name   :   Util7258.C                                         ;;
...
HANDLE  hInst;
U16 input=0, output=0, output2=0;
I16 card=-1, card_number=0;
U16 oldinput=0 , rb_output;
...
int nCmdShow;
{
  ...
  hInst = hInstance;
  hSampleMenu = LoadMenu(hInstance, "U7258Menu");
         ...
         case WM_COMMAND:
              switch(LOWORD(wParam)) {
                ...
                case IDC_COMBO1:
                     ...
                     return(TRUE);
                case IDOK:
                     if ((card=Register_Card(PCI_7258, card_number)) < 0) {
...
LPARAM lParam;
{
  ...
  switch(message) {
    ...
    case WM_PAINT:
        ...
        SetBkColor(hDC, RGB(0, 0, 255));
        TextOut(hDC, output_pos[15].x, output_pos[15].y-30, "Digital Output CH#0", 19);

        // Digital Output Box
        SelectObject(hDC, hGreenBrush);
        Rectangle(hDC, output_pos2[15].x-10, output_pos2[15].y-24, output_pos2[0].x+26, output_pos2[0].y+40);
        SetTextColor(hDC,RGB(255,0,0));
        SetBkColor(hDC, RGB(0, 0, 255));
        TextOut(hDC, output_pos2[15].x, output_pos2[15].y-30, "Digital Output CH#1", 19);

        // Channel Label
        SetTextColor(hDC,RGB(0,0,0));
        SetBkColor(hDC, RGB(0, 255, 0));
        TextOut(hDC, input_pos[15].x, input_pos[15].y+16, "                                                                       1   0", 76);
        SetBkColor(hDC, RGB(0, 128, 0));
        TextOut(hDC, output_pos[15].x, output_pos[15].y+16, "15                                                                        0", 75);
        TextOut(hDC, output_pos2[15].x, output_pos2[15].y+16, "15                                                                        0", 75);
        SetBkColor(hDC, RGB(0, 255, 0));
        ...
        for (i=0; i<16; i++) {
          if ((output2>>i)&0x01)
             ...
             SelectObject(hDC, GetStockObject(WHITE_BRUSH));
          Ellipse(hDC, output_pos2[i].x, output_pos2[i].y, output_pos2[i].x+17, output_pos2[i].y+17);
        }
        EndPaint(hWnd, &ps);
        break;
    case WM_COMMAND:
    ...
    case WM_LBUTTONDOWN:
        ...
        for (i=0; i<16; i++) {
          if (px>output_pos[i].x && px<output_pos[i].x+17 && py>output_pos[i].y && py<output_pos[i].y+17) {
			 ...
			 //...Sleep(10);
			 //..DO_ReadPort(card,0, &rb_output);
			 //..wsprintf(s, "ReadBack = %x\n", rb_output);
             //..MessageBox(hWnd, s, "Error", MB_OK);
             break;
          }
          if (px>output_pos2[i].x && px<output_pos2[i].x+17 && py>output_pos2[i].y && py<output_pos2[i].y+17) {
             hDC = GetDC(hWnd);
             if ((output2>>i)&0x01) {
                output2 -= (((U16)1L)<<i);
                SelectObject(hDC, GetStockObject(WHITE_BRUSH));
