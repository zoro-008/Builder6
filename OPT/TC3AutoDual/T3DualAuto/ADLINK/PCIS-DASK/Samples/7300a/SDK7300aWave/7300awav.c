...
LPARAM lParam;
{
  ...
  switch(message) {
    case WM_CREATE:
        ...
        DI_7300A_Config(card, 16, TRIG_CLK_10MHZ, P7300_WAIT_NO, P7300_TERM_ON, 1, 1, 1);
        DO_7300A_Config (card, 16, TRIG_INT_PACER, P7300_WAIT_NO, P7300_TERM_ON, 1);
    ...
    case WM_COMMAND:
        switch(LOWORD(wParam)) {
           ...
           case IDC_START:
                ...
                if (err < 0) {
                   wsprintf(s, "DO_ContWritePort Error = %d\n", err);
                   MessageBox(hWnd, s, "Error", MB_OK);
                   SendMessage(hWnd, WM_CLOSE, 0, 0L);
                   break;
                }
