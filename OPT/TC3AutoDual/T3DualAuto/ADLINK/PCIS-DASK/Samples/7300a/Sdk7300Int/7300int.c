...
int             nCmdShow;
{
    ...
    hWnd = CreateWindow("IntClass",
                        "PCI-7300 Timer2 Interrupt Samples",
                        WS_OVERLAPPEDWINDOW,
                        200,
                        100,
                        375,
...
LPARAM lParam;
{
    ...
    switch(message) {
        ...
        case WM_PAINT:
             ...
             wsprintf(Buf,"The interrupt signals are generated");
             TextOut(hDC1,20,10,Buf,strlen(Buf));
             wsprintf(Buf," by internal Timer2 Output.");
        ...
        case WM_COMMAND:
             switch(wParam) {
                ...
                case IDM_GO:
                  //  Interrupt Trigger
                  hDC=GetDC(hWnd);
				  DIO_7300SetInterrupt (card, 0, 1, hIntEvent);
				  CTR_Setup (card, 2, 2, c2, BIN);
                  k=0; i=0;
                  while (k<total_count) {
                    if (WaitForSingleObject(hIntEvent[1], 1000)== WAIT_OBJECT_0) {
                          wsprintf(Buf,"Count = %d ",k+1);
                          TextOut(hDC,20,120,Buf,strlen(Buf));
                          k++;
                    }
                    else {
                          wsprintf(s, "****** It is too long to wait Interrupt Event *****\n");
                          MessageBox(hWnd, s, "Err", MB_OK);
                          break;
                    }
                  }
                  DIO_SetDualInterrupt(card, 0, 0, hIntEvent);
