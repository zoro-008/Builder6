...
/* Date    : 2008/08/12                                                       */
/*                                                                            */
/* This sample performs continuous DO operation with delay trigger.           */
/*----------------------------------------------------------------------------*/
#include <windows.h>
#include <stdio.h>
#include <stdlib.h>
...
int main(int argc, char **argv)
{
    ...
    U16 TrigCtrl = P922x_DO_TRGMOD_DELAY|P922x_DO_TRGSRC_GPI0;
    U32 ReTrgCnt = 0; //Ignored if Retrigger mode dose not be enabled
    U32 DLY1Cnt = 40000000; //Delay interval; Delay Time = DLY1Cnt/TIMEBASE Sec.
    ...
    printf("This sample performs continuous DO with delay trigger at %6.2f update rate.\n", SampleRate);
    printf("Card Number? ");
    scanf(" %hd", &card_num);
    ...
    printf("\n\nDO is start, wait trigger from GPI0...\n");
    ...
    if(Stopped){
        ...
        getch();
        printf("\n\nDO Operation has been stopped manually... %d data had been written to DO port...\n", AccessCnt);
    }
