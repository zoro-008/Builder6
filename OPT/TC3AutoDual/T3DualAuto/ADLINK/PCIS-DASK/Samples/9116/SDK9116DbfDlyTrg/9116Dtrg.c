...
HANDLE  hInst;
HWND hListBox;
HWND hEdit,hEdit1, hStart, hStop, hData;
BOOL bCheck[8];
...
long  samp_intrv=240;
U16 channel;
U32 data_size=4096, mem_size;
static HANDLE hMem=NULL;
I16  *ai_buf;
I16 card=-1, card_number = 0, ai_mode, dl_mode;
U32 count;
BOOLEAN clear_op;
HANDLE hThread1;
LPSTR AI_MODE[2]={"DMA", "INT"};
LPSTR DL_MODE[2]={"In Timebase", "In Samples"};
...
int nCmdShow;
{
  ...
  hWnd = CreateWindow("GenericWClass",
                      (LPCSTR)"PCI-9116 Double Buffered DMA/INT with Delay Trigger Mode",
                      WS_OVERLAPPEDWINDOW,
                      CW_USEDEFAULT,
                      CW_USEDEFAULT,
                      520,
  ...
  char s[50];
      switch (message) {
         case WM_INITDIALOG:
              ...
              for (i=0;i<2;i++) {
                SendMessage(GetDlgItem(hWnd,IDC_COMBO3), CB_ADDSTRING, NULL, (LONG)(LPSTR) DL_MODE[i]);
              }
    ...
    case WM_COMMAND:
        switch(LOWORD(wParam)) {
           ...
           case IDC_START:
				...
				if(ai_mode && (data_size%1024)) {
                   MessageBox(hWnd,"Counts must be times of 1024 for DbfINT", "No Memory", MB_OK);
                   break;
                }
    			...
    			if(!ai_mode)				  
					AI_9116_Config(card,0,P9116_AI_DMA|P9116_TRGMOD_DELAY|(dl_mode?P9116_AI_DlyInSamples:P9116_AI_DlyInTimebase), 65535,0,0);
				else 
					AI_9116_Config(card,0,P9116_AI_INT|P9116_TRGMOD_DELAY|(dl_mode?P9116_AI_DlyInSamples:P9116_AI_DlyInTimebase), 65535,0,0);
				AI_9116_CounterInterval (card, samp_intrv, samp_intrv);
                ...
                err = AI_ContReadChannel (card, channel, AD_B_5_V, ai_buf, data_size, (F32)1, ASYNCH_OP);
