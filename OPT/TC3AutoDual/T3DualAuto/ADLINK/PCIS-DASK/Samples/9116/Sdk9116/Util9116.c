...
LPSTR AD_Range[8]={"+/-5V","+/-2.5V","+/-1.25V","+/-0.625V","0-10V","0-5V","0-2.5V","0-1.25V"};
I16 range_value[8] = {AD_B_5_V, AD_B_2_5_V, AD_B_1_25_V, AD_B_0_625_V, AD_U_10_V, AD_U_5_V, AD_U_2_5_V, AD_U_1_25_V};
...
LPARAM lParam;
{
  ...
  switch(message) {
    ...
    case WM_PAINT:
           ...
           TextOut(hDC, input_pos[15].x+243, input_pos[15].y+20,   "7                                                   0", 53);
           SetBkColor(hDC, RGB(0, 128, 0));
           TextOut(hDC, output_pos[15].x+243, output_pos[15].y+20, "7                                                   0", 53);
           SetBkColor(hDC, RGB(0, 255, 0));
           ...
           TextOut(hDC,output_pos[13].x+90, output_pos[15].y+56," PCI-9116 ",9);
    ...
    case WM_TIMER:
        if (card >= 0) {
           ...
           for (i=0;i<8;i++ ) {
              if (bCheck1[i]) {
				  if(range < 14)
					AI_9116_Config(card,P9116_AI_BiPolar,P9116_AI_SoftPolling,0,0,0);
				  else 
					AI_9116_Config(card,P9116_AI_UniPolar,P9116_AI_SoftPolling,0,0,0);
                  err = AI_ReadChannel(card, i, range, &analog_input[i]);
				  /*if(err != NoError) {
                        wsprintf(s, "AI_ReadChannel Error = %d\n", err);
                        MessageBox(hWnd, s, "Error", MB_OK);
				  }*/
                  switch(i) {
                    case 0:
           ...
           for (i=0;i<8;i++ ) {
              if (bCheck2[i]) {
                  ...
                  adinput[i+8]=io_pos[0].y+42-(43*(long)analog_input[i+8])/32767;
                  if (old_adinput2[i]!=-1 ) {
