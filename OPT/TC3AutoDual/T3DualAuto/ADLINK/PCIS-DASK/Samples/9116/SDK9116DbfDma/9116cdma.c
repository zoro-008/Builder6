...
int nCmdShow;
{
  ...
  hWnd = CreateWindow("GenericWClass",
                      (LPCSTR)"PCI-9116 Double Buffered DMA/INT",
                      WS_OVERLAPPEDWINDOW,
                      CW_USEDEFAULT,
                      CW_USEDEFAULT,
                      520,
  ...
  char s[50];
      switch (message) {
         case WM_INITDIALOG:
              ...
              for (i=0;i<2;i++) {
                SendMessage(GetDlgItem(hWnd,IDC_COMBO2), CB_ADDSTRING, NULL, (LONG)(LPSTR) AI_MODE[i]);
              }
    ...
    case WM_COMMAND:
        switch(LOWORD(wParam)) {
           ...
           case IDC_START:
                ...
                if ((err=AI_ContReadChannel(card, channel, AD_B_5_V, ai_buf, data_size, (F64)1, ASYNCH_OP)) != NoError) {
