...
int nCmdShow;
{
  ...
  hWnd = CreateWindow("GenericWClass",
                      (LPCSTR)"PCI-9111 Double Buffered Interrupt with Post-Trigger Mode",
                      WS_OVERLAPPEDWINDOW,
                      CW_USEDEFAULT,
                      CW_USEDEFAULT,
                      520,
    ...
    case WM_COMMAND:
        switch(LOWORD(wParam)) {
           ...
           case IDC_START:
                ...
                GlobalFix(hMem);
                fstop =0;
                if ((err=AI_9111_Config(card,TRIG_INT_PACER, P9111_TRGMOD_POST, postCount))!= NoError) {
