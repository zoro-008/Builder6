...
int DA_wave[8] = {NONE, NONE, NONE, NONE, NONE, NONE, NONE, NONE};
...
int nCmdShow;
{
  ...
  hInst = hInstance;
  hSampleMenu = LoadMenu(hInstance, "U6208Menu");
  ...
  switch(message) {
    case WM_CREATE:
        ...
        for (i=0; i<8; i++) {
            ...
            SendMessage(hDAComboBox[i], CB_ADDSTRING, NULL, (LONG)(LPSTR)"Sine");
            SendMessage(hDAComboBox[i], CB_ADDSTRING, NULL, (LONG)(LPSTR)"Square");
        ...
        for (i=0;i<4; i++) {
            ...
            SetScrollRange(hDAScroll[i],SB_CTL,-32768,32767,TRUE);
    ...
    case WM_PAINT:
        ...
        TextOut(hDC,io_pos[21].x-45, output_pos[3].y+64, "V7", 2);
        TextOut(hDC,io_pos[18].x-45, output_pos[3].y+64, "V6", 2);
        TextOut(hDC,io_pos[21].x-50, output_pos[3].y+112, "-10         0          10",25);
        TextOut(hDC,io_pos[15].x-45, output_pos[3].y+64, "V5", 2);
        TextOut(hDC,io_pos[12].x-45, output_pos[3].y+64, "V4", 2);
        TextOut(hDC,io_pos[15].x-50, output_pos[3].y+112, "-10         0          10",25);
        TextOut(hDC,io_pos[9].x-45, output_pos[3].y+64, "V3", 2);
        TextOut(hDC,io_pos[6].x-45, output_pos[3].y+64, "V2", 2);
        TextOut(hDC,io_pos[9].x-50, output_pos[3].y+112, "-10         0          10",25);
        TextOut(hDC,io_pos[3].x-45, output_pos[3].y+64, "V1", 2);
        TextOut(hDC,io_pos[0].x-45, output_pos[3].y+64, "V0", 2);
        TextOut(hDC,io_pos[3].x-50, output_pos[3].y+112, "-10         0          10",25);
    ...
    case WM_COMMAND:
        switch(LOWORD(wParam)) {
           ...
           case IDC_DA+7:
                ...
                if (HIWORD(wParam)==CBN_CLOSEUP)
                   DA_wave[k]=SendMessage(hDAComboBox[k],CB_GETCURSEL,0,0L);
                break;
           default:
    ...
    case WM_TIMER:
        ...
        for (i=0; i<8; i++) {
              ...
              case SINE:
                 m = sin(angle[i]);
                 angle[i] = angle[i] + Pi / 36;
                 p = (I16)(m * 32767);
				 AO_WriteChannel(card, i, p);

                 break;
              case SQUARE:
				 AO_WriteChannel(card, i, sq[i]);
                 if (((int)(angle[i]) % 36) == 0)
                   if (sq[i] == 32767) sq[i] = -32768; else sq[i] = 32767;
                 angle[i] = angle[i] + 1;
                 break;
              }
        }
