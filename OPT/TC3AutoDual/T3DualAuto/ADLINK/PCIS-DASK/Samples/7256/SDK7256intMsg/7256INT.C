...
int             nCmdShow;
{
    ...
    hInst = hInstance;
    hWnd = CreateWindow("IntClass", "PCI-7256 Interrupt Sample (INT1)", WS_OVERLAPPEDWINDOW,
         ...
         case WM_COMMAND:
              switch(LOWORD(wParam)) {
                ...
                case IDC_COMBO1:
                     ...
                     return(TRUE);
                case IDOK:
                     if ((card=Register_Card(PCI_7256, card_number)) < 0) {
...
LPARAM lParam;
{
    ...
    switch(message) {
        case WM_CREATE:
			 ...
			 int_count = 0;		     
			 DIO_INT1_EventMessage (card, INT1_COS, hWnd, WM_INT, NULL);
			 DIO_SetCOSInterrupt(card, 0, 0x0001, 0, 0);
        ...
        case WM_PAINT:
             ...
             wsprintf(s,"COS Latch Data = %d", COSLData);
             TextOut(hDC, 120, 160, s, strlen(s));
			 EndPaint(hWnd, &ps);
             break;
        case WM_COMMAND:
             switch(LOWORD(wParam)) {
...
void mesgfn()
{
    ...
    wsprintf(Buf,"COS Latch Data = %d", COSLData);
