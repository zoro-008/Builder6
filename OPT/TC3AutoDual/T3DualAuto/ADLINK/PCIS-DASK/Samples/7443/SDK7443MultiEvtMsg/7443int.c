...
HWND hStart, hMainWnd, hEvent[4];
U32 data_out0[4] = {0, 0 ,0 ,0};
U32 data_out1[4] = {1, 1 ,1 ,1};
...
LPARAM lParam;
{
    ...
    switch(message) {
        case WM_CREATE:
			...
			DIO_SetCOSInterrupt32(card, 3, 0x0001, (HANDLE *) &hEvent[3], 1);
		    DIO_INT_Event_Message (card, 1, hEvent[0], hWnd, WM_INT, NULL);
			DIO_INT_Event_Message (card, 1, hEvent[1], hWnd, NULL, (U32) cbfn);
		    DIO_INT_Event_Message (card, 1, hEvent[2], hWnd, WM_INT1, NULL);
			DIO_INT_Event_Message (card, 1, hEvent[3], hWnd, NULL, (U32) cbfn1);
            hMainWnd = hWnd;
            break;
        case WM_PAINT:
             ...
             TextOut(hDC, 120, 100, buf, strlen(buf));
             wsprintf(buf,"COS2 count = %d", int_count[2]);
             TextOut(hDC, 120, 120, buf, strlen(buf));
			 wsprintf(buf,"COS3 count = %d", int_count[3]);
        ...
        case WM_COMMAND:
            switch(LOWORD(wParam)) {
				case IDM_EXIT:
					DIO_INT1_EventMessage (card, INT1_DISABLE, NULL, NULL, NULL);
					...
					DIO_INT_Event_Message (card, 0, hEvent[2], NULL, NULL, NULL);
					DIO_INT_Event_Message (card, 0, hEvent[3], NULL, NULL, NULL);
                    SendMessage(hWnd, WM_CLOSE, 0, 0L);
                    break;
             }
             break;
