...
int nCmdShow;
{
  ...
  hInst = hInstance;
  hSampleMenu = LoadMenu(hInstance, "U7443Menu");
...
LPARAM lParam;
{
  ...
  switch(message) {
             ...
             if (HIWORD(wParam) == CBN_CLOSEUP) {
               if ((i=(int)SendMessage(GetDlgItem(hWnd,IDC_CHAN),CB_GETCURSEL, 0, 0L))>=0 ) {
				   if(i==0){
					...
					if(oPort == 0)
						DO_WritePort(card,P7443_TTL0, output[oPort]);				
					else
						DO_WritePort(card,P7443_TTL1, output[oPort]);
    ...
    case WM_TIMER:
        /* Digital Input */
            if(iPort == 0)	
				DI_ReadPort(card,P7443_TTL0,&input[iPort]);
            else
            	DI_ReadPort(card,P7443_TTL1,&input[iPort]);
		hDC = GetDC(hWnd);
        for (i=0; i<16; i++) {
          if ((input[iPort]>>i)&0x01)
             SelectObject(hDC, hRedBrush);
