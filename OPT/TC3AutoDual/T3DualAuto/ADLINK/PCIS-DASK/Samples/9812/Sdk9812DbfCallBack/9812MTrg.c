...
int nCmdShow;
{
  ...
  hWnd = CreateWindow("GenericWClass",
                      (LPCSTR)"PCI-9812 Double Buffered DMA with Middle Trigger Mode",
                      WS_OVERLAPPEDWINDOW,
                      CW_USEDEFAULT,
                      CW_USEDEFAULT,
                      520,
    ...
    case WM_COMMAND:
        switch(LOWORD(wParam)) {
           ...
           case IDC_START:
                ...
                if ((err = AI_9812_Config(card, midcheck? P9812_TRGMOD_MIDL:P9812_TRGMOD_SOFT, midcheck?P9812_TRGSRC_EXT_DIG:0, P9812_TRGSLP_POS, P9812_AD2_GT_PCI|P9812_CLKSRC_INT,0,postCount))<0) {
...
void AI_CallBack()
{
  ...
  AI_AsyncDblBufferTransfer(card, ai_buf);
  if(count>tfer_size)
	  AI_AsyncDblBufferTransfer(card, ai_buf+tfer_size);
  ...
  for (i=0; i<count; i++) {
         adinput = ai_buf[i]>>4;
         newy = io_pos[0].y+86-(87*(long)adinput)/2048;
         newx = i*336/((count>tfer_size)?count:tfer_size)+output_pos[15].x+35;
         if (i==0)
            MoveToEx(hDC, newx, newy, NULL);
         else
            MoveToEx(hDC, oldx, oldy, NULL);
