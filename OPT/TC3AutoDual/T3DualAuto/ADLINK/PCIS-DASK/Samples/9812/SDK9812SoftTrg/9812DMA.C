         ...
         switch(LOWORD(wParam)) {
           ...
           case IDC_CHAN:
             ...
             case IDC_START:
                  ...
                  if ((err = AI_9812_Config(card,P9812_TRGMOD_SOFT, P9812_TRGSRC_CH0, P9812_TRGSLP_POS, P9812_AD2_GT_PCI|P9812_CLKSRC_INT,0x80,0))<0) {
                  ...
                  AI_ContScanChannels (card, ch_cnt-1, range, ai_buf, data_size, (F64)sample_rate, SYNCH_OP);
                  EnableWindow (hData, TRUE);

                  for (i=0;i<4;i++) oldinput[i]=-1;
                  hDC = GetDC(hWnd);
                  SelectObject(hDC, hBlackBrush);
                  ...
                  for (i=0;i<data_size;i++) {
                   ...
                   adinput = adinput >> 4;
                   adinput=io_pos[0].y+86-(87*(int)adinput)/2048;
    ...
    switch (message)
    {
       case WM_INITDIALOG:
            ...
            for(i=0; i<data_size; i++) {
               cur_channel = i%ch_cnt;
               if (card_type)
                   wsprintf(cBuf, "%5d\t          %5x        %2d", i, pp[i]>>6, cur_channel);
               else
                   wsprintf(cBuf, "%5d\t          %5x        %2d", i, pp[i]>>4, cur_channel);
