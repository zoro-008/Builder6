...
int nCmdShow;
{
  ...
  hWnd = CreateWindow("GenericWClass",
                      (LPCSTR)"PCI-9812 Single Buffered DMA with Middle Trigger Mode",
                      WS_OVERLAPPEDWINDOW,
                      CW_USEDEFAULT,
                      CW_USEDEFAULT,
                      520,
    ...
    case WM_COMMAND:
        switch(LOWORD(wParam)) {
           ...
           case IDC_START:
                ...
                if ((err = AI_9812_Config(card, midcheck? P9812_TRGMOD_MIDL:P9812_TRGMOD_SOFT, midcheck?P9812_TRGSRC_CH0:0, P9812_TRGSLP_POS, P9812_AD2_GT_PCI|P9812_CLKSRC_INT,0x80,postCount))<0) {
                //if ((err = AI_9812_Config(card,P9812_TRGMOD_MIDL, P9812_TRGSRC_EXT_DIG, P9812_TRGSLP_POS, P9812_AD2_GT_PCI|P9812_CLKSRC_INT,0x80,postCount))<0) {
                ...
                if ((AI_ContScanChannels (card, ch_cnt-1, range, ai_buf, data_size, (F32)sample_rate, ASYNCH_OP)) != NoError) {
