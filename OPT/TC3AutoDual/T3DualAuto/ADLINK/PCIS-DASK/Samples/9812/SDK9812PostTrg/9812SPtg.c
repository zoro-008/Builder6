...
int nCmdShow;
{
  ...
  hWnd = CreateWindow("GenericWClass",
                      (LPCSTR)"PCI-9812 Single Buffered DMA with Post Trigger Mode",
                      WS_OVERLAPPEDWINDOW,
                      CW_USEDEFAULT,
                      CW_USEDEFAULT,
                      520,
  ...
  switch(message) {
    case WM_CREATE:
        ...
        CreateWindow("BUTTON","4"       ,
                      WS_CHILD | WS_VISIBLE | BS_AUTORADIOBUTTON | BS_LEFTTEXT | WS_GROUP,
                      output_pos[4].x+80, io_pos[0].y+31+20,25,15, hWnd,IDC_CH+2, hInst, NULL);
    ...
    case WM_COMMAND:
        switch(LOWORD(wParam)) {
           ...
           case IDC_START:
                ...
                if ((err = AI_9812_Config(card,P9812_TRGMOD_POST, P9812_TRGSRC_EXT_DIG, P9812_TRGSLP_POS, P9812_AD2_GT_PCI|P9812_CLKSRC_INT,0x80,0x10))<0) {
                        wsprintf(s, "AI Config Error = %d\n", err);
                        MessageBox(hWnd, s, "Error", MB_OK);
                        SendMessage(hWnd, WM_CLOSE, 0, 0L);
                        break;
                }
                if ((err = AI_ContScanChannels (card, ch_cnt-1, range, ai_buf, data_size, (F64)sample_rate, ASYNCH_OP)) != NoError) {
    ...
    switch (message) {
        case WM_INITDIALOG:
            ...
            for(i=0;i<count; i++) {
               ch_no = i % ch_cnt;
               if (card_type)
                 wsprintf(cBuf, "%5d\t      %5d          %3d", i, pp[i]>>6, ch_no);
               else
                 wsprintf(cBuf, "%5d\t      %5d          %3d", i, pp[i]>>4, ch_no);
