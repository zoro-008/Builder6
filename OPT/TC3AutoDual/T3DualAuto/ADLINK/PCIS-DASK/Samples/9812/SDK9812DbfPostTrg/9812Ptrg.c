...
int nCmdShow;
{
  ...
  hWnd = CreateWindow("GenericWClass",
                      (LPCSTR)"PCI-9812 Double Buffered DMA with Post Trigger Mode",
                      WS_OVERLAPPEDWINDOW,
                      CW_USEDEFAULT,
                      CW_USEDEFAULT,
                      520,
    ...
    case WM_COMMAND:
        switch(LOWORD(wParam)) {
           ...
           case IDC_START:
                ...
                if ((err = AI_9812_Config(card,P9812_TRGMOD_POST, P9812_TRGSRC_EXT_DIG, P9812_TRGSLP_POS, P9812_AD2_GT_PCI|P9812_CLKSRC_INT,0x80,0x4000))<0) {
...
void ProcessThread(void* pArg)
{
  ...
  do {
      ...
      for (i=0; i<data_size; i++) {
         adinput = ai_buf[i]>>4;
         newy = io_pos[0].y+87-(86*(long)adinput)/2048;
