/*;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;  Copyright (c) 1998, ADLink Technology Inc.  All rights reserved.       ;;
;;                                                                         ;;
;;      File Name   :   Util7434r.C                                         ;;
...
int nCmdShow;
{
  ...
  hInst = hInstance;
  hSampleMenu = LoadMenu(hInstance, "U7434Menu");
    ...
    case WM_PAINT:
        ...
        for (i=0; i<32; i++) {
          if ((output_hi>>i)&0x01)
             ...
             SelectObject(hDC, GetStockObject(WHITE_BRUSH));
          Ellipse(hDC, output_pos[i+32].x, output_pos[i+32].y, output_pos[i+32].x+17, output_pos[i+32].y+17);
        }
        EndPaint(hWnd, &ps);
        break;
    case WM_COMMAND:
    ...
    case WM_LBUTTONDOWN:
        ...
        for (i=0; i<32; i++) {
          if (px>output_pos[i].x && px<output_pos[i].x+17 && py>output_pos[i].y && py<output_pos[i].y+17) {
             ...
             Ellipse(hDC, output_pos[i].x, output_pos[i].y, output_pos[i].x+17, output_pos[i].y+17);
             ReleaseDC(hWnd, hDC);
             DO_WritePort(card, PORT_DO_LOW, output_lo);
             DO_WritePort(card, P7434R_DO_LED, output_lo);
             break;
          }
        }
        for (i=0; i<32; i++) {
          if (px>output_pos[i+32].x && px<output_pos[i+32].x+17 && py>output_pos[i+32].y && py<output_pos[i+32].y+17) {
             hDC = GetDC(hWnd);
             if ((output_hi>>i)&0x01) {
                output_hi -= (((U32)1L)<<i);
                SelectObject(hDC, GetStockObject(WHITE_BRUSH));
             ...
             Ellipse(hDC, output_pos[i+32].x, output_pos[i+32].y, output_pos[i+32].x+17, output_pos[i+32].y+17);
             ReleaseDC(hWnd, hDC);
             DO_WritePort(card,PORT_DO_HIGH, output_hi);
