...
int nCmdShow;
{
  ...
  hWnd = CreateWindow("GenericWClass",
                      (LPCSTR)"PCI-7200 Double Buffered DMA",
                      WS_OVERLAPPEDWINDOW,
                      CW_USEDEFAULT,
                      CW_USEDEFAULT,
                      520,
...
#ifdef SELF_OUTPUT
        for (i=0; i<10240; i++)
           DoBuf[i] = (U32)(sin((double)i*PI/512)*0x7fffffff)+0x80000000;
#endif
...
/*
                if (data_size > 32768) {
                   data_size = 32768;  //at most 32K (64K bytes) for DMA
                   MessageBox(hWnd, "DMA transfer cannot exceed 64K bytes boundary\nOnly perform 64K bytes DMA transfer.", "Warning", MB_OK);
                }
*/
...
void ProcessThread(void* pArg)
{
  ...
  do {
      ...
      i = DI_AsyncDblBufferTransfer(card, pMem);
      ...
      for (i=0; i<data_size; i++) {
         adinput = pMem[i];
         newy = io_pos[0].y+174-(177*(F32)adinput)/0xffffffff;
