    ...
    case WM_COMMAND:
        switch(LOWORD(wParam)) {
           ...
           case IDC_START:
                ...
                if (data_size > 40960) {
                   data_size = 40960;
                   MessageBox(hWnd, "Xfer Count can not exceed 40960\nDMA transfer only perform 40960 count.", "Warning", MB_OK);
                }
                ...
                GlobalFix(hMem);
                if ((err=DI_7200_Config(card, TRIG_HANDSHAKE, DI_NOWAITING, DI_TRIG_FALLING, IREQ_RISING)) != NoError) {
...
#ifdef SELF_OUTPUT
                DO_7200_Config(card, TRIG_HANDSHAKE, OREQ_ENABLE, OTRIG_LOW);
...
#ifdef SELF_OUTPUT
                DO_ContWritePort(card, 0, DoBuf, data_size+2, 1, (F64)100, ASYNCH_OP);
#endif
