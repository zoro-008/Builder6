...
U16 input[16], output=0;
I16 oldinput[16],analog_input[16],adinput[16],old_adinput1[16],old_adinput2[16];
U8 input_lo=0, input_hi=0;
U32 kk;

LPSTR AD_Range[]={"+/-10V","+/-5V","+/-2.5V","+/-1.25V","+/-1V","+/-0.1V","+/-0.01V"};
I16 range_value[] = {AD_B_10_V, AD_B_5_V, AD_B_2_5_V, AD_B_1_25_V, AD_B_1_V, AD_B_0_1_V, AD_B_0_01_V};
...
I16 sample_rate=100, time_interval, range=AD_B_10_V;
...
int nCmdShow;
{
  ...
  hInst = hInstance;
  hSampleMenu = LoadMenu(hInstance, "U9114Menu");
  hWnd = CreateWindow("GenericWClass",
                      (LPCSTR)"9114 (AI0~AI15)",
                      WS_OVERLAPPEDWINDOW,
                      CW_USEDEFAULT,
                      CW_USEDEFAULT,
                      530,
         ...
         case WM_COMMAND:
              switch(LOWORD(wParam)) {
                ...
                case IDC_COMBO1:
                     ...
                     return(TRUE);
                case IDOK:
                     if ((card=Register_Card(PCI_9114DG, card_number)) < 0) {
  ...
  switch(message) {
    case WM_CREATE:
           ...
           CreateWindow("BUTTON","Start" ,
                      WS_CHILD | WS_VISIBLE ,
                      output_pos[10].x+110, output_pos[10].y+20,50, 30, hWnd, IDC_START, hInst, NULL);

           CreateWindow("BUTTON","Stop"   ,
                      WS_CHILD | WS_VISIBLE ,
                      output_pos[10].x+170, output_pos[10].y+20,50, 30, hWnd, IDC_STOP, hInst, NULL);

           CreateWindow("BUTTON","Pause"   ,
                      WS_CHILD | WS_VISIBLE ,
                      output_pos[10].x+230, output_pos[10].y+20,50, 30, hWnd, IDC_PAUSE, hInst, NULL);

           CreateWindow("BUTTON","0"       ,
                      WS_CHILD | WS_VISIBLE | BS_RADIOBUTTON | BS_LEFTTEXT ,
                      output_pos[4].x+50, io_pos[0].y-5,25,15, hWnd,IDC_AD, hInst, NULL);

           CreateWindow("BUTTON","1"       ,
                      WS_CHILD | WS_VISIBLE | BS_RADIOBUTTON | BS_LEFTTEXT,
                      output_pos[4].x+50, io_pos[0].y+13,25,15, hWnd,IDC_AD+1, hInst, NULL);

           CreateWindow("BUTTON","2"       ,
                      WS_CHILD | WS_VISIBLE | BS_RADIOBUTTON | BS_LEFTTEXT,
                      output_pos[4].x+50, io_pos[0].y+31,25,15, hWnd,IDC_AD+2, hInst, NULL);

           CreateWindow("BUTTON","3"       ,
                      WS_CHILD | WS_VISIBLE | BS_RADIOBUTTON | BS_LEFTTEXT,
                      output_pos[4].x+50, io_pos[0].y+49,25,15, hWnd,IDC_AD+3, hInst, NULL);

           CreateWindow("BUTTON","4"       ,
                      WS_CHILD | WS_VISIBLE | BS_RADIOBUTTON | BS_LEFTTEXT,
                      output_pos[4].x+100, io_pos[0].y-5,25,15, hWnd,IDC_AD+4, hInst, NULL);

           CreateWindow("BUTTON","5"       ,
                      WS_CHILD | WS_VISIBLE | BS_RADIOBUTTON | BS_LEFTTEXT,
                      output_pos[4].x+100, io_pos[0].y+13,25,15, hWnd, IDC_AD+5, hInst, NULL);

           CreateWindow("BUTTON","6"       ,
                      WS_CHILD | WS_VISIBLE | BS_RADIOBUTTON | BS_LEFTTEXT,
                      output_pos[4].x+100, io_pos[0].y+31,25,15, hWnd,IDC_AD+6, hInst, NULL);

           CreateWindow("BUTTON","7"       ,
                      WS_CHILD | WS_VISIBLE | BS_RADIOBUTTON | BS_LEFTTEXT,
                      output_pos[4].x+100, io_pos[0].y+49,25,15, hWnd, IDC_AD+7, hInst, NULL);

           CreateWindow("BUTTON","0"       ,
                      WS_CHILD | WS_VISIBLE | BS_RADIOBUTTON | BS_LEFTTEXT,
                      output_pos[4].x+50, io_pos[0].y+97,25,15, hWnd, IDC_AD+8, hInst, NULL);

           CreateWindow("BUTTON","1"       ,
                      WS_CHILD | WS_VISIBLE | BS_RADIOBUTTON | BS_LEFTTEXT,
                      output_pos[4].x+50, io_pos[0].y+115,25,15, hWnd, IDC_AD+9, hInst, NULL);

           CreateWindow("BUTTON","2"       ,
                      WS_CHILD | WS_VISIBLE | BS_RADIOBUTTON | BS_LEFTTEXT,
                      output_pos[4].x+50, io_pos[0].y+133,25,15, hWnd, IDC_AD+10, hInst, NULL);

           CreateWindow("BUTTON","3"       ,
                      WS_CHILD | WS_VISIBLE | BS_RADIOBUTTON | BS_LEFTTEXT,
                      output_pos[4].x+50, io_pos[0].y+151,25,15, hWnd, IDC_AD+11, hInst, NULL);

           CreateWindow("BUTTON","4"       ,
                      WS_CHILD | WS_VISIBLE | BS_RADIOBUTTON | BS_LEFTTEXT,
                      output_pos[4].x+100, io_pos[0].y+97,25,15, hWnd, IDC_AD+12, hInst, NULL);

           CreateWindow("BUTTON","5"       ,
                      WS_CHILD | WS_VISIBLE | BS_RADIOBUTTON | BS_LEFTTEXT,
                      output_pos[4].x+100, io_pos[0].y+115,25,15, hWnd, IDC_AD+13, hInst, NULL);

           CreateWindow("BUTTON","6"       ,
                      WS_CHILD | WS_VISIBLE | BS_RADIOBUTTON | BS_LEFTTEXT,
                      output_pos[4].x+100, io_pos[0].y+133,25,15, hWnd, IDC_AD+14, hInst, NULL);

           CreateWindow("BUTTON","7"       ,
                      WS_CHILD | WS_VISIBLE | BS_RADIOBUTTON | BS_LEFTTEXT,
                      output_pos[4].x+100, io_pos[0].y+151,25,15, hWnd, IDC_AD+15, hInst, NULL);
    ...
    case WM_PAINT:
           ...
           Rectangle(hDC, input_pos[15].x-10, input_pos[15].y-44, input_pos[0].x+26, input_pos[0].y+10);
           SetTextColor(hDC,RGB(255,255,255));
           SetBkColor(hDC, RGB(0, 0, 255));
           TextOut(hDC, input_pos[15].x, input_pos[15].y-48, "Digital Input",13);

           // Digital Output Box
           SelectObject(hDC, hGreenBrush);
           Rectangle(hDC, output_pos[15].x-10, output_pos[15].y-44, output_pos[0].x+26, output_pos[0].y+10);
           SetTextColor(hDC,RGB(255,0,0));
           SetBkColor(hDC, RGB(0, 0, 255));
           TextOut(hDC, output_pos[15].x, output_pos[15].y-50, "Digital Output", 14);
           ...
           TextOut(hDC, input_pos[15].x, input_pos[15].y-10,   "15                                                                                                               0", 114);
           SetBkColor(hDC, RGB(0, 128, 0));
           TextOut(hDC, output_pos[15].x, output_pos[15].y-10, "15                                                                                                               0", 114);
           SetBkColor(hDC, RGB(0, 255, 0));
           ...
           switch (range) {
             ...
             case AD_B_0_1_V:
                       ...
                       break;
             case AD_B_0_01_V:
                       TextOut(hDC,input_pos[15].x-5, io_pos[15].y-5,"0.01",4);
                       TextOut(hDC,input_pos[15].x-8, io_pos[15].y+67,"-0.01",5);
                       TextOut(hDC,input_pos[15].x-5, io_pos[15].y+87,"0.01",4);
                       TextOut(hDC,input_pos[15].x-8, io_pos[15].y+159,"-0.01",5);
                       break;
           }
           ...
           for (i=0; i<16; i++) {
              ...
              Ellipse(hDC, input_pos[i].x, input_pos[i].y-30, input_pos[i].x+20, input_pos[i].y-10);
           }
           ...
           for (i=0; i<16; i++) {
              if ((output>>i)&0x01)
                 ...
                 SelectObject(hDC, GetStockObject(WHITE_BRUSH));
              Ellipse(hDC, output_pos[i].x, output_pos[i].y-30, output_pos[i].x+20, output_pos[i].y-10);
           }
        ...
        if (card >= 0) {
           ...
           for (i=0;i<8;i++ ) {
              if (bCheck1[i]) {
                  ...
                  adinput[i]=io_pos[0].y+42-(43*(long)analog_input[i])/32767;
                  if (old_adinput1[i]!=-1 ) {
           ...
           for (i=0;i<8;i++ ) {
              if (bCheck2[i]) {
                  ...
                  adinput[i+8]=io_pos[0].y+123-(43*(long)analog_input[i+8])/32767;
                  if (old_adinput2[i]!=-1 ) {
    ...
    case WM_LBUTTONDOWN:
        if (card >= 0) {
           ...
           py = HIWORD(lParam);
           for (i=0; i<16; i++) {
               if (px>output_pos[i].x && px<output_pos[i].x+17 && py>output_pos[i].y-30 && py<output_pos[i].y-13) {
                  hDC = GetDC(hWnd);
                  if ((output>>i)&0x01) {
