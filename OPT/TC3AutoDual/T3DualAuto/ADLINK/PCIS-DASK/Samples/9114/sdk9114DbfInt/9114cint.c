...
int nCmdShow;
{
  ...
  hWnd = CreateWindow("GenericWClass",
                      (LPCSTR)"PCI-9114 Double Buffered DMA",
                      WS_OVERLAPPEDWINDOW,
                      CW_USEDEFAULT,
                      CW_USEDEFAULT,
                      520,
...
LPARAM lParam;
{
  ...
  switch(message) {
    ...
    case WM_PAINT:
        ...
        TextOut(hDC,input_pos[2].x-10, io_pos[15].y+118,"Range:+/-10V",12);
    ...
    case WM_COMMAND:
        switch(LOWORD(wParam)) {
           ...
           case IDC_START:
                ...
                if ((err=AI_9114_Config(card, TRIG_INT_PACER)) != NoError) {
                ...
                if ((err=AI_ContReadChannel(card, channel, AD_B_10_V, ai_buf, data_size, (F64)sample_rate, ASYNCH_OP)) != NoError) {
    ...
    switch (message) {
        case WM_INITDIALOG:
            ...
            for(i=0;i<data_size; i++) {
                 wsprintf(cBuf, "%5d\t                %7d", i, (I16)(pp[i]&0xffff));
...
void ProcessThread(void* pArg)
{
  ...
  do {
      ...
      for (i=0; i<data_size; i++) {
         adinput = (I16)(ai_buf[i]&0xffff);
         newy = io_pos[0].y+85-(86*(long)adinput)/32767;
