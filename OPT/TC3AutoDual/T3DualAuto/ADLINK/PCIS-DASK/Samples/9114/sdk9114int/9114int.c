...
int nCmdShow;
{
  ...
  hWnd = CreateWindow("GenericWClass",
                      (LPCSTR)"9114 Autoscan Interrupt(AI0~AI7)",
                      WS_OVERLAPPEDWINDOW,
                      CW_USEDEFAULT,
                      CW_USEDEFAULT,
                      520,
         ...
         switch(LOWORD(wParam)) {
           ...
           case IDC_START:
              ...
              for (i=0;i<data_size;i++) {
               ...
               input_data = (I16)(adinput & 0xffff);
               input_data=io_pos[0].y+86-(87*(long)input_data)/32767;
               if (oldinput[cur_channel]!=-1)
                  MoveToEx(hDC,(i*334)/data_size+output_pos[15].x+35,oldinput[cur_channel],NULL);
               else
                  MoveToEx(hDC,(i*334)/data_size+output_pos[15].x+35,input_data,NULL);
               LineTo(hDC,(i*334)/data_size+output_pos[15].x+36,input_data);
               oldinput[cur_channel]=input_data;
              } // for
              ReleaseDC(hWnd, hDC);
              break;
           default:
    ...
    switch (message)
    {
       case WM_INITDIALOG:
            ...
            for(i=0; i<data_size; i++) {
               ...
               wsprintf(cBuf, "%5d\t             %7d     %2d", i, (I16)(ai_buf[i]&0xffff), ch_no);
