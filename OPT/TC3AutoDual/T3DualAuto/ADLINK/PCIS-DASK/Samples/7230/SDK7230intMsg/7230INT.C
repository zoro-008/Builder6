...
int             nCmdShow;
{
    ...
    hInst = hInstance;
    hWnd = CreateWindow("IntClass", "PCI-7230 Interrupt Sample (INT2)", WS_OVERLAPPEDWINDOW,
...
LPARAM lParam;
{
    ...
    switch(message) {
        case WM_CREATE:
			 ...
			 int_count = 0;		     
			 DIO_INT2_EventMessage (card, INT2_EXT_SIGNAL, hWnd, WM_INT, NULL);
             hMainWnd = hWnd;
             break;
        case WM_PAINT:
             hDC = BeginPaint(hWnd, &ps);
             TextOut(hDC, 20, 30, "This program shows the number of occurrence", 43);
             TextOut(hDC, 20, 45, "of INT2 interupt. INT2 interrupt is generated", 45);
             TextOut(hDC, 20, 60, "by external signal from DI bit-1. If you connect", 48);
             TextOut(hDC, 20, 75, "DO bit-1 to DI bit-1, press 'Output' button can", 47);
             TextOut(hDC, 20, 90, "generate an interrupt.", 22);
             wsprintf(s,"INT2 Count = %d ", int_count);
...
void mesgfn()
{
    ...
    wsprintf(Buf,"INT2 Count = %d ", int_count);
