...
int             nCmdShow;
{
    ...
    hInst = hInstance;
    hWnd = CreateWindow("IntClass", "PCI-7396 Interrupt Sample (INT1)", WS_OVERLAPPEDWINDOW,
...
LPARAM lParam;
{
    ...
    switch(message) {
        case WM_CREATE:
             ...
             DIO_PortConfig(card,Channel_P1, INPUT_PORT);
             DIO_PortConfig(card,Channel_P2, OUTPUT_PORT);
             DIO_SetCOSInterrupt(card, Channel_P1, 1, 1, 1);
             DIO_SetCOSInterrupt(card, Channel_P2, 0, 0, 0);
             DIO_SetCOSInterrupt(card, Channel_P3, 0, 0, 0);
             DIO_SetCOSInterrupt(card, Channel_P4, 0, 0, 0);
             DIO_SetDualInterrupt(card, INT1_COS, INT2_DISABLE, hEvent);
        ...
        case WM_PAINT:
             ...
             TextOut(hDC, 20, 45, "of COS INT1 interupt. COS INT1 interrupt is", 43);
             TextOut(hDC, 20, 60, "generated by external signal from DI Port 1.", 44);
             TextOut(hDC, 20, 75, "If you connect P1 to P2, press 'Output' button", 46);
             TextOut(hDC, 20, 90, "can generate an interrupt.", 26);
