...
void __fastcall CSequence::UpdateButton(void) //외부 버튼들 및 윈도우즈 버튼들이 클릭이 되었을때 취합해서 동작진행 시키는 함수.
{
    ...
    if (bStartSw) {
        ...
        if(!InspectExpiry      ()) { FM_MsgOk("WARRING" , "용액 기한이 지났습니다. 용액을 확인하여 주십시오."); bStartSw = false;                        }
...
//---------------------------------------------------------------------------
bool __fastcall CSequence::InspectExpiry(void)
{
    bool isOk = true;

    TDateTime LQSply;
    LQSply = IncDay(OM.CmnOptn.LQSupplyDay.Val, OM.CmnOptn.iExpiryDay);
    int iLQSulyDay = LQSply.Val - Now().Val;

    isOk = iLQSulyDay >= 0;
    if(OM.CmnOptn.bIgnrDsprExpr) isOk = true;
