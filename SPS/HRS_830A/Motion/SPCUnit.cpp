...
bool CLotData::SaveArrayData(TData & _tData , int _iMgzNo , int _iSlotNo , CArray & _rData)
{
    ...
    UserINI.Save(sPath.c_str() , "Data" , "dTickTime " , _tData.dTickTime );
...
bool CLotData::LoadSaveLotIni(bool _bLoad)
{
    ...
    if(_bLoad) {
        ...
        UserINI.Load(sPath.c_str() , "Data" , "dTickTime " , m_tData.dTickTime );
        ...
        UserINI.Save(sPath.c_str() , "Data" , "dTickTime " , m_tData.dTickTime );
...
void CLotData::DispLotList(AnsiString _sDate , TStringGrid * _sgLotList)
{
    ...
    _sgLotList -> Cells[2][0] = "WorkCnt"     ; //_sgLotList -> ColWidths[2] = _sgLotList -> Cells[2][0].Length() * 15 ;
    //_sgLotList -> Cells[3][0] = "TickTime" ; //_sgLotList -> ColWidths[4] = _sgLotList -> Cells[4][0].Length() * 15 ;
    _sgLotList -> Cells[3][0] = "Start"    ; //_sgLotList -> ColWidths[5] = _sgLotList -> Cells[5][0].Length() * 15 ;
    _sgLotList -> Cells[4][0] = "End"      ; //_sgLotList -> ColWidths[6] = _sgLotList -> Cells[6][0].Length() * 15 ;
    _sgLotList -> Cells[5][0] = "WorkTime" ; //_sgLotList -> ColWidths[7] = _sgLotList -> Cells[7][0].Length() * 15 ;
    _sgLotList -> Cells[6][0] = "TotalTime"; //_sgLotList -> ColWidths[8] = _sgLotList -> Cells[8][0].Length() * 15 ;
    _sgLotList -> Cells[7][0] = "JobFile"  ; //_sgLotList -> ColWidths[9] = _sgLotList -> Cells[9][0].Length() * 15 ;
    ...
    if (FindFirst(sSerchFile , faAnyFile , sr) == 0) {
        do {
            if ((sr.Attr & faDirectory) != sr.Attr) {
                ...
                //UserINI.Load(sIniPath.c_str() , "Data" , "dTickTime " , tData.dTickTime ); _sgLotList -> Cells[3][iLotCnt+1] = tData.dTickTime  ;
...
void CLotData::DispLotInfo(TStringGrid * _sgLotInfo) //오퍼레이션 창용.
{
    ...
    _sgLotInfo -> Cells[0][1] = "TickTime "  ;                                       _sgLotInfo -> Cells[1][1] = m_tData.dTickTime ;
    _sgLotInfo -> Cells[0][2] = "SttTime  "  ; tDateTime.Val = m_tData.dSttTime   ;  _sgLotInfo -> Cells[1][2] = tDateTime.FormatString("hh:nn:ss");
    _sgLotInfo -> Cells[0][3] = "EndTime  "  ; tDateTime.Val = m_tData.dEndTime   ;  _sgLotInfo -> Cells[1][3] = tDateTime.FormatString("hh:nn:ss");
    _sgLotInfo -> Cells[0][4] = "WorkTime "  ; tDateTime.Val = m_tData.dWorkTime  ;  _sgLotInfo -> Cells[1][4] = tDateTime.FormatString("hh:nn:ss");
    _sgLotInfo -> Cells[0][5] = "TotalTime"  ; tDateTime.Val = m_tData.dTotalTime ;  _sgLotInfo -> Cells[1][5] = tDateTime.FormatString("hh:nn:ss");
...
void CLotData::Update(EN_SEQ_STAT Stat)
{
    ...
    if(bExist && !bPreExist) { //자제 작업 끝나서 포스트버퍼에 들어옴.
        ...
        sTickTime.printf("%.2f" , WRK.m_dWorkCycleTime / 1000);
        m_tData.dTickTime = sTickTime.ToDouble();
...
AnsiString CLotData::GetCrntLotNo()
{
    ...
    else if(!DM.ARAY[riRWK].CheckAllStat(csNone)) return DM.ARAY[riRWK].GetLotNo() ;
    else if(!DM.ARAY[riLWK].CheckAllStat(csNone)) return DM.ARAY[riLWK].GetLotNo() ;
...
bool CDayData::LoadSaveDayIni(bool _bLoad , TData & _tData)
{
    ...
    if(_bLoad) {
        ...
        UserINI.Load(sPath.c_str() , "Data" , "iLotCnt   " , _tData.iLotCnt    );  //1
    }
    else {
        UserINI.Save(sPath.c_str() , "Data" , "dWorkTime " , _tData.dWorkTime  );
        UserINI.Save(sPath.c_str() , "Data" , "dStopTime " , _tData.dStopTime  );
...
bool CDayData::SaveDayData(TData & _tData , TDateTime _tDateTime) //혹시 나중에 Lot하고 동기화 할까봐 TDateTime 인자 넣음.
{
    ...
    UserINI.Save(sPath.c_str() , "Data" , "iLotCnt   " , _tData.iLotCnt    );  //2
...
void CDayData::DispDayInfo (TStringGrid * _sgDayInfo) //오퍼레이션 창용.
{
    ...
    _sgDayInfo -> Cells[0][6] = "LotCnt   ";                                      _sgDayInfo -> Cells[1][6] = m_tData.iLotCnt  ;
}
