...
bool __fastcall CVisnComUnit::ReadResult(int iVisnNo, EN_ARAY_ID riId)
{
    ...
    if(iVisnNo == 3) m_iVsTtlErrCnt = iFailCnt ;
...
//580T        iRslt = GetHighLevel(DM.ARAY[riId].GetStat(OM.DevInfo.iRowCnt-iFailRow ,iFailCol-1),(EN_CHIP_STAT)iFailItm);
        ...
        if(iVisnNo == 3) m_iVsErrCnt[iRslt]++ ;
...
int CVisnComUnit::GetLastErrCnt(EN_CHIP_STAT _csStat)
{
    return m_iVsErrCnt[_csStat] ;
}

int CVisnComUnit::GetLastTtlErrCnt()
{
    return m_iVsTtlErrCnt ;
}

int CVisnComUnit::ClearErrCnt()
{
    memset(&m_iVsErrCnt , 0 ,sizeof(int) * MAX_CHIP_STAT);
...
void CVisnComUnit::ThreadReset()
{
    ...
    bool bVisn1 = iVisnResetSel == vrOri ;
    bool bVisn2 = iVisnResetSel == vrLsr ;
    bool bVisn3 = iVisnResetSel == vrInp ;
    ...
    while(1) {
        ...
        switch (iStep) {
            ...
            default : sTemp = sTemp.sprintf("Cycle Default Clear %s iStep=%02d" , __FUNC__ , iStep );
                      Trace("CVisnComUnit", sTemp.c_str());
                      if(bVisn1)IO_SetY(yVSN_OriReset , false) ;
                      if(bVisn2)IO_SetY(yVSN_LsrReset , false) ;
                      if(bVisn3)IO_SetY(yVSN_InpReset , false) ;
                      //if(bVisn4)IO_SetY(yVSN_OriReset , false) ;
                      iStep = 0 ;
                      return ;
            ...
            case  11: //iSendCnt ++ ;
                      ...
                      if(bVisn1 && !IO_GetX(xVSN_OriReady)) {IO_SetY(yVSN_OriReset , true) ; bVisn1Rdy = true ;}
                      if(bVisn2 && !IO_GetX(xVSN_LsrReady)) {IO_SetY(yVSN_LsrReset , true) ; bVisn2Rdy = true ;}
                      if(bVisn3 && !IO_GetX(xVSN_InpReady)) {IO_SetY(yVSN_InpReset , true) ; bVisn3Rdy = true ;}
                      if(bVisn1Rdy || bVisn2Rdy || bVisn3Rdy ) {iStep = 12 ; continue ; }
                      iStep = 14;
                      continue ;

            case  12: if(bVisn1 && bVisn1Rdy && !IO_GetX(xVSN_OriReady)) continue ;
                      if(bVisn2 && bVisn2Rdy && !IO_GetX(xVSN_LsrReady)) continue ;
                      if(bVisn3 && bVisn3Rdy && !IO_GetX(xVSN_InpReady)) continue ;
                      if(bVisn1 && bVisn1Rdy) {  IO_SetY(yVSN_OriReset,false) ; bVisn1Rdy = false ; }
                      if(bVisn2 && bVisn2Rdy) {  IO_SetY(yVSN_LsrReset,false) ; bVisn2Rdy = false ; }
                      if(bVisn3 && bVisn3Rdy) {  IO_SetY(yVSN_InpReset,false) ; bVisn3Rdy = false ; }
                      Timer.Clear() ;            
            ...
            case  14: iSendCnt ++ ;
                      if(bVisn1)IO_SetY(yVSN_OriReset , true) ;
                      if(bVisn2)IO_SetY(yVSN_LsrReset , true) ;
                      if(bVisn3)IO_SetY(yVSN_InpReset , true) ;

                      iStep++;
                      continue ;

            case  15: if(bVisn1 && IO_GetX(xVSN_OriReady)) continue ;
                      if(bVisn2 && IO_GetX(xVSN_LsrReady)) continue ;
                      if(bVisn3 && IO_GetX(xVSN_InpReady)) continue ;
                      iStep++;
                      continue ;

            case  16: if(bVisn1)IO_SetY(yVSN_OriReset ,false) ;
                      if(bVisn2)IO_SetY(yVSN_LsrReset ,false) ;
                      if(bVisn3)IO_SetY(yVSN_InpReset ,false) ;
            ...
            case  17: if(bVisn1 && !IO_GetX(xVSN_OriReady)) continue ;
                      if(bVisn2 && !IO_GetX(xVSN_LsrReady)) continue ;
                      if(bVisn3 && !IO_GetX(xVSN_InpReady)) continue ;
            ...
            case  18: if(!Timer.OnDelay(true , 500)) continue ;
                      ...
                      if(iSendCnt == 1 ) {
                          if((bVisn1 &&!IO_GetX(xVSN_OriBusy)) ||
                             (bVisn2 &&!IO_GetX(xVSN_LsrBusy)) ||
                             (bVisn3 &&!IO_GetX(xVSN_InpBusy)) ) {  //비젼이 이상하게 2번 보내 줘야 검사위치가 클리어됌.
                              iStep = 14 ;
                              continue ;
                          }
                      }
                      ...
                      if((bVisn1 && IO_GetX(xVSN_OriBusy)) ||
                         (bVisn2 && IO_GetX(xVSN_LsrBusy)) ||
                         (bVisn3 && IO_GetX(xVSN_InpBusy)) ) {
                          if(iSendCnt > 2) {  //2번 해서도 비젼쪽 비지가 않풀리면 에러.
                              EM_SetDisp(true) ;
                              EM_SetErr(eiVSN_ResetCnt) ;
                              return ;
                          }
                          iSendCnt++;
                          iStep = 14 ;
...
void CVisnComUnit::ThreadLotStart()
{
    ...
    while(1) {
        ...
        if (m_tmLotStart.OnDelay(iStep == iPreStep && !OM.MstOptn.bDebugMode , 2000 )) {
            EM_SetDisp(true);
            EM_SetErr(eiVSN_CommTO);
            sTemp = sTemp.sprintf("%s TIMEOUT STATUS : iStep=%02d" , __FUNC__ , iStep );
            Trace("CVisnComUnit",sTemp.c_str());
            iStep = 0 ;
            IO_SetY(yVSN_OriStart , false) ;
            IO_SetY(yVSN_LsrStart , false) ;
            IO_SetY(yVSN_InpStart , false) ;
        ...
        switch (iStep) {
            ...
            case  10: if(!IO_GetX(xVSN_OriReady) && !OM.CmnOptn.bIgnrOriVisn) {
                          EM_SetErr(eiVSN_OriNotReady);
                          EM_SetDisp(true) ;
                          return ;
                      }
                      if(!IO_GetX(xVSN_LsrReady) && !OM.CmnOptn.bIgnrLsrVisn) {
                          EM_SetErr(eiVSN_LsrNotReady);
                          EM_SetDisp(true) ;
                          return ;
                      }
                      if(!IO_GetX(xVSN_InpReady) && !OM.CmnOptn.bIgnrInpVisn) {
                          EM_SetErr(eiVSN_InpNotReady);
                          EM_SetDisp(true) ;
                          return ;
                      }

                      WriteVisnLot();   //요놈 시점 부터 작업 하면됌.
                      IO_SetY(yVSN_OriStart , true) ;
                      IO_SetY(yVSN_LsrStart , true) ;
                      IO_SetY(yVSN_InpStart , true) ;

                      iStep++;
                      continue ;

            case  11: //if(!Timer.OnDelay(true , 100)) continue ;
                      if(IO_GetX(xVSN_OriReady) && !OM.CmnOptn.bIgnrOriVisn) continue ;
                      if(IO_GetX(xVSN_LsrReady) && !OM.CmnOptn.bIgnrLsrVisn) continue ;
                      if(IO_GetX(xVSN_InpReady) && !OM.CmnOptn.bIgnrInpVisn) continue ;
                      iStep++;
                      continue ;

            case  12: IO_SetY(yVSN_OriStart ,false) ;
                      IO_SetY(yVSN_LsrStart ,false) ;
                      IO_SetY(yVSN_InpStart ,false) ;
                      iStep++;
                      continue ;

            case  13: if(!IO_GetX(xVSN_OriReady)&& !OM.CmnOptn.bIgnrOriVisn) continue ;
                      if(!IO_GetX(xVSN_LsrReady)&& !OM.CmnOptn.bIgnrLsrVisn) continue ;
                      if(!IO_GetX(xVSN_InpReady)&& !OM.CmnOptn.bIgnrInpVisn) continue ;
                      Timer.Clear();
                      iStep++;
            ...
            case  14: //if(Timer.OnDelay(true , 1000)) {
                      ...
                      if(!OM.CmnOptn.bIgnrOriVisn && IO_GetX(xVSN_OriBusy)) continue ;
                      if(!OM.CmnOptn.bIgnrLsrVisn && IO_GetX(xVSN_LsrBusy)) continue ;
                      if(!OM.CmnOptn.bIgnrInpVisn && IO_GetX(xVSN_InpBusy)) continue ;
...
void CVisnComUnit::ThreadModelChange()
{
    ...
    while(1) {
        ...
        switch (iStep) {
            ...
            case  11: if(!IO_GetX(xVSN_InpReady) && !OM.CmnOptn.bIgnrInpVisn)  {
                          EM_SetErr(eiVSN_InpNotReady);
                          EM_SetDisp(true) ;
                          return ;
                      }
                      if(!OM.CmnOptn.bIgnrInpVisn) IO_SetY(yVSN_InpChange , true) ;
                      Timer.Clear();
                      iStep++;
                      continue ;

            case  12: if(!OM.CmnOptn.bIgnrInpVisn && IO_GetX(xVSN_InpReady)) continue ;
                      //if(!Timer.OnDelay(true , 1000)) continue ;

                      iStep++;
                      continue ;

            case  13: if(!OM.CmnOptn.bIgnrInpVisn) IO_SetY(yVSN_InpChange ,false) ;
                      iStep++;
                      continue ;

            case  14: if(!OM.CmnOptn.bIgnrInpVisn && !IO_GetX(xVSN_InpReady)) continue ;
                      Timer.Clear();
                      iStep++;
                      continue ;
...
bool __fastcall CVisnComUnit::WriteVisnLot()
{
    ...
    sData = DM.ARAY[riPSB].GetLotNo().Trim()+";";
    //LTQ.GetLotCnt().Trim()+";" ;
    //
...
bool __fastcall CVisnComUnit::WriteLogVisnErr(AnsiString _sLog) //비젼 결과값들을 로그로 관리 한다.
{
    ...
    //Set Path.
    if(LTQ.GetLotOpen()) tLotStartTime.Val = SPC.LOT.GetData().dSttTime;
    else                 tLotStartTime     = Now()                 ;
    sTime = tLotStartTime.FormatString("hhnnss");

    if (!DirectoryExists(VISION_FOLDER)) CreateDir(VISION_FOLDER);

    if(LTQ.GetLotOpen())sPath = VISION_FOLDER + DM.ARAY[riPSB].GetLotNo() + "_" + sTime + ".dat";
