...
bool CLoader::CheckSafe(EN_MOTR_ID _iMotr , EN_PSTN_ID _iPstnId)
{
    ...
    bool isFwPusher      = AT_GetCmd(aiLDR_PshFwBw)==ccFwd; //IO_GetX(xLD1_PusherFw) || !IO_GetX(xLD1_PusherBw);
    bool isRailGateCnsr  = IO_GetX(xPRB_PkgIn)  ;
...
bool CLoader::CycleHome()     //sun DLL direct access.
{
    ...
    switch (Step.iHome) {
        ...
        case 11: if(!IO_GetX(xLDR_PahrBw))return false ;
...
bool CLoader::Autorun(void) //오토런닝시에 계속 타는 함수.
{
    ...
    if (Step.iSeq == 0) {
        ...
        bool isCycleWork   =  DM.ARAY[riLDR].GetCntStat  (csUnkwn)  &&  DM.ARAY[riPRB].CheckAllStat(csNone )  && !IO_GetX(xPRB_PkgIn) &&
                              PRB.GetCycleStep() == CPreBuff::scIdle&& WRK.GetCycleStep() != CWork::scIn && !OM.CmnOptn.bLoadingStop;
        bool isCyclePlace  =  DM.ARAY[riLDR].CheckAllStat(csEmpty)  && !IO_GetX(xPRB_PkgIn) ;
...
//Check Cycle Time Out.
    ...
    switch (Step.iCycle) {
        ...
        case  13: if(IO_GetX(xLDR_PshrOvld)) {
                      MoveActr(aiLDR_PshFwBw , ccBwd);
                  ...
                  if(!MoveActr(aiLDR_PshFwBw, ccFwd)) return false;
                  ...
                  if(IO_GetX(xPRB_PkgIn)){
                      ...
                      if(OM.CmnOptn.bEmptyStrpCnt){
                          m_iEmptyStrpCnt++;
                          if(m_iEmptyStrpCnt >= OM.CmnOptn.iEmptyStrpCnt){
                              m_iEmptyStrpCnt = 0;
                              DM.ARAY[riLDR].SetStat(csEmpty);
                          }
                      }
                  }  //연속으로 빈 슬롯 일 때 배출 하는 파트 추가..
...
//                  DM.ShiftArrayData(riLDR,riPRB);
        ...
        case  14: if(!MoveActr(aiLDR_PshFwBw, ccBwd)) return false;
                  //if(!IO_GetX(xPRB_PkgIn)) return false ;
