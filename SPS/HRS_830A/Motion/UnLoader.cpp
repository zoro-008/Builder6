...
bool CUnLoader::CheckSafe(EN_MOTR_ID _iMotr , EN_PSTN_ID _iPstnId)
{
    ...
    bool isBwPusher        = AT_Complete(aiPSB_PshFwBw , ccBwd) ;
    bool isRailGateCnsr    = IO_GetX(xPSB_PkgOut) ; //IO_GetX(xPSB_PkgOut1) || IO_GetX(xPSB_PkgOut2) ;
...
bool CUnLoader::CycleHome()
{
    ...
    switch (Step.iHome) {
        ...
        case 10: //MT_Reset(miULD_ZClmp);
                 ...
                 PSB.MoveActr(aiPSB_PshFwBw,ccBwd);
        ...
        case 11: //if(!MoveActr(aiULD_Cmp,ccBwd)) return false ;
                 if(!PSB.MoveActr(aiPSB_PshFwBw,ccBwd)) return false ;
...
String CUnLoader::GetNextStrpLotNo()
{
    ...
    if(!DM.ARAY[riRWK].CheckAllStat(csNone)) return  DM.ARAY[riRWK].GetLotNo() ;
    if(!DM.ARAY[riLWK].CheckAllStat(csNone)) return  DM.ARAY[riLWK].GetLotNo() ;
    if(!DM.ARAY[riPRB].CheckAllStat(csNone)) return  DM.ARAY[riPRB].GetLotNo() ;
    if(!DM.ARAY[riLDR].CheckAllStat(csNone)) return  DM.ARAY[riLDR].GetLotNo() ;
...
String CUnLoader::GetNextStrpID()
{
    ...
    if(!DM.ARAY[riRWK].CheckAllStat(csNone)) return  DM.ARAY[riRWK].GetID() ;
    if(!DM.ARAY[riLWK].CheckAllStat(csNone)) return  DM.ARAY[riLWK].GetID() ;
    if(!DM.ARAY[riPRB].CheckAllStat(csNone)) return  DM.ARAY[riPRB].GetID() ;
    if(!DM.ARAY[riLDR].CheckAllStat(csNone)) return  DM.ARAY[riLDR].GetID() ;
...
bool CUnLoader::Autorun (void) //오토런닝시에 계속 타는 함수.
{
    ...
    if (Step.iSeq == 0) {
        ...
        if(DM.ARAY[riULD].CheckAllStat(csNone) && !IO_GetX(xULD_MgzIn) && Stat.bChckSplyEmpty &&
          (DM.ARAY[riPSB].GetCntExist() || DM.ARAY[riRWK].GetCntExist() || DM.ARAY[riLWK].GetCntExist()) && !OM.MstOptn.bDryRun ) EM_SetErr(eiULD_NeedMgz  ) ;
...
bool CUnLoader::ToStart(void) //스타트를 하기 위한 함수.
{
    ...
    switch (Step.iToStart) {
        ...
        case 10: PSB.MoveActr(aiPSB_PshFwBw,ccBwd);
                 Step.iToStart ++ ;
                 return false ;

        case 11: if(!PSB.MoveActr(aiPSB_PshFwBw,ccBwd)) return false ;
                 Step.iToStart = 0 ;
                 return true ;
    }
}
...
//Check Cycle Time Out.
    ...
    switch (Step.iCycle) {
        ...
        case  10: MoveActr(aiULD_CmpUpDn , ccFwd );
                  MoveMotr(miULD_YClmp   , piULD_YClmpWork) ;
                  IO_SetY(yULD_MgzInAC,true);
                  Step.iCycle++;
                  return false;

        case  11: if(!MoveActr(aiULD_CmpUpDn    , ccFwd  )) return false ;
        ...
        case  15: if(!MoveActr(aiULD_CmpUpDn , ccBwd)) return false ;
                  //IO_SetY(yULD_MgzInAC,true);
...
//Check Cycle Time Out.
    ...
    switch (Step.iCycle) {
        ...
        case  14: if(!MoveMotr(miULD_ZClmp , piULD_ZClmpClampOff)) return false ;
                  MoveActr(aiULD_CmpUpDn , ccFwd);
                  DM.ARAY[riULD].ClearMap();
                  Step.iCycle++;
                  return false ;

        case  15: if(!MoveActr(aiULD_CmpUpDn , ccFwd)) return false ;
        ...
        case  18: if(!m_tmDelay.OnDelay(true , 2000)) return false;
                  IO_SetY(yULD_MgzOutAC , false);
                  if(!MoveMotr(miULD_ZClmp , piULD_ZClmpPickFwd)) return false ;
                  Step.iCycle++;
                  return false ;

                  /////////////////
        case  19: if(IO_GetX(xULD_MgzOutFull)) {
                      IO_SetY(yULD_MgzOutAC,false);
...
bool CUnLoader::CheckStop()
{
    ...
    if(!AT_Done(aiULD_CmpUpDn   )) return false ;
