...
bool CRailDie::FindChip(EN_ARAY_ID _riWhre , int &r , int &c )
{
    ...
    if(_riWhre == riRAD) { //X Idx
             if(DM.ARAY[riRAD].GetCntStat(csWait )) DM.ARAY[riRAD].FindLastColRow(csWait  , r , c) ;
        else if(DM.ARAY[riRAD].GetCntStat(csMask )) DM.ARAY[riRAD].FindLastColRow(csMask  , r , c) ;
        else if(DM.ARAY[riRAD].GetCntStat(csWait2)) DM.ARAY[riRAD].FindLastColRow(csWait2 , r , c) ;
        else if(DM.ARAY[riRAD].GetCntStat(csUnkwn)){
            c = DM.ARAY[riRAD].FindLastCol(csUnkwn);
            if(c % 2)DM.ARAY[riRAD].FindLastColRow    (csUnkwn , r , c) ;
            else     DM.ARAY[riRAD].FindLastColFrstRow(csUnkwn , r , c) ;
        }
        else return false;
...
//    else if(_riWhre == riWRD) { //Y Ins
//             if(DM.ARAY[riWRD].GetCntStat(csWait )) DM.ARAY[riWRD].FindLastColRow(csWait,r,c);
//        else if(DM.ARAY[riWRD].GetCntStat(csMask )) DM.ARAY[riWRD].FindLastColRow(csMask,r,c);
//        else if(DM.ARAY[riWRD].GetCntStat(csUnkwn)) {
//            c = DM.ARAY[riWRD].FindLastCol(csUnkwn);
//            if(c % 2)DM.ARAY[riWRD].FindLastColRow    (csUnkwn , r , c) ;
//            else     DM.ARAY[riWRD].FindLastColFrstRow(csUnkwn , r , c) ;
//        }
//        else {
//            r= 0 ; c = 0 ;
//        }
...
double CRailDie::GetWorkPos(EN_MOTR_ID _iMotr)
{
    ...
    if(_iMotr == miWRD_XIdx){
        if(FindChip(riRAD,r,c)){
            c = OM.DevInfo.iColCnt - c - 1 ;
            dWorkPos = PM.GetValue (_iMotr , pvWRD_XIdxWorkStt ) + ( OM.DevInfo.dColPitch * c ) ;

            if(DM.ARAY[riRAD].GetStep() >1 ) {
                for(int i =0 ; i < DM.ARAY[riRAD].GetStep() - 1 ; i++) dReGrip += OM.DevOptn.iWRDRgDis[i] * OM.DevInfo.dColPitch;
                dWorkPos -= dReGrip ;
            }
        }
    }
...
bool CRailDie::CheckSafe(EN_MOTR_ID _iMotr , EN_PSTN_ID _iPstnId)
{
    ...
    if(!bRet){
        ...
        if(!Step.iCycle && !Step.iToStart )FM_MsgOk(MT_GetName(_iMotr).c_str(),sMsg);
    }
...
bool CRailDie::CycleHome()     //sun DLL direct access.
{
    ...
    switch (Step.iHome) {
        ...
        case 10: MT_SetHomeEnd(miWRD_XIdx,false);
                 ...
                 bWRDNone = DM.ARAY[riRAD].CheckAllStat(csNone) ;
                 Step.iHome++;
                 return false ;
...
bool CRailDie::ToStart(void) //스타트를 하기 위한 함수.
{
    ...
    switch (Step.iToStart) {
        ...
        case 10: bNone = DM.ARAY[riRAD].CheckAllStat(csNone);
                 if(!bNone) {
                     Step.iToStart = 15 ;
                     return false ;
                 }
        ...
        case 13: if(!MoveMotr(miWRD_XIdx,piWRD_XIdxWait)) return false ;
                 DM.ARAY[riRAD].ChangeStat(csMask , csWait);
...
bool CRailDie::ToStop(void) //스탑을 하기 위한 함수.
{
    ...
    switch (Step.iToStop) {
        ...
        case 10: //DM.ARAY[riRAD].ChangeStat(csWait,csUnkwn);
...
bool CRailDie::Autorun(void) //오토런닝시에 계속 타는 함수.
{
    ...
    DM.ARAY[riRAD].SetDispItem(csNone   ,"자제없음"   ) ; DM.ARAY[riWRD].SetDispItem(csNone   ,"자제없음"   ) ;
    DM.ARAY[riRAD].SetDispItem(csUnkwn  ,"작업전"     ) ; DM.ARAY[riWRD].SetDispItem(csUnkwn  ,"작업전"     ) ;
    DM.ARAY[riRAD].SetDispItem(csWait   ,"BF비전대기" ) ; DM.ARAY[riWRD].SetDispItem(csRslt0  ,"방향불량"   ) ;
    DM.ARAY[riRAD].SetDispItem(csMask   ,"칩놓기대기" ) ; DM.ARAY[riWRD].SetDispItem(csRslt1  ,"에폭시불량" ) ;
    DM.ARAY[riRAD].SetDispItem(csWait2  ,"AT비전대기" ) ; DM.ARAY[riWRD].SetDispItem(csRslt2  ,"칩위치불량" ) ;
    DM.ARAY[riRAD].SetDispItem(csWork   ,"작업끝"     ) ; DM.ARAY[riWRD].SetDispItem(csRslt3  ,"기준점불량" ) ;
        ...
        //Check
        int  iCNow     = DM.ARAY[riRAD].FindLastCol(csUnkwn) ;
        int  iCRvs     = OM.DevInfo.iColCnt - iCNow ;

        bool isVisnZone   = PM.GetValue(miWRD_YHed , pvWRD_YHedStripWait) < MT_GetEncPos(miWRD_YHed) ;

        int  iReGribStep  = DM.ARAY[riRAD].GetStep() ;
        bool bAllNone     = DM.ARAY[riRAD].CheckAllStat(csNone ) ;
        ...
        bool isWorkChip1     = ( DM.ARAY[riRAD].GetCntStat(csUnkwn) ||  DM.ARAY[riRAD].GetCntStat(csMask) ||  DM.ARAY[riRAD].GetCntStat(csWait)) ; //졸려서 대충 때운거.
        bool isWorkChip2     = ( DM.ARAY[riRAD].GetCntStat(csUnkwn) && !DM.ARAY[riRAD].GetCntStat(csMask) && !DM.ARAY[riRAD].GetCntStat(csWait)) ;

        bool isVisnAtGrib   = DM.ARAY[riRAD].GetCntStat(csMask) && PM.GetValue(miWRD_YHed,pvWRD_YHedStripWait) <= MT_GetCmdPos(miWRD_YHed) ;
        ...
        //Judgment
        bool isCycleWorkStt =  DM.ARAY[riRAD].CheckAllStat(csUnkwn) && iReGribStep == 0 && isPSBWorkSttCon ; // Unkwn -> Wait
        bool isCycleReGrip  = !bAllNone &&  iReGribStep > 0 && bReGrip ;//거의 안씀.                  //
        bool isCycleAtVisn  =  DM.ARAY[riRAD].GetCntStat(csWait) && isVisnZone ;
        bool isCycleBfVisn  =  DM.ARAY[riRAD].GetCntStat(csWait) && isVisnZone ;
...
//        bool isVisnZone      = PM.GetValue(miWRD_YHed , pvWRD_YHedStripWait)-0.2 < MT_GetEncPos(miWRD_YHed) ;
        ...
        bool bCheckXIdxPos   = MT_CmprPos(miWRD_XIdx,GetWorkPos(miWRD_XIdx)) ;
        bool bCheckYIdxPos   = MT_CmprPos(miWRD_YIns,GetWorkPos(miWRD_YIns)) ;
        bool bNeedWork       = DM.ARAY[riRAD].GetCntStat(csUnkwn) && //자제 넣고 스타트 하면 csWait로 되지 않아 홀드 되는 것 때문에 추가.
                              !DM.ARAY[riRAD].GetCntStat(csWait ) &&
                              !DM.ARAY[riRAD].GetCntStat(csWait2) &&
                              !DM.ARAY[riRAD].GetCntStat(csMask ) ;

        int  iReGribStep     = DM.ARAY[riRAD].GetStep() ;
        bool bExstUnknwn     = DM.ARAY[riRAD].GetCntStat(csUnkwn ) ;
        bool bAllNone        = DM.ARAY[riRAD].CheckAllStat(csNone ) ;
        bool bPSBNone        = DM.ARAY[riRAP].CheckAllStat(csNone ) ;
        bool isPSBWorkSttCon = DM.ARAY[riRAP].GetStep() >= 1 || bPSBNone ;
        bool isWorkPsbCon    = bPSBNone ||

        !(iCRvs >= OM.DevOptn.iWRD_XWkCon && DM.ARAY[riRAP].GetStep() >= 1 ) ;
//        !(iCRvs >= OM.DevOptn.iWRD_XWkCon && DM.ARAY[riRAP].GetStep() == 2 ) ||
        bool bReGrip         = iReGribStep > 0 &&  iCRvs >= OM.DevOptn.iWRDRgCol[iReGribStep-1] && iReGribStep < OM.DevOptn.iWRD_XRgCnt + 1 ;

        //Judgment
        //첫위치 찾아가는놈.
        bool isCycleWorkStt =  DM.ARAY[riRAD].CheckAllStat(csUnkwn) && iReGribStep == 0 && isPSBWorkSttCon ;
        //리그립 하는놈.
        bool isCycleReGrip  = !bAllNone &&  iReGribStep > 0 && bReGrip ;//거의 안씀.
        ...
        //AT 비전 검사.
        bool isCycleAtVisn  =  DM.ARAY[riRAD].GetCntStat(csWait2) && isVisnZone ;
        //BF 비전 검사.
        bool isCycleBfVisn  =  DM.ARAY[riRAD].GetCntStat(csWait ) && isVisnZone ;
        //배출.
        bool isCycleWorkEnd = !bAllNone && DM.ARAY[riRAD].CheckAllStat(csWork) && DM.ARAY[riRAP].CheckAllStat(csNone);
        //종료.
        bool isConEnd       =  bAllNone ;//&&DM.ARAY[riWRE].GetCntStat(csWork) ;
...
//Check Cycle Time Out.
    ...
    if (m_tmCycle.OnDelay(Step.iCycle && Step.iCycle == PreStep.iCycle && CheckStop() && !OM.MstOptn.bDebugMode, 5000 )) {
        ...
        Trace(m_sPartName.c_str(),sTemp.c_str());
//        DM.ARAY[riRAD].ChangeStat(csWait,csUnkwn);
        Step.iCycle = 0 ;
        return true;
    }
    ...
    switch (Step.iCycle) {
        ...
        case  10: MoveActr(aiWRD_Clamp , ccBwd);
                  MoveActr(aiWRD_CAlign, ccBwd);
                  Step.iCycle++;
                  return false;
...
//                  if(!MoveActr(aiWRD_Align, ccBwd)) return false ;
        ...
        case  14: if(!MoveMotr(miWRD_XIdx , piWRD_XIdxWorkStt )) return false ;
                  ...
                  DM.ARAY[riRAD].SetStep(1);
                  Step.iCycle++;
                  return false ;

        case  15: if(!MoveActr(aiWRD_CAlign, ccFwd)) return false ;

                  FindChip(riRAD,r,c);
                  if(OM.MstOptn.bIgnrDieAttach) {
                      for (int i= 0; i<OM.DevInfo.iRowCnt; i++){
                          DM.ARAY[riRAD].SetStat(i,c,csWork);
                          Step.iCycle = 0 ;
                          return true ;
                      }
                  }
                  ...
                  for (int i= 0; i<OM.DevInfo.iRowCnt; i++){
                      if(DM.ARAY[riWRD].GetStat(i,c) == csRslt0) DM.ARAY[riRAD].SetStat(i,c,csWork);
                  }
//                  FindChip(riWRD,r1,c1);
                  if(DM.ARAY[riWRD].GetStat(r,c) == csUnkwn) { //Start Position Up
                      DM.ARAY[riRAD].SetStat(r,c,csWait);
                  }
...
//Check Cycle Time Out.
    ...
    switch (Step.iCycle) {
        ...
        case  10: dReGrip  = 0.0 ;
                  dReGrip  = OM.DevOptn.iWRDRgDis[DM.ARAY[riRAD].GetStep()-1] * OM.DevInfo.dColPitch;
        ...
        case  11: if(!MoveActr(aiWRD_Clamp  , ccBwd)) return false ;
                  ...
                  MT_GoIncRun(miWRD_XIdx,-dReGrip); //충돌 체크 안됨.
                  Step.iCycle++;
                  return false;
        ...
        case  14: if(!MoveActr(aiWRD_CAlign, ccFwd)) return false ;
                  DM.ARAY[riRAD].SetStep(DM.ARAY[riRAD].GetStep()+1);
...
//        DM.ARAY[riRAD].ChangeStat(csWait,csUnkwn);
    ...
    switch (Step.iCycle) {
        ...
        case  13: if(!m_tmTemp.OnDelay(true , OM.DevOptn.iRalAtVsBfDly)) return false ;
                  VSM.Grab(viRalAt);  //이거 그립 딜레이 _bInsYReady
                  m_tmTemp.Clear() ;
                  Step.iCycle++;
                  return false ;

        case  14: if(m_tmTemp.OnDelay(!VSM.GetGrabEnd(viRalAt),OM.DevOptn.iRalAtVsGrabTO )) {
        ...
        case  15: if(!OM.MstOptn.bDryRun) VSM.Inspect(viRalAt,false); //쓰레드로 빼던가 나중에 ...
                  Step.iCycle++;
                  return false ;

        case  16: if(!MoveActr(aiWRD_CAlign  , ccBwd)) return false ;
                  //비전에 따라 딜레이 밑으로 빼던가 다른데로 빼던가.
                  m_tmTemp.Clear() ;
                  Step.iCycle++;
                  return false ;

        case  17: if(m_tmTemp.OnDelay(!VSM.GetInspEnd(viRalAt),OM.DevOptn.iRalAtVsInspTO )) {
                      EM_SetErr(eiHED_RalAtVsInspFail);
                      Step.iCycle = 0 ;
                      return true ;
                  }
                  if(!OM.MstOptn.bDryRun && !VSM.GetInspEnd(viRalAt)) return false ;


                  FindChip(riRAD,r,c);

                  if(!OM.MstOptn.bDryRun)VSM.GetRslt(viRalAt,&RsltAt);
                  else {
                      RsltAt.bRslt  = true ;
                      RsltAt.iRet   = 0 ;
                      RsltAt.fRsltX = 0.0 ;
                  ...
                  if(!RsltAt.bRslt){
                      ...
                      RsltAt.iRet == 0 ? sTemp = "GOOD" : sTemp = "NG" ;
                      sTemp += AnsiString(" X=")+ RsltAt.fRsltX + AnsiString(" Y=") + RsltAt.fRsltY + AnsiString(" T=") + RsltAt.fRsltT ;
                      Trace("AfterVisn",sTemp.c_str());

                      if(RsltAt.iRet == 0 ) { //Good
                          DM.ARAY[riWRD].SetStat(r,c,csWork);
                          DM.ARAY[riRAD].SetStat(r,c,csWork);
                          iVisnFailCnt = 0 ;
                      }
                      else if(RsltAt.iRet == 1 ) { //칩크기 불량.
                          DM.ARAY[riWRD].SetStat(r,c,csRslt2);
                          DM.ARAY[riRAD].SetStat(r,c,csWork);
                          iVisnFailCnt++;
                      }
                      else if(RsltAt.iRet == 2 ) { //자제 안착 오프셑 불량.
                          DM.ARAY[riWRD].SetStat(r,c,csRslt2);
                          DM.ARAY[riRAD].SetStat(r,c,csWork);
                          iVisnFailCnt++;
                      }
                  ...
                  Trace("AT VISION",("X=" + AnsiString(RsltAt.fRsltX)).c_str() );
                  Trace("AT VISION",("Y=" + AnsiString(RsltAt.fRsltY)).c_str() );
                  Trace("AT VISION",("T=" + AnsiString(RsltAt.fRsltT)).c_str() );


                  SPCS.WriteAtVsDataCsv(&DM.ARAY[riRAD],&RsltAt);
...
//    static int iVisnFailCnt = 0 ; //제거 에폭시 불량은 그냥 에러 띄우고 버튼으로 눌러서 패스 할지 한다.
    ...
    switch (Step.iCycle) {
        ...
        case  14: if(m_tmTemp.OnDelay(!VSM.GetGrabEnd(viRalBf),OM.DevOptn.iRalBfVsGrabTO )) {
                  ...
                  if(!OM.MstOptn.bDryRun)VSM.Inspect(viRalBf,false); //쓰레드로 빼던가 나중에 ...
                  //비전에 따라 딜레이 밑으로 빼던가 다른데로 빼던가.
                  m_tmTemp.Clear() ;
                  Step.iCycle++;
                  return false ;

        case  15: if(m_tmTemp.OnDelay(!VSM.GetInspEnd(viRalBf),OM.DevOptn.iRalBfVsInspTO )) {
                      EM_SetErr(eiHED_RalBfVsInspFail);
                      //m_bEpxFailIgnr = false ;
                      Step.iCycle = 0 ;
                      return true ;
                  }
                  if(!OM.MstOptn.bDryRun && !VSM.GetInspEnd(viRalBf)) return false ;

                  FindChip(riRAD,r,c);


                  if(!OM.MstOptn.bDryRun)VSM.GetRslt(viRalBf,&RsltBf);
                  else {
                      RsltBf.bRslt  = true ;
                      RsltBf.iRet   = 0 ;
                      RsltBf.fRsltX = 0.0 ;
                  ...
                  Trace("BF VISION",("X=" + AnsiString(RsltBf.fRsltX)).c_str() );
                  Trace("BF VISION",("Y=" + AnsiString(RsltBf.fRsltY)).c_str() );
                  Trace("BF VISION",("T=" + AnsiString(RsltBf.fRsltT)).c_str() );
                  ...
                  if(!RsltBf.bRslt){
                      ...
                      if(RsltBf.iRet == 0 ) {
                          DM.ARAY[riRAD].SetStat(r,c,csMask);
                          if( fabs(RsltBf.fRsltX) > (OM.DevInfo.dWf_BldOfs + OM.DevInfo.dWf_Width )/2 ||
                              fabs(RsltBf.fRsltY) > (OM.DevInfo.dWf_BldOfs + OM.DevInfo.dWf_Height)/2 ||
                              fabs(RsltBf.fRsltT) > OM.CmnOptn.dHedTLmtErr ) {
                              RsltBf.fRsltX = 0.0 ;
                              RsltBf.fRsltY = 0.0 ;
                              RsltBf.fRsltT = 0.0 ;
...
//                          iVisnFailCnt = 0 ;
                      ...
                      //이건 안날라옴.
                      //else if(RsltBf.iRet == 1 ) { //기준점 못찾음.
                      //    //EM_SetErr(eiHED_RalAtVsThchFail);
                      //    DM.ARAY[riWRD].SetStat(r,c,csRslt3);
                      //    DM.ARAY[riRAD].SetStat(r,c,csWork);
                      //    Step.iCycle = 0 ;
                      //    return true ;
                      //}
                      else if(RsltBf.iRet == 2 ) { //에폭시 불량.
...
//                              iVisnFailCnt++;
                              ...
                              DM.ARAY[riWRD].SetStat(r,c,csRslt1);
                              EM_SetErr(eiWRD_RalBfVsFailCnt);
//                              iVisnFailCnt = 0 ;
...
//                  DM.ARAY[riPSB].SetLotNo(DM.ARAY[riRAD].GetLotNo());
//                  DM.ARAY[riPSB].SetID   (DM.ARAY[riRAD].GetID   ());

                  DM.ARAY[riRAP].SetLotNo(DM.ARAY[riRAD].GetLotNo());
                  DM.ARAY[riRAP].SetID   (DM.ARAY[riRAD].GetID   ());



                  DM.ARAY[riRAD].SetStat(csNone );
                  DM.ARAY[riRAP].SetStat(csUnkwn);



//                  DM.ARAY[riPSB].ChangeStat(csGood,csUnkwn);
...
/*
bool CRailDie::CycleVisnAtGrib(void)
{
//Check Cycle Time Out.
    AnsiString sTemp ;
    if (m_tmCycle.OnDelay(Step.iCycle && Step.iCycle == PreStep.iCycle && CheckStop() && !OM.MstOptn.bDebugMode, 5000 )) {
    ...
    switch (Step.iCycle) {
        ...
        case  10: if(OM.CmnOptn.bIgnrRalAtVs ) DM.ARAY[riRAD].ChangeStat(csMask,csWork);
        ...
        case  11: if(m_tmTemp.OnDelay(!VSM.GetGrabEnd(viRalAt),OM.DevOptn.iRalAtVsGrabTO )) {
                      EM_SetErr(eiHED_RalAtVsGrabFail);
                      Step.iCycle = 0 ;
                      return true ;
                  }
                  if(!VSM.GetGrabEnd(viRalAt)) return false ;
                  VSM.Inspect(viRalAt,false); //쓰레드로 빼던가 나중에 ...
                  //비전에 따라 딜레이 밑으로 빼던가 다른데로 빼던가.
                  m_tmTemp.Clear() ;
                  Step.iCycle++;
                  return false ;

        case  12: if(m_tmTemp.OnDelay(!VSM.GetInspEnd(viRalAt),OM.DevOptn.iRalAtVsInspTO )) {
                      EM_SetErr(eiHED_RalAtVsInspFail);
                      Step.iCycle = 0 ;
                      return true ;
                  }
                  ...
                  if(!RsltAt.bRslt){
                      ...
                      if(DM.ARAY[riRAD].GetCntStat(csMask)) {
                          if(RsltAt.iRet == 0 ) {
                              FindChip(riRAD,r,c) ;
                              DM.ARAY[riWRD].SetStat(r,c,csWork);
                              DM.ARAY[riRAD].SetStat(r,c,csWork);
