...
#define MAX_ARRAY_COL  256              //Default Col Size of Tray. X.
#define MAX_ARRAY_ROW  MAX_ARRAY_COL    //Default Row Size of Tray. Y.

extern TColor     clStat   [MAX_CHIP_STAT];
//extern AnsiString sMenuName[MAX_CHIP_STAT];

typedef void (__closure*CallbackAray)(int _iCol ,int _iRow , TShiftState Shift);
...
class CArray
{
    ...
    private:
        ...
        //부득이 하게 추가하게 되엇당...맵을 누르면 웨이퍼를 움직여 달라능...
        //int m_iRtUpX    ;
        //int m_iRtUpY    ;
        //int m_iRtUpS    ;
        ...
        void __fastcall ImageArayHint (int X, int Y);
    ...
    public:
        //int __fastcall GetRtUpX(){return m_iRtUpX ;}
        //int __fastcall GetRtUpY(){return m_iRtUpY ;}
        //int __fastcall GetRtUpS(){return m_iRtUpS ;}
        String __fastcall GetName(){ return m_sName; }

        CallbackAray CallAray;
        bool __fastcall CallBackReg(CallbackAray _CallbackKey);
        ...
        void __fastcall SetStat( int r , int c , EN_CHIP_STAT _iStat);
        void __fastcall SetStat(EN_CHIP_STAT _iStat);
        void __fastcall ChangeStat(EN_CHIP_STAT _iFrom , EN_CHIP_STAT _iTo) { for(int r = 0 ; r < m_iMaxRow ; r++) for(int c = 0 ; c < m_iMaxCol ; c++) if(CHPS[r][c].GetStat() == _iFrom) CHPS[r][c].SetStat(_iTo);}//sun UpdateAray();

        void __fastcall SetMaxColRow (int c,int r) { m_iMaxCol = c; m_iMaxRow = r; /*UpdateAray();sun*/}
        void __fastcall SetMaxRow    (int n      ) { m_iMaxRow = n     ;/*UpdateAray();sun*/}
        void __fastcall SetMaxCol    (int n      ) { m_iMaxCol = n     ;/*UpdateAray();sun*/}
        ...
        int  __fastcall FindFrstRow       (EN_CHIP_STAT FindMode                   ); int  __fastcall FindFrstRow       (EN_CHIP_STAT FindMode                   , int sr , int er , int sc , int ec);
        int  __fastcall FindFrstCol       (EN_CHIP_STAT FindMode                   ); int  __fastcall FindFrstCol       (EN_CHIP_STAT FindMode                   , int sr , int er , int sc , int ec);
        int  __fastcall FindLastRow       (EN_CHIP_STAT FindMode                   ); int  __fastcall FindLastRow       (EN_CHIP_STAT FindMode                   , int sr , int er , int sc , int ec);
        int  __fastcall FindLastCol       (EN_CHIP_STAT FindMode                   ); int  __fastcall FindLastCol       (EN_CHIP_STAT FindMode                   , int sr , int er , int sc , int ec);
        bool __fastcall FindFrstRowCol    (EN_CHIP_STAT FindMode , int &r , int &c ); bool __fastcall FindFrstRowCol    (EN_CHIP_STAT FindMode , int &r , int &c , int sr , int er , int sc , int ec);
        bool __fastcall FindFrstColRow    (EN_CHIP_STAT FindMode , int &r , int &c ); bool __fastcall FindFrstColRow    (EN_CHIP_STAT FindMode , int &r , int &c , int sr , int er , int sc , int ec);
        bool __fastcall FindLastRowCol    (EN_CHIP_STAT FindMode , int &r , int &c ); bool __fastcall FindLastRowCol    (EN_CHIP_STAT FindMode , int &r , int &c , int sr , int er , int sc , int ec);
        bool __fastcall FindFrstRowLastCol(EN_CHIP_STAT FindMode , int &r , int &c ); bool __fastcall FindFrstRowLastCol(EN_CHIP_STAT FindMode , int &r , int &c , int sr , int er , int sc , int ec);
        bool __fastcall FindLastRowFrstCol(EN_CHIP_STAT FindMode , int &r , int &c ); bool __fastcall FindLastRowFrstCol(EN_CHIP_STAT FindMode , int &r , int &c , int sr , int er , int sc , int ec);
        bool __fastcall FindLastColFrstRow(EN_CHIP_STAT _iStat   , int &r , int &c ); bool __fastcall FindLastColFrstRow(EN_CHIP_STAT _iStat   , int &r , int &c , int sr , int er , int sc , int ec);
        bool __fastcall FindFrstColLastRow(EN_CHIP_STAT FindMode , int &r , int &c ); bool __fastcall FindFrstColLastRow(EN_CHIP_STAT FindMode , int &r , int &c , int sr , int er , int sc , int ec);
        bool __fastcall FindLastColRow    (EN_CHIP_STAT FindMode , int &r , int &c ); bool __fastcall FindLastColRow    (EN_CHIP_STAT FindMode , int &r , int &c , int sr , int er , int sc , int ec);

        inline bool __fastcall CheckIndex      (int sr , int er , int sc , int ec) ;
...
//        bool __fastcall FindFrstColLastRow_Indx (EN_CHIP_STAT FindMode , int iStrCol , int iEndCol , int &R , int &C);
        ...
        //Loading Para.
        //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        void __fastcall Load   (bool IsLoad , AnsiString _sPath , AnsiString _sIniPath , int _iNo);

        //binary 이용 않고 ini로만 로드 세이브.
        void __fastcall LoadIni(bool IsLoad , AnsiString _sFilePath);
        ...
        void   __fastcall SetTemp(int r , int c , int _iIdx , double _dVal);
        double __fastcall GetTemp(int r , int c , int _iIdx);
        void   __fastcall ClearTemp();
