...
bool CUnLoader::GetLastStrip()
{
    ...
    bool bLotEnd = (OM.CmnOptn.iLotEndChipCnt && EPX.GetEpxLotCnt() >= OM.CmnOptn.iLotEndChipCnt) &&
                    DM.ARAY[riRAD].CheckAllStat(csNone) && //헤드구간 보는게 조오금 ...모랄까? 흠...
...
bool CUnLoader::Autorun(void) //오토런닝시에 계속 타는 함수.
{
    ...
    if (Step.iSeq == 0) {
        ...
        bool isStripNone    = DM.ARAY[riRAD].CheckAllStat(csNone) && DM.ARAY[riRAP].CheckAllStat(csNone) ;
...
//                              ( DM.ARAY[riULD].GetLotNo()!="" && DM.ARAY[riULD].GetID().ToIntDef(9999)/100 != DM.ARAY[riRAD].GetID().ToIntDef(9999)/100 && !DM.ARAY[riRAD].CheckAllStat(csNone) && DM.ARAY[riRAP].CheckAllStat(csNone)) ;
        ...
        bool isCyclePick    =  DM.ARAY[riULD].CheckAllStat(csNone ) && !isMgzDetected && !isStripNone &&
                             ((EPX.GetEpxLotCnt() <= OM.CmnOptn.iLotEndChipCnt && (!DM.ARAY[riRAD].CheckAllStat(csNone ) || !DM.ARAY[riRAP].CheckAllStat(csNone ))) || !OM.CmnOptn.iLotEndChipCnt );
        ...
        bool isCyclePlace   =  DM.ARAY[riULD].CheckAllStat(csWork ) ||
                             ...
                             //((DM.ARAY[riULD].GetCntStat  (csWork ) ||  DM.ARAY[riULD].GetCntStat(csEmpty)) &&  isStripNone && LDR.GetStat().bWorkEnd) ;
                              (DM.ARAY[riULD].GetCntStat  (csWork ) &&  isStripNone && LDR.GetStat().bWorkEnd) ;
                              //TODO :: 에폭시 카운트로 어쩌고 저쩌고 할 차례다.
        bool isConEnd       = (DM.ARAY[riULD].CheckAllStat(csNone ) &&  isStripNone );//&& LDR.GetStat().bWorkEnd ) || (EPX.GetEpxLotCnt() <= OM.CmnOptn.iLotEndChipCnt && (!DM.ARAY[riRAD].CheckAllStat(csNone ) || !DM.ARAY[riRAP].CheckAllStat(csNone )));
...
//                  MoveMotr(miULD_YCmp , piULD_YCmpWait   ); //확인 17
        ...
        case  18: //if(!MT_GetHomeEnd(miULD_YCmp)) return false ;
//                  if(!MoveMotr(miULD_YCmp   , piULD_YCmpWait)) return false ;
