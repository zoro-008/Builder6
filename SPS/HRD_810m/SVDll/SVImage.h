...
class IImage {
    ...
    public :
        ...
        //외부에서 더블버퍼링 할때
        //Scale
        virtual int __stdcall ShowImg      (HDC _hHdc , int _x, int _y, int _iWidth, int _iHeight)=0;
        virtual int __stdcall ShowImgGrey  (HDC _hHdc , int _x, int _y, int _iWidth, int _iHeight)=0;
        virtual int __stdcall ShowImgFalse (HDC _hHdc , int _x, int _y, int _iWidth, int _iHeight)=0;
        virtual int __stdcall ShowImgColor (HDC _hHdc , int _x, int _y, int _iWidth, int _iHeight)=0;
        virtual int __stdcall ShowImgBinary(HDC _hHdc , int _x, int _y, int _iWidth, int _iHeight , int _iThreshold )=0;
        virtual int __stdcall ShowImgR     (HDC _hHdc , int _x, int _y, int _iWidth, int _iHeight)=0;
        virtual int __stdcall ShowImgG     (HDC _hHdc , int _x, int _y, int _iWidth, int _iHeight)=0;
        virtual int __stdcall ShowImgB     (HDC _hHdc , int _x, int _y, int _iWidth, int _iHeight)=0;
        
        virtual bool __stdcall SetSize   (int _iWidth , int _iHeight , int _iBitCnt)=0;
        
        //화면에 이미지만 다이렉트로 그릴경우 더블버퍼링 지원 함수.
        virtual HBITMAP __stdcall GetHBitmap(HDC _hHdc)=0;
        virtual void    __stdcall ShowBitmap(HDC _hHdc, int _x, int _y, int _iWidth, int _iHeight)=0;
