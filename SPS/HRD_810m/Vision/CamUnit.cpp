...
bool CImi::Init()
{
    ...
    */
    ...
    SetTrgMode(tmSw);//밑에가 이상해서 하나 더 넣어봄.

    ImCamStart(Para.iPhysicalNo);

    SetTrgMode(tmSw);//이게 이상하게 처음 켤때 안먹을 때가 있어서...
    ...
    if (ImCamGetTriggerInfo(Para.iPhysicalNo, &TrigInfo) != IMC_SUCCESS ) {
    	ShowMessage("Failed ImCamGetTriggerInfo Function!.");
    	return false;
    }
    return true ;
}
...
bool CImi::SetTrgMode(EN_TRG_MODE _iMode)
{
    ...
    //ImCamGetTriggerMode(Para.iPhysicalNo, &TrigProp);

    if( _iMode == tmSw) {
	TrigProp.bTriggerOn = true;
	TrigProp.nTriggerSource = 7;		// SW trigger

        //TrigProp.bTriggerPolarity = 1;
        //TrigProp.nTriggerMode     = 0;
        //TrigProp.nTriggerParam    = 0;
        //TrigProp.nExposureTime    = 10;
        //TrigProp.nExposureTimeInterval = 0 ;
    }
    else if (_iMode == tmHw){
        TrigProp.bTriggerOn = true;
    ...
    */
    ...
    if ( ImCamSetTriggerMode(Para.iPhysicalNo, TrigProp) != IMC_SUCCESS ) {
    	...
    	//ShowMessage("Failed ImCamSetTriggerMode Function!..");
    	return false;
    }
    return true ;
}
...
bool CEur::Close()   //여기 부터 하기.... m_Channel 바꾸고 하면 됌.
{
