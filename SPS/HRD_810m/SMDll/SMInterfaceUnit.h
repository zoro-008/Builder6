...
extern "C" {
    ...
    __declspec(dllexport) AnsiString __stdcall FM_GetVer (              ); //버그 있음. 고처야함. 픽스함 2011.11.8 선계원.
    ...
    __declspec(dllexport) void __stdcall MT_GoIncSlow(int _iAxisNo , double _dPos );  //Inc move with AutoRun ManualVel. 모션돈 확인은 GetStopInpos() 함수로.
    ...
    __declspec(dllexport) void __stdcall MT_GoIncAcDc(int _iAxisNo , double _dPos , double _dVel , double _dAcc , double _dDcc);  //Inc move  모션돈 확인은 GetStopInpos() 함수로.
...
//    __declspec(dllexport) void       __stdcall AT_DelCntrlPn  (int _iActrNo                                                );
    ...
    //DIO Class.//
    /////////////
    __declspec(dllexport) void __stdcall IO_SetY  (int _iNo , bool _bVal ,bool _bDirect = false); //Direct = true일경우 1싸이클 동안 엣지는 못본다.
    ...
    __declspec(dllexport) bool       __stdcall EM_GetErr        (int _iNo); //특정 에러 상태 확인.
    ...
    //TowerLamp Class.//
    ////////////////////
    __declspec(dllexport) void       __stdcall TL_MuteSnd       (); //타워램프 소리 끄기.
