...
*/
    ...
    if( OM.CmnOptn.bUseTrimMachine) {
        pnPunchSkip  -> Caption = "PUNCH USE" ;
//        pnPunchSkip  -> Visible = true ;
    }
    else {
        pnPunchSkip  -> Caption = "PUNCH SKIP" ;

//        pnTrimSkip  -> Caption = "REJECT SKIP" ;
//        pnTrimSkip  -> Visible = SEQ._bFlick ;
    }
    ...
    if(OM.CmnOptn.bUseTrimMachine) isUlr = DM.ARAY[riULD].CheckAllStat(csNone ) ;
    else                           isUlr = true ;


    if(PSB.m_bLotEnded && DM.ARAY[riULD].CheckAllStat(csNone) &&
                          isUlr ){ //TODO :: dd ULR 이거 사용여부 미결정.dd
