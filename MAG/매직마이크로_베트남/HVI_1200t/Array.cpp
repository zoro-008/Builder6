...
__fastcall CArray::CArray(void)
{
    ...
    if(bFirst) {
        ...
        clStat[csFail   ] = clFuchsia         ;//csFuchsia         = 28 ,
        clStat[csEror   ] = clRed             ;//csRed             = 29 ,
        clStat[csGood   ] = clLime            ;//csLime            = 30 ,
        clStat[csEtc    ] = clNone            ;//etc               = 31 ,
        clStat[csToBuf  ] = clBlack           ;//clBlack           = 32,
        clStat[csFromBuf] = clBlack           ;//clBlack           = 33,
    ...
    pmStatusSub = new TPopupMenu(Application);
    pmStatusSub -> OwnerDraw = true ;

    for ( int i = 0; i<3; i++){
        miStatusSub[i] = new TMenuItem(Application);
        miStatusSub[i] -> OnDrawItem = MenuDrawItemSub ;

        miStatusSub[i] -> Caption = "Err_Error" + (String)i + "   ";
        pmStatusSub    -> Items -> Insert(i  , miStatusSub[i]);
        pmStatusSub    -> AutoHotkeys = maManual ;
        miStatusSub[i] -> AutoHotkeys = maManual ;
    }
    for( int i = 0 ; i < MAX_STATUS_POPUP_ID ; i++) {
...
void __fastcall CArray::PopupMenuVisible(bool bState)
{
    for(int i = 0 ; i < MAX_STATUS_POPUP_ID ; i++ ) {
        for(int j = 0 ; j < MAX_CHIP_STAT ; j++ ) {
            miStatus[i][j] -> Visible = bState ;
        }
    }
}
...
void __fastcall CArray::ClearDispItem()
{
    ...
    for(int i = 0 ; i < MAX_STATUS_POPUP_ID ; i++ ) {
        for(int j = 0 ; j < MAX_CHIP_STAT ; j++ ) {
            sTemp = (String)i + " , " + (String)j;
            Trace("****BugTest****", sTemp.c_str() );
            sTemp = miStatus[i][j] -> Caption ;
            Trace("****BugTest****", sTemp.c_str() );
...
/*
void __fastcall CArray::SetDispItemRC(EN_CHIP_STAT _iStat , AnsiString _sSubCaption )
{
    bool isAllVisible = true ;
    for(int j = 0 ; j < 3 ; j++ ) {
            if(!miStatus[j] -> Visible) {
                isAllVisible = false ;
                break ;
            }
        }
    ...
    for(int i = 0 ; i < 3; i++ ) {
        miRCStatus[i] -> Visible = true ;
        miRCStatus[i] -> AutoHotkeys = maManual ; //동작 안함....
        pmRCStatus[i] -> AutoHotkeys = maManual ;
        miStatus  [i] -> Caption = _sSubCaption
...
//---------------------------------------------------------------------------
void __fastcall CArray::MenuDrawItemSub(TObject *Sender,TCanvas *ACanvas,const TRect &ARect, bool Selected)
{
    if(Selected) {
        ACanvas->Brush->Color = 0x339900;
        ACanvas->Font ->Color = clWhite;
...
void __fastcall CArray::ImageMouseUp(TObject *Sender,TMouseButton Button, TShiftState Shift, int X, int Y)
{
    ...
    int iR = ClickStat.iSelR1 ;
    int iC = ClickStat.iSelC1 ;
    int iStats = -1 ;
...
void __fastcall CArray::UpdateArayNrm()
{
    ...
    imArray -> Canvas -> Pen  -> Color = TColor(cl3DDkShadow); //배경색.dd
