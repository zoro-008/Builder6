...
bool CUnLoader::CheckSafe(EN_MOTR_ID _iMotr , EN_PSTN_ID _iPstnId)
{
    ...
    bool isRailGateCnsr    = IO_GetX(xPSB_PkgOut4) ; //IO_GetX(xPSB_PkgOut1) || IO_GetX(xPSB_PkgOut2) ;
...
bool CUnLoader::Autorun(void) //오토런닝시에 계속 타는 함수.
{
    ...
    if (Step.iSeq == 0) {
        ...
        bool isExstStrip   = !DM.ARAY[riPRB].CheckAllStat(csNone) || !DM.ARAY[riWR1].CheckAllStat(csNone) ||
                             !DM.ARAY[riWR2].CheckAllStat(csNone) || !DM.ARAY[riWR3].CheckAllStat(csNone) ||
        ...
        bool isCyclePlace  = !DM.ARAY[riULD].CheckAllStat(csNone ) &&
                             (DM.ARAY[riULD].CheckAllStat(csWork ) || isLotChanged || (!isExstStrip && LDR.GetStat().bWorkEnd));
        ...
        //카세트 사라짐.
        if(!isCyclePlace){
            if( !DM.ARAY[riULD].CheckAllStat(csNone) && ( !IO_GetX(xULD_MgzDetect1 ) && !IO_GetX(xULD_MgzDetect2 )) && !OM.MstOptn.bDryRun ) EM_SetErr(eiULD_MgzDispr  ) ; //sun bDryRun 옵션 추가.
        }
        //카세트 OUT 매거진 풀
...
//Check Cycle Time Out.
    ...
    switch (Step.iCycle) {
        ...
        case  11: r1 = m_tmTemp.OnDelay(true , 3000) ;
...
//Check Cycle Time Out.
    ...
    switch (Step.iCycle) {
        ...
        case  19: if(MoveMotr(miULD_ZCmp , piULD_ZPickFwd)) return false ;
                  ...
                  /////////////////
        case  20: if(m_tmTemp.OnDelay(true , 2000)){
        ...
        case  21: if(!m_tmTemp.OnDelay(true , 3000)) return false;
                  IO_SetY(yULD_MgzOutAC,false);
