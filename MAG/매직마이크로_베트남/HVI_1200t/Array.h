...
class CArray
{
    ...
    private:
        ...
        TPanel     * pnParent;
//        TImage     * imArray ;
//        좀 쓰려고 public 으로 이번에만 빼겟어요...
//        더블클릭 연결 시켜 줄라고 햇는데 여기서는 생각나는게 없음. dd
        TPopupMenu * pmStatus[MAX_STATUS_POPUP_ID] ;
        TMenuItem  * miStatus[MAX_STATUS_POPUP_ID][MAX_CHIP_STAT] ;

        TPopupMenu * pmStatusSub;
        TMenuItem  * miStatusSub[3];
        ...
        void __fastcall MenuDrawItemSub(TObject *Sender,TCanvas *ACanvas,const TRect &ARect, bool Selected);
...
//HVI-2300 장비에서만 이렇게 사용. dd private 로 옮기세요 ...dd
//        struct SClickStat {
//            bool bMouseDown;
...
//        }ClickStat;
    ...
    public:
        TImage     * imArray ; //HVI-2300 장비에서만 이렇게 사용. dd private 로 옮기세요 ...dd
...
//        void __fastcall SetDispItemRC(EN_CHIP_STAT _iStat , AnsiString _sSubCaption ) ; //우 클릭에 넣어봄.dd
        ...
        void __fastcall SetStat(EN_CHIP_STAT _iStat)
        {
            ...
            if(_iStat == csNone){SetStep(0);}
        }//{SetID(""); SetLotNo(""); SetStep(0);}} 별로 쓸일 없을거 같음...로더2매거진 이상일때 문제되서 삭제.
        void __fastcall ChangeStat(EN_CHIP_STAT _iFrom , EN_CHIP_STAT _iTo) { for(int r = 0 ; r < m_iMaxRow ; r++) for(int c = 0 ; c < m_iMaxCol ; c++) if(CHPS[r][c].GetStat() == _iFrom) CHPS[r][c].SetStat(_iTo);}
        ...
        void __fastcall PopupMenuVisible(bool bState);
