object FormOption: TFormOption
  Left = 788
  Top = 25
  Align = alClient
  BorderStyle = bsNone
  Caption = '-'
  ClientHeight = 866
  ClientWidth = 1080
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object GroupBox1: TGroupBox
    Left = 8
    Top = 8
    Width = 649
    Height = 769
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 0
    object Label85: TLabel
      Left = 19
      Top = 79
      Width = 252
      Height = 18
      Alignment = taCenter
      Caption = '2nd Inspection Stopper Down Delay'
      Color = clBtnFace
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
    end
    object Label3: TLabel
      Left = 4
      Top = 511
      Width = 254
      Height = 18
      Alignment = taCenter
      Caption = 'Insert X Velocity to Stopper Position'
      Color = clBtnFace
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
      Visible = False
    end
    object Label11: TLabel
      Left = 463
      Top = 76
      Width = 20
      Height = 18
      Alignment = taCenter
      Caption = 'ms'
      Color = clBtnFace
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
    end
    object Label16: TLabel
      Left = 449
      Top = 511
      Width = 36
      Height = 18
      Alignment = taCenter
      Caption = 'mm/s'
      Color = clBtnFace
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
      Visible = False
    end
    object Label1: TLabel
      Left = 4
      Top = 463
      Width = 265
      Height = 18
      Alignment = taCenter
      Caption = 'PreBuffer To WRK1 Zone Move Delay'
      Color = clBtnFace
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
      Visible = False
    end
    object Label2: TLabel
      Left = 449
      Top = 466
      Width = 20
      Height = 18
      Alignment = taCenter
      Caption = 'ms'
      Color = clBtnFace
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
      Visible = False
    end
    object Label4: TLabel
      Left = 20
      Top = 255
      Width = 221
      Height = 18
      Alignment = taCenter
      Caption = 'Marking To Unloader Out Delay'
      Color = clBtnFace
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
      Visible = False
    end
    object Label5: TLabel
      Left = 463
      Top = 250
      Width = 20
      Height = 18
      Alignment = taCenter
      Caption = 'ms'
      Color = clBtnFace
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
      Visible = False
    end
    object Label6: TLabel
      Left = 4
      Top = 335
      Width = 291
      Height = 18
      Alignment = taCenter
      Caption = 'Loader, UnLoader Change Position(16 ~ )'
      Color = clBtnFace
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
      Visible = False
    end
    object Label7: TLabel
      Left = 455
      Top = 332
      Width = 19
      Height = 18
      Alignment = taCenter
      Caption = 'EA'
      Color = clBtnFace
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
      Visible = False
    end
    object Label8: TLabel
      Left = 20
      Top = 295
      Width = 143
      Height = 18
      Alignment = taCenter
      Caption = 'UnLoader Out Delay'
      Color = clBtnFace
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
      Visible = False
    end
    object Label9: TLabel
      Left = 463
      Top = 290
      Width = 20
      Height = 18
      Alignment = taCenter
      Caption = 'ms'
      Color = clBtnFace
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
      Visible = False
    end
    object Label10: TLabel
      Left = 19
      Top = 112
      Width = 247
      Height = 18
      Alignment = taCenter
      Caption = '1st Inspection Stopper Down Delay'
      Color = clBtnFace
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
    end
    object Label12: TLabel
      Left = 463
      Top = 109
      Width = 20
      Height = 18
      Alignment = taCenter
      Caption = 'ms'
      Color = clBtnFace
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
    end
    object Label13: TLabel
      Left = 19
      Top = 145
      Width = 222
      Height = 18
      Alignment = taCenter
      Caption = 'Pre Buffer Stopper Down Delay'
      Color = clBtnFace
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
    end
    object Label14: TLabel
      Left = 463
      Top = 136
      Width = 20
      Height = 18
      Alignment = taCenter
      Caption = 'ms'
      Color = clBtnFace
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
    end
    object edWK2StprDnDelay: TEdit
      Left = 329
      Top = 71
      Width = 124
      Height = 30
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -19
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ImeName = 'Microsoft Office IME 2007'
      ParentFont = False
      TabOrder = 0
      Text = '0'
    end
    object edWK1_XInsVel: TEdit
      Left = 321
      Top = 499
      Width = 124
      Height = 30
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -19
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ImeName = 'Microsoft Office IME 2007'
      ParentFont = False
      TabOrder = 1
      Text = '0'
      Visible = False
    end
    object cbAutoVsChange: TCheckBox
      Tag = 4
      Left = 19
      Top = 22
      Width = 269
      Height = 22
      Caption = 'Auto Visn Job Change'
      Color = clBtnFace
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
      TabOrder = 2
    end
    object edPrbToWk1Delay: TEdit
      Left = 321
      Top = 455
      Width = 124
      Height = 30
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -19
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ImeName = 'Microsoft Office IME 2007'
      ParentFont = False
      TabOrder = 3
      Text = '0'
      Visible = False
    end
    object edPsbToOutDelay: TEdit
      Left = 329
      Top = 247
      Width = 124
      Height = 30
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -19
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ImeName = 'Microsoft Office IME 2007'
      ParentFont = False
      TabOrder = 4
      Text = '0'
      Visible = False
    end
    object cbEmptyOnlyTrm: TCheckBox
      Tag = 4
      Left = 11
      Top = 383
      Width = 269
      Height = 22
      Caption = 'Empty Chip Only Trim'
      Color = clBtnFace
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
      TabOrder = 5
      Visible = False
    end
    object edLDRChangePs: TEdit
      Left = 321
      Top = 327
      Width = 124
      Height = 30
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -19
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ImeName = 'Microsoft Office IME 2007'
      ParentFont = False
      TabOrder = 6
      Text = '0'
      Visible = False
    end
    object cbDWidth: TCheckBox
      Tag = 4
      Left = 3
      Top = 549
      Width = 318
      Height = 22
      Caption = 'Used Double Width Type Cyclinder'
      Color = clBtnFace
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
      TabOrder = 7
      Visible = False
    end
    object edUldOutDelay: TEdit
      Left = 329
      Top = 287
      Width = 124
      Height = 30
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -19
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ImeName = 'Microsoft Office IME 2007'
      ParentFont = False
      TabOrder = 8
      Text = '0'
      Visible = False
    end
    object edWK1StprDnDelay: TEdit
      Left = 329
      Top = 104
      Width = 124
      Height = 30
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -19
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ImeName = 'Microsoft Office IME 2007'
      ParentFont = False
      TabOrder = 9
      Text = '0'
    end
    object edPRBStprDnDelay: TEdit
      Left = 329
      Top = 137
      Width = 124
      Height = 30
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -19
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ImeName = 'Microsoft Office IME 2007'
      ParentFont = False
      TabOrder = 10
      Text = '0'
    end
    object cbAutoOperation: TCheckBox
      Tag = 4
      Left = 19
      Top = 354
      Width = 269
      Height = 22
      Caption = 'Auto Operation Switch (HRT-250P)'
      Color = clBtnFace
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
      TabOrder = 11
      Visible = False
    end
    object GroupBox2: TGroupBox
      Left = 304
      Top = 568
      Width = 313
      Height = 169
      Caption = 'Pen Marking Option'
      TabOrder = 12
      Visible = False
      object Label15: TLabel
        Left = 13
        Top = 118
        Width = 173
        Height = 18
        Alignment = taCenter
        Caption = 'Pen Marking Scratch Y : '
        Color = clBtnFace
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
      end
      object Label17: TLabel
        Left = 258
        Top = 120
        Width = 24
        Height = 18
        Alignment = taCenter
        Caption = 'mm'
        Color = clBtnFace
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
      end
      object Label18: TLabel
        Left = 14
        Top = 55
        Width = 173
        Height = 18
        Alignment = taCenter
        Caption = 'Pen Marking Scratch X : '
        Color = clBtnFace
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
      end
      object Label19: TLabel
        Left = 256
        Top = 56
        Width = 24
        Height = 18
        Alignment = taCenter
        Caption = 'mm'
        Color = clBtnFace
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
      end
      object cbUsePenMrkMvY: TCheckBox
        Tag = 4
        Left = 13
        Top = 89
        Width = 268
        Height = 22
        Caption = 'Use Pen Marking Scratch Y (Down)'
        Color = clBtnFace
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
        TabOrder = 0
      end
      object edPenMrkMvY: TEdit
        Left = 188
        Top = 115
        Width = 58
        Height = 30
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -19
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ImeName = 'Microsoft Office IME 2007'
        ParentFont = False
        TabOrder = 1
        Text = '0.0'
      end
      object cbUsePenMrkMvX: TCheckBox
        Tag = 4
        Left = 14
        Top = 26
        Width = 275
        Height = 22
        Caption = 'Use Pen Marking Scratch X (Right)'
        Color = clBtnFace
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
        TabOrder = 2
      end
      object edPenMrkMvX: TEdit
        Left = 189
        Top = 52
        Width = 58
        Height = 30
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -19
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ImeName = 'Microsoft Office IME 2007'
        ParentFont = False
        TabOrder = 3
        Text = '0.0'
      end
    end
    object cb1stBMOnlyTrm: TCheckBox
      Tag = 4
      Left = 11
      Top = 410
      Width = 269
      Height = 22
      Caption = '1St BM Only Trim'
      Color = clBtnFace
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
      TabOrder = 13
      Visible = False
    end
    object cbIgnrEmptyTrm: TCheckBox
      Tag = 4
      Left = 19
      Top = 50
      Width = 269
      Height = 22
      Caption = 'Ignore Empty Trim'
      Color = clBtnFace
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
      TabOrder = 14
    end
  end
  object btSave: TBitBtn
    Left = 767
    Top = 784
    Width = 305
    Height = 73
    Caption = ' SAVE'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -20
    Font.Name = 'Arial'
    Font.Style = []
    ParentFont = False
    TabOrder = 1
    OnClick = btSaveClick
    Glyph.Data = {
      
      7878787878FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
  end
end
