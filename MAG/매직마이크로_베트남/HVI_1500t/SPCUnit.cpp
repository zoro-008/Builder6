...
bool __fastcall CSPC::SaveLotMap(String sPath, String sSaveFileName)
{
    //Local Var.
    TUserINI   UserINI;
    AnsiString sTemp  ;
    ...
    //Init. Grid Data.
//    for (register int i = 0 ;i < Grid -> RowCount ; i++) Grid -> Rows[i] -> Clear();
//    Grid -> RowCount = 2;

//폴더만 띄어 낼라고 햇는데 ...쩝...dd 멀하고 있는지...쩝쩝쩝...정신이 나감...dd
/*
    SerchFile = sPath ;
    int iTemp = 0 ;
    while(1){
        if(SerchFile.Pos("\\")) {
            iTemp = sTemp.Pos("\\");
            SerchFile = SerchFile.Delete(SerchFile.Pos("\\"),1);
            continue ;
        }
        else
        {
            SerchFile = sPath.SubString(0,iTemp);
            break;
        }
    }
*/
    ...
    //Search.
    SerchFile = ExpandFileName(sPath) + "\\*.*";
    ...
    //여기까지 폴더 파일 몇개 있는지 count

    sRsltT = "" ;
    for(int i=0; i<Count; i++)
    {
        sFilePath = sPath + "\\" + Item[i] ;

        if(!FileExists(sFilePath)) return false ;

        UserINI.Load(sFilePath.c_str()  , "ETC"  , "ID"    , &sId    );
        UserINI.Load(sFilePath.c_str()  , "ETC"  , "LotNo" , &sLotNo );
        UserINI.Load(sFilePath.c_str()  , "ETC"  , "Row"   , &iRow   );
        UserINI.Load(sFilePath.c_str()  , "ETC"  , "Col"   , &iCol   );

        sRsltT = "\r\n";
        for(int r = 0 ; r < iRow ; r++) {
            sTemp.printf("R%02d", r);
            UserINI.Load(sFilePath.c_str()  , "Data"  , sTemp , sRslt );
            sRsltT += sRslt ;
            sRsltT += "\r\n";
        }
//        sRsltT += "\r\n";
        UserINI.Save(sSaveFileName , sLotNo , sId , sRsltT);
    }

/*
    String sSavePath = "" ;
    String sTemp1    = "MapData\\";
    sSavePath  = SPC_FOLDER + sTemp1  ;
    sSavePath += sLotNo ;

//    String sTemp1 = "" ;
    sTemp2 = "" ;
//    sTemp1 = "LotNo : " + sLotNo ;
    sTemp2 = "Row : " + (String)iRow + " Col : " + (String)iCol ;

    UserINI.Save(sSaveFileName , "Map Data" , sId , sRsltT);
*/
