...
{
    bool bRet = true ;
         if(_iId == miWRK_XVsn) bRet = true ;
    else if(_iId == miWRK_YVsn) bRet = true ;
    else if(_iId == miPSB_XTrm) {
        if(!AT_Complete(aiPSB_PnchDnUp , ccBwd)) { AT_MoveCyl(aiPSB_PnchDnUp , ccFwd); bRet = false ; }
    }
    else if(_iId == miPSB_YTrm)
    {
        if(!AT_Complete(aiPSB_PnchDnUp , ccBwd)) { AT_MoveCyl(aiPSB_PnchDnUp , ccFwd); bRet = false ; }
    }
    else if(_iId == miPSB_TFlp) bRet = true ;
    else if(_iId == miPSB_XIns) bRet = true ;
    else if(_iId == miWR1_XCvr) bRet = true ;
    else if(_iId == miWR2_XCvr) bRet = true ;
    else if(_iId == miWR3_XCvr) bRet = true ;
    else if(_iId == miLDR_YCmp) bRet = true ;
    else if(_iId == miLDR_ZCmp) bRet = true ;
    else if(_iId == miULD_YCmp) bRet = true ;
    else if(_iId == miULD_ZCmp) bRet = true ;
