    ...
    : TForm(Owner)
{
    ...
    btMan2_21 -> Tag = (int)mcRAL_VisnYZeroPs    ;
    ...
    btMan3_7  -> Tag = (int)mcPSB_TAlignBw       ;
    btMan3_8  -> Tag = (int)mcPSB_TFlpNorPs      ;
    btMan3_9  -> Tag = (int)mcPSB_TFlpInsPs      ;
    btMan3_10 -> Tag = (int)mcPSB_TTrimAllTest   ;
    btMan3_11 -> Tag = (int)mcPSB_TTrimUpDn      ;
    btMan3_12 -> Tag = (int)mcPSB_TTrimXMv       ;
    btMan3_13 -> Tag = (int)mcPSB_TTrimYMv       ;
    btMan3_14 -> Tag = (int)mcPSB_IdxUp          ;
    btMan3_15 -> Tag = (int)mcPSB_IdxDn          ;
    ...
    btMan5_10-> Tag = (int)mcETC_AllAlignLiftUp ;
    btMan5_11-> Tag = (int)mcETC_AllLiftDn      ;
    btMan5_12-> Tag = (int)mcETC_MainAirOnOff   ;
    btMan5_13-> Tag = (int)mcETC_ManLotEnd      ;
    ...
    for (int i = 0; i < ComponentCount; i++)
    {
        if (Components[i]->ClassNameIs("TBitBtn"))
        {
            ...
            if(iTag)
            {
                ((TBitBtn*)FindComponent(Components[i]->Name))->Visible = true   ;
                btMan1_14 -> Visible = false ;
                btMan1_15 -> Visible = false ;
                btMan1_16 -> Visible = false ;
                btMan1_17 -> Visible = false ;
                btMan1_18 -> Visible = false ;
                btMan1_19 -> Visible = false ;
                btMan1_20 -> Visible = false ;

                btMan3_15 -> Visible = false ;

                btMan4_13 -> Visible = false ;
                btMan4_14 -> Visible = false ;
                btMan4_15 -> Visible = false ;
                btMan4_16 -> Visible = false ;
                btMan4_17 -> Visible = false ;
                btMan4_18 -> Visible = false ;
                btMan4_19 -> Visible = false ;
                btMan4_20 -> Visible = false ;
                
                btMan5_16 -> Visible = false ;
            }
            else     ((TBitBtn*)FindComponent(Components[i]->Name))->Visible = false  ;
        }
    }
    tmSetUp -> Enabled = false;
...
void __fastcall TFormManual::tmSetUpTimer(TObject *Sender)
{
    tmSetUp -> Enabled = false;

    switch(m_iSetUpTag){
        case 1 : if(AT_Complete(aiWK1_AlgnFwBw , ccFwd)) AT_MoveCyl(aiWK1_AlgnFwBw , ccBwd);
                 else                                    AT_MoveCyl(aiWK1_AlgnFwBw , ccFwd);
                 if(AT_Complete(aiWK2_AlgnFwBw , ccFwd)) AT_MoveCyl(aiWK2_AlgnFwBw , ccBwd);
                 else                                    AT_MoveCyl(aiWK2_AlgnFwBw , ccFwd);
                 if(AT_Complete(aiWK3_AlgnFwBw , ccFwd)) AT_MoveCyl(aiWK3_AlgnFwBw , ccBwd);
                 else                                    AT_MoveCyl(aiWK3_AlgnFwBw , ccFwd);
                 if(AT_Complete(aiPSB_AlgnFwBw , ccFwd)) AT_MoveCyl(aiPSB_AlgnFwBw , ccBwd);
                 else                                    AT_MoveCyl(aiPSB_AlgnFwBw , ccFwd);
                 break;

        case 2 : if(AT_Complete(aiWK1_SttnUpDn , ccFwd)) AT_MoveCyl(aiWK1_SttnUpDn , ccBwd);
                 else                                    AT_MoveCyl(aiWK1_SttnUpDn , ccFwd);
                 if(AT_Complete(aiWK2_SttnUpDn , ccFwd)) AT_MoveCyl(aiWK2_SttnUpDn , ccBwd);
                 else                                    AT_MoveCyl(aiWK2_SttnUpDn , ccFwd);
                 if(AT_Complete(aiWK3_SttnUpDn , ccFwd)) AT_MoveCyl(aiWK3_SttnUpDn , ccBwd);
                 else                                    AT_MoveCyl(aiWK3_SttnUpDn , ccFwd);
                 break;

        case 3 : if(AT_Complete(aiPRB_StprUpDn , ccFwd)) AT_MoveCyl(aiPRB_StprUpDn , ccBwd);
                 else                                    AT_MoveCyl(aiPRB_StprUpDn , ccFwd);
                 if(AT_Complete(aiWK1_StprUpDn , ccFwd)) AT_MoveCyl(aiWK1_StprUpDn , ccBwd);
                 else                                    AT_MoveCyl(aiWK1_StprUpDn , ccFwd);
                 if(AT_Complete(aiWK2_StprUpDn , ccFwd)) AT_MoveCyl(aiWK2_StprUpDn , ccBwd);
                 else                                    AT_MoveCyl(aiWK2_StprUpDn , ccFwd);
                 if(AT_Complete(aiWK3_StprUpDn , ccFwd)) AT_MoveCyl(aiWK3_StprUpDn , ccBwd);
                 else                                    AT_MoveCyl(aiWK3_StprUpDn , ccFwd);
                 break;
    }

    tmSetUp -> Enabled = true;
}
//---------------------------------------------------------------------------


void __fastcall TFormManual::btAlignSClick(TObject *Sender)
{
    m_iSetUpTag = ((TBitBtn *)Sender) -> Tag ;
    tmSetUp -> Enabled = true;
}
//---------------------------------------------------------------------------

void __fastcall TFormManual::btAlignEClick(TObject *Sender)
{
    tmSetUp -> Enabled = false;
}
//---------------------------------------------------------------------------
