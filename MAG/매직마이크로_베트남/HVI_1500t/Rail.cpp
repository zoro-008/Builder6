...
bool CRail::ToStop(void) //스탑을 하기 위한 함수.
{
    ...
    switch (Step.iToStop) {
        ...
        case 12: if(!MoveActr(aiWK1_SttnUpDn , ccBwd)) return false ;
                 ...
                 if(IO_GetX(xWR1_Pkg)) DM.ARAY[riVS1].SetStat(csUnkwn);
                 if(IO_GetX(xWR2_Pkg)) DM.ARAY[riVS2].SetStat(csUnkwn);
                 if(IO_GetX(xWR3_Pkg)) DM.ARAY[riVS3].SetStat(csUnkwn);
...
//        bool isDoingOut    = CPreBuff::scOut == PRB.GetSeqStep() ;
        ...
        EN_ARAY_STEP bTempp1 = DM.ARAY[riWR1].GetStep();
        bool bTemp1 = (DM.ARAY[riWR1].GetCntExist()&& IO_GetX(xWR1_Pkg) && DM.ARAY[riWR1].GetStep()==asInspNeed && !DM.ARAY[riVS1].CheckAllStat(csWork));
        bool bTemp2 = (DM.ARAY[riWR2].GetCntExist()&& IO_GetX(xWR2_Pkg) && DM.ARAY[riWR2].GetStep()==asInspNeed && !DM.ARAY[riVS2].CheckAllStat(csWork));
        bool bTemp3 = (DM.ARAY[riWR3].GetCntExist()&& IO_GetX(xWR3_Pkg) && DM.ARAY[riWR3].GetStep()==asInspNeed && !DM.ARAY[riVS3].CheckAllStat(csWork));
        bool isCycleBind   = /*!isWaitWR1 &&*/ (bTemp1 || bTemp2 || bTemp3) && !isPrbOut;//!isDoingInsp &&                                                                //리프트 블럭 없으면 센서 감지 됨.dd




                             //!isWaitWR1 && //!isDoingInsp &&                                                                //리프트 블럭 없으면 센서 감지 됨.dd
...
//        bool isCycleUnBind =  DM.ARAY[riVS1].CheckAllStat(csWork) && DM.ARAY[riVS2].CheckAllStat(csWork) && DM.ARAY[riVS3].CheckAllStat(csWork) ;
        bool isCycleUnBind = (((!DM.ARAY[riWR1].GetCntExist() || DM.ARAY[riVS1].CheckAllStat(csWork)&& DM.ARAY[riWR1].GetStep()==asInspBind )) &&
                              ((!DM.ARAY[riWR2].GetCntExist() || DM.ARAY[riVS2].CheckAllStat(csWork)&& DM.ARAY[riWR2].GetStep()==asInspBind )) &&
                              ((!DM.ARAY[riWR3].GetCntExist() || DM.ARAY[riVS3].CheckAllStat(csWork)&& DM.ARAY[riWR3].GetStep()==asInspBind ))) && isExistStrip && HED.GetSeqStep() != CHead::scInsp;

        bool isCycleMove   = ( isCanMovePRB && PRB.GetSeqStep() != CPreBuff::scIn && HED.GetSeqStep() != CHead::scInsp && PSB.GetSeqStep() != CPstBuff::scIdle) || //PRB
...
//                             PRB.GetSeqStep() == CPreBuff::scOut ;// Rail 에서 프리버퍼 있을때만으로 무브 사이클 태우기는에는 안될때가 있음.dd
        ...
        //모르는 스트립에러.
        if(!DM.ARAY[riWR1 ].CheckExist(0,0) &&  IO_GetX(xWR1_Pkg) && AT_Complete(aiWK1_SttnUpDn , ccBwd)) EM_SetErr(eiWR1_PkgUnknown ) ;
        if(!DM.ARAY[riWR2 ].CheckExist(0,0) &&  IO_GetX(xWR2_Pkg) && AT_Complete(aiWK2_SttnUpDn , ccBwd)) EM_SetErr(eiWR2_PkgUnknown ) ;
        if(!DM.ARAY[riWR3 ].CheckExist(0,0) &&  IO_GetX(xWR3_Pkg) && AT_Complete(aiWK3_SttnUpDn , ccBwd)) EM_SetErr(eiWR3_PkgUnknown ) ;
...
bool CRail::CycleBind(void) //
{
    ...
    switch (Step.iCycle) {
        ...
        case  15: if(!MoveActr(aiWK1_AlgnFwBw , ccBwd)) return false ;
                  ...
                  if(DM.ARAY[riWR1].GetCntExist()){DM.ARAY[riVS1].SetStat(csUnkwn); DM.ARAY[riWR1] = riWR1Buff ; DM.ARAY[riWR1].SetStep(asInspBind);}
                  if(DM.ARAY[riWR2].GetCntExist()){DM.ARAY[riVS2].SetStat(csUnkwn); DM.ARAY[riWR2] = riWR2Buff ; DM.ARAY[riWR2].SetStep(asInspBind);}
                  if(DM.ARAY[riWR3].GetCntExist()){DM.ARAY[riVS3].SetStat(csUnkwn); DM.ARAY[riWR3] = riWR3Buff ; DM.ARAY[riWR3].SetStep(asInspBind);}
