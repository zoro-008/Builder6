...
bool __fastcall TOutBandRectTracker::TrackerDw(TMouseButton Button,TShiftState Shift ,int x, int y , float _fScaleX , float _fScaleY , float _fOffsetX, float _fOffsetY)
{
    ...
    //Band
    float fBandRectCntX = GetOutRect().Left + GetOutRect().Width()/2 ;
    float fBandRectTopY = GetOutRect().Bottom   ;
    float fBCntX  = (fBandRectCntX-PointSize * 2) ;
    float fBCntY  =  fBandRectTopY                ;
    ...
    else if (fx > fBCntX -PointSize && fx < fBCntX +PointSize && fy > fBCntY -PointSize && fy < fBCntY +PointSize && FBandEnable) {ret = Stat.IsBND  = true; FFocused = true ;}
...
*/
    ...
    //Band
    float fBandRectCntX = GetOutRect().Left + GetOutRect().Width()/2 ;
    float fBandRectCntY = GetOutRect().Bottom ;
    float fOriBCntX     = fBandRectCntX-PointSize * 2 ;
    float fOriBCntY     = fBandRectCntY               ; //fBtmRectTopY               * _fScaleY ;
    float fBCntY        = 0 ;
    ...
    fWidth  = fRight  - fLeft ; //* _fScaleX  ; * _fScaleX  ;
    fHeight = fBottom - fTop  ; //* _fScaleY  ; * _fScaleY  ;
