...
int CPackage::GetInspRowCnt(){return VOM.VDevOptn.iCountY ;}
int CPackage::GetInspColCnt(){return VOM.VDevOptn.iCountX ;}
void CPackage::SetInspRowCnt(int _iVal){VOM.VDevOptn.iCountY = _iVal;}
void CPackage::SetInspColCnt(int _iVal){VOM.VDevOptn.iCountX = _iVal;}

bool CPackage::Init(TVisnStaticPara _tVisnStaticPara)
{
...
void CPackage::Inspection()
{
    DWORD dwStartTime = GetTickCount() ;

    memset(&Rslt , 0 , sizeof(TRetResult));

    EN_VISN_ERR_ID iInspRslt    = vePkgOk ;
    int            iInspFailCnt = 0          ;
    bool           bRet         = true       ;
    ...
    while(1) { //고투문 대용.
        ...
        bRet = OCV_Inspect(m_pImg , Orig.pTrArea , Orig.pTrImage , Orig.pTrckInsp->GetOutRect() , VOM.VDevPara.PackageOcv , Orig.pRsltArea , &Orig.tRslt) ;
        //}
        if(!bRet) {
            iInspRslt = vePkgOriFail ;
            m_sRsltMsg = AnsiString("FAILED ORIGIN : ") + SVL_GetErrMsg();
            break ;
        }

        if(Orig.tRslt.fSinc < VOM.VDevOptn.dPackageOriSinc) {
            iInspRslt = vePkgOriNg ;
        ...
        float fColGap = float(GetInspColCnt() - 1)?(Insp.pTrckInsp2->Left - Insp.pTrckInsp1->Left) / float(GetInspColCnt() - 1) : 1.0;
        float fRowGap = float(GetInspRowCnt() - 1)?(Insp.pTrckInsp2->Top  - Insp.pTrckInsp1->Top ) / float(GetInspRowCnt() - 1) : 1.0;
        ...
        for(int r = 0 ; r < GetInspRowCnt() ; r++) {
            for(int c = 0 ; c < GetInspColCnt() ; c++) {
                ...
                bRet = TSD_Inspect(m_pImg , tInspRect , VOM.VDevPara.PackageTsd , &Insp.pRslt[GetInspColCnt() * r + c]);

                iInspPxCnt = Insp.pRslt[GetInspColCnt() * r + c].tRect.Height() * Insp.pRslt[GetInspColCnt() * r + c].tRect.Width() ;

                if(!bRet) {
                    iInspRslt = vePkgInspFail ;
                    m_sRsltMsg = AnsiString("FAILED Insp Y:") + r + AnsiString(" X:") + c +AnsiString(" ")+ SVL_GetErrMsg();
                    break ;
                }

                fPxPer = Insp.pRslt[GetInspColCnt() * r + c].iPxCnt * 100 / (float)iInspPxCnt ;
                if(fMinPxPer > fPxPer) fMinPxPer = fPxPer ;
                if(fMaxPxPer < fPxPer) fMaxPxPer = fPxPer ;
                /*
                iInspPxCnt = Insp.pRslt[GetInspColCnt() * r + c].tRect.Width() * Insp.pRslt[VOM.VDevOptn.iCountX * r + c].tRect.Height() ;
                fPercent = Insp.pRslt[VOM.VDevOptn.iCountX * r + c].iPxCnt * 100 /(float)iInspPxCnt ;

                */


                if(fPxPer < VOM.VDevOptn.dPackagePxMinPer) {
                    iInspRslt = vePkgInspNg ;
                    Rslt.bFailMap[r][c] = true ;
                    iInspFailCnt++;
                }
            }
            if(!iInspRslt == vePkgInspFail) break ;
        }
        if(iInspRslt != vePkgOk) {
...
void CPackage::Train()
{
    if(m_iActivPage == piPkgOcv) {
        //(IImage * _pImg , TRect _Rect , OCV_Para _tPara ,  IArea * _pTrainArea , IImage * _pTrainImg) ;
        if(!OCV_Train(GetImg() , Orig.pTrckInsp->GetTRect() , VOM.VDevPara.PackageOcv , Orig.pTrArea , Orig.pTrImage)) {
            ShowMessage(SVL_GetErrMsg());
        }
    }
}

void CPackage::Paint(HDC _hDc , EN_DISP_OPTN _iDpOptn ,
                              ...
                              float _fScaleX , float _fScaleY ,int _iThreshold , bool _bCntrLine)
{
    ...
    float fColGap = float(GetInspColCnt() - 1) ? (Insp.pTrckInsp2->Left - Insp.pTrckInsp1->Left) * _fScaleX / float(GetInspColCnt() - 1) : 1.0;
    float fRowGap = float(GetInspRowCnt() - 1) ? (Insp.pTrckInsp2->Top  - Insp.pTrckInsp1->Top ) * _fScaleY / float(GetInspRowCnt() - 1) : 1.0;
    ...
    if(g_bSettingMode){
        if(m_iActivPage == piPkgLCS) {
            //PaintCntLine(_hDc , _iDstW , _iDstH);
        }
        else if(m_iActivPage == piPkgOcv) {
    ...
    if(Stat.bDispRslt){
        ...
        dPxPer = Insp.pRslt[VOM.VDevOptn.iCountX * r + c].iPxCnt * 100 / iInspPxCnt ;
                ...
                if(dPxPer < VOM.VDevOptn.dBfPlcePxMinPer) {
                    iInspRslt = veBfPlceInspNg ;
                    iInspFailCnt++;
                }
        ...
        */
        ...
        for(int r = 0 ; r < VOM.VDevOptn.iCountY ; r++) {
            for(int c = 0 ; c < VOM.VDevOptn.iCountX ; c++) {
                ...
                fPercent = Insp.pRslt[VOM.VDevOptn.iCountX * r + c].iPxCnt * 100 /(float)iInspPxCnt ;
                if(fPercent  < VOM.VDevOptn.dPackagePxMinPer) {Gdi -> m_tPen.Color = clRed  ; Gdi -> m_tText.Color = clRed  ;}
...
void CPackage::SetPaintCallback(PaintCallbackFunc _pCalback)
{
...
bool CPackage::OnMouseDw(TMouseButton Button, TShiftState Shift, int X, int Y , float _fScaleX , float _fScaleY, float _fOffsetX, float _fOffsetY)
{
    if(g_bSettingMode){
        if(m_iActivPage == piPkgLCS) {
        }
...
bool CPackage::OnMouseMv(TShiftState Shift, int X, int Y , float _fScaleX , float _fScaleY, float _fOffsetX, float _fOffsetY)
{
...
void CPackage::OnMouseUp(TMouseButton Button, TShiftState Shift, int X, int Y , float _fScaleX , float _fScaleY, float _fOffsetX, float _fOffsetY)
{
...
void CPackage::ShowAlgoPara(TValueListEditor * _vePara)
{
...
void CPackage::UpdateAlgoPara(bool _bTable , TValueListEditor * _vePara)
{
    if(m_iActivPage == piPkgOcv) {
        UpdateParaOcv(_bTable , _vePara , VOM.VDevPara.PackageOcv);
    }
    else if(m_iActivPage == piPkgTsd) {
        UpdateParaTsd(_bTable , _vePara , VOM.VDevPara.PackageTsd);
    }
}

void CPackage::Load(bool _bLoad)
{
    AnsiString sJobPath = g_sJobFilePath + "Package.INI" ;
    ...
    //Origin
    LoadParaOcv(_bLoad , VOM.VDevPara.PackageOcv , sJobPath , "Orig");
    ...
    //Insp
    LoadParaTsd(_bLoad , VOM.VDevPara.PackageTsd , sJobPath , "Insp");
...
void CPackage::TrackerInit()
{
    Orig.pTrckInsp -> Left = 10 ;
    Orig.pTrckInsp -> Top  = 10 ;
}
void CPackage::InspectInit()
{
    Insp.pTrckInsp1 -> Left = 10  ;
    Insp.pTrckInsp1 -> Top  = 10  ;

    Insp.pTrckInsp2 -> Left = 100 ;
    Insp.pTrckInsp2 -> Top  = 100 ;
}
