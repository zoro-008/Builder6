...
#define DATA_ARAY_F riFLS
#define DATA_ARAY_B riLLS
...
void CErrData::Update(AnsiString _sCrntLotNo)
{
    ...
    bool isErr = EM_IsErr() ;
    if(isErr && !bPreErr) {
        SetErr(EM_GetLastErr() , EM_GetErrName(EM_GetLastErr()) , EM_GetErrMsg() , _sCrntLotNo  );
    }
    bPreErr = isErr ;
}
...
void CLotData::Init()
{
    m_dTempTm = 0.0;
    LoadSaveLotIni(true);
    LoadSaveUserIni(true);
}

void CLotData::Close()
{
    LoadSaveLotIni(false);
    LoadSaveUserIni(false);
}
...
bool CLotData::SaveArrayData(TData & _tData , int _iMgzNo , int _iSlotNo , CArray & _rData)
{
    ...
    tDateTime.Val =  Now(); // _tData.dSttTime ;

    if(_tData.sLotId != OM.MstOptn.sCrntLotID) _tData.sLotId = OM.MstOptn.sCrntLotID;
...
bool CLotData::LoadSaveUserIni(bool _bLoad)
{
    //Local Var.
    TUserINI   UserINI      ;
    AnsiString sPath        ;
    AnsiString sUserWorkCnt ;

    TDateTime  tDateTime ;

    tDateTime.Val =  Now(); // _tData.dSttTime ;

    sPath = LOT_FOLDER + tDateTime.FormatString("yyyymmdd\\") + m_tData.sLotId + "_User" + ".ini" ;
    //sPath = LOT_FOLDER + "UserInfo.ini" ;

    if(_bLoad) {
        UserINI.Load(sPath.c_str() , "Data" , "iUserCnt    " , m_tUserData.iUserCnt     ); sUserWorkCnt = "iUserWorkCnt_" + (AnsiString)m_tUserData.iUserCnt; SPC.m_iUserCnt     = m_tUserData.iUserCnt    ;
        UserINI.Load(sPath.c_str() , "Data" , sUserWorkCnt   , m_tUserData.iUserWorkCnt );                                                                    SPC.m_iUserWorkCnt = m_tUserData.iUserWorkCnt;

    }
    else {
        m_tUserData.iUserCnt     = SPC.m_iUserCnt     ;
        m_tUserData.iUserWorkCnt = SPC.m_iUserWorkCnt ;
        UserINI.Save(sPath.c_str() , "Data" , "iUserCnt    " , m_tUserData.iUserCnt     ); sUserWorkCnt = "iUserWorkCnt_" + (AnsiString)m_tUserData.iUserCnt;
        UserINI.Save(sPath.c_str() , "Data" , sUserWorkCnt   , m_tUserData.iUserWorkCnt );
    }
...
void CLotData::DispLotList(AnsiString _sDate , TStringGrid * _sgLotList)
{
    ...
    _sgLotList -> Cells[3][0] = "Start"    ; //_sgLotList -> ColWidths[3] = _sgLotList -> Cells[3][0].Length() * 15 ;
    _sgLotList -> Cells[4][0] = "End"      ; //_sgLotList -> ColWidths[4] = _sgLotList -> Cells[4][0].Length() * 15 ;
    _sgLotList -> Cells[5][0] = "WorkTime" ; //_sgLotList -> ColWidths[5] = _sgLotList -> Cells[5][0].Length() * 15 ;
    _sgLotList -> Cells[6][0] = "TotalTime"; //_sgLotList -> ColWidths[6] = _sgLotList -> Cells[6][0].Length() * 15 ;
    _sgLotList -> Cells[7][0] = "JobFile"  ; //_sgLotList -> ColWidths[7] = _sgLotList -> Cells[7][0].Length() * 15 ;
    ...
    if (FindFirst(sSerchFile , faAnyFile , sr) == 0) {
        do {
            if ((sr.Attr & faDirectory) != sr.Attr) {
                ...
                //랏 정보 블러오기.
                                                                                           _sgLotList -> Cells[0][iLotCnt+1] = iLotCnt          ;
                ...
                UserINI.Load(sIniPath.c_str() , "Data" , "dSttTime  " , tData.dSttTime  ); tTime.Val = tData.dSttTime  ; _sgLotList -> Cells[3][iLotCnt+1] = tTime.FormatString("hh:nn:ss") ;
                UserINI.Load(sIniPath.c_str() , "Data" , "dEndTime  " , tData.dEndTime  ); tTime.Val = tData.dEndTime  ; _sgLotList -> Cells[4][iLotCnt+1] = tTime.FormatString("hh:nn:ss") ;
                UserINI.Load(sIniPath.c_str() , "Data" , "dWorkTime " , tData.dWorkTime ); tTime.Val = tData.dWorkTime ; _sgLotList -> Cells[5][iLotCnt+1] = tTime.FormatString("hh:nn:ss") ;
                UserINI.Load(sIniPath.c_str() , "Data" , "dTotalTime" , tData.dTotalTime); tTime.Val = tData.dTotalTime; _sgLotList -> Cells[6][iLotCnt+1] = tTime.FormatString("hh:nn:ss") ;
                UserINI.Load(sIniPath.c_str() , "Data" , "sJobFile  " , tData.sJobFile  ); _sgLotList -> Cells[7][iLotCnt+1] = tData.sJobFile   ;
...
void CLotData::DispLotInfo(TStringGrid * _sgLotInfo) //오퍼레이션 창용.
{
    ...
    static DWORD dTickTime;

         if(DM.ARAY[riFFD].CheckAllStat(csUnkwn)) dTickTime = FFD.m_dWorkTotalTm;
    else if(DM.ARAY[riLFD].CheckAllStat(csUnkwn)) dTickTime = LFD.m_dWorkTotalTm;
    //else                                          dTickTime = 0.0               ;

    _sgLotInfo -> ColCount = 2 ;
    _sgLotInfo -> RowCount = 8 ;
    _sgLotInfo -> Cells[0][0] = "Lot Id    "  ;                                       _sgLotInfo -> Cells[1][0] = m_tData.sLotId    ;
    _sgLotInfo -> Cells[0][1] = "Work Cnt  "  ;                                       _sgLotInfo -> Cells[1][1] = m_tData.iWorkCnt  ;
    _sgLotInfo -> Cells[0][2] = "Stt Time  "  ; tDateTime.Val = m_tData.dSttTime   ;  _sgLotInfo -> Cells[1][2] = tDateTime.FormatString("hh:nn:ss");
    _sgLotInfo -> Cells[0][3] = "End Time  "  ; tDateTime.Val = m_tData.dEndTime   ;  _sgLotInfo -> Cells[1][3] = tDateTime.FormatString("hh:nn:ss");
    _sgLotInfo -> Cells[0][4] = "Run Time  "  ; tDateTime.Val = m_tData.dWorkTime  ;  _sgLotInfo -> Cells[1][4] = tDateTime.FormatString("hh:nn:ss");
    _sgLotInfo -> Cells[0][5] = "Total Time"  ; tDateTime.Val = m_tData.dTotalTime ;  _sgLotInfo -> Cells[1][5] = tDateTime.FormatString("hh:nn:ss");
    _sgLotInfo -> Cells[0][6] = "Err Time  "  ; tDateTime.Val = m_tData.dErrTime   ;  _sgLotInfo -> Cells[1][6] = tDateTime.FormatString("hh:nn:ss");
    _sgLotInfo -> Cells[0][7] = "Work Time "  ;                                       _sgLotInfo -> Cells[1][7] = ((int)dTickTime/1000.0) ;
    _sgLotInfo -> Cells[0][8] = "Stop Time "  ; tDateTime.Val = m_tData.dStopTime  ;  _sgLotInfo -> Cells[1][8] = tDateTime.FormatString("hh:nn:ss");
}
...
void CLotData::Update(EN_SEQ_STAT Stat)
{
    ...
    bool bLotOpen = LTQ.GetLotOpen() ;

    static bool bPreLotOpen = bLotOpen;

    if(sLotId != m_tData.sLotId && !bPreLotOpen /*sLotId != ""*/) { //어맛! 새로운 랏.
        //이런식으로하면 AnsiString 안에 Char * Data 가 메모리 해제 않되서 안됌.
        //memset(&m_tData , 0 , sizeof(TData));
        m_tData.Clear();

        m_tData.sLotId    = LTQ.GetLotNo(0).Trim();
        m_tData.dSttTime  = dCrntTime ;
        m_tData.sJobFile  = OM.m_sCrntDev;
    }
    bPreLotOpen = bLotOpen;
    ...
    bool bWork_F = DM.ARAY[DATA_ARAY_F].CheckAllStat(csWork ) ;
    bool bWork_B = DM.ARAY[DATA_ARAY_B].CheckAllStat(csWork ) ;

    static bool bPreWork_F = bWork_F ;
    static bool bPreWork_B = bWork_B ;

    //bool bLotOpen = LTQ.GetLotOpen();

    if(!bPreWork_F && bWork_F && bLotOpen) {    // 작업 카운트 업데이트 하는 부분 다시 봐야 한다...JH
        m_tData.iWorkCnt  += OM.MstOptn.iPkgCnt;   // DM.ARAY[DATA_ARAY_F].GetMaxCol() * DM.ARAY[DATA_ARAY_F].GetMaxRow();
        m_tData.dEndTime  = dCrntTime ;
        m_tData.sJobFile  = OM.m_sCrntDev ;

        iSlotNo = DM.ARAY[DATA_ARAY_F].GetID().ToIntDef(0)%100 ;
        iMgzNo  = DM.ARAY[DATA_ARAY_F].GetID().ToIntDef(0)/100 ;
        SaveArrayData(m_tData , iMgzNo , iSlotNo , DM.ARAY[DATA_ARAY_F]);
    }

    if(!bPreWork_B && bWork_B && bLotOpen) {
        m_tData.iWorkCnt  += OM.MstOptn.iPkgCnt;   // DM.ARAY[DATA_ARAY_F].GetMaxCol() * DM.ARAY[DATA_ARAY_F].GetMaxRow();
        m_tData.dEndTime  = dCrntTime ;
        m_tData.sJobFile  = OM.m_sCrntDev ;

        iSlotNo = DM.ARAY[DATA_ARAY_B].GetID().ToIntDef(0)%100 ;
        iMgzNo  = DM.ARAY[DATA_ARAY_B].GetID().ToIntDef(0)/100 ;
        SaveArrayData(m_tData , iMgzNo , iSlotNo , DM.ARAY[DATA_ARAY_B]);
    }
    bPreWork_F = bWork_F ;
    bPreWork_B = bWork_B ;
}
...
AnsiString CLotData::GetCrntLotNo()
{
         ...
         if(!DM.ARAY[riFFD].CheckAllStat(csNone)) return DM.ARAY[riFFD].GetLotNo() ;
    else if(!DM.ARAY[riLFD].CheckAllStat(csNone)) return DM.ARAY[riLFD].GetLotNo() ;
    else if(!DM.ARAY[riFLS].CheckAllStat(csNone)) return DM.ARAY[riFLS].GetLotNo() ;
    else if(!DM.ARAY[riLLS].CheckAllStat(csNone)) return DM.ARAY[riLLS].GetLotNo() ;
    else                                            return "";
}

void CLotData::SetLotID(AnsiString _sLotID) //혹시 나중에 Lot하고 동기화 할까봐 TDateTime 인자 넣음.
{

    m_tData.sLotId  = _sLotID ;
    DM.ARAY[riFFD].SetLotNo(_sLotID) ;
    DM.ARAY[riLFD].SetLotNo(_sLotID) ;
}
...
void CDayData::Update(AnsiString _sCrntLotNo, EN_SEQ_STAT Stat)
{
    ...
    static double dPreDate = tDateTime.CurrentDate() ;
    double dCrntDate = tDateTime.CurrentDate() ;
    ...
    bool bDateChanged = dCrntDate != dPreDate ;
    ...
    if(!bPreWork_F && bWork_F && bLotOpen) {    // 작업량 업데이트 부분 다시 봐야 한다...JH
        m_tData.iWorkCnt += OM.MstOptn.iPkgCnt; //DM.ARAY[DATA_ARAY].GetMaxCol() * DM.ARAY[DATA_ARAY].GetMaxRow();
...
void CDayData::DispDayInfo (TStringGrid * _sgDayInfo) //오퍼레이션 창용.
{
    ...
    _sgDayInfo -> Cells[0][0] = "Work Time "; tDateTime.Val = m_tData.dWorkTime  ; _sgDayInfo -> Cells[1][0] = tDateTime.FormatString("hh:nn:ss");
    _sgDayInfo -> Cells[0][1] = "Stop Time "; tDateTime.Val = m_tData.dStopTime  ; _sgDayInfo -> Cells[1][1] = tDateTime.FormatString("hh:nn:ss");
    _sgDayInfo -> Cells[0][2] = "Err Time  "; tDateTime.Val = m_tData.dErrTime   ; _sgDayInfo -> Cells[1][2] = tDateTime.FormatString("hh:nn:ss");
    _sgDayInfo -> Cells[0][3] = "Total Time"; tDateTime.Val = m_tData.dTotalTime ; _sgDayInfo -> Cells[1][3] = tDateTime.FormatString("hh:nn:ss");
    _sgDayInfo -> Cells[0][4] = "Work Cnt  ";                                      _sgDayInfo -> Cells[1][4] = m_tData.iWorkCnt   ;
    _sgDayInfo -> Cells[0][5] = "Uers Cnt  ";                                      _sgDayInfo -> Cells[1][5] = SPC.m_iUserCnt     ;
    _sgDayInfo -> Cells[0][6] = "U Work Cnt";                                      _sgDayInfo -> Cells[1][6] = SPC.m_iUserWorkCnt ;
