...
bool TFraMotr::SetIdType(EN_MOTR_ID _iId , EN_MTOR_DIRECTION _iType )
{
    ...
    */
    ...
    {
        btNeg -> Caption   = "BWD"     ; btNeg -> Glyph = m_pDnimg ;  //btNeg    -> OnMouseUp = btMtStop  ;
        btPos -> Caption   = "FWD"     ; btPos -> Glyph = m_pUpimg ;  //btPos    -> OnMouseUp = btMtStop  ;
    }
    else if(_iType == mdDU)
    {
    ...
    {
        btNeg -> Caption   = "CW"      ; btNeg -> Glyph = m_pAcimg ; //btNeg    -> OnMouseUp = btMtStop  ;
        btPos -> Caption   = "CCW"     ; btPos -> Glyph = m_pClimg ; //btPos    -> OnMouseUp = btMtStop  ;
    }
      ...
      TMouseButton Button, TShiftState Shift, int X, int Y)
{
    ...
    if(m_iId == miFED_YFrt){if(bFrntStgFwd){FM_MsgOk("WARNING","Stage is not Safe"); return;}}
    if(m_iId == miLSR_TFrt){if(bFrntStgFwd){FM_MsgOk("WARNING","Stage is not Safe"); return;}}
    if(m_iId == miFED_YLer){if(bLearStgFwd){FM_MsgOk("WARNING","Stage is not Safe"); return;}}
    if(m_iId == miLSR_TLer){if(bLearStgFwd){FM_MsgOk("WARNING","Stage is not Safe"); return;}}
