...
void CLaserConUnit::ReceiveMsg(DWORD _cbInQue)
{
    ...
    //상시 data 전송.   if(!sRcvMsg.Pos("\r"  )) return ; 이줄하고 순서 바꿔야 될수도 있음.....
    ...
    Trace("LaserCom Total",sRcvMsg.c_str());
    sRcvMsg  = "" ;
}
...
bool CLaserConUnit::CycleTX()
{
    ...
    if (m_tmCycle.OnDelay(m_iLaserStep && m_iLaserStep == m_iPreLaserStep , 15000 )) {
        EM_SetErr(eiLSR_Laser);
    ...
    switch (m_iLaserStep) {
        ...
        case 100: if(Rs232_L -> sData == "") return false;
                  ...
                  EM_SetErrMsg(eiLSR_LaserErrCode , sReadMsg.c_str());
                  m_bWrkLaser = false;
                  m_iLaserStep = 0 ;
                  return true ;
    }
...
bool CLaserConUnit::SendMsg(AnsiString _sMsg)
{
    ...
    bool bRet = Rs232_L -> SendData(iMsgLeng , str) ;
    delete[] str;

    return bRet ;

    //20150127 선계원 이렇게 하면 리턴펄스 일때 딜리트 안되서 수정함.
