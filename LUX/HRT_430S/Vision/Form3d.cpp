...
__fastcall TFrm3d::TFrm3d(TComponent* Owner , CImage * _pImage , CTracker * _pTracker)// , double * _pRefOstX , double * _pRefOstY)
    : TForm(Owner)
{
    Left = 100 ;
    Top  = 100 ;
...
void __fastcall TFrm3d::OnSetImage(CImage * _pImage)
{
    m_pImage   = _pImage   ;
}

void __fastcall TFrm3d::OnSetTracker(CTracker * _pTracker)
{
    m_pTracker = _pTracker ;
}
...
int __fastcall TFrm3d::DrawGLScene()
{
    ...
    TRect tTrackerRect = m_pTracker -> GetRect() ; 
    int iLeft   = tTrackerRect.left   ;
    int iTop    = tTrackerRect.top    ;
    int iRight  = tTrackerRect.right  ;
    int iBottom = tTrackerRect.bottom ;
