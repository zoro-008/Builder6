...
double CLoader::GetMotrPos(EN_MOTR_ID _iMotr , EN_PSTN_ID _iPstnId )
{
    ...
    //X , Y Pos Setting.
    double dZ1Pos = PM.GetValue(miLDR_ZCmp , pvLDR_ZWorkStartPs) + (OM.DevInfo.iCsSlCnt-r-1) * OM.DevInfo.dCsSlPitch ;
//    double dZ2Pos = PM.GetValue(miLD2_Z , pvLD2_ZWorkStartPs) + (                    r  ) * OM.DevInfo.dCsSlPitch ;
    ...
    if(_iMotr == miLDR_ZCmp){
        switch(_iPstnId) {
            ...
            case piLDR_ZWait      : dPos = PM.GetValue (_iMotr , pvLDR_ZWaitPs     ); break ;
            case piLDR_ZPickFwd   : dPos = PM.GetValue (_iMotr , pvLDR_ZPickFwdPs  ); break ;
            case piLDR_ZClampOn   : dPos = PM.GetValue (_iMotr , pvLDR_ZClampOnPs  ); break ;
            case piLDR_ZPickBwd   : dPos = PM.GetValue (_iMotr , pvLDR_ZPickBwdPs  ); break ;
            case piLDR_ZWorkStart : dPos = PM.GetValue (_iMotr , pvLDR_ZWorkStartPs); break ;
            case piLDR_ZPlaceFwd  : dPos = PM.GetValue (_iMotr , pvLDR_ZPlaceFwdPs ); break ;
            case piLDR_ZClampOff  : dPos = PM.GetValue (_iMotr , pvLDR_ZClampOffPs ); break ;
            case piLDR_ZPlaceBwd  : dPos = PM.GetValue (_iMotr , pvLDR_ZPlaceBwdPs ); break ;
        ...
        switch(_iPstnId) {
            default               : dPos = MT_GetCmdPos  (_iMotr                    ); break ;
            case piLDR_YWait      : dPos = PM.GetValue   (_iMotr , pvLDR_YWaitPs    ); break ;
            case piLDR_YPick      : dPos = PM.GetValue   (_iMotr , pvLDR_YPickPs    ); break ;
            case piLDR_YWork      : dPos = PM.GetValue   (_iMotr , pvLDR_YWorkPs    ); break ;
            case piLDR_YPlace     : dPos = PM.GetValue   (_iMotr , pvLDR_YPlacePs    ); break ;
        }
    }
    else {
...
bool CLoader::CheckSafe(EN_MOTR_ID _iMotr , EN_PSTN_ID _iPstnId)
{
    ...
    bool isFwPusher      = AT_Complete(aiLDR_Psh,ccFwd); //IO_GetX(xLDR_PusherFw) || !IO_GetX(xLDR_PusherBw);
...
bool CLoader::CycleHome()     //sun DLL direct access.
{
    ...
    switch (Step.iHome) {
        ...
        case 10: MoveActr(aiLDR_Psh, ccBwd);
...
bool CLoader::Autorun(void) //오토런닝시에 계속 타는 함수.
{
    ...
    if (Step.iSeq == 0) {
        ...
        bool isCycleSupply =  DM.ARAY[riLDR].CheckAllStat(csNone )   && !Stat.bChckSplyEmpty && !IO_GetX(xLDR_MgzIn )&& ((m_iMgzCnt < OM.DevOptn.iLotEndMgzCnt)  || (GetLotNo()!="" || OM.MstOptn.bNotUstLot)) ;
        bool isCyclePick   =  DM.ARAY[riLDR].CheckAllStat(csNone )   && (IO_GetX(xLDR_MgzIn) ||  OM.CmnOptn.bDryRun )&& ((m_iMgzCnt < OM.DevOptn.iLotEndMgzCnt)  || (GetLotNo()!="" || OM.MstOptn.bNotUstLot)) && !OM.CmnOptn.bLoadingStop;;
        bool isCycleWork   =  DM.ARAY[riLDR].GetCntStat  (csUnkwn)   &&  DM.ARAY[riPRB].CheckAllStat(csNone ) && //DM.ARAY[riPR2].CheckAllStat(csNone ) &&
        ...
        if (((m_iMgzCnt < OM.DevOptn.iLotEndMgzCnt)  || GetLotNo()!="")&& Stat.bChckSplyEmpty && !IO_GetX(xLDR_MgzIn) && !OM.CmnOptn.bLoadingStop) EM_SetErr(eiLDR_NeedMgz) ;
...
bool CLoader::ToStart(void) //스타트를 하기 위한 함수.
{
    ...
    switch (Step.iToStart) {
        ...
        case 10: Stat.bChckSplyEmpty = false ;
                 MoveActr(aiLDR_Psh, ccBwd);
                 Step.iToStart ++ ;
                 return false ;

        case 11: if(!MoveActr(aiLDR_Psh, ccBwd)) return false;
                 Step.iToStart = 0 ;
                 return true ;
    }
}
...
//Check Cycle Time Out.
    ...
    switch (Step.iCycle) {
        ...
        case  11: if(!MoveActr(aiLDR_Cmp    , ccFwd      )) return false ;
                  ...
                  if(!MoveActr(aiLDR_Psh , ccBwd      )) return false ;
        ...
        case  13: if(!MoveMotr(miLDR_YCmp , piLDR_YPick))return false ;
                  ...
                  //IO_SetY(yLDR_MgzInAC,false); 매거진이 뒤로 밀리는 현상 발생해서 나중에 끔.
        ...
        case  15: MoveActr(aiLDR_Cmp , ccBwd);

                  Step.iCycle++;
                  return false ;

        case  16: if(!MoveActr(aiLDR_Cmp , ccBwd)) return false ;
                  //IO_SetY(yLDR_MgzInAC,true);
        ...
        case  19: if(!MoveMotr(miLDR_YCmp , piLDR_YWork)) return false ;
                  ...
                  if(m_iMgzCnt <= 0) bFrstMgz = true;
                  m_iMgzCnt++;              //1

                  if(bFrstMgz){
                      sLotNo = PopLotNo();
                      if(sLotNo != "") {DM.ARAY[riLDR].SetLotNo(sLotNo) ; }
                      else             {EM_SetErr(eiETC_LotErr);          }
                  }
                  if(m_iMgzCnt > OM.DevOptn.iLotEndMgzCnt) {
                      ...
                      LT.LotInfo.sOperator = OM.CmnOptn.sOperator;
        ...
        case  20: if(!IO_GetX(xLDR_MgzDetect1) && !IO_GetX(xLDR_MgzDetect2)){
                      EM_SetErr(eiLDR_PickMiss);
                  }
                  Step.iCycle = 0;
                  return true;
    }
...
//                  }
        ...
        case  13: if(IO_GetX(xLDR_PusherOverLoad)) {
                      MoveActr(aiLDR_Psh, ccBwd);
                      EM_SetErr(eiLDR_PusherOverload);
                      Step.iCycle = 0;
                      return true ;
                  }
                  if(!MoveActr(aiLDR_Psh, ccFwd)) return false;
//                  DM.ShiftArrayData(riLDR,riPRB);
        ...
        case  14: if(!MoveActr(aiLDR_Psh, ccBwd)) return false;
    ...
    switch (Step.iCycle) {
        ...
        case  10: MoveMotr(miLDR_YCmp , piLDR_YWork) ;
                  ...
                  if(bStripEnd) {
                      ...
                      WRK.EndedLot.sLotNo     = LT.LotInfo.sLotNo     ;
                      WRK.EndedLot.iCrntLotNo = LT.LotInfo.iCrntLotNo ;
                      WRK.EndedLot.iLastLotNo = LT.LotInfo.iLastLotNo ;
                      WRK.EndedLot.sJobFile   = LT.LotInfo.sJobFile   ;
                      WRK.EndedLot.sOperator  = LT.LotInfo.sOperator  ;
                      WRK.EndedLot.iLotMgzCnt = LT.LotInfo.iLotMgzCnt ;
                      WRK.EndedLot.dStartTime = LT.LotInfo.dStartTime ;
                      WRK.EndedLot.dEndTime   = LT.LotInfo.dEndTime   ;
                      WRK.EndedLot.dRunTime   = LT.LotInfo.dRunTime   ;
                      WRK.EndedLot.dIdleTime  = LT.LotInfo.dIdleTime  ;
                      WRK.EndedLot.dJamTime   = LT.LotInfo.dJamTime   ;
                      WRK.EndedLot.dTotalTime = LT.LotInfo.dTotalTime ;
                      WRK.EndedLot.iStrpUPEH  = LT.LotInfo.iStrpUPEH  ;
                      WRK.EndedLot.iChipUPEH  = LT.LotInfo.iChipUPEH  ;
                      WRK.EndedLot.iStrpUPH   = LT.LotInfo.iStrpUPH   ;
                      WRK.EndedLot.iChipUPH   = LT.LotInfo.iChipUPH   ;
                      WRK.EndedLot.iWorkStrp  = LT.LotInfo.iWorkStrp  ;
                      WRK.EndedLot.iWorkChip  = LT.LotInfo.iWorkChip  ;
                      WRK.EndedLot.iFailChip  = LT.LotInfo.iFailChip  ;

                      Trace("LastStrip", DM.ARAY[riLDR].GetLotNo().c_str());
