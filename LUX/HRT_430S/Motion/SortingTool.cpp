...
//    이상하다 dXCmdPos == dFailPos 일때 같지 않다고 리턴됨.
    ...
    bool   bReGripZero = OM.DevOptn.iPSBReGripCol == 0 && PSB._iReGripCnt == 0;


    //bool   bNeedMove   = bXInposStop && dXCmdPos != dFailPos ;                //인포지션스탑이고 최종명령했던 포지션과 페일자제 있는 포지션이 다를 경우 이동필요.
    bool   bReadyPos   = bXInposStop && abs(dXCmdPos - dFailPos)< dXInpos && !bReGripZero ;     //이상하다 dXCmdPos == dFailPos 일때 같지 않다고 리턴됨.
...
bool CSortingTool::CycleHome()     //sun DLL direct access.
{
    ...
    switch (Step.iHome) {
        ...
        case 10: AT_MoveCyl(aiPSB_Cmp,ccFwd); //혹시 모를 간섭 .
...
//        case  15: if(!m_tmTemp.OnDelay(true ,1000)) return false ;
        ...
        case  17: if(!MoveActr(aiSTL_Btm , ccBwd)) return false;
                  MoveActr(aiSTL_Top , ccBwd);
                  //리젝없는 랏도 한칸 옮기게 하고 싶어서 PSB쪽에서 호출하게함.
                  //RejectMove(); //합치고 싶으나 귀찮음. 밑에있는거 계산함.
                  //MoveMotr(miREJ_XBtm , piREJ_XBWork  );
                  Step.iCycle++;
                  return false;
        ...
        case  18: if(!MoveActr(aiSTL_Top  , ccBwd        )) return false;
                  ...
                  if(!IO_GetX(xSTL_Vacuum) && !OM.CmnOptn.bIgnrVacuum) { // 배큠 센서가 감지 안되고 배큠 무시 옵션이 아닐경우
...
//Check Cycle Time Out.
    ...
    switch (Step.iCycle) {
        ...
        case  14: if(!m_tmTemp.OnDelay(true ,OM.DevOptn.iTrimRjtDly)) return false ;
                  IO_SetY(ySTL_VaccumEje,false);
                  if(IO_GetX(xSTL_Vacuum)) //앤 아직 옵션 추가 안함...굳이 해야 되나 싶음...
                  {
...
//                  MoveMotr(miSTL_YTop , piSTL_YTWait);  //여기부터 잠시 추가.
