...
EN_PROB_RSLT iMChipRslt[MAX_ARRAY_ROW][MAX_ARRAY_COL][MAX_CHIPCNT];
double       dMChipVf  [MAX_ARRAY_ROW][MAX_ARRAY_COL][MAX_CHIPCNT];
double       dMChipVr  [MAX_ARRAY_ROW][MAX_ARRAY_COL][MAX_CHIPCNT];
...
//    RcvData = "PUT:";
    ...
    */
    //RL00N0279RL01N0177
    // 대겸 테스트                   ex iContColCnt = 2 iRowCnt = 8 ;
    int iCnt = -1 ;
    for(int j=0; j>-OM.DevOptn.iContColCnt / 2; j-- )
    {
        for(int i = OM.DevInfo.iRowCnt - 1 ; i >= 0 ; i --)
        {
            ...
            for(int k=0; k< OM.DevOptn.iDgChipCnt; k++ ) //대겸 칩 카운트가 아니라 그냥 칩카운트로 바꾸고 GL 에서도 사용해야됨.
            {
                ...
                if(_iCh == PROBE_CH1) {
                    ...
                    dChipData = dChipData + OM.DevOptn.d1chLedOffset ;
                ...
                if(_iCh == PROBE_CH2) {
                    ...
                    dChipData = dChipData + OM.DevOptn.d2chLedOffset ;
                ...
                if(iCChipRslt[i][c+j][k] != prOk || _bRpt) {
                    ...
                    //dMChipVf[i][c+j][k] = dCChipVf[i][c+j][k];
                }
                ...
                */
                ...
                //20151020. JS
                //ITW에서 Chip Value가 튀는 현상 발생에 따른 임시조치사항.
                if(bVfFail) DM.ARAY[aiWhere].SetVf(i, c+j, dCChipVf[i][c+j][k]);
                bVfFail = bVfRslt;
                ...
                if(OM.DevOptn.bUseZener) {
                   //RcvData.Delete(RcvData.Pos("Z") , 1);
                    ...
                    if(_iCh == PROBE_CH1) {
                        ...
                        dChipData = dChipData + OM.DevOptn.d1chZennerOffset ;
                    ...
                    if(_iCh == PROBE_CH2) {
                        ...
                        dChipData = dChipData + OM.DevOptn.d2chZennerOffset ;
                    ...
                    if(iCChipRslt[i][c+j][k] != prOk || _bRpt) {
                        ...
                        //dMChipVr[i][c+j][k] = dCChipVr[i][c+j][k] ;
                    }
                    ...
                    //20151020. JS
                    //ITW에서 Chip Value가 튀는 현상 발생에 따른 임시조치사항.
                    if(bVrFail) DM.ARAY[aiWhere].SetVr(i,c+j,dCChipVr[i][c+j][k]);
                    bVrFail = bVrRslt;
                ...
                //if(OM.MstOptn.bUseDaegyeomProbe) { //
                     ...
                     if ( bVfRslt &&  bVrRslt &&  bDrRslt) {iCChipRslt[i][c+j][k] = prOk     ; DM.ARAY[aiWhere].SetFailStat(i,c+j,fsGood) ; }
                else if ( bVfRslt &&  bVrRslt && !bDrRslt) {iCChipRslt[i][c+j][k] = prDRNg   ; DM.ARAY[aiWhere].SetFailStat(i,c+j,fsDrop) ; }
                else if ( bVfRslt && !bVrRslt &&  bDrRslt) {iCChipRslt[i][c+j][k] = prVRNg   ; DM.ARAY[aiWhere].SetFailStat(i,c+j,fsZnnr) ; }
                else if ( bVfRslt && !bVrRslt && !bDrRslt) {iCChipRslt[i][c+j][k] = prVRNg   ; DM.ARAY[aiWhere].SetFailStat(i,c+j,fsZnnr) ; }
                else if (!bVfRslt &&  bVrRslt &&  bDrRslt) {iCChipRslt[i][c+j][k] = prVFNg   ; DM.ARAY[aiWhere].SetFailStat(i,c+j,fsLed ) ; }
                else if (!bVfRslt &&  bVrRslt && !bDrRslt) {iCChipRslt[i][c+j][k] = prVFNg   ; DM.ARAY[aiWhere].SetFailStat(i,c+j,fsLed ) ; }
                else if (!bVfRslt && !bVrRslt &&  bDrRslt) {iCChipRslt[i][c+j][k] = prVFVRNg ; DM.ARAY[aiWhere].SetFailStat(i,c+j,fsAll ) ; }
                else if (!bVfRslt && !bVrRslt && !bDrRslt) {iCChipRslt[i][c+j][k] = prVFVRNg ; DM.ARAY[aiWhere].SetFailStat(i,c+j,fsAll ) ; }

                //iMChipRslt[i][c+j][k] = iCChipRslt[i][c+j][k];
...
void __fastcall CProbeComUnit::WriteProbeDataCsv(EN_ARAY_ID _iId)
{
    ...
    Temp += ",1ChLedCur,";
    Temp += OM.DevOptn.i1ChLedCur ;
    Temp += ",1ChZenCur,";
    Temp += OM.DevOptn.i1ChZenCur ;
    Temp += ",2ChLedCur,";
    Temp += OM.DevOptn.i2ChLedCur ;
    Temp += ",2ChZenCur,";
    Temp += OM.DevOptn.i2ChZenCur ;
    Temp += ",Temperature,";
    Temp += OM.CmnOptn.iTmprtr    ;
    Temp += ",WaitDelay,";
    Temp += OM.DevOptn.iWaitDelay ;
...
void __fastcall CProbeComUnit::WriteMasterDataCsv(EN_ARAY_ID _iId, bool _bDateFolder)
{
    ...
    //Set Path.
    //sTemp        = "Master" ;
    sTemp         = CurrDateTime.CurrentDateTime().FormatString("yyyymmdd") + "\\";

    if(_bDateFolder) sPath = LOT_LOG_FOLDER + sTemp ;
    else             sPath = LOT_LOG_FOLDER;

    if(!DirectoryExists(sPath)) CreateDir(sPath.c_str());

    if(_bDateFolder) sPath = sPath + CurrDateTime.CurrentDateTime().FormatString("hh_nn") + "_" + "Master.csv" ;
    else             sPath = sPath + "Master.csv" ;
...
//
    ...
    for(int k =0 ; k < OM.DevOptn.iDgChipCnt ; k++ )
    {
        for(int r = 0 ; r < OM.DevInfo.iRowCnt ; r++)
        {
            ...
            for(int c = 0 ; c < OM.DevInfo.iColCnt ; c++)
            {
                dMChipVf[r][c][k] = dCChipVf[r][c][k];
                sTemp2 = sTemp2.sprintf("%06.3f",dMChipVf[r][c][k]) + "_";
                sRslt += sTemp2 ;
            }
        ...
        for(int r = 0 ; r < OM.DevInfo.iRowCnt ; r++)
        {
            ...
            for(int c = 0 ; c < OM.DevInfo.iColCnt ; c++)
            {
                dMChipVr[r][c][k] = dCChipVr[r][c][k];
                sTemp2 = sTemp2.sprintf("%06.3f",dMChipVr[r][c][k]) + "_";
                sRslt += sTemp2 ;
            }
            sRslt.Delete(sRslt.Length(),1);
        ...
        for(int r = 0 ; r < OM.DevInfo.iRowCnt ; r++)
        {
            ...
            for(int c = 0 ; c < OM.DevInfo.iColCnt ; c++)
            {
                iMChipRslt[r][c][k] = iCChipRslt[r][c][k];
                     if(iMChipRslt[r][c][k] == prReady ) sTemp2.sprintf("%02d",(int)csUnkwn ) ;
                else if(iMChipRslt[r][c][k] == prOk    ) sTemp2.sprintf("%02d",(int)csGood  ) ;
                else if(iMChipRslt[r][c][k] == prVFNg  ) sTemp2.sprintf("%02d",(int)csRslt0 ) ;
                else if(iMChipRslt[r][c][k] == prVRNg  ) sTemp2.sprintf("%02d",(int)csRslt1 ) ;
                else if(iMChipRslt[r][c][k] == prVFVRNg) sTemp2.sprintf("%02d",(int)csRslt2 ) ;
                sRslt += sTemp2 +"_";
            }
            sRslt.Delete(sRslt.Length(),1);
    ...
    //
    //
    //sTemp        = "Master\\Master.csv" ;
    //Path         = LOT_LOG_FOLDER + sTemp;
    //
    ////if(DM.ARAY[_iId].GetLotNo() != "")  Path += "\\" + DM.ARAY[_iId].GetLotNo() + "\\Log";//   LT.GetCrntLot() ;
    ////else                                Path += "\\NoLot"                                ;
    //
    //if( DirectoryExists(Path)) UserINI.ClearFile(Path) ;
    //if(!DirectoryExists(Path)) UserINI.MakeDir  (Path);
    //
    //
    //iMgzCnt = DM.ARAY[riWRK].GetID().ToIntDef(9999) / 100;
    //
...
bool __fastcall CProbeComUnit::ReadMasterData(EN_ARAY_ID _iId)
{
    ...
    for(int k =0 ; k < OM.DevOptn.iDgChipCnt ; k++ )
    {
        ...
        for(int r = 0 ; r < OM.DevInfo.iRowCnt ; r++)
        {
            ...
            for(int c = 0 ; c < OM.DevInfo.iColCnt ; c++)
            {
                ...
                if(sVal.ToIntDef(25) == (int)csUnkwn) iMChipRslt[r][c][k] = prReady ;
                if(sVal.ToIntDef(25) == (int)csGood ) iMChipRslt[r][c][k] = prOk    ;
                if(sVal.ToIntDef(25) == (int)csRslt0) iMChipRslt[r][c][k] = prVFNg  ;
                if(sVal.ToIntDef(25) == (int)csRslt1) iMChipRslt[r][c][k] = prVRNg  ;
                if(sVal.ToIntDef(25) == (int)csRslt2) iMChipRslt[r][c][k] = prVFVRNg;

                if(sVal.ToIntDef(25) != (int)csGood && sVal.ToIntDef(25) != (int)csUnkwn) DM.ARAY[_iId].SetStat(r, c, csFail);
                else                                                                      DM.ARAY[_iId].SetStat(r, c, (EN_CHIP_STAT)sVal.ToInt());
...
//    if( OM.DevOptn.i1Ch3P && OM.DevOptn.i1Ch3N ){ iLedChip = 3 ; } 혼용이 안됨.
    ...
    i1ChZenCur = (OM.DevOptn.i1ChZenCur * 0.01 * 4096) + (OM.DevOptn.i1ChZenCur * OM.DevOptn.d1chZennerOffset);
    ...
    i2ChZenCur = (OM.DevOptn.i2ChZenCur * 0.01 * 4096) + (OM.DevOptn.i2ChZenCur * OM.DevOptn.d2chZennerOffset);
