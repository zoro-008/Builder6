    ...
    : TForm(Owner)
{
    ...
    sgMasterCur -> DoubleBuffered = true;
...
//    sgProbePast -> Refresh();
    ...
    sgMasterCur -> ColCount = OM.DevInfo.iColCnt + 1;
    sgMasterCur -> RowCount = OM.DevInfo.iRowCnt + 1;
    ...
    for (int iCnt =0 ; iCnt <= OM.DevInfo.iRowCnt + 1 ; iCnt ++ ) sgMasterCur -> Cells[0   ][iCnt] = iCnt;
    for (int iCnt =0 ; iCnt <= OM.DevInfo.iColCnt + 1 ; iCnt ++ ) sgMasterCur -> Cells[iCnt][0   ] = iCnt;

    PCM.ReadMasterData(riMST);
...
//    }
    ...
    btMstDataSave -> Enabled = (FM_GetLevel() == (EN_LEVEL)lvMaster) ;
}
//---------------------------------------------------------------------------
...
void __fastcall TFrmProbe::btMstDataSaveClick(TObject *Sender)
{
    //if( !SaveDialog1->Execute() ) return ;
    //AnsiString sPath;
    //sPath = SaveDialog1 -> FileName ;
    DM.ShiftArrayData(riWRK, riMST);
    PCM.WriteMasterDataCsv(riMST, false);
    SEQ.bMstDataSave = true;
}
//---------------------------------------------------------------------------

void __fastcall TFrmProbe::sgMasterCurDrawCell(TObject *Sender, int ACol,
      int ARow, TRect &Rect, TGridDrawState State)
{
    AnsiString sItemVf[3] , sItemVr[3] ;
    if(ARow == 0 || ACol == 0) return ;
    ...
    sItemVf[0].printf("%0.2f",dMChipVf[ARow-1][ACol-1][0]);
    sItemVr[0].printf("%0.2f",dMChipVr[ARow-1][ACol-1][0]);
    sItemVf[1].printf("%0.2f",dMChipVf[ARow-1][ACol-1][1]);
    sItemVr[1].printf("%0.2f",dMChipVr[ARow-1][ACol-1][1]);
    sItemVf[2].printf("%0.2f",dMChipVf[ARow-1][ACol-1][2]);
    sItemVr[2].printf("%0.2f",dMChipVr[ARow-1][ACol-1][2]);
    ...
    switch(iRoom)
    {
        default :
        break ;
        case 1 : if( sgMasterCur->RowHeights[ARow] != iDftRowHeight    ) {
                     sgMasterCur->RowHeights[ARow]  = iDftRowHeight     ;
//                     sgProbeCur->DefaultColWidth   = 25 ;
                     return ;
                 }
//                 dRect[0].Bottom = dRect[0].Top + iDftRowHeight ;
//                 dRect[1].Top    = dRect[0].Bottom                 ;
        break ;
        case 2 : if( sgMasterCur->RowHeights[ARow] != iDftRowHeight * 2) {
                     sgMasterCur->RowHeights[ARow]  = iDftRowHeight * 2 ;
//                     sgProbePast->DefaultColWidth   = 25 ;
                     return ;
                 }
                 dRect[0].Bottom = dRect[0].Top + iDftRowHeight ;
                 dRect[1].Top    = dRect[0].Bottom                 ;
        break ;
        case 3 : if( sgMasterCur->RowHeights[ARow] != iDftRowHeight * 3 ) {
                     sgMasterCur->RowHeights[ARow]  = iDftRowHeight * 3 ;
//                     sgProbePast->DefaultColWidth   = 25 ;
                     return;
                 }
                 ...
                 dRect[2].Top    = dRect[1].Bottom               ;
        break ;
        case 4 : if( sgMasterCur->RowHeights[ARow] != iDftRowHeight * 4 ) {
                     sgMasterCur->RowHeights[ARow]  = iDftRowHeight * 4 ;
//                     sgProbePast->DefaultColWidth   = 25 ;
                     return;
                 }
                 ...
                 //Rect4.Bottom = Rect4.Top + iRectHeight * 4;
        break ;
        case 6 : if( sgMasterCur->RowHeights[ARow] != iDftRowHeight * 6 ){
                     sgMasterCur->RowHeights[ARow]  = iDftRowHeight * 6 ;
//                     sgProbePast->DefaultColWidth   = 25 ;
                     return ;
                 }
    ...
    {
        iCount ++ ;
        if( iMChipRslt [ARow-1][ACol-1][i] == prReady) {
            sgMasterCur->Canvas->Brush->Color = clYellow ;
            sgMasterCur->Canvas->FillRect(Rect)          ;
        }
        else if( iMChipRslt[ARow-1][ACol-1][i] == prOk) {
            sgMasterCur->Canvas->Brush->Color = clLime   ;
            sgMasterCur->Canvas->FillRect(dRect[iCount])          ;
            sgMasterCur->Canvas->TextOut(dRect[iCount].Left + 1, dRect[iCount].Top , sItemVf[i]);
            if(OM.DevOptn.bUseZener) {
                sgProbePast->Canvas->TextOut(dRect[iCount+1].Left + 1, dRect[iCount+1].Top , sItemVr[i]);
                iCount++;
            }
        }
        else if(iMChipRslt[ARow-1][ACol-1][i] == prVFVRNg) {
            sgMasterCur->Canvas->Brush->Color = clRed ;
            sgMasterCur->Canvas->FillRect(dRect[iCount])       ;
            sgMasterCur->Canvas->TextOut(dRect[iCount].Left + 1, dRect[iCount].Top , sItemVf[i]);
            if(OM.DevOptn.bUseZener) {
                sgProbePast->Canvas->TextOut(dRect[iCount+1].Left + 1, dRect[iCount+1].Top , sItemVr[i]);
        ...
        else if( iMChipRslt[ARow-1][ACol-1][i] == prVFNg) {
            sgMasterCur->Canvas->Brush->Color = clFuchsia  ;
            sgMasterCur->Canvas->FillRect(dRect[iCount])          ;
            sgMasterCur->Canvas->TextOut(dRect[iCount].Left + 1, dRect[iCount].Top , sItemVf[i]);
            //sgProbePast->Canvas->Brush->Color = clLime ;
            //sgProbePast->Canvas->FillRect(DnRect)      ;
        ...
        else if( iMChipRslt[ARow-1][ACol-1][i] == prVRNg) {
            sgMasterCur->Canvas->Brush->Color = clOlive ;
            sgMasterCur->Canvas->FillRect(dRect[iCount])          ;
            sgMasterCur->Canvas->TextOut(dRect[iCount].Left + 1, dRect[iCount].Top , sItemVf[i]);
            //sgProbePast->Canvas->Brush->Color = clRed  ;
            //sgProbePast->Canvas->FillRect(DnRect)      ;
            if(OM.DevOptn.bUseZener) {
                sgMasterCur->Canvas->TextOut(dRect[iCount+1].Left + 1, dRect[iCount+1].Top , sItemVr[i]);
                iCount++;
            }
        }
    }
