...
//    else if (m_iManNo == mcETC_PstRemoveStrip ){Trace("Manual Man","mcETC_PstRemoveStrip Stt");}
    ...
    else if (m_iManNo == mcETC_VisnTest        ){}
    else if (m_iManNo == mcETC_MasterInsp      ){}
...
//    else if (m_iManNo == mcETC_PstRemoveStrip ) { m_iManStep = 10 ; m_iPreManStep = 0 ; }
    ...
    else if (m_iManNo == mcETC_VisnTest       ) {WRK.InitCycleStep(); }
    else if (m_iManNo == mcETC_MasterInsp     ) {m_iManStep = 10; m_iPreManStep = 0;}
...
void __fastcall CManualMan::Update(void)
{
    ...
    else if (m_iManNo == mcETC_VisnTest    ) { if(WRK.CycleVisn      ()) m_iManNo = mcNoneCycle; }
    else if (m_iManNo == mcETC_MasterInsp  ) { if(CycleMasterInsp    ()) m_iManNo = mcNoneCycle; }
...
bool CManualMan::CycleMasterInsp()
{
//Check Cycle Time Out.
    AnsiString sTemp ;
    if (m_tmCycle.OnDelay(m_iManStep && m_iManStep == m_iPreManStep && !OM.MstOptn.bDebugMode, OM.DevOptn.iWaitDelay + 1000 )) {
        EM_SetErr(eiETC_ManCycleTO);
        sTemp = sTemp.sprintf( "%s TIMEOUT STATUS : m_iManStep=%02d" , __FUNC__ , m_iManStep );
        Trace("ManualMan",sTemp.c_str());
        m_iManStep = 0 ;
    ...
    switch (m_iManStep) {
        ...
        case  10: DM.ARAY[riPRB].SetStat(csUnkwn);
                  WRK.MoveActr(aiPRB_HtBlk, ccFwd);
                  m_tmDelay.Clear();
                  m_iManStep++;
                  return false;

        case  11: if(!WRK.MoveActr(aiPRB_HtBlk, ccFwd))return false;
                  if(!m_tmDelay.OnDelay(true, OM.DevOptn.iWaitDelay))return false;
                  PRB.InitCycleStep();
                  PRB.CycleDrag();
                  m_iManStep++;
                  return false ;

        case  12: if(!PRB.CycleDrag())return false;
                  PRB.InitCycleStep();
                  PRB.CycleOut();
                  m_iManStep++;
                  return false;

        case  13: if(!PRB.CycleOut())return false;
                  WRK.InitCycleStep();
                  WRK.CyclePreWork();
                  m_iManStep++;
                  return false;

        case  14: if(!WRK.CyclePreWork())return false;
                  WRK.InitCycleStep();
                  WRK.CycleVisn();
                  m_iManStep++;
                  return false;
        ...
        //아래서 씀.
        case  15: if(!WRK.CycleVisn())return false;
                  WRK.InitCycleStep();
                  WRK.CycleWork();
                  m_iManStep++;
                  return false;

        case  16: if(!WRK.CycleWork())return false;
                  //작업이 끝났으면 종료시키자.
                  ...
                  fc = (iMaxCol - c) -1  ; //원래 데이터를 뒤집은 것.

                  isReGrip1 =  fc >= OM.DevOptn.iReGrabCnt1 - OM.DevOptn.iContColCnt && WRK._iReGripCnt == 0 ;
                  isReGrip2 =  fc >= OM.DevOptn.iReGrabCnt2 - OM.DevOptn.iContColCnt && WRK._iReGripCnt == 1 ;

                  if(isReGrip1 || isReGrip2){
                      WRK.InitCycleStep();
                      WRK.CycleWkReGrip();
                      m_iManStep = 20;
                      return false;
        ...
        //위에서 씀.
        //리그립 하고 다시 돌아가자.
        case  20: if(!WRK.CycleWkReGrip())return false;
                  m_iManStep = 15;
                  return false;

        case  50: WRK.InitCycleStep();
                  WRK.CyclePostWork();
                  m_iManStep++;
                  return false;

        case  51: if(!WRK.CyclePostWork())return false;
                  PSB.MoveMotr(miPSB_XCmp, piPSB_XGrip);
                  m_iManStep++;
                  return false;

        case  52: if(!PSB.MoveMotr(miPSB_XCmp, piPSB_XGrip))return false;
                  PSB.InitCycleStep();
                  PSB.CycleDrag();
                  m_iManStep++;
                  return false;

        case  53: if(!PSB.CycleDrag())return false;
                  //데이터 비교하자.JS
                  _bInspMaster = true;
                  PCM.ReadMasterData(riMST);
                  for(int r = 0; r < OM.DevInfo.iRowCnt; r++){
                      for(int c = 0; c < OM.DevInfo.iColCnt; c++){
                          if(DM.ARAY[riWRK].GetStat(r, c) != DM.ARAY[riMST].GetStat(r, c)) _bInspMaster = false;
                      }
                  }
                  WRK.MoveMotr(miWRK_ZBtm, piWRK_ZBChange);
                  PSB.MoveActr(aiPSB_Cmp, ccBwd);
                  m_iManStep++;
                  return false;

        case  54: if(!WRK.MoveMotr(miWRK_ZBtm, piWRK_ZBChange))return false;
                  if(!PSB.MoveActr(aiPSB_Cmp, ccBwd))return false;
                  WRK.MoveActr(aiPRB_HtBlk, ccBwd);
                  SEQ._bInspMstr = _bInspMaster;
                  //DM.ShiftArrayData(riWRK, riMST);
                  PCM.WriteMasterDataCsv(riWRK, true);
                  if(_bInspMaster){
                      DM.ARAY[riWRK].SetStat(csNone);
                      FM_MsgOk("Confirm", "검사를 완료하였습니다.");
                      m_iManStep = 0;
                      return true;
                  }
                  else {
                      FM_MsgOk("Confirm", "검사를 실패하였습니다. 검사를 다시 실행 해 주세요.");
                  }
