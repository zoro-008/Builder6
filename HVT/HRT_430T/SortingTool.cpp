...
*/
        ...
        bool isGetReadyPos =  OM.CmnOptn.bIgnrDecChk ? PSB.GetReadyPos(csFail) : PSB.GetReadyPos(csWork) && !PSB.GetReadyPos(csFail) && PSB.GetCycleStep() == CPostBuff::scIdle; //PSB.GetCycleStep() == CPstBuffer::scIdle 이거 있어야 홀드 안남. 3번 인덱스 프리버퍼 동작중에  포지션 인식되어 디바이스 체크 들어감.
