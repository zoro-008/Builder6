...
bool CPostBuff::Autorun(void) //오토런닝시에 계속 타는 함수.
{
    ...
    if (Step.iSeq == 0) {
/*  원래꺼인데 가끔 워크가 있는데 작업열이 넘어가서 홀드 나는 문제로 다시 짠다.
...
//        iReGribCol = OM.DevOptn.iPSBReGripCol ; //30이라고 넣으면 30까지 작업 하고 31부터 리그립.   iMaxCol / 2 ; //스트립 중간까지 작업 하고 리그립한다.
        ...
        bool isCyclePreDrag = !DM.ARAY[riPS1].GetCntStat(csNone) && !DM.ARAY[riPS1].GetStep() ; //데이터맵 하나 만들기 싫어서 스텝을 쓴다.
        ...
        bool isCycleReGrip  = !isExstTrim1 &&  isExstTrim2       &&
                              !GetReadyPos(csFail)&& !GetReadyPos(csWork) && !GetReadyPos(csMask) &&
                               m_iReGripCnt == 0 &&  //중간이상 작업 할 차례고 리그립을 안했으면 리그립 조건.  ;
                               DM.ARAY[riULD].GetCntStat(csEmpty) && ULD.GetSeqStep() == CUnLoader :: scIdle &&
                               STL.GetSeqStep() == CSortingTool :: scIdle ; // 미얀 일단 이렇게 가자.

        bool isCycleWork    = ((isExstTrim1 && m_iReGripCnt == 0) || (isExstTrim2 && m_iReGripCnt == 1)) && //regripcnt가 있어야만 메가진 체인지시에 리그립후 페일 자제 있을시 삽질 안함.
...
*/
        ...
        bool isCyclePreDrag =!DM.ARAY[riPS1].GetCntStat(csNone) && !DM.ARAY[riPS1].GetStep() ; //처음에 자제를 인수 받고 워킹존까지 끌어 오는 스텝 데이터맵 하나 만들기 싫어서 스텝을 쓴다.
        bool isCycleDrag    = bFinished && !DM.ARAY[riPS1].CheckAllStat(csNone) ;              //작업이 다 끝났을때 드레그.
