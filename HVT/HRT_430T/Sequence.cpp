...
bool __fastcall CSequence::InspectEmergency(void)
{
    ...
    //Check Emergency
    if (IO_GetX(xETC_LEmgLD1Sw) || IO_GetX(xETC_EmgSw1) || IO_GetX(xETC_EmgSw2) ) {
...
bool __fastcall CSequence::InspectActuator(void)
{
    ...
    if(AT_Err(aiLD1_Psh  )) {EM_SetErr(eiLD1_PushTO  ) ; isOk = false ; }
...
//         if (!DM.ARAY[riPR2].CheckAllStat(csNone) && !IO_GetX(xPRB_3Pkg) ) return false ;
    if(( DM.ARAY[riWRP].GetCntExist() || DM.ARAY[riWRK].GetCntExist()) &&  (!IO_GetX(xWRK_1Pkg) && !IO_GetX(xWRK_2Pkg) )  && !OM.CmnOptn.bDryRun ) isOk = false ;
    if(( DM.ARAY[riPS1].GetCntExist() || DM.ARAY[riPS2].GetCntExist()) && !OM.CmnOptn.bDryRun) {
        if(!IO_GetX(xPSB_1Pkg) && !IO_GetX(xPSB_2Pkg) && !IO_GetX(xPSB_3Pkg) && !IO_GetX(xPSB_4Pkg) && !IO_GetX(xPSB_PkgOut1) ) isOk = false ;
    }
//    else if ( DM.ARAY[riPS2].GetCntExist() && (!IO_GetX(xPSB_1Pkg) && !IO_GetX(xPSB_2Pkg) && !IO_GetX(xPSB_3Pkg) && !IO_GetX(xPSB_PkgOut1) && !IO_GetX(xPSB_PkgOut2)) && !OM.CmnOptn.bDryRun) return false ;
    ...
    if (OM.EqpOptn.bExistLoader  ) { if(!DM.ARAY[riLD1].CheckAllStat(csNone) && ( !IO_GetX(xLD1_MgzDetect1) && !IO_GetX(xLD1_MgzDetect2) ) ) isOk = false ; }
