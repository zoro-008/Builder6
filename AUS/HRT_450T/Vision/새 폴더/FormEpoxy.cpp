...
void TFrmEpoxy::ApplyPara(bool _bToTable)
{
    ...
    if(_bToTable) {
        ...
        cbOverUnder     -> ItemIndex = VOM.VDevOptn.bEpxOverUnder ? 1 : 0 ;
        ...
        VOM.VDevOptn.bEpxOverUnder         = cbOverUnder     -> ItemIndex ;
...
void __fastcall TFrmEpoxy::btLtClick(TObject *Sender)
{
    ...
    float fXPitch   = (((CEpoxy *)VISN[m_iVisnId])->Insp.pTrckInsp2 -> Left - ((CEpoxy *)VISN[m_iVisnId])->Insp.pTrckInsp1 -> Left) / (float)VOM.VDevOptn.iCountX ;
    float fYPitch   = (((CEpoxy *)VISN[m_iVisnId])->Insp.pTrckInsp2 -> Top  - ((CEpoxy *)VISN[m_iVisnId])->Insp.pTrckInsp1 -> Top ) / (float)VOM.VDevOptn.iCountY ;
    float fTrackerW = ((CEpoxy *)VISN[m_iVisnId])->Insp.pTrckInsp1 -> Width  ;
    float fTrackerH = ((CEpoxy *)VISN[m_iVisnId])->Insp.pTrckInsp1 -> Height ;

    if(iSelButton == 0 ) {//left top
        fWidth  = fXPitch * 2;
        fHeight = fYPitch * 2;
        fLeft   = ((CEpoxy *)VISN[m_iVisnId])->Insp.pTrckInsp1 -> Left - fWidth /2 + fTrackerW/2;
        fTop    = ((CEpoxy *)VISN[m_iVisnId])->Insp.pTrckInsp1 -> Top  - fHeight/2 + fTrackerH/2;
    }
    else if(iSelButton == 1) { //right top
        fWidth  = fXPitch * 2;
        fHeight = fYPitch * 2;
        fLeft   = ((CEpoxy *)VISN[m_iVisnId])->Insp.pTrckInsp2 -> Left - fWidth /2 + fTrackerW/2;
        fTop    = ((CEpoxy *)VISN[m_iVisnId])->Insp.pTrckInsp1 -> Top  - fHeight/2 + fTrackerH/2;
    }
        ...
        fHeight = fYPitch * 2;
        fLeft   = ((CEpoxy *)VISN[m_iVisnId])->Insp.pTrckInsp1 -> Left - fWidth /2 + fTrackerW/2;
        fTop    = ((CEpoxy *)VISN[m_iVisnId])->Insp.pTrckInsp2 -> Top  - fHeight/2 + fTrackerH/2;
    }
