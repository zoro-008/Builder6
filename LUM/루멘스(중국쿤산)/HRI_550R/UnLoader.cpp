...
double CUnLoader::GetMotrPos(EN_MOTR_ID _iMotr , EN_PSTN_ID _iPstnId )
{
    ...
    if(_iMotr == miULD_ZCmp){
        ...
        switch(_iPstnId) {
            ...
            case piULD_YWork         : dPos = PM.GetValue   (_iMotr , pvULD_YWorkStartPs       ); break ;
        }
    }
...
bool CUnLoader::CheckSafe(EN_MOTR_ID _iMotr , EN_PSTN_ID _iPstnId)
{
    ...
    bool isBwPusher        = AT_Complete(aiPSB_Pusher , ccBwd) ;
    bool isRailGateCnsr    = IO_GetX(xPSB_PkgOut3) ; //IO_GetX(xPSB_PkgOut1) || IO_GetX(xPSB_PkgOut2) ;
...
bool CUnLoader::CycleHome()
{
    ...
    switch (Step.iHome) {
        ...
        case 10: //MT_Reset(miULD_ZCmp);
                 ...
                 PSB.MoveActr(aiPSB_Pusher,ccBwd);
        ...
        case 11: //if(!MoveActr(aiULD_Cmp,ccBwd)) return false ;
                 if(!PSB.MoveActr(aiPSB_Pusher,ccBwd)) return false ;
...
bool CUnLoader::GetLastStrip()
{
    ...
    bool bLDRDiff  = DM.ARAY[riULD].GetLotNo() != DM.ARAY[riLDR].GetLotNo() && DM.ARAY[riULD].GetLotNo()!="" ;
    bool bPRBDiff  = DM.ARAY[riULD].GetLotNo() != DM.ARAY[riPRB].GetLotNo() && DM.ARAY[riULD].GetLotNo()!="" ;
    bool bWR1Diff  = DM.ARAY[riULD].GetLotNo() != DM.ARAY[riWR1].GetLotNo() && DM.ARAY[riULD].GetLotNo()!="" ;
    bool bWR2Diff  = DM.ARAY[riULD].GetLotNo() != DM.ARAY[riWR2].GetLotNo() && DM.ARAY[riULD].GetLotNo()!="" ;
    bool bWR3Diff  = DM.ARAY[riULD].GetLotNo() != DM.ARAY[riWR3].GetLotNo() && DM.ARAY[riULD].GetLotNo()!="" ;
    bool bPSBDiff  = DM.ARAY[riULD].GetLotNo() != DM.ARAY[riPSB].GetLotNo() && DM.ARAY[riULD].GetLotNo()!="" ;
...
bool CUnLoader::Autorun(void) //오토런닝시에 계속 타는 함수.
{
    ...
    if (Step.iSeq == 0) {
        ...
        //                    (DM.ARAY[riULD].GetLotNo()!="" && DM.ARAY[riWR3].GetLotNo() != DM.ARAY[riPSB].GetLotNo() && !DM.ARAY[riWR3].CheckAllStat(csNone) && DM.ARAY[riPSB].CheckAllStat(csNone)) ;

        bool isMgzChanged = (DM.ARAY[riULD].GetLotNo()!="" && DM.ARAY[riULD].GetID().ToIntDef(9999)/100 != DM.ARAY[riPSB].GetID().ToIntDef(9999)/100 && !DM.ARAY[riPSB].CheckAllStat(csNone)) ||
                            (DM.ARAY[riULD].GetLotNo()!="" && DM.ARAY[riWR3].GetID().ToIntDef(9999)/100 != DM.ARAY[riPSB].GetID().ToIntDef(9999)/100 && !DM.ARAY[riWR3].CheckAllStat(csNone) && DM.ARAY[riPSB].CheckAllStat(csNone)) ;
        ...
        bool isCycleWork   = DM.ARAY[riULD].GetCntStat  (csEmpty) && !isZWorkPos && !isPSBOutCycle ;
        ...
        bool isCyclePlace  = !DM.ARAY[riULD].CheckAllStat(csNone ) && !isPSBOutCycle &&
                             (DM.ARAY[riULD].CheckAllStat(csWork ) || isLotChanged || isMgzChanged || (!isExstStrip && LDR.GetStat().bWorkEnd));
...
bool CUnLoader::ToStart(void) //스타트를 하기 위한 함수.
{
    ...
    switch (Step.iToStart) {
        ...
        case 10: PSB.MoveActr(aiPSB_Pusher,ccBwd);
                 Step.iToStart ++ ;
                 return false ;

        case 11: if(!PSB.MoveActr(aiPSB_Pusher,ccBwd)) return false ;
                 Step.iToStart = 0 ;
                 return true ;
    }
}
...
//Check Cycle Time Out.
    ...
    switch (Step.iCycle) {
        ...
        case  19: if(m_tmTemp.OnDelay(true , 2000)){
                  ...
                  //끝까지 나오게 하기 위해. IO Output 딜레이로 해결해요.
                  IO_SetY(yULD_MgzOutAC,false);
