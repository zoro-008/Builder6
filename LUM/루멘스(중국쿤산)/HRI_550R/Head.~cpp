...
double GetRightPos ()
{
    ...
    double dTemp = OM.DevInfo.dColPitch * GetVtlColCnt() +
    ...
    dTemp1 = OM.DevInfo.dColPitch * GetVtlColCnt();
    dTemp2 = (OM.DevInfo.dColGrGap - OM.DevInfo.dColPitch) *        (iColOneGrCnt ? (GetVtlColCnt()/iColOneGrCnt) : 0) ;
    dTemp3 = (OM.DevInfo.iColInspCnt - 1) * OM.DevInfo.dColPitch / 2.0 ;
...
void CHead::SetTrgPos()
{
    ...
    dSttPos = PM.GetValue(miWRK_XVsn , pvWRK_XVsnWorkStartPs );
    ...
    double dColGroupValue  = OM.DevInfo.iColGrCnt ? OM.DevInfo.iColCnt / OM.DevInfo.iColGrCnt : 0;
    double dGrPitch        = OM.DevInfo.dColGrGap - OM.DevInfo.dColPitch ;
    int    iSubCnt         = 0;
    int    iInspRmdr       = 0; //Rmdr = remainder 나머지!!

    double dGrValue;
    int    iGrValueCnt;

    for(int i = 0 ; i < iTrgCnt ; i++) {
        iGrValueCnt = OM.DevInfo.iColGrCnt ? (i * OM.DevInfo.iColInspCnt / dColGroupValue) : 0;
        dGrValue = dGrPitch * iGrValueCnt;
        if(dColGroupValue != 0 && iGrValueCnt != 0) {
            iInspRmdr = (int)dColGroupValue % OM.DevInfo.iColInspCnt;
            iInspRmdr = (OM.DevInfo.iColInspCnt - iInspRmdr) * iGrValueCnt;
        }
        dTemp = dSttPos +                    //첫포지션
                OM.DevInfo.dColPitch * (OM.DevInfo.iColInspCnt * i - iInspRmdr) + //
                dGrValue;
        ...
        //             //(OM.DevInfo.iColGrCnt ? (OM.DevInfo.dColGrGap - OM.DevInfo.dColPitch)* OM.DevInfo.iColInspCnt * (i + 1) / OM.DevInfo.iColGrCnt : 0 ) ;
        //
        //dTemp1 = (OM.DevInfo.iColGrCnt ? (OM.DevInfo.dColGrGap - OM.DevInfo.dColPitch)* OM.DevInfo.iColInspCnt * (i + 1) / OM.DevInfo.iColGrCnt : 0 ) ;
        //                                                                                              //빼기 검사 그룹별로
