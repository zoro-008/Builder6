...
bool CLoader::CheckSafe(EN_MOTR_ID _iMotr , EN_PSTN_ID _iPstnId)
{
    ...
    bool isFwPusher      = AT_Complete(aiLDR_Pusher,ccFwd); //IO_GetX(xLDR_PusherFw) || !IO_GetX(xLDR_PusherBw);
...
bool CLoader::CheckSafe(EN_ACTR_ID _iActr , bool _bFwd)
{
    ...
    if(_iActr == aiLDR_Pusher){
        if(_bFwd) if(isZMoving) {sMsg = "LDRZisMoving" ; bRet = false ;}
    }
...
bool CLoader::CycleHome()     //sun DLL direct access.
{
    ...
    switch (Step.iHome) {
        ...
        case 10: AT_MoveCyl(aiLDR_Pusher, ccBwd);
...
//        bool isWaitForPreLot=  isChangeLot && (DM.ARAY[riPRB].GetCntExist() ||
//                                               DM.ARAY[riWR1].GetCntExist() ||
//                                               DM.ARAY[riWR2].GetCntExist() ||
//                                               DM.ARAY[riWR3].GetCntExist() ||
//                                               DM.ARAY[riPSB].GetCntExist() );
        ...
        bool isPrbReady     =  !IO_GetX(xPRB_Pkg) && DM.ARAY[riPRB].CheckAllStat(csNone ) ;
                               //(DM.ARAY[riWR1].CheckAllStat(csNone ) || (DM.ARAY[riWR1].GetCntExist() && IO_GetX(xWR1_Pkg))) ;//&& AT_Complete(aiPRB_Stopper,ccFwd);
        bool isCycleSupply  =  DM.ARAY[riLDR].CheckAllStat(csNone )  && !Stat.bChckSplyEmpty && !IO_GetX(xLDR_MgzIn )&& ((m_iMgzCnt < OM.DevOptn.iLotEndMgzCnt)|| GetLotNo()!="") ; //|| (GetLotNo()!="" || OM.MstOptn.bNotUstLot)) ;

        bool isCyclePick    =  DM.ARAY[riLDR].CheckAllStat(csNone )  && (IO_GetX(xLDR_MgzIn) ||  OM.CmnOptn.bDryRun )&& //HED.GetSeqStep() == C
                             ((m_iMgzCnt < OM.DevOptn.iLotEndMgzCnt)|| isPreLotReady) ; //|| (GetLotNo()!="" || OM.MstOptn.bNotUstLot)) ;
//                             ((m_iMgzCnt < OM.DevOptn.iLotEndMgzCnt)|| !isWaitForPreLot) ; //|| (GetLotNo()!="" || OM.MstOptn.bNotUstLot)) ;
        bool isCycleWork    =  DM.ARAY[riLDR].GetCntStat  (csUnkwn)  && !OM.CmnOptn.bLoadingStop && isPrbReady && RAL.GetSeqStep() == ssNone ;  //RAL.GetSeqStep() != CRail::scIn;
...
//                      else DM.ARAY[riLDR].SetLotNo(PopLotNo()) ;    //NotUseLot X
        ...
        case  20: MoveMotr(miLDR_ZCmp , piLDR_ZWork);
                  Step.iCycle = 0 ;
                  return true ;
    }
}
...
//                  }
        ...
        case  13: if(!MoveActr(aiLDR_Pusher, ccFwd)) return false;
                  if(IO_GetX(xLDR_PusherOverLoad)) {
                      MoveActr(aiLDR_Pusher, ccBwd);
...
//Check Cycle Time Out.
    ...
    switch (Step.iCycle) {
        ...
        case  18: if(MoveMotr(miLDR_ZCmp , piLDR_ZPickFwd)) return false ;
                  IO_SetY(yLDR_MgzOutAC,false);
                  IO_SetY(yLDR_MgzInAC ,false); 
                  Step.iCycle = 0;
                  return true ;
