...
/*    for (int i = 0 ; i < m_iMaxIn ; i++) {
    ...
    for (int i = 0 ; i < m_iMaxIn ; i++) {
        ...
        //Check
        if(m_pIn[i].iOnDelay  && !m_pInOnDelay [i].OnDelay (bGetXVal != bPreXVal && !bPreXVal , m_pIn[i].iOnDelay )) continue ;
        if(m_pIn[i].iOffDelay && !m_pInOffDelay[i].OnDelay (bGetXVal != bPreXVal &&  bPreXVal , m_pIn[i].iOffDelay)) continue ;
