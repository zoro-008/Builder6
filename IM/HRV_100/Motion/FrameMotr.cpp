...
bool TFraMotr::CheckSafe(int _iId)
{
    ...
    if(_iId == (int)miSTG_XIndx){
        bool bSTGChckNotBwd = !AT_Complete(aiSTG_ChckDnUp , ccBwd);
        bool bSTGProbNotBwd = !AT_Complete(aiSTG_ProbFwBw , ccBwd);

        if(bSTGChckNotBwd) {sMsg = "Chucking cylinder is Down" ; bRet = false; }
        if(bSTGProbNotBwd) {sMsg = "probe cylinder is fwd!" ; bRet = false; }
    }
