...
void CPstnMan::Init()
{
    ...
    Value[miSTG_XIndx][pvSTG_XIndxWait   ].sName = "Wait   " ; Value[miSTG_XIndx][pvSTG_XIndxWait   ].iType = vtPosition;
    Value[miSTG_XIndx][pvSTG_XIndxWorkStt].sName = "WorkStt" ; Value[miSTG_XIndx][pvSTG_XIndxWorkStt].iType = vtPosition;
    Value[miSTG_XIndx][poSTG_XIndxPtch   ].sName = "Pitch  " ; Value[miSTG_XIndx][poSTG_XIndxPtch   ].iType = vtOffset  ;
...
double CPstnMan::GetValue(EN_MOTR_ID _iAxisNo , EN_PSTN_VALUE _iPstnNo)
{
...
EN_VALUE_TYPE CPstnMan::GetType(EN_MOTR_ID _iAxisNo , EN_PSTN_VALUE _iPstnNo)
{
    if( _iAxisNo < 0 || _iAxisNo >= MAX_MOTR          ) {ShowMessageT("Motor Axis Range is wrong"    ); return 0.0 ;}
    if( _iPstnNo < 0 || _iPstnNo >= PSTN_CNT[_iAxisNo]) {
        ShowMessageT("Position Index Range is wrong");
...
bool CPstnMan::SetType  (EN_MOTR_ID _iAxisNo , EN_PSTN_VALUE _iPstnNo , EN_VALUE_TYPE _iType)
{
    if( _iAxisNo < 0 || _iAxisNo >= MAX_MOTR          ) {ShowMessageT("Motor Axis Range is wrong"    ); return 0.0 ;}
    if( _iPstnNo < 0 || _iPstnNo >= PSTN_CNT[_iAxisNo]) {
        ShowMessageT("Position Index Range is wrong");
...
double CPstnMan::SetPos(EN_MOTR_ID _iAxisNo , EN_PSTN_VALUE _iPstnNo , double _dValue )
{
      ...
      TMouseButton Button, TShiftState Shift, int X, int Y)
{
    ...
    if(1 == GridInfo.iCol && Value[(EN_MOTR_ID)GridInfo.iGrid][(EN_PSTN_VALUE)GridInfo.iRow].iType != vtFixValue) {
        m_pPstnEdit[GridInfo.iGrid] -> Left    = GridInfo.iColX ;
    ...
    if(2 == GridInfo.iCol ) {
         ...
         if(CheckSafe[GridInfo.iGrid]!=NULL){
             if(CheckSafe[GridInfo.iGrid]((EN_MOTR_ID)GridInfo.iGrid  , (EN_PSTN_VALUE)GridInfo.iRow)){
                 MT_GoAbsMan(    (EN_MOTR_ID)GridInfo.iGrid    ,
                                 Value[(EN_MOTR_ID)GridInfo.iGrid] [(EN_PSTN_VALUE)GridInfo.iRow].dValue );
             }
             else {
             ...
             MT_GoAbsMan((EN_MOTR_ID)GridInfo.iGrid , Value[(EN_MOTR_ID)GridInfo.iGrid][(EN_PSTN_VALUE)GridInfo.iRow].dValue );
         }

         //if(GridInfo.iGrid == miSTG_XIndx) //MoveMotr( , (EN_PSTN_ID)GridInfo.iRow);
    }
    ...
    if(3 == GridInfo.iCol ) {
        ...
        if(Value[(EN_MOTR_ID)GridInfo.iGrid][(EN_PSTN_VALUE)GridInfo.iRow].iType == vtPosition) m_pPstnList[GridInfo.iGrid] -> Cells[1][GridInfo.iRow] = MT_GetCmdPos(GridInfo.iGrid) ;
    }
...
void CPstnMan::SetParent(TPanel * _pnBase , EN_MOTR_ID _iAxisNo)
{
    m_pPstnList[_iAxisNo]-> Parent = _pnBase ;
    m_pPstnEdit[_iAxisNo]-> Parent = _pnBase ;
}

void CPstnMan::SetCheckSafe(EN_MOTR_ID _iAxisNo , CheckSafeCallbackPM _pCallback)
{
    CheckSafe[_iAxisNo] = _pCallback;
}


void CPstnMan::SetWindow(TPanel * _pnBase , EN_MOTR_ID _iAxisNo)
{
