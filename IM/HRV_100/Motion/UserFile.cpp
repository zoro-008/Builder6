...
int  __fastcall TUserFile::SearchDir(AnsiString Path)
{
    ...
    //Init. mem.
    for (register int i = 0 ; i < MAX_FIND ; i++) m_FindItem[i] ="";
...
bool __fastcall TUserFile::SaveGridToCsv (TStringGrid *Grid , AnsiString DestName)
{
    //Set Data.
    AnsiString sData ;
    int hFile;
    ...
    if(FileExist(DestName)){
        if( MessageBox( Application->Handle, "File is Already Exist \r\n Overwrite?", "Confirm",MB_YESNO ) == IDNO ) return false ;
    }
    ...
    for(int i=0; i<Grid->RowCount; i++){
        for(int j=0; j<Grid->ColCount; j++){
            sData += Grid -> Cells[j][i] ;
            if(j!=Grid->ColCount-1)sData += "," ;
        }
        sData += "\r\n" ;
    }
    ...
    hFile = FileOpen(DestName.c_str() , fmOpenWrite);
    if (hFile == -1) {
        hFile = FileCreate(DestName.c_str());
        if (hFile == -1) {                                                                 
            ShowMessage("Could not make "+ DestName);
            return false;
        }
    }
