...
void __stdcall CImage::Del()
{
    ...
    if(m_pImage  ){GlobalFree(m_pImage );m_pBitInfo8  = NULL ;}//{delete [] m_pImage  ;m_pBitInfo8  = NULL ;}
    if(m_pImageC ){GlobalFree(m_pImageC);m_pBitInfo24 = NULL ;}//{delete [] m_pImageC ;m_pBitInfo24 = NULL ;}
}
...
bool __stdcall CImage::SetSize(int _iWidth , int _iHeight , int _iBitCnt)
{
    ...
    if(m_iWidth != _iWidth || m_iHeight != _iHeight || m_iBitCnt != _iBitCnt) {
        ...
        if(m_pImage  ){GlobalFree(m_pImage );}  //delete [] m_pImage  ;}
        if(m_pImageC ){GlobalFree(m_pImageC);}//delete [] m_pImageC ;}

        m_pImage  = (unsigned char*)GlobalAlloc(GMEM_FIXED|GMEM_ZEROINIT,iMemWidth8 *_iHeight); //m_pImage  = new unsigned char [iMemWidth8  * _iHeight ] ;
        m_pImageC = (unsigned char*)GlobalAlloc(GMEM_FIXED|GMEM_ZEROINIT,iMemWidth24*_iHeight); //m_pImageC = new unsigned char [iMemWidth24 * _iHeight ] ;
    }
...
void __stdcall CImage::ClrImage()
{
    ...
    if(m_iBitCnt == 8) {
        memset(m_pImage  , 0  , m_iMemWidth8 * m_iHeight);
    }
    else {
        memset(m_pImageC , 0  , m_iMemWidth8 * m_iHeight* BitToByte(m_iBitCnt));
    }
}
...
int __stdcall CImage::ShowImgGrey(HDC _hHdc , int _iDstX, int _iDstY, int _iDstW, int _iDstH , int _iSrcX, int _iSrcY, int _iSrcW, int _iSrcH)
{
    ...
    if(m_iBitCnt == 8){
        //m_pBitInfo8 -> bmiHeader.biHeight가 양수이면 실제 픽셀데이터들이 밑에서 위로 되어 있어야 하지만.
        //픽셀 좌표 계산할때 간편히 하기 위해 bmiHeader.biHeight가 양수 일때 데이터만 위아래 미러링 하여 가지고 있고.
        //그림 그릴때만 bmiHeader.biHeight의 부호를 바꿔 준다.
        //세이브 할때도 위아래 바꿔서 세이브 한다.
        //카메라에서 들어올때는 윈쪽 위부터 온다.
        //사진이 위아래가 미러링 되어 나온다.
        //_iSrcY도 뒤집혔기 때문에 미러링 계산 해야함.
        //m_pBitInfo8 -> bmiHeader.biHeight = -m_pBitInfo8 -> bmiHeader.biHeight ;
        iSrcMirroredY = m_iHeight - _iSrcH - _iSrcY; //존나 헛갈림....
        iRet = StretchDIBits(_hHdc, _iDstX , _iDstY , _iDstW , _iDstH , _iSrcX , iSrcMirroredY, _iSrcW, _iSrcH , m_pImage , m_pBitInfo8 , DIB_RGB_COLORS , SRCCOPY) ;
        //m_pBitInfo8 -> bmiHeader.biHeight = -m_pBitInfo8 -> bmiHeader.biHeight ;
    }
    else {
        //m_pBitInfo24-> bmiHeader.biHeight = -m_pBitInfo24-> bmiHeader.biHeight ;
        iSrcMirroredY = m_iHeight - _iSrcH - _iSrcY; //존나 헛갈림....
...
void __stdcall CImage::TrimScale(TRect _tRect , IImage * _tDstImg , float _fScale)
{
    SetRect(&_tRect);
...
bool __stdcall CImage::SetX (int *_pX , int _iMargin)
{
    bool bRet = false ;
    if(*_pX < _iMargin             ) {*_pX = _iMargin ; bRet = true ;}
    if(*_pX >= m_iWidth  - _iMargin) {*_pX = m_iWidth  - _iMargin - 1 ; bRet = true ;}

    return bRet ;

}
bool __stdcall CImage::SetY (int *_pY , int _iMargin)
{
    bool bRet = false ;
    if(*_pY < _iMargin              ) {*_pY = _iMargin ; bRet = true ;}
    if(*_pY >= m_iHeight  - _iMargin) {*_pY = m_iHeight  - _iMargin - 1 ; bRet = true ;}

    return bRet ;

}

bool __stdcall CImage::SetX (double *_pX , int _iMargin)
{
    bool bRet = false ;
    if(*_pX < _iMargin             ) {*_pX = _iMargin ; bRet = true ;}
    if(*_pX >= m_iWidth  - _iMargin) {*_pX = m_iWidth  - _iMargin - 1 ; bRet = true ;}
...
bool __stdcall CImage::SetY (double *_pY , int _iMargin)
{
    bool bRet = false ;
    if(*_pY < _iMargin              ) {*_pY = _iMargin ; bRet = true ;}
    if(*_pY >= m_iHeight  - _iMargin) {*_pY = m_iHeight  - _iMargin - 1 ; bRet = true ;}
