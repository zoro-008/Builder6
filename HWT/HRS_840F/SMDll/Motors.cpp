...
CMotors::CMotors()
{
    ...
    for(int i = 0 ; i < m_iMaxMotor ; i++) {
        ...
        //JH_AjinMotion = NULL ;
        //JH_AjinMotion = new CAjinMotion(i);
...
bool CMotors::GoAbsAcDc(int _iAxisNo , double _dPos , double _dVel , double _dAcc , double _dDec)
{
    _NG_RANGE_BOOL_

    return Motor[_iAxisNo]->GoAbsAcDc(_dPos , _dVel , _dAcc , _dDec);

}
void CMotors::GoIncAcDc(int _iAxisNo , double _dPos , double _dVel , double _dAcc , double _dDec)
{
    _NG_RANGE_VOID_

    Motor[_iAxisNo]->GoIncAcDc(_dPos , _dVel , _dAcc , _dDec);
}
...
{
    Motor[_iAxisNo]-> FindEdgeStop(_dVel ,_dAcc , _ftFindEdgeType);
}

bool CMotors::FindEdgeEmgStop(int _iAxisNo ,double _dVel ,double _dAcc ,CAjinMotion::EN_FINDEDGE_TYPE _ftFindEdgeType)
{
    Motor[_iAxisNo]-> FindEdgeEmgStop(_dVel ,_dAcc , _ftFindEdgeType);
}
...
// 보간 구동을 위해 추가
void CMotors::JH_ContiWriteClear(int _iCoord)
{
//    _NG_RANGE_VOID_
//    JH_AjinMotion -> ContiWriteClear(_iCoord) ;
}

void CMotors::JH_ContiSetAxisMap(int _iCoord, int _iAxisCnt, int *_ipAxisNo) // _ipAxisNo 값이 쓸모가 없어지는... 이 문제 해결해야함...
{
//    _NG_RANGE_VOID_

    //int * ipTemp[2] ;
    //
    //ipTemp[0] = &JH_AjinMotion -> AjinPara.iPhysicalNo ;
    //ipTemp[1] = &JH_AjinMotion -> AjinPara.iPhysicalNo + 1 ;
    //
    //_ipAxisNo = *ipTemp ;

    int *ipRealAxisNo;
    ipRealAxisNo = new int[_iAxisCnt] ; // Motor 개수만큼 int 포인터 배열 선언

    JH_m_iMasterAxisPhyAdd = 9999 ; // 마스터 축 피지컬 주소 값 9999로 초기화
    ...
    for(int i = 0; i < _iAxisCnt ; i++){
        ...
        if(iPhyscAdd < JH_m_iMasterAxisPhyAdd) JH_m_iMasterAxisPhyAdd = iPhyscAdd ; //가장 작은 축이 Master가 되게
        ipRealAxisNo[i] = iPhyscAdd ;
    }

    //JH_AjinMotion -> JH_ContiSetAxisMap(_iCoord, _iAxisCnt, _ipAxisNo) ; // 이 두개의 차이점은??
    CAjinMotion::JH_ContiSetAxisMap(_iCoord, _iAxisCnt, ipRealAxisNo) ;

    delete []ipRealAxisNo ;
}

void CMotors::JH_ContiSetAbsRelMode(int _iCoord, DWORD _dwAbsRelMode)
{
//    _NG_RANGE_VOID_
    CAjinMotion::JH_ContiSetAbsRelMode(_iCoord, _dwAbsRelMode) ;
}

void CMotors::JH_ContiBeginNode(int _iCoord)
{
//    _NG_RANGE_VOID_
    CAjinMotion::JH_ContiBeginNode(_iCoord) ;
}

void CMotors::JH_ContiEndNode(int _iCoord)
{
//    _NG_RANGE_VOID_
    CAjinMotion::JH_ContiEndNode(_iCoord) ;
}

void CMotors::JH_ContiStart(int _iCoord, DWORD _dwProfileset, int _iAngle)
{
//    _NG_RANGE_VOID_
    CAjinMotion::JH_ContiStart(_iCoord, _dwProfileset, _iAngle) ;
}

void CMotors::JH_LineMove(int _iCoord, int *_ipAxisNo, int _iAxisCnt, double *_dpEndPos, double _dVel, double _dAcc, double _dDec)
{
/*
...
//    _NG_RANGE_VOID_
    JH_AjinMotion -> JH_LineMove(_iCoord, _dpEndPos, _dVel, _dAcc, _dDec) ;
*/
    CAjinMotion::JH_LineMove(_iCoord, _dpEndPos, Motor[JH_m_iMasterAxisPhyAdd] -> GetPulseFromUnit(_dVel), // 인자 * ( Pulse / Unit ), Unit를 Pulse로 바꾸어줌
                                                 Motor[JH_m_iMasterAxisPhyAdd] -> GetPulseFromUnit(_dAcc), // pulses / Sec^2 ??
                                                 Motor[JH_m_iMasterAxisPhyAdd] -> GetPulseFromUnit(_dDec)) ;
}

void CMotors::JH_CircleCenterMove(int _iCoord, int *_ipAxisNo, int _iAxisCnt, double *_dpCenterPos, double *_dpEndPos, double _dVel, double _dAcc, double _dDec, DWORD _dwCWDir)
{
/*
    int *iPulsePer, *iUnitPer ;
    ...
    for(int i = 0; i < _iAxisCnt ; i++){
        ...
        iUnitPer    [i] = Motor[i] -> Para.dUnitPerRev  ;
        _dpEndPos   [i] = _dpEndPos   [i] * (iPulsePer[i] / iUnitPer[i]) ;
        _dpCenterPos[i] = _dpCenterPos[i] * (iPulsePer[i] / iUnitPer[i]) ;
    }
//    _NG_RANGE_VOID_
    CAjinMotion::JH_CircleCenterMove(_iCoord, _ipAxisNo, _dpCenterPos, _dpEndPos, _dVel, _dAcc, _dDec, _dwCWDir);
...
*/
    ...
    CAjinMotion::JH_CircleCenterMove(_iCoord, ipRealAxisNo, _dpCenterPos, _dpEndPos, Motor[JH_m_iMasterAxisPhyAdd] -> GetPulseFromUnit(_dVel),
                                                                                     Motor[JH_m_iMasterAxisPhyAdd] -> GetPulseFromUnit(_dAcc),
                                                                                     Motor[JH_m_iMasterAxisPhyAdd] -> GetPulseFromUnit(_dDec), _dwCWDir) ;
    delete []ipRealAxisNo ;
}
