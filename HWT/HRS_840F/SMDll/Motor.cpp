...
#pragma package(smart_init)
        // Motor static 함수 내에 모터 배열 인자 모두 넘겨서 
...
CAjinMotion * CMotor::GetAjinMoton()
{
    return AjinMotion ;
}
...
int CMotor::GetPulseFromUnit(double _dUnit)
{
    return _dUnit * PULSE_PER_UNIT ;

}

bool CMotor::GoAbsAcDc(double _dPos , double _dVel , double _dAcc , double _dDec)
{
    //return
    ...
    if(Stat.dTrgPos != _dPos || GetCmdPos()  == _dPos  ) {
        if (GetStop() ) {
            ...
            if(UsrPara.bUseRate) bReceive = AjinMotion -> GoAbs(_dPos * PULSE_PER_UNIT , _dVel * PULSE_PER_UNIT , _dAcc  * PULSE_PER_UNIT, _dDec * PULSE_PER_UNIT);
            else                 bReceive = AjinMotion -> GoAbs(_dPos * PULSE_PER_UNIT , _dVel * PULSE_PER_UNIT , _dAcc                  , _dDec                 );
            Stat.dTrgPos = _dPos ;
    ...
    if(Stat.dTrgPos != _dPos || GetCmdPos()  != _dPos  ) {
        if (GetStop() ) {
            ...
            Stat.dTrgPos = _dPos ;
            double dTemp1 = GetTrgPos() ;
            Trace(Para.sMotorName.c_str() , ("From " + String(GetCmdPos()) +" Move to " + AnsiString(_dPos)).c_str());
        }
        else {
            if (fabs(_dPos - Stat.dTrgPos) > UsrPara.dInposition) {
                Trace(Para.sMotorName.c_str() , ("Position Override Move to " + AnsiString(_dPos)).c_str());
...
void CMotor::GoIncAcDc(double _dPos , double _dVel , double _dAcc , double _dDec)
{
    //Local Var.
    double dCmdPos = GetCmdPos();

    //Go Relative.
    GoAbsAcDc(_dPos + dCmdPos , _dVel , _dAcc , _dDec);
}
