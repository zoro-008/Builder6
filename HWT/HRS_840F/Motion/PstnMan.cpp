...
void CPstnMan::Init()
{
    ...
    //if(MAX_MOTR != 2) {ShowMessageT("Motor Count is wrong");}

    memset(&GridInfo , 0 , sizeof(SGridSelInfo)             );
    //memset( Value    , 0 , sizeof(SValue)*MAX_MOTR*MAX_PSTN );

    //Value[miSTG_XTemp][pvSTG_XTempWait       ].sName = "Wait        " ;
      ...
      TMouseButton Button, TShiftState Shift, int X, int Y)
{
    ...
    if(2 == GridInfo.iCol ) {
         ...
         if(GridInfo.iGrid == miSTG_XTemp) STG.MoveMotr((EN_MOTR_ID)GridInfo.iGrid , (EN_PSTN_ID)GridInfo.iRow);
         if(GridInfo.iGrid == miSTG_YTemp) STG.MoveMotr((EN_MOTR_ID)GridInfo.iGrid , (EN_PSTN_ID)GridInfo.iRow);
    }
