...
class CArrayData
{
    ...
    public:
        ...
        void __fastcall SetGrColRow  (int c,int r);
        void __fastcall SetinspColRow(int c,int r);
        ...
        int  __fastcall GetGrRow   (void ) { return  m_iGrRow   ;}
        int  __fastcall GetGrCol   (void ) { return  m_iGrCol   ;}

        int  __fastcall GetInspRow (void ) { return  m_iInspRow ;}
        int  __fastcall GetInspCol (void ) { return  m_iInspCol ;}
...
class CArrayTrg : public CArrayData
{
    public:
        __fastcall  CArrayTrg(void);
        __fastcall ~CArrayTrg(void);
    ...
    public:
        void SetColRowPich(double _dColPich, double _dRowPich){
            m_dColPich = _dColPich;
            m_dRowPich = _dRowPich;
        };
        void SetColRowGrPich(double _dColGrPich, double _dRowGrPich){
            m_dColGrPich = _dColGrPich;
            m_dRowGrPich = _dRowGrPich;
        };

        double GetColGrPich (void){return m_dColGrPich; }
        double GetRowGrPich (void){return m_dRowGrPich; }
        
        double GetColPich   (void){return m_dColPich  ; }
        double GetRowPich   (void){return m_dRowPich  ; }
        
        int GetColTrgCnt(void){
            int iTrgCnt = 0;
            int iCheckColGrCnt = 0;
            ...
            if(uColGr){
                uColGrIn  = uCol % uColGr ? uCol / uColGr + 1 : uCol / uColGr ;
                for(int c = 0 ; c < uCol ; c+=GetInspCol()){
                    iTrgCnt++;
                    iGrCountC = (c / uColGrIn < uColGr) ? ( c / uColGrIn) : (uColGr-1);
                    if(iLastColGr != iGrCountC){
                        c = iGrCountC * uColGrIn ;
                        iCheckColGrCnt = iGrCountC;
                        iLastColGr = iGrCountC;
                    }
                    iLastCol = c;
                }
                iGrCountC = (iLastCol + GetInspCol() / uColGrIn < uColGr) ? ( iLastCol + GetInspCol() / uColGrIn) : (uColGr-1);
                if(iLastColGr != iGrCountC) iTrgCnt += 1;
            }
            else {
                iTrgCnt  = 0;
            }
        ...
        int GetRowTrgCnt(void){
            int iTrgCnt = 0;
            int iCheckRowGrCnt = 0;
            ...
            if(uRowGr){
                uRowGrIn  = uRow % uRowGr ? uRow / uRowGr + 1 : uRow / uRowGr ;
                for(int r = 0 ; r < uRow ; r+=GetInspRow()){
                    iTrgCnt++;
                    iGrCountR = (r / uRowGrIn < uRowGr) ? ( r / uRowGrIn) : (uRowGr-1);
                    if(iLastRowGr != iGrCountR){
                        r = iGrCountR * uRowGrIn ;
                        iCheckRowGrCnt = iGrCountR;
                        iLastRowGr = iGrCountR;
                    }
                    iLastRow = r;
                }
                iGrCountR = (iLastRow + GetInspRow() / uRowGrIn < uRowGr) ? ( iLastRow + GetInspRow() / uRowGrIn) : (uRowGr-1);
                if(iLastRowGr != iGrCountR) iTrgCnt += 1;
            }
            else {
                iTrgCnt  = 0;
            }
        ...
        double * GetColTrgPos(double dSttPos, double * dTrgPos){
            int iTrgCnt = 0;
            int iCheckColGrCnt = 0;
            iTrgCnt = GetColTrgCnt();
            ...
            if(uColGr){
                uColGrIn = uCol % uColGr ? uCol / uColGr + 1 : uCol / uColGr ;
                for(int c = 0 ; c < GetMaxCol() ; c+=GetInspCol()){
                    iGrCountC = (c / uColGrIn < uColGr) ? ( c / uColGrIn) : (uColGr-1);
                    if(iLastColGr != iGrCountC){
                        c = iGrCountC * uColGrIn ;
                        iCheckColGrCnt = iGrCountC;
                    ...
                    double dTemp = c * GetColPich() + iGrCountC * (GetColGrPich() - GetColPich());
                    dTrgPos[iTrgCnt] = dSttPos + c * GetColPich() + iGrCountC * (GetColGrPich() - GetColPich());
                    iTrgCnt++;
                    uLastCol = c + GetInspCol();
                }
                if(iTrgCnt < GetColTrgCnt()){
                    iGrCountC = (uLastCol / uColGrIn < uColGr) ? ( uLastCol / uColGrIn) : (uColGr-1);
                    if(iLastColGr != iGrCountC){
                        uLastCol = iGrCountC * uColGrIn ;
                        iCheckColGrCnt = iGrCountC;
                    }
                    double dTemp = uLastCol * GetColPich() + iGrCountC * (GetColGrPich() - GetColPich());
                    dTrgPos[iTrgCnt] = dSttPos + uLastCol * GetColPich() + iGrCountC * (GetColGrPich() - GetColPich());
                    iTrgCnt++;
                }
            }
            return dTrgPos;
        ...
        double * GetRowTrgPos(double dSttPos, double * dTrgPos){
            int iTrgCnt = 0;
            int iCheckRowGrCnt = 0;
            iTrgCnt = GetRowTrgCnt();
            ...
            if(uRowGr){
                uRowGrIn = uRow % uRowGr ? uRow / uRowGr + 1 : uRow / uRowGr ;
                for(int r = 0 ; r < GetMaxRow() ; r+=GetInspRow()){
                    iGrCountR = (r / uRowGrIn < uRowGr) ? ( r / uRowGrIn) : (uRowGr-1);
                    if(iLastRowGr != iGrCountR){
                        r = iGrCountR * uRowGrIn ;
                        iCheckRowGrCnt = iGrCountR;
                    ...
                    double dTemp = r * GetRowPich() + iGrCountR * (GetRowGrPich() - GetRowPich());
                    dTrgPos[iTrgCnt] = dSttPos + r * GetRowPich() + iGrCountR * (GetRowGrPich() - GetRowPich());
                    iTrgCnt++;
                    uLastRow = r + GetInspRow();
                }
                if(iTrgCnt < GetRowTrgCnt()){
                    iGrCountR = (uLastRow / uRowGrIn < uRowGr) ? ( uLastRow / uRowGrIn) : (uRowGr-1);
                    if(iLastRowGr != iGrCountR){
                        uLastRow = iGrCountR * uRowGrIn ;
                        iCheckRowGrCnt = iGrCountR;
                    }
                    double dTemp = uLastRow * GetRowPich() + iGrCountR * (GetRowGrPich() - GetRowPich());
                    dTrgPos[iTrgCnt] = dSttPos + uLastRow * GetRowPich() + iGrCountR * (GetRowGrPich() - GetRowPich());
                    iTrgCnt++;
                }
            }
            return dTrgPos;
