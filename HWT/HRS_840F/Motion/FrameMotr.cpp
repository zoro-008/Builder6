...
bool TFraMotr::CheckSafe(int _iId)
{
    ...
    if(_iId == (int)miSTG_XTemp){
        ...
        if(bVTIPshNotBwd) {sMsg = "실린더가 내려와 있습니다." ; bRet = false; }
        if(bVTITolNotBwd) {sMsg = "실린더가 내려와 있습니다." ; bRet = false; }
        if(bOSTPshNotBwd) {sMsg = "실린더가 내려와 있습니다." ; bRet = false; }
        if(bOSTTolNotBwd) {sMsg = "실린더가 내려와 있습니다." ; bRet = false; }
        if(bPckrZDown   ) {sMsg = "픽커Z축이 Move포지션 보다 밑에 있습니다."; bRet = false; }
    }
    if(_iId == (int)miSRT_XPckr ||
       _iId == (int)miSRT_YPckr ||
       _iId == (int)miLDR_YIndx ||
       _iId == (int)miULD_YIndx ){
       bool bPckrZDown    = MT_GetTrgPos(miSRT_ZPckr) > PM.GetValue(miSRT_ZPckr , pvSRT_ZPckrMove) + 1.0 ;
