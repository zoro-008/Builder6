...
void CPreRail::Init(bool _bFront)
{
    //m_sPartName = "Rail " ;
    //m_lStrpCycleTime = 0 ;
    //m_lPreCycleTime  = 0 ;
    ...
    if(_bFront){
        m_sPartName                 = "Front Pre Rail"         ;
        Add.iPartId                 = piPRE_F                  ;

//        Add.riPRE                   = riPRE_F                  ;
//        Add.riPST                   = riPST_F                  ;

        Add.xRAL_MaskDetect         = xRAL_FMaskDetect1        ;

        Add.yRAL_FoldAirBlow        = yRAL_FFoldAirBlow1       ;
        ...
        m_sPartName                 = "Rear Pre Rail"         ;
        Add.iPartId                 = piPRE_R                  ;

//        Add.riPRE                   = riPRE_R                  ;
//        Add.riPST                   = riPST_R                  ;
        
        Add.xRAL_MaskDetect         = xRAL_RMaskDetect1        ;

        Add.yRAL_FoldAirBlow       = yRAL_RFoldAirBlow1       ;
...
bool CPreRail::ToStart(void) //스타트를 하기 위한 함수.
{
    ...
    switch (Step.iToStart) {
        ...
        case 10: if(DM.ARAY[riULD_F].CheckAllStat(csNone)&&
                    DM.ARAY[riULD_R].CheckAllStat(csNone))IO_SetY(yRAL_ConMotorCw , true);
                 Step.iToStart++;
                 return false ;
...
bool CPreRail::Autorun(void) //오토런닝시에 계속 타는 함수.
{
    ...
    if (Step.iSeq == 0) {
        ...
        //오토런시에 서플라이쪽에 픽에러가 나면 자꾸 스타트시에 컨베이어 켜서
        //레일 뒷쪽에 마스크가 쌓여서 병신됌.
        //오토런에서 언로더에 자제가 없을때만 온시킨다.
        //끄는것은 ToStop에서...
        if( DM.ARAY[riULD_F].CheckAllStat(csNone)&&
            DM.ARAY[riULD_R].CheckAllStat(csNone))IO_SetY(yRAL_ConMotorCw , true);



        bool isCycleWork = IO_GetX(Add.xRAL_MaskDetect); /* && DM.ARAY[Add.riPRE].CheckAllStat(csUnkwn); */ // Rail 쪽 Array 빼버림.
        bool isConEnd     ; // DM.ARAY[Add.riPRE].GetCntStat(csNone) ;
...
//Check Cycle Time Out.
    ...
    switch (Step.iCycle) {
        ...
        case  10: m_tmDelay.Clear();
                  IO_SetY(Add.yRAL_FoldAirBlow , true);
                  Step.iCycle++;
                  return false;

        case  11: if(!m_tmDelay.OnDelay(true , OM.CmnOptn.dFirstAirBlowTime)) return false;
                  IO_SetY(Add.yRAL_FoldAirBlow , false);
        ...
        case  12: if(!m_tmDelay.OnDelay(true , 3000)) return false ;
                  ...
                  //DM.ShiftArrayData(Add.riPRE , Add.riPST); // Rail 쪽 Arry 빼버림.
                  //DM.ARAY[Add.riPST].SetStat(csUnkwn);
                  Step.iCycle=0;
                  return true;
    }
}
...
void CPreRail::Load(bool IsLoad)
{
    ...
    Trace( "CPreRail","Load End") ;
}

bool CPreRail::GetLastStrip()
{
