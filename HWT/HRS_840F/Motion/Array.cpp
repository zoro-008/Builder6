...
void __fastcall CArrayData::SetGrColRow (int c,int r)
{
    m_iGrCol = c;
    m_iGrRow = r;
}
void __fastcall CArrayData::SetinspColRow(int c, int r)
{
    m_iInspCol = c;
    m_iInspRow = r;
}
...
//ARRAY TRIGER.
//==============================================================================
__fastcall CArrayTrg::CArrayTrg(void)
{
    SetGrColRow  (1,1);
    SetinspColRow(1,1);
}
...
__fastcall CArray::CArray(void)
{
    ...
    m_iClStat[csRslt6  ] = clHotLight        ;//csMoneyGreen      = 6  ,
...
void __fastcall CArray::UpdateArayNrm()
{
    ...
    */
    ...
    int uRowGr, uColGr, uRow, uCol, uColGrIn , uRowGrIn ;
    ...
    int iGetWidth  = imArray -> Width  -(5 * (uColGr-1));  // 사각형들이 이미지안에 다 들어오게 하기 위함.. 그냥 원래 사이즈로 하면 삐져나감 사각형들이..
    int iGetHeight = imArray -> Height -(5 * (uRowGr-1));

    int iSetWidth = 0 , iSetHeight = 0;

    double uGw   = (double)iGetWidth  / (double)(uCol);
    double uGh   = (double)iGetHeight / (double)(uRow);
    int    iWOff = (int)((double)(iGetWidth  - uGw * (uCol)) / 2.0);
    int    iHOff = (int)((double)(iGetHeight - uGh * (uRow)) / 2.0);
    TColor sColor;
    ...
    if(CheckAllStat(csNone)) {
        ...
        if(GetInspRow() && GetInspCol()) {

            int iRowMax = uRow % GetInspRow() ? uRow : uRow + GetInspRow();
            int iColMax = uCol % GetInspCol() ? uCol : uCol + GetInspCol();

            imArray -> Canvas -> Brush -> Color = clRed;
            imArray -> Canvas -> Pen -> Color = clRed;

            //Col, Row 그룹을 그려주는데 문제 때문에.
            int iCheckColGrCnt = 0;
            int iCheckRowGrCnt = 0;
            int iLastColGr  = 0;

            for (int r = 0 ; r < iRowMax; r+=GetInspRow()){
                for(int c = 0; c < iColMax; c+=GetInspCol()){
                    //if(GetTrg()){  JS.
                        imArray -> Canvas -> Brush -> Color = clRed;
                        imArray -> Canvas -> Pen -> Color = clRed;

                        iGrCountR = (r/uRowGrIn < uRowGr) ? (5 * (r/uRowGrIn)) :5 * (uRowGr-1) ;
                        iGrCountC = (c/uColGrIn < uColGr) ? (5 * (c/uColGrIn)) :5 * (uColGr-1) ;

                        if(iGrCountR || iGrCountC){
                            //if(iCheckColGrCnt != iGrCountC / 5 && iGrCountC && c / uColGrIn <= iGrCountC / 5 && (c + GetInspCol()) / uColGrIn > iGrCountC / 5) {
                            if(iLastColGr != iGrCountC / 5){
                                c = iGrCountC / 5 * uColGrIn ;
                                iCheckColGrCnt = iGrCountC / 5;
                                iLastColGr = iGrCountC / 5;
                            }
                            uX1 = iWOff +  c                     *       uGw + 1 + iGrCountC;
                            uX2 = iWOff + (c + GetInspCol() - 1) * uGw + uGw - 1 + iGrCountC;
                            uY1 = iHOff +  r                     *       uGh + 1 + iGrCountR;
                            uY2 = iHOff + (r + GetInspRow() - 1) * uGh + uGh - 1 + iGrCountR;
                        ...
                        imArray -> Canvas -> Rectangle(uX1     , uY1     , uX2     , uY2    );
