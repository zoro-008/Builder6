...
void CTempConUnit::Init()
{
    ...
    // juhyeon
    if(OM.MstOptn.bUsedTempRS232) { // 마스터 옵션 값이 트루로 저장되어 있는데 왜 펄스로 로딩 되지?
        RS232C_TempCon = new TRS232C ;
        if(!RS232C_TempCon->Open(2)) ShowMessage("RS-232C TempCon 4 Ch Open Failed!");
        // 0부터 3까지 인자로 시작하는데
        // 0이 실제 Com1번 포트 순서임
        // 0 = Com1번, 1 = Com2번, 2 = Com3번, 3 = Com4번
        // [3_RS232] <- 4번째 Com 포트 & PortNo=4 <- 4번째 Com 포트
...
bool CTempConUnit::RqstCrntTemp()
{
    ...
    for(int i = 1 ; i <= MAX_TEMP_CH ; i++) {
        ...
        //sSendMsg += ",011"  ;
        //sSendMsg += sTemp.sprintf("%01d",i); //현제 온도 메모리 영역 0001~0008
    }
    bool bRet ;
...
void CTempConUnit::SetRcvMsg(AnsiString _sMsg)
{
    ...
    if(bDispErr && sSum != sCheckSum && sCheckStr != NULL) { //체크썸 확인 해보자.
