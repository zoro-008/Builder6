...
//                 Trace(m_sPartName.c_str(), sTemp.c_str());
        ...
        case 14: if(!MT_GoAbsMan(miWRK_YBtm, GetMotrPos(miWRK_YBtm,piWRK_YBWait))) return false ;
                 ...
                 MT_GoAbsMan(miWRK_ZBtm, GetMotrPos(miWRK_ZBtm,piWRK_ZBWait));//2015.02.13 장비 종료시에 불안해서.
...
bool CWorkZone::Autorun(void) //오토런닝시에 계속 타는 함수.
{
    ...
    if (Step.iSeq == 0) {
        ...
        if(OM.DevOptn.iReGrabCnt2<0) isReGrip2 = false;
...
bool CWorkZone::ToStart(void) //스타트를 하기 위한 함수.
{
    ...
    switch (Step.iToStart) {
        ...
        case 11: if(!MoveMotr(miWRK_YBtm,piWRK_YBWork)) return false ;
...
void CWorkZone::HeatProbe1Ch(DWORD _cbInQue) // Juhyeon 주석 설명
{
    ...
    {
        AnsiString sTemp = exception.Message;
        Trace("catch 1 exception"  ,sTemp      .c_str());
        Trace("catch 1 sRcvMsg"    ,sRcvMsg    .c_str());
        Trace("catch 1 sRcvMsgDrop",sRcvMsgDrop.c_str());
...
//Sleep(1);
    ...
    {
        AnsiString sTemp = exception.Message;
        Trace("catch 2 exception"  ,sTemp      .c_str());
        Trace("catch 2 sRcvMsg"    ,sRcvMsg    .c_str());
        Trace("catch 2 sRcvMsgDrop",sRcvMsgDrop.c_str());
