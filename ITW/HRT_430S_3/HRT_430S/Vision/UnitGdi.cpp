...
//    if (m_hDc==NULL) return;
//    TPoint p;
//    MoveToEx(m_hDc, x+m_iXOffset, y+m_iYOffset, &p);
//}
...
void __stdcall CGdi::Line(int x1, int y1, int x2, int y2)
{
    ...
    x1 += m_iXOffset; x1 *= m_fXScale;
    y1 += m_iYOffset; y1 *= m_fYScale;
    x2 += m_iXOffset; x2 *= m_fXScale;
    y2 += m_iYOffset; y2 *= m_fYScale;
...
void __stdcall CGdi::Pixel(int x, int y, COLORREF color)
{
    if(m_fXScale <= 1.0 && m_fYScale <= 1.0) {
        x += m_iXOffset; x *= m_fXScale;
        y += m_iYOffset; y *= m_fYScale;
        SetPixelV(m_hDc, x, y, color);
    }


    //else if(m_fXScale <= 1.0 ) {
    //    Line(x, y, x+1 , y);  // Line(int x1, int y1, int x2, int y2)
    //}
    //else if(m_fYScale <= 1.0) {
