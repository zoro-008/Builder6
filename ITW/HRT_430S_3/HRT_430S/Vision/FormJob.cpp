...
void __fastcall TFrmJob::FormShow(TObject *Sender)
{
    ...
    for (int i=0 ; i<sgDevice->RowCount; i++) {
        //sunsun if (sgDevice->Cells[1][i]==g_sJobFileName) {
        //sunsun     sgDevice->Row = i;
        //sunsun }
    }
...
void __fastcall TFrmJob::btNewClick(TObject *Sender)
{
    ...
    for (int i=0; i<sgDevice->RowCount; i++) {
        //sunsun if (sgDevice->Cells[1][i] == g_sJobFileName) sgDevice->Row = i;
    }
}
//---------------------------------------------------------------------------
...
void __fastcall TFrmJob::btRenameClick(TObject *Sender)
{
    ...
    //sunsun if (edSelDevice->Text == g_sJobFileName         ) {ShowMessage("현재 사용중인 JOB FILE은 변경할 수 없습니다."); return;}

    UserFile.RenameDir(Path + edSelDevice->Text , Path + edNewDevice->Text);

    sgDevice->Cells[1][sgDevice->Row] = edSelDevice->Text;
    //sunsun g_sJobFileName = edSelDevice->Text;
...
void __fastcall TFrmJob::btDeleteClick(TObject *Sender)
{
    ...
    //sunsun if (edSelDevice->Text == g_sJobFileName) {ShowMessage("현재 사용중인 JOB FILE은 삭제할 수 없습니다."); return;}
