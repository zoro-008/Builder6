...
bool __stdcall CArea::ShowArea(HDC _hHdc , int _x, int _y, float _fScaleX , float _fScaleY )
{
    ...
    for(int y = 0 ; y <  m_iHeight ; y++) {
        for(int x = 0 ; x <  m_iWidth ; x++) {
            ...
            if(Px) {
                ...
                if(GetColor(Px)) {
                    Gdi -> m_tPen.Color = GetColor(GetPixel(x,y)) ;
                    Gdi -> Rect(true , x+_x-1 , y+_y-1 , x+_x+1 , y+_y+1 , 0);
                }
            }
        }
    }
    ...
    /* 준선이가 한것. 화면이 위아래 뒤집히는데 확인 하고 쓰기...
    if(!m_pArea || m_iHeight <=0 || m_iWidth <= 0) { return false ; }
    ...
    //뒤집는 방법....아래..
    //StretchBlt(_hHdc , 0, 0, _iWidth , _iHeight , hMemDC , 0 , m_iHeight , m_iWidth , -m_iHeight , SRCCOPY);

    //투명화 하여 위에 덮에 씌운다.
    TransparentBlt(_hHdc , 0 , 0 , _iWidth , _iHeight , hMemDC , 0 , 0, m_iWidth , m_iHeight , RGB(0, 0, 0));
...
HBITMAP __stdcall CArea::GetHBitmapOnBlack(HDC _hHdc)
{
    ...
    //DeleteObject(hbitmap); CreateDIBitmap로 만 해줘야 할텐데....과연...
}
