...
bool CImi::Init()
{
    ...
    if(bFirst) {
        ...
        CImi::m_bDllLoaded = LoadImCamApi("ImCam.dll") ;
    ...
    MemChProperty.nChannelIndex = 5;//imi셑업에서 5번만 쓴다.
  ...
  if(pAS->m_iPixelFormat == XAS_PF_GRAY1 || pAS->m_iPixelFormat == XAS_PF_GRAY2)
    ...
    */
	...
	if(ImCamOpen(m_iImiCamNo, true, false, NULL , IMC_VIEW_STATIC, false, IMC_GRAB_RAW, 0, IMC_BAYER_GB_RG) != IMC_SUCCESS) {
		//ShowMessage("can't open ImiCamera");
                ShowMessagePos("Can't open Camera. Please ReStart Computer",0,0);
		return false;
	}
    ...
    /*
    */
    if(ImCamGetROISize(m_iImiCamNo,&iWidth , &iHeight) != IMC_SUCCESS || //짤라낸 알오아이.
       ImCamGetDataBits (m_iImiCamNo,&iBit             ) != IMC_SUCCESS ){
         if(ImCamGetImageSize(m_iImiCamNo,&iWidth , &iHeight) != IMC_SUCCESS || //실제 이미지 센서 크기.
            ImCamGetDataBits (m_iImiCamNo,&iBit             ) != IMC_SUCCESS ){
...
void CNeptune::LoadPara(bool _bLoad , AnsiString _sPath)
{
    ...
    //Set Dir.
    sTemp = "Camera_" + AnsiString(m_iNeptuneCamNo);
    sPath = _sPath;
