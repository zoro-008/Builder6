...
void CUnLoader::ResetChckSplyEmpty() //실린더를 움직일때 쓰는 함수.
{
    if(DM.ARAY[riULD].GetCntStat(csNone) != 0 && (!IO_GetX(xULD_MgzDetect1) && !IO_GetX(xULD_MgzDetect2)) &&
       !IO_GetX(xPSB_PkgOut2))
    {
...
bool CUnLoader::Autorun(void) //오토런닝시에 계속 타는 함수.
{
    ...
    if (Step.iSeq == 0) {
        ...
        bool isLotChanged = (DM.ARAY[riULD].GetLotNo()!="" && DM.ARAY[riULD].GetLotNo() != DM.ARAY[riPSB].GetLotNo() && !DM.ARAY[riPSB].CheckAllStat(csNone)) ||
                            (DM.ARAY[riULD].GetLotNo()!="" && DM.ARAY[riULD].GetLotNo() != DM.ARAY[riWRK].GetLotNo() &&  DM.ARAY[riPSB].CheckAllStat(csNone)  && !DM.ARAY[riWRK].CheckAllStat(csNone)) ; //||
//                            (DM.ARAY[riULD].GetLotNo()!="" && DM.ARAY[riULD].GetLotNo() != DM.ARAY[riPS1].GetLotNo() && !DM.ARAY[riPS1].CheckAllStat(csNone) && DM.ARAY[riPS2].CheckAllStat(csNone));
        ...
        //                     (DM.ARAY[riULD].CheckAllStat(csWork ) || isLotFinished || (!isExstStrip && LDR.GetStat().bWorkEnd));
        ...
        //모르는 스트립에러.
        if(  DM.ARAY[riULD].CheckAllStat(csNone) && Step.iSeq != scPlace && (  IO_GetX(xULD_MgzDetect1 ) || IO_GetX(xULD_MgzDetect2 ))                      )
            EM_SetErr(eiULD_MgzUnknown) ;
        //카세트 사라짐.
...
//Check Cycle Time Out.
    ...
    switch (Step.iCycle) {
        ...
        case  16: if(!MoveMotr(miULD_ZCmp , piULD_ZPickBwd)) return false ;
                  MoveMotr(miULD_YCmp , piULD_YWork) ;
                  if(IO_GetX(xULD_MgzDetect1) || IO_GetX(xULD_MgzDetect2)){
...
//Check Cycle Time Out.
    ...
    switch (Step.iCycle) {
        ...
        case  20: if(!m_tmTemp.OnDelay(true , 1000)) return false;
                  IO_SetY(yULD_MgzOutAC,false);
